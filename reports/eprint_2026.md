...
### [推荐] [2026/2103] LaBinius – Fast Lattice-Based Binary Polynomial Commitment Scheme

- **匹配关键字:** lattice

- **作者:** Michał Osadnik, Gregor Seiler

- **分类:** Cryptographic protocols

- **关键词:** lattice, SNARK, binary-field, avx-512

- **链接:** [论文](https://eprint.iacr.org/2026/2103) | [PDF](https://eprint.iacr.org/2026/2103.pdf)


> **研究背景:** 现有的基于格的证明系统在承诺和约束域之间存在耦合，限制了承诺效率并影响了二进制计算的效率。
>
> **主要贡献:** $\mathsf{LaBinius}$通过分离评估算术与承诺算术，提出了一个基于格的多项式承诺方案。
>
> **达到效果:** 该方案实现了单核超过67次哈希函数证明的速度，并且证明时间最多是相应哈希验证时间的1.2倍。
>
> **技术梗概:** 使用复合模数进行承诺，并将所有素因子设置为NTT友好的，评估声明则位于二进制扩展域中，无需进行位检查或额外的二进制算术化开销。

---
### [推荐] [2026/2109] Attacking CRT-RSA with Small Exponents via Unravelled Linearization

- **匹配关键字:** lattice

- **作者:** Zhaopeng Ding, Zhaopeng Dai, Yanshuo Zhang, Ziyang Yan, Ying Chen

- **分类:** Attacks and cryptanalysis

- **关键词:** CRT-RSA, Coppersmith’s method, unravelled linearization, MSB leakage

- **链接:** [论文](https://eprint.iacr.org/2026/2109) | [PDF](https://eprint.iacr.org/2026/2109.pdf)


> **研究背景:** 本文针对具有小CRT指数$d_p, d_q$的平衡CRT-RSA提出了一个新的Coppersmith格攻击方法，利用展开线性化技术。
>
> **主要贡献:** 该研究贡献了一种新的攻击方法，其效果与Takayasu–Lu–Peng攻击相当，但使用的格维数约为后者的一半。
>
> **达到效果:** 实验结果表明，理论预测与实际结果高度一致，并且通过LLL归约降低了运行时间，使得可以攻击更大的$d_p, d_q$值。
>
> **技术梗概:** 通过巧妙的变量替换，该研究将攻击扩展到两个最显著位泄漏模型，首次利用$p+q$的MSB泄漏并改进了$d_p,d_q$的MSB泄漏攻击方法。

---
### [推荐] [2026/2113] Post-Quantum PKE and More from a Noisy Unstructured Linear Algebraic Assumption: Beyond LWE and LPN

- **匹配关键字:** lattice, post-quantum, LWE

- **作者:** Riddhi Ghosal, Paul Lou, Amit Sahai

- **分类:** Public-key cryptography

- **关键词:** Public Key Encryption, Post-Quantum, New hardness assumption

- **链接:** [论文](https://eprint.iacr.org/2026/2113) | [PDF](https://eprint.iacr.org/2026/2113.pdf)


> **研究背景:** 研究背景：经典的线性代数假设，如LWE和LPN，长期以来是构建后量子安全公钥加密（PKE）方案的基础。然而，近期的经典攻击和量子技术的迅速发展揭示了这些假设可能不再安全，因此迫切需要新的假设来确保后量子安全性。
>
> **主要贡献:** 主要贡献：作者引入了一个新的线性代数假设，该假设在面对常噪声率LPN问题时仍然保持安全，并且即使LWE被破解的情况下也依然安全。此外，他们还改进了Ghosal等人关于PKE和两轮恶意安全的透明零知识证明方案。
>
> **达到效果:** 达到的效果：通过引入新的线性代数假设，作者成功地克服了之前工作中的弱点，构建了一个在面对不同噪声率LPN攻击时仍保持安全性的后量子公钥加密方案。
>
> **技术梗概:** 技术梗概：该研究采用了结合两种不同的线性代数假设的方法，并通过引入一个新假设来增强安全性。这种方法不仅提高了现有方案的鲁棒性，还为设计新的后量子密码学方案提供了新的思路。

---
### [推荐] [2026/2117] Provable dual attack on LWE via lattice projection

- **匹配关键字:** lattice, post-quantum, LWE

- **作者:** Hongyuan Qu, Chengliang Tian, Geng Wang, Guangwu Xu

- **分类:** Attacks and cryptanalysis

- **关键词:** provable dual attack, LWE, lattice projection, Poisson summation formula, Banaszczyk's inequality

- **链接:** [论文](https://eprint.iacr.org/2026/2117) | [PDF](https://eprint.iacr.org/2026/2117.pdf)


> **研究背景:** Learning with Errors (LWE)问题是后量子密码学中的基础，而双攻击是评估其实际难度的主要工具。然而，在证明性的双攻击中，一个主要瓶颈是需要在对偶格上生成大量的离散高斯样本。
>
> **主要贡献:** 作者提出了一种新的基于格投影的证明性双攻击框架，通过选择一个精心挑选的子格进行采样，而不是在整个对偶格上进行采样，从而减少了格维度并显著加速了离散高斯采样的过程。
>
> **达到效果:** 通过对参数条件的严格分析，并通过列举所有可行参数，作者实现了比原始证明性双攻击框架高出26.1到43.7位的效果提升。
>
> **技术梗概:** 该研究利用Banazczyk不等式的改进来获得更精细的区别不等式，提供了更精确的攻击成功条件；同时通过格投影技术减少采样维度以加速离散高斯样本生成。

---
### [推荐] [2026/2118] I Prove, Therefore I Am: Spatiotemporal Multi-Party Computation

- **匹配关键字:** post-quantum, LWE

- **作者:** Ziqing Guo, Fuyuki Kitagawa, Xiao Liang

- **分类:** Foundations

- **关键词:** Secure Multi-Party Computation, Position Verification, Universal Composability, Quantum Cryptography, Proof of Knowledge

- **链接:** [论文](https://eprint.iacr.org/2026/2118) | [PDF](https://eprint.iacr.org/2026/2118.pdf)


> **研究背景:** 研究背景：传统安全多方计算（MPC）允许互不信任的各方在私有数字输入上进行联合计算。本文扩展了这一范式，提出了一种时空多方计算（spatiotemporal MPC），该计算不仅依赖于参与方的私密信息，还依赖于其地理位置、时间或轨迹等物理事实。
>
> **主要贡献:** 主要贡献：提出了时空知识论证的概念，并基于“I证明，因此我存在”的原则定义了时空一致性。在此基础上，给出了时空MPC的安全性定义，并提供了实现该新概念的构造方法。
>
> **达到效果:** 达到的效果：成功构建了满足时空MPC安全性的协议，确保了隐私、物理一致性和可组合性。
>
> **技术梗概:** 技术梗概：通过引入时空知识论证和时空验证协议，操作性地定义了参与方的物理存在，并在此基础上提出了时空MPC的安全模型。

---
### [推荐] [2026/2120] Multi-Key FHE with Compact Ciphertexts (or: MKFHE Strikes Back)

- **匹配关键字:** homomorphic encryption, LWE

- **作者:** Intak Hwang, Seonhong Min, Yongsoo Song

- **分类:** Applications

- **关键词:** fully homomorphic encryption, FHE, multi-key fully homomorphic encryption, MKFHE

- **链接:** [论文](https://eprint.iacr.org/2026/2120) | [PDF](https://eprint.iacr.org/2026/2120.pdf)


> **研究背景:** Multi-Key Fully Homomorphic Encryption (MKFHE)允许多个密钥参与非交互式的动态计算，但现有方案的性能较差，因为随着参与者数量增加，密文大小线性增长。这使得MKFHE在实际应用中长期被忽视。
>
> **主要贡献:** 本文提出了一种具有紧凑密文的第一个MKFHE方案，无论参与方数量如何变化，在整个同态评估过程中密文大小保持不变。该方法适用于广泛的RLWE基FHE方案，并实现了高效的多密钥RGSW和FV类似方案。
>
> **达到效果:** 实验结果显示，与16个参与者相比，我们的方案在乘法上快30.6倍，在自同构操作上快60.6倍。这使得MKFHE同时保留了所有优点并提供了类似于单密钥方案的快速评估性能。
>
> **技术梗概:** 该方法通过开发一种编译器将广泛的RLWE基FHE方案转换为其多密钥对应方案，从而实现高效性。

---
### [推荐] [2026/2121] Lattice-Based Synchronous Signatures: Efficiently Aggregatable and Thresholdizable

- **匹配关键字:** lattice, post-quantum

- **作者:** Dan Boneh, Binyi Chen, Trisha Datta

- **分类:** Public-key cryptography

- **关键词:** synchronous signatures, post-quantum, lattice-based cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2121) | [PDF](https://eprint.iacr.org/2026/2121.pdf)


> **研究背景:** 在基于权益证明的系统中，需要将多个验证者的签名聚合为一个短签名以提高验证效率。现有的方案如Chipmunk和Lemur使用梅克尔树来实现这一目标，但存在长度依赖于聚合次数$\ell$的问题。
>
> **主要贡献:** 本文提出了一种基于格的新方法，实现了与聚合次数无关的短签名，并证明了在自适应攻击模型下的安全性。
>
> **达到效果:** 该方案不仅提高了签名的效率和验证速度，还支持高效的阈值签名，并且签名长度更短，对于某些参数选择具有实际优势。
>
> **技术梗概:** 通过设计一种新的基于格的同步聚合签名方案，利用代数验证算法实现高效性和安全性证明。

---
### [推荐] [2026/2124] Every Signing Leaks: Breaking Falcon via Floating-Point Conversion Leakage

- **匹配关键字:** lattice, post-quantum

- **作者:** Yuanyuan Zhou, Weijia Wang, Yiteng Sun, Yu Yu

- **分类:** Attacks and cryptanalysis

- **关键词:** alcon, Post-quantum cryptography, Side-channel attack, NTRU, Lattice-based signatures, Lattice reduction

- **链接:** [论文](https://eprint.iacr.org/2026/2124) | [PDF](https://eprint.iacr.org/2026/2124.pdf)


> **研究背景:** Falcon签名方案虽然具有紧凑的签名和良好的数学安全性，但其侧信道安全问题仍然存在挑战。特别是其浮点高斯采样器成为主要的泄漏源，保护它也因依赖于浮点运算而变得困难。
>
> **主要贡献:** 作者提出了一种新的攻击方法，显著减少了在现实噪声泄漏条件下所需的踪迹数量，并且可以在未优化和高度优化的实现中工作。
>
> **达到效果:** 该攻击通过利用浮点高斯中心$\mu$在递归叶节点处转换为浮点表示时产生的符号指数泄漏以及较弱的仅符号泄漏，成功减少了所需踪迹的数量，从而提高了攻击效率。
>
> **技术梗概:** 作者提出了一种名为CB-MLE的技术，使用梯度下降法获得一个估计值$f^\star$，该估计值的计算高斯中心分类与实际观测结果一致。

---
### [推荐] [2026/2125] A Survey of Constraint-Based Side-Channel Analysis: From Algebraic Attacks to Exact Probabilistic Inference
...
- **关键词:** Cryptanalysis, Side Channel Analysis, Probabilistic reasoning algorithms, Constraint and logic programming

- **链接:** [论文](https://eprint.iacr.org/2026/2125) | [PDF](https://eprint.iacr.org/2026/2125.pdf)


> **研究背景:** 侧信道分析（SCA）在过去十五年中，从将泄漏视为需要结合算法结构求解的精确方程组证据，转变为将其视为用于概率查询的联合概率分布证据。
>
> **主要贡献:** 作者提出了一个统一的贝叶斯因子图框架，展示了四种不同的SCA范式之间的关系，并证明了代数SCA是零温度极限下的最大后验估计。
>
> **达到效果:** 通过该框架，作者能够系统地回顾并整合了从代数SCA到精确概率推理的不同SCA方法，为未来的研究提供了统一视角。
>
> **技术梗概:** 文章采用了贝叶斯因子图、软分析SCA和知识编译的概率电路等技术来建模和解决复杂的侧信道攻击问题。

---
### [推荐] [2026/2130] On the Necessity of Anamorphic Encryption for Covert Communication
...
- **关键词:** Covert Communication, Supervisor, Anamorphic Encryption, Public-Key Encryption, LWE

- **链接:** [论文](https://eprint.iacr.org/2026/2130) | [PDF](https://eprint.iacr.org/2026/2130.pdf)


> **研究背景:** 研究背景：传统的加密方法可能不足以实现隐蔽通信，而安形加密作为一种新的技术手段被提出以增强隐蔽通信的安全性和有效性。
>
> **主要贡献:** 主要贡献：作者通过理论分析和实验验证了在隐蔽通信中使用安形加密的必要性，并提出了相应的安全模型和协议设计。
>
> **达到效果:** 达到的效果：证明了安形加密能够显著提高隐蔽通信的鲁棒性和安全性，为隐蔽信道提供了新的解决方案。
>
> **技术梗概:** 技术梗概：该研究采用了概率论和信息论的方法来分析隐蔽通信中的信息泄露问题，并结合密码学原理设计了基于安形变换的加密方案。

---
### [推荐] [2026/2131] Forging 1024-bit RSA signatures in nearly SNFS time

- **匹配关键字:** post-quantum

- **作者:** Laura Shea, Miro Haller, Adam Suhl, Nadia Heninger, Emmanuel Thomé

- **分类:** Attacks and cryptanalysis

- **关键词:** RSA, number field sieve, 1024, blind RSA, hardware security module

- **链接:** [论文](https://eprint.iacr.org/2026/2131) | [PDF](https://eprint.iacr.org/2026/2131.pdf)


> **研究背景:** 研究背景：RSA的安全性通常基于分解的复杂度，但实际场景中的安全性可能与通用数域筛法（GNFS）估计有所不同。Joux, Naccache和Thomé在2007年提出的一种算法允许攻击者在短时间内通过访问原始的RSA签名/解密 oracle 来伪造签名。
>
> **主要贡献:** 主要贡献：作者实现了并运行了这一算法针对1024位RSA，展示了如何通过黑盒API交互来模拟硬件安全模块（HSM）而无需泄露密钥。此外，研究还推断出在有签名oracle的情况下，实际的RSA安全性比基于分解的安全性估计低15到30比特。
>
> **达到效果:** 达到的效果：攻击总共耗时约1380 CPU核心年，并进行了$2^{32}$次 oracle 查询，在预计算完成后可以离线伪造任意选择的签名。研究结果表明，4096位RSA在该模型下也不足以提供128比特的安全性。
>
> **技术梗概:** 技术梗概：通过实现Joux, Naccache和Thomé提出的算法，作者展示了如何利用SNFS（特殊数域筛法）的时间复杂度来伪造签名。攻击过程中大部分时间用于预计算，实际签名生成所需时间相对较少。

---
### [推荐] [2026/2132] United: Shuffle-Free Multiparty PSU via Oblivious  Invertible Sketches

- **匹配关键字:** homomorphic encryption

- **作者:** Truong Son Nguyen, Lucas Piske, Ni Trieu

- **分类:** Cryptographic protocols

- **关键词:** Multiparty Private Set Union, Set Operation

- **链接:** [论文](https://eprint.iacr.org/2026/2132) | [PDF](https://eprint.iacr.org/2026/2132.pdf)


> **研究背景:** 现有的MPSU协议通常需要进行元素级别的操作并依赖于分布式洗牌来隐藏元素的所有权，这引入了O(n)的轮次复杂性瓶颈。
>
> **主要贡献:** United框架提出了一个新的数据结构抽象——不可见插入只读草图(OIIS)，直接构建保护后的对称草图，确保每个不同的联合元素恰好插入一次，并且隐藏插入条件和修改位置。
>
> **达到效果:** 通过两种实现OIIS的方法，HE基构造使用多密钥部分同态加密保护标准可逆查找表，而MPC基构造采用加法秘密共享并引入了新的行本地幂和OIIS技术以提高条件插入效率。在LAN设置中，MPC基构造的运行时最多降低3.10倍；而在WAN设置中，HE基构造的运行时最多降低2.73倍，优于现有协议。
>
> **技术梗概:** United框架通过引入OIIS和基于HE及MPC的技术来实现shuffle-free MPSU，提高了计算效率并减少了通信轮次。

---
### [推荐] [2026/2134] ANSA-IBS: Identity-Based Signatures from Annular NTRU Trapdoors and Bimodal Fiat-Shamir with Aborts

- **匹配关键字:** lattice

- **作者:** Zhaohui Cheng, Hengfu Yang, Kaixin Xiong

- **分类:** Public-key cryptography

- **关键词:** identity-based signature, lattice cryptography, NTRU trapdoors, bimodal signatures, Fiat- Shamir with aborts

- **链接:** [论文](https://eprint.iacr.org/2026/2134) | [PDF](https://eprint.iacr.org/2026/2134.pdf)


> **研究背景:** 该研究旨在设计一种基于身份的签名方案，利用环NTRU陷阱门和带有中止的Fiat-Shamir机制，以提高安全性并优化签名长度。
>
> **主要贡献:** 作者提出了一种新颖的身份基签名方案ANSA-IBS，并通过调整参数集实现了不同安全级别的固定签名大小。
>
> **达到效果:** 研究达到了在保持高安全性的前提下，显著缩短了签名长度的效果，满足了NIST的不同安全要求。
>
> **技术梗概:** 该方案采用Gärtner的迭代拒绝方法处理挑战单项式，减少了高斯掩码宽度和预期的签名尝试次数，从而实现了更短的签名。

---
### [推荐] [2026/2135] Improved Soundness for Compressed Permutation Oracles and Tight Quantum Preimage and Collision Bounds for the Sponge
...
- **关键词:** Post-quantum cryptography, Quantum random permutation model, Compressed oracle, Quantum query complexity

- **链接:** [论文](https://eprint.iacr.org/2026/2135) | [PDF](https://eprint.iacr.org/2026/2135.pdf)


> **研究背景:** 研究旨在改进压缩置换 oracle 的可靠性边界，并为海绵结构的预象和碰撞搜索提供更紧致的量子查询复杂性界限。
>
> **主要贡献:** 通过引入精确置换状态在压缩 oracle 数据库空间中的表示，作者得出了在 q ≤ N/4 查询下 soundness 错误的界为 4q/√N。
>
> **达到效果:** 此分析不仅提高了单次更多问题和循环查找的界限，还为无密钥 Davies–Meyer 碰撞搜索提供了常数成功概率下的紧致查询复杂性。
>
> **技术梗概:** 利用 Rosmanis 的表示论方法，直接比较了随机置换 oracle 和压缩置换 oracle 的查询操作。

---
### [推荐] [2026/2136] MAMBA-Frost: A Lattice KEM from Learning With Quantization

- **匹配关键字:** lattice, post-quantum, LWE, LWR

- **作者:** Shanxiang Lyu, Ke Ma, Junzuo Lai

- **分类:** Public-key cryptography

- **关键词:** Post-Quantum Cryptography, Lattice-Based Cryptography, Key Encapsulation Mechanism, Learning With Quantization

- **链接:** [论文](https://eprint.iacr.org/2026/2136) | [PDF](https://eprint.iacr.org/2026/2136.pdf)


> **研究背景:** 研究背景：基于学习误差（LWE）问题构建的密码系统提供了保守且成熟的安全性基础，而通过确定性舍入改进了带宽效率的学习加权舍入（LWR）问题，则面临内在相关噪声的问题，这阻碍了其与标准LWE的安全归约。
>
> **主要贡献:** 主要贡献：本文提出了基于学习量化（LWQ）问题的密钥封装机制MAMBA-Frost，通过精确的误差分布和明确定义的映射关系，实现了未结构化LWE安全性和量化压缩的无缝结合。
>
> **达到效果:** 达到的效果：该方案仅依赖于硬件友好的幂次二进制算术进行操作，并且通过E8编码信息嵌入优化了正确性余量而不影响基本的LWQ难度论证。
>
> **技术梗概:** 技术梗概：利用对齐的2的幂模数明确实例化量化格，推导出精确的有限支持误差分布并构造明确定义的映射关系，从而构建MAMBA-Frost机制。

---
### [推荐] [2026/2137] Too Small to Hide: Single-Trace Key Recovery from  ML-KEM Key Generation
...
- **链接:** [论文](https://eprint.iacr.org/2026/2137) | [PDF](https://eprint.iacr.org/2026/2137.pdf)


> **研究背景:** 本文研究了从ML-KEM（CRYSTALS-Kyber）的单个侧信道泄漏中恢复密钥的方法，特别关注其关键生成过程中的信息泄露机制。
>
> **主要贡献:** 作者证明了一个Arm Cortex-M4上的优化pqm4实现的一次功率踪迹足以用于密钥恢复，并提出了一种基于原始抽样器的修正方法来减少泄漏。
>
> **达到效果:** 通过结合CBD采样器和NTT操作的泄漏信息，研究达到了将ML-KEM-768的安全性估计从$2^{182.2}$降低到$2^{37.7}$的效果。
>
> **技术梗概:** 技术上，作者利用了系数表示法中的符号泄漏与幅度泄漏之间的互补性，并提出了一种$q$随机化方法来减少敏感信息的泄露。

---
### [推荐] [2026/2139] A Latency-Aware FPGA Architecture for SHAKE256 with Guarded Final-Block Construction: Targeting Post-Quantum XMSS Signatures

- **匹配关键字:** post-quantum

- **作者:** Maryam Taghi Zadeh

- **分类:** Applications

- **关键词:** SHAKE256, eXtended Merkle Signature Scheme(XMSS), FPGA acceleration, Keccak, latency optimization, final-block processing

- **链接:** [论文](https://eprint.iacr.org/2026/2139) | [PDF](https://eprint.iacr.org/2026/2139.pdf)


> **研究背景:** 研究背景：NIST SP 800-208 eXtended Merkle Signature Scheme (XMSS) 中的 XMSS-SHAKE256_10_256 需要频繁调用 SHAKE256 原语，这使得基于现场可编程门阵列（FPGA）的加速器对 Keccak 轮函数的时间和吸收/最终化控制开销非常敏感。
>
> **主要贡献:** 主要贡献：提出了一种结合保护型 SHAKE256 率感知合格性、直接合格最终块构造以及顺序最终化旁路的技术，无需修改 Keccak，从而显著降低了 L2 最终化延迟。
>
> **达到效果:** 达到的效果：在固定深度下，SAFE FBC 相对于 Structural Controller Decoupling (SCD) 基线减少了 13 个周期的总 L2 最终化延迟；在每周期三轮的情况下，总延迟从 305 减少到 292 个周期，且没有单一案例出现退步。
>
> **技术梗概:** 技术梗概：SAFE FBC 结合了保护型 SHAKE256 率感知合格性、直接合格最终块构造和顺序最终化旁路，通过控制 12 种配置研究了四种控制器/最终化组织与一到三轮每周期数据路径在九种 XMSS 案例中的表现。

---
### [推荐] [2026/2142] $\mathsf{PRAWNS}$: Threshold Hash-Based Signatures from Threshold PRFs

- **匹配关键字:** post-quantum

- **作者:** Dan Boneh, Benedikt Bünz, Justin D., Yavor Litchev, Kamilla Nazirkhanova

- **分类:** Cryptographic protocols

- **关键词:** post-quantum cryptography, threshold signatures, hash-based signatures, threshold pseudorandom functions

- **链接:** [论文](https://eprint.iacr.org/2026/2142) | [PDF](https://eprint.iacr.org/2026/2142.pdf)


> **研究背景:** 该研究旨在提出一种适用于某些哈希签名机制的高效门限方案，以增强其安全性和灵活性。
>
> **主要贡献:** 论文贡献了一种新颖的门限机制，能够将一次性签名和有限次数签名等方案转化为门限化版本，并保持了非门限化签名的格式和验证方式。
>
> **达到效果:** 实验结果表明，该框架在大量参与方和高门限设置下仍能高效运作，且不会泄露参与方数量和门限值信息。
>
> **技术梗概:** 通过设计基于门限伪随机函数（PRF）的机制，实现了签名分享过程中的隐私保护，并确保了签名验证的一致性和灵活性。

---
### [推荐] [2026/2146] Succinct Shortness Check Under a Few Kilobytes via Tensor Train Random Projections

- **匹配关键字:** lattice

- **作者:** Zhiyuan Geng, Maxime Plançon

- **分类:** Cryptographic protocols

- **关键词:** Lattices, Tensor Train, Approximate range proofs

- **链接:** [论文](https://eprint.iacr.org/2026/2146) | [PDF](https://eprint.iacr.org/2026/2146.pdf)


> **研究背景:** 现有基于格的证明系统依赖于Johnson-Lindenstrauss (JL)引理，但标准方法导致验证者复杂度线性增长，而结构化JL虽然投影向量较短，但需要以昂贵的形式传输，且长度随着维度增加。
>
> **主要贡献:** 作者提出了一种名为Tensor Train Random Projection (TTRP)的新技术，结合了结构化和非结构化的优点，实现了紧凑的投影矩阵和向量表示。
>
> **达到效果:** 使用TTRP与求和协议相结合的方法，验证者复杂度降低至$O(\lambda \log m (\log\log m)^2)$，证明大小约为4KB，显著提高了效率并保持了安全性。
>
> **技术梗概:** TTRP通过张量训练（Tensor Train）矩阵表示减少了投影长度，并利用求和协议线性化投影的正确性验证过程。

---
### [推荐] [2026/2149] Identify Post-Quantum Cryptographic Algorithms for Automotive Security: Performance-Driven Guidance for Secure Boot, OTA, and V2X Communication

- **匹配关键字:** lattice, post-quantum

- **作者:** Dileep Singh Kushwaha, Parneet Kaur

- **分类:** Cryptographic protocols

- **关键词:** Post Quantum Cryptography, PQC over TLS, PQC in automotive, Embedded, Cryptography, Liboqs, OQS

- **链接:** [论文](https://eprint.iacr.org/2026/2149) | [PDF](https://eprint.iacr.org/2026/2149.pdf)


> **研究背景:** 量子计算的快速发展对依赖于经典公钥加密算法如RSA和ECC的安全性构成了威胁，这些算法可能因Shor算法而失效。为了确保汽车行业的安全通信，包括空中下载（OTA）更新、安全启动、固件签名、PKI验证及V2X连接等关键功能，需要进行早期且明智的后量子密码学（PQC）迁移规划。
>
> **主要贡献:** 本文通过跨平台性能评估NIST选定和候选的PQC算法在TLS协议中的应用，包括独立运行和混合模式配置，并针对资源受限的Raspberry Pi 4B和基于Ubuntu的Intel Core i5系统进行了测试。
>
> **达到效果:** 结果表明，基于格的方案在性能、互操作性和长期保障方面表现出色，适合V2X和PKI密集型通信；而紧凑的签名方案如MAYO和FALCON则更适合其他场景。
>
> **技术梗概:** 通过测量执行时间、内存消耗和CPU核心利用率来评估算法-密钥封装机制（KEM）组合在实际应用中的可行性，从而为汽车安全通信提供指导。

---
### [推荐] [2026/2151] Tighter  Nonuniform Trace Reductions from Ring-LWE to MP-LWE

- **匹配关键字:** lattice, LWE

- **作者:** Yihang Cheng, Xiangzheng Zhao, Hengyi Luo, Yanbin Pan, Siwei Sun

- **分类:** Foundations

- **关键词:** Ring-LWE, Middle-Product LWE, Ideal lattices, Noise loss

- **链接:** [论文](https://eprint.iacr.org/2026/2151) | [PDF](https://eprint.iacr.org/2026/2151.pdf)


> **研究背景:** 该研究旨在改进从环LWE（Ring-LWE）到中间产物LWE（MP-LWE）的直接归约方法，特别是在系数绝对值有限制的情况下，提供更紧致的安全性证明。
>
> **主要贡献:** 作者通过分析Peikert和Pepin的方法，证明了在特定条件下存在具有多项式描述的小乘数，并且提出了当模数满足一定条件时，能够匹配NP26方法的线性噪声边界。
>
> **达到效果:** 研究结果表明，在系数绝对值有限制的情况下，可以通过选择适当的模数来实现与现有方法相当的安全性保证，从而提高了归约的有效性和紧致度。
>
> **技术梗概:** 作者使用了代数数论中的迹归约技术和乘数理论，结合多项式类的存在性证明，提出了新的分析框架和算法策略。

---
### [推荐] [2026/2153] New algorithms for quaternion ideals in SQIsign

- **匹配关键字:** lattice, post-quantum

- **作者:** Antonin Leroux, Sina Schaeffler

- **分类:** Implementation

- **关键词:** SQIsign, Quaternion

- **链接:** [论文](https://eprint.iacr.org/2026/2153) | [PDF](https://eprint.iacr.org/2026/2153.pdf)


> **研究背景:** 许多基于isogeny的方案依赖于Deuring对应关系，需要进行 quaternion 理想的相关计算。
>
> **主要贡献:** 论文提出了新的算法来表示和操作 quaternion 理想，并且在特定条件下（$p=3 mod 4$）实现了更简洁有效的计算方法。
>
> **达到效果:** 这些新算法使得参与计算的整数大小易于控制并且比之前的工作要小，特别地，在应用于SQIsign时，将最坏情况下的整数边界从 $O(p^6)$ 改善到了 $O(p^4)$。
>
> **技术梗概:** 通过引入规范化表示和一系列简单高效的算法来实现 quaternion 理想的计算，并且在特定条件下优化了 lattice reduction 的部分。

---
### [推荐] [2026/2164] On the Limits of LWE and PCE based UPKE

- **匹配关键字:** lattice, post-quantum, LWE

- **作者:** Martin R. Albrecht, Benjamin Benčina, Russell W. F. Lai

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2164) | [PDF](https://eprint.iacr.org/2026/2164.pdf)


> **研究背景:** 研究背景：更新公钥加密（UPKE）在异步环境中实现前向安全性，如安全群组消息传递中至关重要。现有方案要么无法支持无限次更新，要么存在公共密钥和密文大小过大的问题。
>
> **主要贡献:** 主要贡献在于提出了一种基于置换码等价性（PCE）和二元秘密二元误差下的计算假设LWE的UPKE方案，显著减小了密钥大小，并分析了将模块格问题引入带来的攻击风险。
>
> **达到效果:** 达到的效果是实现了更紧凑的密钥和密文尺寸，同时证明了一般化到模块格问题是不具优势的。此外，该方案在Alwen等人的模型中达到了IND-CCA安全性，但未实现其他后量子候选方案的‘加入者安全’特性。
>
> **技术梗概:** 技术梗概包括使用计算假设LWE替代信息论上的空心格问题以减小密钥大小，并通过分析PCE实例的线性化攻击展示了模块格带来的潜在风险。

---
### [推荐] [2026/2166] A Provable Correctness Analysis of the MATZOV-Style FFT Dual Attack on ML-KEM

- **匹配关键字:** lattice, LWE

- **作者:** Jiale Li, Li-Ping Wang, Huaxiong Wang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2166) | [PDF](https://eprint.iacr.org/2026/2166.pdf)


> **研究背景:** 该研究针对MATZOV风格的FFT双攻击方法在ML-KEM中的应用进行了证明正确性分析，填补了实际攻击估计与可证明保证之间的差距。
>
> **主要贡献:** 作者通过分别处理正确的和不正确的候选者，并应用不同的条件论证，得出了明确的成功标准和可证明的FFT分数阈值分离。
>
> **达到效果:** 基于Pouly和Shen（EUROCRYPT 2024）的成本估算方法，研究估计了ML-KEM-512、ML-KEM-768和ML-KEM-1024的具体攻击成本分别为2^10, 300和410位。
>
> **技术梗概:** 分析通过将正确的候选者与错误的候选者分开处理，并针对两部分应用不同的条件论证来实现证明正确性，从而获得明确的成功标准和可证明的阈值分离。

---
### [推荐] [2026/2167] New Applications of RSA

- **匹配关键字:** lattice

- **作者:** Yao-Ching Hsieh, Abhishek Jain, Brent Waters

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2167) | [PDF](https://eprint.iacr.org/2026/2167.pdf)


> **研究背景:** 本文提出了基于RSA的新颖构造，旨在实现简洁的密文通信，并且这些构造在解密批次大小、签名者数量或接收者集合大小方面达到了最优的简洁性。
>
> **主要贡献:** 作者通过应用投影伪随机生成器（PPRGs）的方法，提出了批量门限加密、聚合签名和广播加密的新颖构造，这些构造在标准模型中实现了黑盒使用密码学技术的安全性。
>
> **达到效果:** 这些新颖的构造不仅达到了最优的简洁度，在解密批次大小或接收者集合大小方面也超越了或匹配了已知的构造，并且在安全性上提供了与之前基于双线性映射或其他假设的方案相当的保障。
>
> **技术梗概:** 通过识别并利用投影伪随机生成器（PPRGs）的新增强特性，作者成功地将RSA假设应用于这些加密问题中，从而实现了简洁性和安全性的双重目标。

---
### [推荐] [2026/2168] High-Performance Implementations of HAETAE and SMAUG-T on Arm Cortex-M55 Using MVE and Slothy

- **匹配关键字:** post-quantum

- **作者:** Seung-Won Lee, Min-Joo Sim, Hwa-Jeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/2168) | [PDF](https://eprint.iacr.org/2026/2168.pdf)


> **研究背景:** 针对资源受限的嵌入式环境，优化后量子密码算法（PQC）的高效执行需要同时优化算法向量化、数据流和指令执行顺序。
>
> **主要贡献:** 论文提出了基于Arm Cortex-M55 MVE扩展的手动优化汇编代码，并结合Slothy进一步优化指令顺序和寄存器分配，显著提升了性能。
>
> **达到效果:** 实验结果表明，与原生MVE实现相比，HAETAE的前向Number-Theoretic Transform加速了2.349–2.424倍，FFT包括平方值累积加速了1.701–1.732倍，SMAUG-T矩阵-向量乘法加速了1.322–1.350倍。
>
> **技术梗概:** 通过MVE和Slothy技术对HAETAE和SMAUG-T的关键内核进行了数据布局、寄存器重用和内存访问模式的重新设计，优化了指令顺序和寄存器分配。

---
### [推荐] [2026/2170] Swing the Lure: How to Cheaply Mitigate Sign Leakage in Falcon

- **匹配关键字:** lattice, post-quantum

- **作者:** Xiuhan Lin, Mehdi Tibouchi, Yang Yu

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2170) | [PDF](https://eprint.iacr.org/2026/2170.pdf)


> **研究背景:** Falcon作为NIST标准候选方案之一，尽管高效且签名短小，但由于浮点运算和秘密依赖的高斯抽样导致难以正确安全实现，尤其在侧信道攻击方面存在巨大挑战。
>
> **主要贡献:** 本文提出了一种新的基于中心符号泄漏的侧信道攻击方法，并展示了其不同于以往攻击的特点。
>
> **达到效果:** 该攻击方法成功揭示了Falcon签名算法中输入中心值的符号信息，从而可能威胁到密钥的安全性。
>
> **技术梗概:** 通过分析一维高斯采样器中的输入中心变量符号泄漏，提出了一种低成本的防御策略以减轻签名泄露风险。

---
### [推荐] [2026/2173] Switch Commitments by Coincidence: Confidential Transactions Were Quantum-Upgradable All Along

- **匹配关键字:** post-quantum

- **作者:** Yevhen Hrubiian, Mykyta Redko, Tim Ruffing

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2173) | [PDF](https://eprint.iacr.org/2026/2173.pdf)


> **研究背景:** 升级加密货币以实现后量子安全面临挑战，因为链上状态将用户的持有量与公钥关联起来。在使用隐秘交易（CT）的网络中，如Liquid、Monero和Grin，这一问题更为严重，单个离散对数计算可能导致通货膨胀而非仅限于资金盗窃。
>
> **主要贡献:** 作者展示了Liquid和Monero实际上已经部署了一种巧合式的切换承诺方案，并证明了其在量子随机预言模型下的安全性。同时，他们还修复了Grin中切换绑定的构造问题。
>
> **达到效果:** 通过这一研究，作者不仅解决了现有系统中的安全漏洞，还为未来的后量子加密货币设计提供了新的思路和方法。
>
> **技术梗概:** 该研究采用了重新解释已有的隐秘交易机制，并结合切换承诺的概念来构建新的安全方案。

---
### [推荐] [2026/2178] Breaking the Spell: Cryptanalysis of VDOO

- **匹配关键字:** post-quantum

- **作者:** Max Cartor, Ryann Cartor, Ansta Rakotondrafara

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2178) | [PDF](https://eprint.iacr.org/2026/2178.pdf)


> **研究背景:** VDOO是一种彩虹类多变量签名方案，具有额外的‘对角线’结构，旨在抵御结构性攻击。
>
> **主要贡献:** 研究揭示了VDOO的安全性主要由前两层油层决定，而对角线结构的影响微乎其微，并提供了复杂度分析修正和新的混合简单-填充MinRank攻击方法。
>
> **达到效果:** 研究表明所有提议的参数未能达到NIST声明的安全级别，澄清了VDOO的真实安全性并为未来的后量子彩虹设计提供了指导。
>
> **技术梗概:** 研究采用了简单的和矩形的最小秩攻击，并引入了一种新的混合简单-填充最小秩攻击方法进行比较分析。

---
### [推荐] [2026/2180] Gaussian Kernel Lattices and Smoothing Bounds from Theta Integrals

- **匹配关键字:** lattice

- **作者:** Samed Düzlü, Nihar Gargava, Erkan Tairi

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2180) | [PDF](https://eprint.iacr.org/2026/2180.pdf)


> **研究背景:** 文章探讨了Gaussian leftover hash lemma (LHL)在矩阵X具有离散高斯列和足够宽的向量v时，乘积Xv接近于离散高斯分布的现象。该研究扩展了这一工具的应用范围，并提供了一种统一的方法来证明核格的关键事实。
>
> **主要贡献:** 贡献在于通过解析估计证明了核格的两个关键性质：满射性和光滑参数的小值，无需单独的论证和假设。
>
> **达到效果:** 结果表明，对于给定的光滑误差$2^{-\lambda}$，光滑参数为$O_K(\sqrt{\lambda})$，且当矩阵X具有$m \gtrsim dr\ln(srm)$列时成立。此外，该理论适用于某些特定类型的域。
>
> **技术梗概:** 技术上，通过将核格的对偶乘以指数的积分表示为周期高斯和的一维积分来实现分析估计，并利用这些积分的下界来证明结论。

---
### [推荐] [2026/2181] Attacking UOV-based Signatures with Schur--Macaulay Matrices

- **匹配关键字:** post-quantum

- **作者:** Simon-Philipp Merz, Lars Ran

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2181) | [PDF](https://eprint.iacr.org/2026/2181.pdf)


> **研究背景:** 研究背景：Unbalanced Oil and Vinegar (UOV)及其变体是NIST后量子签名候选算法之一，其安全性依赖于恢复隐藏子空间的困难性。在特征为2的情况下，该系统的极形式为交替形式，这被近期攻击利用，在外代数而非多项式环中进行。
>
> **主要贡献:** 主要贡献：作者提出了两种独立改进方法来削弱这种攻击：一是通过去除因子2使方程组在整数域上产生新的方程；二是引入Schur--Macaulay矩阵扩展了Macaulay矩阵的概念。
>
> **达到效果:** 达到的效果：这两种方法消除了参数阈值限制，提高了复杂度，并验证了所提出的Hilbert级数猜想。
>
> **技术梗概:** 技术梗概：通过外代数和Schur模块理论，作者设计了一种新的攻击策略，利用交替形式的性质生成更多方程，并提出了适用于任意形状λ的Schur--Macaulay矩阵的概念。

---
### [推荐] [2026/2186] Provable FFT-Accelerated Dual Attack on LWE Using the General Dual Lattice
...
- **链接:** [论文](https://eprint.iacr.org/2026/2186) | [PDF](https://eprint.iacr.org/2026/2186.pdf)


> **研究背景:** 该研究针对Learning with Errors (LWE)问题的双攻击方法进行了深入探讨，特别是在基于格的密码系统中，如ML-KEM。先前的研究通过快速傅里叶变换(FFT)加速了双攻击，并取得了具体的安全性估计，但这些结果依赖于一个已知在某些参数范围内失效的经验独立性假设。
>
> **主要贡献:** 本文提供了Guo--Johansson风格的基于FFT的双攻击方法在一般对偶格上的形式正确性分析。通过将分析从正交格扩展到$L_q(\mathbf{A}^{T})$，其中$\mathbf{y}$不限于$\mathbf{0}$，减少了所需的LWE样本数量。
>
> **达到效果:** 研究结果表明，在更广泛的参数范围内，基于FFT的双攻击方法的有效性得到了证明，并且在某些情况下可以显著减少所需的数据量。
>
> **技术梗概:** 通过几何分析和模切换技术，本文对双攻击方法进行了形式化处理，从而能够对其复杂度进行精确估计。

---
### [推荐] [2026/2190] Stay in Your Lane: Fast Arithmetic over Large Finite Rings with CKKS

- **匹配关键字:** LWE

- **作者:** Hyeongmin Choe, Robin Köstler, Tim Seuré

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2190) | [PDF](https://eprint.iacr.org/2026/2190.pdf)


> **研究背景:** 该研究基于CKKS方案，探索了在保持CKKS原生多项式环的基础上，通过分解模环来支持更广泛的整数环运算，从而简化同态计算并提高效率。
>
> **主要贡献:** 作者提出了一种新的方法，将GBFV的密文解释为CKKS的密文，并利用多项式的分解技术，使得在保持原生多项式环的同时能够处理多种不同的整数环。
>
> **达到效果:** 通过这种方法，研究达到了在不增加额外编码线性变换的情况下，实现快速且高效的同态运算和重新加密的效果。特别是在$N=2^{16}$时，实现了小于4.5秒的计算时间。
>
> **技术梗概:** 该技术利用了多项式环的分解，并结合GBFV密文结构，在CKKS原生环中进行算术运算和重新加密操作，减少了额外编码步骤带来的复杂性。

---
### [推荐] [2026/2194] A Black-Box Impossibility Result for Threshold Signing Standard Hash-Based Signatures

- **匹配关键字:** post-quantum

- **作者:** Yashvanth Kondi, Naman Kumar

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2194) | [PDF](https://eprint.iacr.org/2026/2194.pdf)


> **研究背景:** 研究背景：哈希基础签名方案因其假设保守性和高效性而备受青睐，但在与阈值签名的兼容性方面存在空白，尤其是在向后量子密码学迁移的背景下，这使得依赖于阈值签名的应用前景不明。
>
> **主要贡献:** 主要贡献：作者开发了一种新的分析框架，并证明了设计高效（完全分布式的）阈值签名协议存在固有的障碍。
>
> **达到效果:** 达到的效果：研究结果表明，对于某些哈希基础方案，当参与方超过一定数量时，必须使用哈希函数的电路表示形式进行阈值签名，而不能将其作为黑盒使用。
>
> **技术梗概:** 技术梗概：作者通过将哈希基础签名与随机Oracle模型中的直线提取文献联系起来，并借鉴Doerner等人（Crypto '24）的工作方法，展示了这些限制。

---
### [推荐] [2026/2195] Multi-Key FHE Almost as Fast as Single-Key FHE

- **匹配关键字:** homomorphic encryption, LWE

- **作者:** Abtin Afshar, Rishab Goyal

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2195) | [PDF](https://eprint.iacr.org/2026/2195.pdf)


> **研究背景:** 研究背景：现有的多密钥全同态加密（MKFHE）方案在进行同态计算时，成本与用户数量线性相关。López-Alt等人提出了这一问题，并指出需要一个独立于用户数量的同态评估方案。
>
> **主要贡献:** 主要贡献：本文首次构建了一个同态评估成本理论上与单密钥全同态加密（FHE）相当的多密钥全同态加密方案，同时保持了独立生成密钥和选择参与用户的特性。
>
> **达到效果:** 达到的效果：该方案在进行同态计算时，所有电路元素的成本不再依赖于用户数量N，实现了与单密钥FHE相近的效率。
>
> **技术梗概:** 技术梗概：通过引入一个公共、确定性和非交互式的预处理阶段，将选定的公钥聚合，并将独立加密的输入转换为基于虚拟密钥（即参与秘密密钥之和）的短单密钥密文，从而使得评估过程可以使用普通的单密钥密文进行。

---
### [推荐] [2026/2198] Lifting Symmetries for Dimension Reduction in Module-LIP: rank 2 and rank 4

- **匹配关键字:** lattice

- **作者:** Hengyi Luo, Kaijie Jiang, Yanbin Pan

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2198) | [PDF](https://eprint.iacr.org/2026/2198.pdf)


> **研究背景:** 研究背景：模块格同构问题（module-LIP）在HAWK密钥恢复中被广泛研究，现有算法将$2n$维问题减少为多项式数量的精确SVP调用，但高秩模块-LIP尚未得到充分探索。
>
> **主要贡献:** 主要贡献：提出了利用对称平方进行降维的方法，在秩四情况下获得了与现有方法相当的效果，并通过共轭霍奇星算子从外积空间中构造了一个同构于$\mathbb{Z}^{3n}$的公开可计算固定格。
>
> **达到效果:** 达到的效果：在秩二和秩四的情况下，新方法匹配了当前最优的结果，为高秩模块-LIP的研究提供了新的视角和技术手段。
>
> **技术梗概:** 技术梗概：通过分析对称平方结构，利用共轭霍奇星算子从外积空间中构造了一个同构于$\mathbb{Z}^{3n}$的格，并基于此进行维度缩减。

---
### [2026/2092] Indifferentiability of the Sum of Two Permutations: Tight 3n/4 Security for the Uniform Simulator and a New Simulator for 4n/5 Security

- **作者:** Sunyeop Kim

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2092) | [PDF](https://eprint.iacr.org/2026/2092.pdf)


> **研究背景:** 研究探讨了两个n位置换的和的安全性界限，旨在改进现有模拟器的安全性能。
>
> **主要贡献:** 提出了一个名为Gyroscope的新模拟器，实现了比之前更高的安全性。
>
> **达到效果:** 证明了对于均匀模拟器达到3n/4比特的安全性，并通过新模拟器达到了4n/5比特的攻击阈值。
>
> **技术梗概:** 通过跟踪未揭示构造值的概率分布中的偏差来限制KL散度，引入Gyroscope模拟器调整逆查询接受概率以补偿早期响应留下的偏差。

---
### [2026/2097] End-to-End Hard-Label Cryptanalytic Model Extraction Using Efficient Sign Recovery

- **作者:** Akira Ito, Takayuki Miura, Yosuke Todo

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2097) | [PDF](https://eprint.iacr.org/2026/2097.pdf)


> **研究背景:** 研究背景：深度神经网络（DNN）的参数被视为有价值的资产，因此如何在不直接访问模型内部结构的情况下提取这些参数成为一个重要的研究方向。现有的攻击方法主要针对硬标签设置，在这种情况下，对手只能观察到最终的输出标签。
>
> **主要贡献:** 主要贡献：提出了一种新的基于不同原理的签名恢复算法，无需专门查询即可实现高效的签名恢复。
>
> **达到效果:** 达到的效果：该方法在实验中实现了比现有方法更高的签名恢复准确性，并且能够完全黑盒地进行硬标签模型提取的所有步骤，从而首次实现了针对训练好的ReLU MLP模型的端到端黑盒攻击演示。
>
> **技术梗概:** 技术梗概：通过设计一种全新的算法来解决签名恢复问题，该算法能够在不增加额外查询成本的情况下提高签名恢复的准确性，并且能够完全在黑盒环境中实现整个模型提取过程。

---
### [2026/2099] Succinct Arguments for QMA in the Quantum Random Oracle Model

- **作者:** Alessandro Chiesa, Zihan Hu

- **分类:** Foundations

- **关键词:** succinct arguments for QMA, quantum random oracle model, quantum interactive oracle proofs

- **链接:** [论文](https://eprint.iacr.org/2026/2099) | [PDF](https://eprint.iacr.org/2026/2099.pdf)


> **研究背景:** 研究背景：简洁论证是用于通过少量通信验证计算声明的基本密码学工具。在经典设置中，可以从无结构的困难性（如哈希函数）构造NP的简洁论证，但已知的QMA简洁论证依赖于特定的加密假设或量子PCP猜想。
>
> **主要贡献:** 主要贡献：首次在量子随机预言模型下构建了无需额外加密假设的QMA简洁论证，展示了理想哈希函数不仅足以支持NP的简洁论证，也适用于QMA。
>
> **达到效果:** 达到的效果：证明了可以从无结构的困难性中构造出QMA的简洁论证，为量子计算中的验证问题提供了新的解决方案。
>
> **技术梗概:** 技术梗概：通过一个保持效率的转换将量子交互预言证明（QIOP）转化为量子论证，并在量子随机预言模型下构建了可局部开窗的提取向量承诺机制作为关键步骤。

---
### [2026/2101] Interactive Secret-Key PIR

- **作者:** Nir Bitansky, Geoffroy Couteau, Noam Mazor

- **分类:** Foundations

- **关键词:** Private information retrieval, Random oracle model

- **链接:** [论文](https://eprint.iacr.org/2026/2101) | [PDF](https://eprint.iacr.org/2026/2101.pdf)


> **研究背景:** 在私有信息检索（PIR）中，通常需要使用公钥密码学。然而，近期的研究表明，在秘密密钥PIR中，客户端可以先预处理一个N位数据库并仅保留一个短的密钥，从而可能避免这种障碍。
>
> **主要贡献:** 本文展示了交互性可以在随机预言机模型中实现对数通信量。具体而言，作者构建了一个具有O(log N)轮次和多项式对数总通信的秘密密钥PIR协议，并且对于任何常数ε，可以得到一个常数轮次的协议，其通信量为O(N^ε)。
>
> **达到效果:** 在随机预言机模型下，该研究实现了秘密密钥PIR协议的交互性，使得通信量从N^{1/2}压缩到多项式对数级别。此外，在最弱版本的LPN假设下（最大噪声率为1/2 - o(1)），也达到了相似的效果。
>
> **技术梗概:** 本文的主要创新在于借鉴了电路加密中免费-XOR技术的思想，使得秘密密钥预处理在XOR下同态，并利用相关密钥攻击的安全性来实现这一目标。

---
### [2026/2104] On the Complexity of Statistical and Multiplicative Zero-Knowledge

- **作者:** Daniel Alabi, Marshall Ball

- **分类:** Foundations

- **关键词:** Zero-Knowledge, Computational Complexity

- **链接:** [论文](https://eprint.iacr.org/2026/2104) | [PDF](https://eprint.iacr.org/2026/2104.pdf)


> **研究背景:** 研究了统计零知识（$\mathsf{SZK}$）与乘法零知识（$\mathsf{MZK}$）之间的复杂性差异，其中$\mathsf{MZK}$通过放宽实验证据视图之间的统计距离要求来提高效率。
>
> **主要贡献:** 论文探讨了在保持一定误差的情况下，$\mathsf{MZK}$的计算能力，并证明了其与$\mathsf{PSPACE}$的关系以及与$\mathsf{SZK}$的区别。
>
> **达到效果:** 研究结果表明，在特定条件下，$\mathsf{MZK}$可以包含$\mathsf{SZK}$且具有更强的计算能力；同时，通过泄漏协议分离了$\mathsf{MZK}$和$\mathsf{SZK}$之间的界限。
>
> **技术梗概:** 使用了多项式泄漏、误差容忍度以及复杂性假设来分析零知识证明系统的性质。

---
### [2026/2105] Actively Secure PPML with Soft-Boundary Overflow Detection

- **作者:** Yuntian Chen, Tianpei Lu, Keyu Ji, Zhiyuan Ning, Zhanyong Tang, Cong Wang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2105) | [PDF](https://eprint.iacr.org/2026/2105.pdf)


> **研究背景:** 现有的PPML方法在有限环上操作，易受算术溢出风险的影响，而现有检测技术通常需要评估非线性函数，导致高计算和通信开销。
>
> **主要贡献:** 提出了一种Soft Boundary Detection机制，将边界验证简化为分区区间上的概率函数，成本与线性计算相当，并自然扩展到恶意攻击者场景。
>
> **达到效果:** 该方法显著提高了安全性和效率，在实际应用中提供了稳健且主动安全的PPML解决方案。
>
> **技术梗概:** 通过将溢出检测转化为概率函数处理，降低了复杂度并减少了资源消耗，同时保持了系统的安全性。

---
### [2026/2106] Couplet: Multivariate Polynomial Commitments with Two-Group-Element Openings

- **作者:** Weijie Wang, Alin Tomescu, Rex Fernando, Charalampos Papamanthou

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2106) | [PDF](https://eprint.iacr.org/2026/2106.pdf)


> **研究背景:** Couplet是一种基于配对的新型多元多项式承诺方案，旨在提供高效的验证机制，并支持多点批量评估证明，而不使用随机预言机。
>
> **主要贡献:** 该方案首次实现了仅需两个群元素即可完成开销证明的多元多项式承诺方案，并且在处理多线性多项式时，证明者所需的操作数量为亚线性的。
>
> **达到效果:** Couplet通过一个准线性、多项式特定的一次预处理阶段实现上述特性，该阶段的成本可以在多次后续验证中摊薄。在BN254上，经过预处理后，它能够以约20倍的速度打开具有20个变量的多线性多项式的开销证明。
>
> **技术梗概:** Couplet的主要技术在于压缩Papamanthou--Shi--Tamassia (PST)评估证明，生成一个承诺，该承诺编码了通常在PST证明中单独给出的所有对数n次商多项式。通过这种方法，将验证问题简化为单变量求和检验问题，并使用Groth16技术仅额外使用一个群元素来解决。

---
### [2026/2107] Enhancing SAT Solving to Find (Near) Collisions in 6-Round SHA-3 Variants

- **作者:** Yi Tu, Ling Song, Huaijin Wu, Jian Guo, Jian Weng, Chaoping Xing

- **分类:** Attacks and cryptanalysis

- **关键词:** symmetric-key, analysis

- **链接:** [论文](https://eprint.iacr.org/2026/2107) | [PDF](https://eprint.iacr.org/2026/2107.pdf)


> **研究背景:** 针对Keccak哈希函数设计的SHA-3标准，现有攻击已能实现五轮碰撞攻击，但六轮SHAKE128仅存在理论上的攻击成本为$2^{123.5}$次评估。本文旨在首次提出经典条件下对六轮SHAKE128的实际碰撞攻击。
>
> **主要贡献:** 作者通过引入参数化的碰撞路径SAT模型和增强的启发式策略，成功实现了六轮SHAKE128的碰撞攻击，并报告了六轮SHA3-224和SHA3-256接近碰撞以及四轮和五轮SHAKE256的实际碰撞。
>
> **达到效果:** 该研究显著降低了六轮SHAKE128的碰撞攻击成本，总复杂度约为$2^{47.82}$；同时发现了其他相关哈希函数实例的新类型攻击。
>
> **技术梗概:** 采用Dinur框架和Guo等人开发的SAT工具，结合差异路径编码、优化连接器阶段的技术（如最小化第二轮和第三轮的传播成本，并惩罚DDT=2过渡），以及通过位粒度贪婪算法选择候选χ兼容方程来最大化自由度。

---
### [2026/2108] $\Delta$-KLPT$^+$: Improved Norm Bounds for the Quaternion Isogeny Path Problem and an Application to Ring Signatures

- **作者:** Kohei Nakagawa, Hiroshi Onuki

- **分类:** Cryptographic protocols

- **关键词:** Isogeny, Quaternion, KLPT, SQIsign

- **链接:** [论文](https://eprint.iacr.org/2026/2108) | [PDF](https://eprint.iacr.org/2026/2108.pdf)


> **研究背景:** 研究背景：计算具有已知端元环的超奇异椭圆曲线之间的光滑度等势线是基于同构的密码学中的基本任务。KLPT型算法通过将已知连接理想转换为等价的理想来解决此问题，但现有算法对于一般输入只能达到约$\widetilde{O}(p^{4.5})$的规范。
>
> **主要贡献:** 主要贡献：提出了一种新的KLPT型算法$\Delta$-KLPT$^+$，将连接理想的规范边界降低到$\widetilde{O}(p^4)$，并基于此构建了Delfar同签方案。
>
> **达到效果:** 达到的效果：与Erebor-full相比，Delfar实现了更短的签名和更低的签署及验证成本，尤其在大环中表现突出。
>
> **技术梗概:** 技术梗概：通过引入新的子程序IdealNormReduce来降低理想对的共同规范，并迭代应用该子程序以获得所需连接理想的光滑规范。

---
### [2026/2110] Forkcipher-Based Committing Security from Explicit Collision Security Assumptions

- **作者:** Elena Andreeva, Maria Eichlseder, Simon Gerhalter, Marcel Nageler, Oliver Christoph Popa, Andreas Weninger

- **分类:** Secret-key cryptography

- **关键词:** Committing Security, Forkciphers, PRFs

- **链接:** [论文](https://eprint.iacr.org/2026/2110) | [PDF](https://eprint.iacr.org/2026/2110.pdf)


> **研究背景:** 研究背景：现有的认证加密方案通常缺乏将密文与其加密上下文唯一绑定的能力，而通用的承诺转换依赖于理想化的组件，这限制了它们的实际应用。
>
> **主要贡献:** 主要贡献在于从显式的分叉密码碰撞抗性属性出发，提出了基于分叉密码的承诺认证加密方案，并通过FCPRF和FixM将其提升为固定长度的碰撞抵抗伪随机函数。
>
> **达到效果:** 达到了在标准模型下实现保密性、完整性和CMT-4安全性的目标，并构造了AEaH-2K和FCTR-CMT两种模式，分别利用两个密钥和单一分叉密码实现了上述安全性。
>
> **技术梗概:** 技术梗概：通过MILP和SAT方法对分叉密码进行了差分和减少轮数的碰撞分析，支持其通用安全级别，并引入了基于nonce的伪随机函数安全性来捕捉所需的非重用伪随机性。

---
### [2026/2111] Computing C(N, R) mod m for Arbitrary Composite Moduli and N <= 10^18: A Prime-Power Decomposition Engine with Two Memory Regimes and Division-Free Arithmetic

- **作者:** Vadik Malik, Rudr Pratap, Sarthak Vashishtha

- **分类:** Implementation

- **关键词:** number theory, modular arithmetic, Montgomery multiplication, Hensel lifting, algorithm engineering

- **链接:** [论文](https://eprint.iacr.org/2026/2111) | [PDF](https://eprint.iacr.org/2026/2111.pdf)


> **研究背景:** 研究背景：二项式系数模整数运算$\binom{N}{R} \pmod m$是组合计数的基本操作，但传统的两种方法在大规模计算中失效。
>
> **主要贡献:** 主要贡献：提出了一种针对任意合数模数$m < 2^{62}$和$N < 2^{64}$的完整、正确引擎，解决了二项式系数模整数运算的问题。
>
> **达到效果:** 达到的效果：该方法能够在有限内存下高效计算二项式系数模复合模数的结果，并且无需预先知道系数的具体值。
>
> **技术梗概:** 技术梗概：通过素因子分解和Pollard-Brent rho算法，结合Kummer的进位计数、Gauss广义威尔逊定理以及中国剩余定理实现了高效的计算过程。

---
### [2026/2112] Mist: Transparent Polynomial Commitments from Class Group Subset-Product Accumulators

- **作者:** Sriram Sridhar

- **分类:** Cryptographic protocols

- **关键词:** SNARK, Polynomial Commitment Scheme, Groups of Unknown Order

- **链接:** [论文](https://eprint.iacr.org/2026/2112) | [PDF](https://eprint.iacr.org/2026/2112.pdf)


> **研究背景:** 本文构建了一种无需信任设置、验证者时间接近线性且开销恒定的多项式承诺方案，填补了该领域的空白。
>
> **主要贡献:** 主要贡献在于实现了类群子集积攒器关系，支持固定类群元素的积累及证明其代数关系，从而实现恒定大小范围论证。
>
> **达到效果:** 结合DewTwo框架后，首次提供了验证者时间接近线性且开销恒定的透明多项式承诺方案，并给出最短已知开销证明（1.3 KB）。
>
> **技术梗概:** 技术上采用了类群子集积攒器关系和DewTwo框架，通过新颖的二进制分数提取方法确保安全性。

---
### [2026/2115] Fast Quantum-Circuit Superoptimization

- **作者:** Aws Albarghouthi

- **分类:** Implementation

- **关键词:** quantum computing, quantum circuits, optimization, compilers

- **链接:** [论文](https://eprint.iacr.org/2026/2115) | [PDF](https://eprint.iacr.org/2026/2115.pdf)


> **研究背景:** 量子电路优化至关重要，以确保它们适合量子计算机的资源限制，并减少不必要的操作以降低失败概率。
>
> **主要贡献:** 作者提出了一种新的快速量子电路优化算法，能够处理大量操作并达到或超过现有最佳优化器和超级优化器的效果。
>
> **达到效果:** 该方法实现了显著的电路简化，运行速度快得多，并且能够扩展到包含数百万操作的电路。
>
> **技术梗概:** 通过构建最小单元表示映射（MURMs），算法能够在线检查每个子电路并用更小的等效电路替换它，利用增量计算和符号算术确保每次替换快速且准确。

---
### [2026/2116] Smooth evaluations of cyclotomic quadratics

- **作者:** Bruno Sterner

- **分类:** Foundations

- **关键词:** Smooth integers, cyclotomic polynomials, integer factorisation

- **链接:** [论文](https://eprint.iacr.org/2026/2116) | [PDF](https://eprint.iacr.org/2026/2116.pdf)


> **研究背景:** 研究背景：本文旨在寻找特定形式的素数p，使得其对应的cyclotomic多项式的值$\Phi_k(p)$在较小的光滑度界B下是B-光滑的。这为通过传统算法易于分解的整数因式分解挑战提供了新的实例化方法。
>
> **主要贡献:** 主要贡献：作者提出了一个有效的方法来寻找满足上述条件的素数p，特别地针对$k=3,4$和$6$的情况。
>
> **达到效果:** 达到的效果：通过这种方法，首次实现了理论上难以分解但实际上却可以用传统算法轻松分解的整数因式分解挑战实例化。
>
> **技术梗概:** 技术梗概：利用特定形式的cyclotomic多项式的性质，结合光滑度的概念，提出了寻找满足条件素数p的有效策略。

---
### [2026/2119] CKKS Bootstrapping in less than 10ms

- **作者:** Rending Ouyang, Jian Liu, Peimin Gao, Pan Xiao, Heng Zhang, Man Ho Au, Cong Zhang, Kui Ren, Chun Chen

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2119) | [PDF](https://eprint.iacr.org/2026/2119.pdf)


> **研究背景:** CKKS加密方案通过补丁操作实现深度同态计算，但该操作是整个方案中最耗时的部分。
>
> **主要贡献:** 作者提出了一种新的CKKS补丁构造方法，结合了SHIP技术，并引入了多项算法创新以减少通信和计算开销，支持高效并行执行。
>
> **达到效果:** 实验结果表明，单个复数密文的补丁操作耗时8.2毫秒，对于64个密文批次，平均耗时降至4.6毫秒，实现了接近HEAAN报告的性能。
>
> **技术梗概:** 该方法基于SHIP技术，并通过减少通信和计算开销以及支持高效并行执行来提高补丁操作的效率。

---
### [2026/2122] Continuum: Concretely Efficient Asynchronous Dynamic MPC with Guaranteed Output Delivery

- **作者:** Wenxuan Yu, Minghui Xu, Yuan Lu, Yingzi Gao, Xiuzhen Cheng

- **分类:** Cryptographic protocols

- **关键词:** Secure multiparty computation, Fluid MPC, asynchronous protocol, guaranteed output delivery

- **链接:** [论文](https://eprint.iacr.org/2026/2122) | [PDF](https://eprint.iacr.org/2026/2122.pdf)


> **研究背景:** 传统的安全多方计算（MPC）协议假设参与方在计算过程中保持固定不变。动态MPC允许参与者在过程中的加入或退出，但在异步互联网环境中实现具有保证输出交付（GOD）的动态异步MPC面临挑战。
>
> **主要贡献:** Continuum通过引入聚合切换协议和批量乘法协议，解决了前人设计中的主要实际效率问题，实现了非交互式的计算阶段，并将交互集中在高效的切换阶段。
>
> **达到效果:** 实验结果表明，与先前的设计相比，在22个地理位置分散的服务器上执行深度6宽度100的电路时，Continuum将线性门传输延迟降低了80%，乘法门评估时间减少了93%。
>
> **技术梗概:** 该协议采用了聚合切换协议以确保各阶段之间的转移一致性，并去除了每阶段的遮罩随机性；基于BGW范式的批量乘法协议则推迟了次数减少并在切换阶段进行所有乘法验证的汇总证明。

---
### [2026/2123] Two Standard Deviations Are Necessary for the Kadison-Singer Problem

- **作者:** Zhao Song, Song Yue

- **分类:** Foundations

- **关键词:** Kadison-Singer, Discrepancy, Lower Bound

- **链接:** [论文](https://eprint.iacr.org/2026/2123) | [PDF](https://eprint.iacr.org/2026/2123.pdf)


> **研究背景:** 该研究旨在解决Kadison-Singer问题的矩阵不等式形式，通过引入随机变量和符号的选择来量化向量组之间的差异。
>
> **主要贡献:** 作者证明了常数$C_1$的下界为2，并且$C_0$不超过2.176，改进了先前的研究结果。
>
> **达到效果:** 研究结果表明，对于给定的向量集和随机变量支持，可以找到合适的符号组合使得矩阵范数满足特定条件，从而更接近问题的最佳解。
>
> **技术梗概:** 通过引入随机变量和支持集的概念，并利用概率方法和算子范数来分析矩阵之间的关系，证明了新常数的边界。

---
### [2026/2126] Cryptanalysis of LDVR: Polynomial-Time Adaptive Attacks against Threshold Schnorr Signatures

- **作者:** Àlex Rodríguez García, Stefano Tessaro

- **分类:** Cryptographic protocols

- **关键词:** Threshold Signatures, Digital Signatures, Cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2126) | [PDF](https://eprint.iacr.org/2026/2126.pdf)


> **研究背景:** 研究背景：论文针对低维向量表示（LDVR）问题的特定参数选择，提出了首个多项式时间适应性攻击方法。该问题由Crites等人引入，旨在证明FROST等阈值签名方案在代数组模型下的自适应安全性。
>
> **主要贡献:** 主要贡献在于首次展示了如何利用多项式时间复杂度对LDVR问题进行攻击，并进一步推导出针对暴露给攻击者密钥份额g^sk_i的阈值Schnorr签名方案的适应性攻击方法。
>
> **达到效果:** 达到的效果是提供了首个针对实际应用中阈值签名方案的多项式时间自适应攻击，这些方案包括FROST、Sparkle等无遮罩协议。实验结果表明，在Curve25519子群上需要5020个签名者和204个被攻破。
>
> **技术梗概:** 技术梗概：通过改进Crites等人先前的工作，利用多项式数量的签名者进行攻击，并通过理论分析确定了攻击所需的签名者数量。

---
### [2026/2127] Polynomial Time Cryptanalytic Extraction of Deep Neural Networks in the Limited Architecture Knowledge Setting

- **作者:** Yiqing Li, Gaoli Wang

- **分类:** Attacks and cryptanalysis

- **关键词:** ReLU-Based Deep Neural Networks, Neural Network Extraction, Limited Architecture Knowledge Attack

- **链接:** [论文](https://eprint.iacr.org/2026/2127) | [PDF](https://eprint.iacr.org/2026/2127.pdf)


> **研究背景:** 研究背景：深度神经网络(DNNs)作为现代AI系统的核心，其内部参数被视为重要的知识产权。因此，DNNs的安全性受到广泛关注，尤其是针对模型提取攻击的研究。现有的多项式时间密码分析方法已经证明，在完全了解目标网络架构的情况下，可以恢复DNN的参数。然而，这些方法在有限的架构知识下尚未得到验证。
>
> **主要贡献:** 主要贡献：本文提出了一个适应性的提取工作流，并结合了一个新颖的错误检测算法，能够在未知隐藏层结构的情况下成功地恢复正确的参数。这是首次在有限的架构知识设置中从CIFAR-10数据集上成功提取一个包含超过一百万个参数的四隐藏层ReLU网络。
>
> **达到效果:** 达到的效果：通过实验，在完全不知道目标网络具体架构的情况下，实现了对一个大型DNN模型的有效参数恢复，并且能够检测出缺失神经元的问题，保证了错误检测的全面性。
>
> **技术梗概:** 技术梗概：该研究采用了一种适应性的提取方法和错误检测算法来应对有限的架构知识挑战，通过识别并填补缺失的神经元信息，实现了对DNN参数的有效恢复。

---
### [2026/2133] How to Fold Linear Error-Correcting Codes with Optimal Proximity Gaps

- **作者:** Zhe Li, Chaoping Xing, Yizhou Yao, Chen Yuan, Ruiqi Zhu

- **分类:** Cryptographic protocols

- **关键词:** optimal proximity gaps, interactive oracle proofs of proximity, folded Reed-Solomon codes, univariate multiplicity codes

- **链接:** [论文](https://eprint.iacr.org/2026/2133) | [PDF](https://eprint.iacr.org/2026/2133.pdf)


> **研究背景:** 研究背景：折叠技术是构建高效多项式承诺方案的关键，这些方案具有polylogarithmic的证明大小和验证。目前仅两种线性纠错码允许折叠，但均未达到最优接近间隙，这成为结果证明尺寸的基本瓶颈。
>
> **主要贡献:** 主要贡献：首次提出了具有最优接近间隙的折叠方案，构建了适用于折迭RS（FRS）代码和一元多重（UM）代码的交互式近邻证明系统（IOPP），实现了最优接近间隙$1-R-\varepsilon$。
>
> **达到效果:** 达到的效果：该工作改进了FRS和UM代码的最优接近间隙分析，提供了更紧致的具体可信度边界，并且IOPPs具有$O(N)$的验证者时间和oracle证明大小，以及$O(\lambda\log{N})$的查询复杂性和验证时间。
>
> **技术梗概:** 技术梗概：通过观察到适当的模数配对下偶/奇折叠与多项式减少的一致性检查可以通过中国剩余定理实现，提出了新的通用折叠框架，恢复了FRS代码的FRI折叠，并使UM代码高效折叠成为可能。

---
### [2026/2138] DASH: Distributed Asynchronous Schnorr with High Throughput

- **作者:** Jiawei Ni, Yanhong Xu, Chen Yuan, Hongqing Liu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2138) | [PDF](https://eprint.iacr.org/2026/2138.pdf)


> **研究背景:** DASH旨在改进现有的高吞吐量门限 Schnorr 签名方案，如SPRINT，以减少在线签名时的计算开销，同时保持SPRINT的通信成本优势。
>
> **主要贡献:** DASH通过分布式计算和批量验证技术，将每个节点每轮次的平均群运算次数从O(log n)降低到O(1)，并提出了一种联合验证多项式份额的方法。
>
> **达到效果:** 该方案显著提高了签名效率，同时保持了SPRINT的通信成本优势，适用于需要高效分布式签名的应用场景。
>
> **技术梗概:** DASH通过将群运算分散给各个节点处理，并采用批量验证技术来减少计算瓶颈，从而实现了高效的门限 Schnorr 签名。

---
### [2026/2140] Multi-Reprogramming with Quantum Advice in the Quantum Random Oracle Model

- **作者:** Yu-Hsuan Huang, Heming Liao

- **分类:** Foundations

- **关键词:** QROM, quantum advice, adaptive reprogramming, Fiat-Shamir, time-space tradeoff

- **链接:** [论文](https://eprint.iacr.org/2026/2140) | [PDF](https://eprint.iacr.org/2026/2140.pdf)


> **研究背景:** 研究背景：现有技术在量子随机预言机模型（QROM）中，对于适应性重编程的证明主要局限于经典建议，并且仅支持单一重编程。这限制了其在实际应用中的灵活性和效率。
>
> **主要贡献:** 主要贡献：本文提出了一个能够处理量子建议的适应性重编程定理，该定理的安全损失更小，与重编程次数的三次根成比例，从而显著提高了理论上的安全性边界。
>
> **达到效果:** 达到的效果：通过应用新的重编程定理，论文改进了伪随机生成器（PRG）安全性和盐化伪一次性密钥的安全证明，并为量子建议设置下的Fiat-Shamir和哈希签名方案提供了通用的安全分析框架。
>
> **技术梗概:** 技术梗概：该研究采用了两步模块化分析方法，首先通过重编程定理进行通用UF-CMA到UF-NMA的归约，然后确定满足UF-NMA安全性的条件，并讨论了多个方案的安全性。

---
### [2026/2141] BitZ: proofs and commitments in arbitrary rings through binary fields

- **作者:** Remco Bloemen, Albert Garreta, Marcin Kostrzewa, Shreyas Londhe, John Wu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2141) | [PDF](https://eprint.iacr.org/2026/2141.pdf)


> **研究背景:** BitZ是一种基于哈希的多项式承诺方案，用于在任意生成环中对多线性多项式进行承诺，并支持通过环同态证明在另一个环上的评估声明。
>
> **主要贡献:** BitZ的主要贡献在于它能够以几乎仅依赖于系数位数的成本，在任何生成环上实现多项式的承诺和验证，同时提供免费的范围检查功能。
>
> **达到效果:** 使用BitZ构建了针对整数多项式约束的SNARK（BitZ-SNARK），并通过指纹技术实现了跨越不同环的有效证明机制。
>
> **技术梗概:** BitZ通过将待承诺的多项式分解为位串，并在二进制域中以打包形式进行承诺，然后证明目标环上的线性声明来实现其功能。

---
### [2026/2143] SAK: Sparse Arguments of Knowledge, from Sparse Lookup Arguments

- **作者:** Abhiram Kothapalli, Sriram Sridhar, Arantxa Zapico

- **分类:** Public-key cryptography

- **关键词:** Sparse SNARKs

- **链接:** [论文](https://eprint.iacr.org/2026/2143) | [PDF](https://eprint.iacr.org/2026/2143.pdf)


> **研究背景:** 该研究旨在设计一种适用于稀疏证明的论证知识系统，以提高在非零值较少的情况下验证效率。
>
> **主要贡献:** 作者提出了Sparse Arguments of Knowledge (SAK)，特别针对可定制约束系统的稀疏实例进行了优化。
>
> **达到效果:** 通过这种技术，实现了O(N log N)预处理时间、O(n log n)证明者时间和O(1)验证者时间的效果，显著提升了验证效率。
>
> **技术梗概:** 研究采用了子线性查找论证来选择与非零值相关的约束系统部分，并结合标准的知识论证方法，从而实现高效验证。

---
### [2026/2144] Algorithms for Computing Affine Equivalence and Self-Equivalence Groups

- **作者:** Joakim Sunde

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2144) | [PDF](https://eprint.iacr.org/2026/2144.pdf)


> **研究背景:** 研究探讨了函数$F,G:\mathbb{F}_q^n\to\mathbb{F}_q^m$之间的仿射和线性等价问题，特别是针对具有非平凡自等价的结构化实例，这些实例在密码学中广泛存在。
>
> **主要贡献:** 作者提出了实用的等价算法，能够处理任意函数（包括任何代数阶数、置换和非置换），特别适用于具有非平凡自等价的结构化实例，并提供了一种专门用于计算满足$A_2\circ F = F\circ A_1$的自等价组生成元的算法。
>
> **达到效果:** 这些算法在处理复杂函数时表现出色，能够有效识别和分类仿射等价类，对于密码学中的多项式函数具有重要意义。
>
> **技术梗概:** 通过结合代数几何和群论的方法，作者设计了高效的搜索策略来找到满足特定条件的自等价变换，并利用这些方法解决了以往缺乏高效通用算法的问题。

---
### [2026/2145] Fully Anonymous Perfect Secret-Sharing

- **作者:** Roni Con, Ray Li, Noam Mazor

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2145) | [PDF](https://eprint.iacr.org/2026/2145.pdf)


> **研究背景:** 该研究解决了非平凡精确门限参数(2 < t < n)下完全匿名完美秘密共享方案的存在性问题，特别是在t=3时的具体实现。
>
> **主要贡献:** 作者提供了首个针对t=3的显式完全匿名方案，并证明了对于任意1 ≤ t ≤ n存在一个一比特秘密的完全匿名完美(t,n)门限秘密共享方案。
>
> **达到效果:** 该研究首次实现了最小未解决门限参数下的完全匿名完美秘密共享，且给出了具体的一比特秘密方案；进一步证明了一般情形下门限的秘密共享方案的存在性。
>
> **技术梗概:** 通过构造特定的分享和恢复算法来实现完全匿名性和精确恢复性，并使用了对数级长度的分享以保证效率。

---
### [2026/2147] ReedWeave: Faster Reed-Solomon Polynomial Commitments from Interleaving and Folding

- **作者:** Yuhao Jia, Zhe Li, Chaoping Xing, Yizhou Yao, Chen Yuan, Jielong Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2147) | [PDF](https://eprint.iacr.org/2026/2147.pdf)


> **研究背景:** 多项式承诺方案（PCSs）允许证明者提交一个多项式并在之后简洁地证明其评估值。FRI及其后续工作通过递归地'折叠'Reed-Solomon码实现了多项式的聚对数证明，但实际评估成本仍然较高。
>
> **主要贡献:** \emph{ReedWeave}结合了交织与折叠技术，实现了两者的优势。该方案将一个$d$次多项式分解为$m$个较小的分量，并对其RS编码进行承诺。
>
> **达到效果:** 通过这种新颖的分解方式，随机线性组合这些码字正好是$m$元折叠。ReedWeave在保持高效聚对数验证的同时，证明者的成本从$\mathcal{O}(d \log d)$降低到$\mathcal{O}(d\log(d/m))$（对于常数编码率），并且我们的方法适用于任意的$m$。
>
> **技术梗概:** ReedWeave 采用了一种新颖的方法，首先进行$m$元折叠，然后是标准二进制折叠。这种方法减少了证明者的成本，并通过交织RS编码而非标准RS编码来实现这一目标。

---
### [2026/2148] Weighted Threshold Encryption with Silent Setup

- **作者:** Guru-Vamsi Policharla

- **分类:** Cryptographic protocols

- **关键词:** Silent Setup, Batched, Threshold Encryption, Weighted

- **链接:** [论文](https://eprint.iacr.org/2026/2148) | [PDF](https://eprint.iacr.org/2026/2148.pdf)


> **研究背景:** 该研究旨在设计一种具有静默设置的加权门限加密方案，以提高多方协作环境下的隐私保护和灵活性。
>
> **主要贡献:** 贡献在于首次提出了非平凡的加权门限加密方案，并且实现了动态选择权重分配和解密阈值的功能。
>
> **达到效果:** 该方案在CPA安全性和CCA安全性方面表现出色，同时减少了公共参数的数量，提高了效率。
>
> **技术梗概:** 通过使用生成函数模型证明了方案的安全性，并优化了部分解密过程以减少计算成本。

---
### [2026/2150] Optimal Byzantine Atomic Broadcast with Message Drops

- **作者:** Dalin Zheng, Yuchen Ye, Xiaolin Gui, Qiang Tang, Zhenliang Lu

- **分类:** Cryptographic protocols

- **关键词:** Atomic Broadcast, Omission Fault, Byzantine Agreement

- **链接:** [论文](https://eprint.iacr.org/2026/2150) | [PDF](https://eprint.iacr.org/2026/2150.pdf)


> **研究背景:** 研究背景：现有文献在混合故障模型中探讨了拜占庭一致性的协议设计，但在所有节点持有相同输入的情况下才能保证有意义的输出这一假设下，如何高效构建原子广播协议仍然是一个开放问题。
>
> **主要贡献:** 主要贡献在于提出了首个具有最优抗故障能力的混合故障环境下原子广播协议UABC，并通过一种称为不死可验证信息分散（UVID）的新协议和未死多值共识技术，实现了在输入大小适度较大时接近最优的通信复杂度。
>
> **达到效果:** 达到的效果是，在混合故障模型下实现了最优抗故障能力的原子广播协议设计，且当输入大小 $\ell \ge n\lambda^2$ 时，通信复杂度达到了理论上最优水平。
>
> **技术梗概:** 技术梗概在于利用不死可验证信息分散（UVID）协议来高效地分散和恢复输入，并结合未死多值共识技术构建了新的弱不死原子广播协议。

---
### [2026/2152] Structural Analysis of Seven Hash Functions Submitted to the NGCC

- **作者:** Yufei Yuan, Ruichen Wu, Shanpeng Wei, Junxu Shen, Jinpeng Liu, Yixin Zhang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2152) | [PDF](https://eprint.iacr.org/2026/2152.pdf)


> **研究背景:** 本文基于ICCS发起的NGCC计划，对七种提交的哈希函数进行了结构分析。
>
> **主要贡献:** 研究者通过分析揭示了多种潜在的碰撞攻击路径和方法。
>
> **达到效果:** 研究表明，这些哈希函数在特定条件下存在碰撞风险，包括状态变化、初始化值影响及内部置换特性等。
>
> **技术梗概:** 采用消息差异分析法、状态演化追踪以及子空间不变性检测等技术手段进行研究。

---
### [2026/2154] Recovering SNOVA Secret Keys from Biased Vinegar Sampling

- **作者:** Ward Beullens, Basil Hess

- **分类:** Public-key cryptography

- **关键词:** Multivariate Crypto, UOV, SNOVA, LPN, BKW

- **链接:** [论文](https://eprint.iacr.org/2026/2154) | [PDF](https://eprint.iacr.org/2026/2154.pdf)


> **研究背景:** 研究背景：SNOVA签名方案在偏置的醋酸采样过程中存在信息泄露问题，特别是对于六个奇数特征的替代参数集。
>
> **主要贡献:** 主要贡献在于揭示了这种偏置导致的秘密密钥信息泄漏，并提出了有效的密钥恢复攻击方法。
>
> **达到效果:** 达到的效果是通过实践证明了这些攻击的有效性，使得攻击成本远低于官方声称的安全水平。
>
> **技术梗概:** 技术梗概采用了$q$-元LPN问题的解决策略，结合BKW归约和傅里叶假设检验等技术手段，实现了对四个参数集的实际密钥恢复。

---
### [2026/2156] Cryptanalysis of Goldreich's PRGs with MAJ–XOR Predicates

- **作者:** Tianning Wang, Haoyang Wang, Fukang Liu, Willi Meier

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2156) | [PDF](https://eprint.iacr.org/2026/2156.pdf)


> **研究背景:** 本文研究了Goldreich的伪随机生成器（PRG）在MAJ-XOR谓词下的具体和渐近安全性，特别是当本地谓词结合多数表决与异或操作时。
>
> **主要贡献:** 作者提出了Deterministic Pooled Recovery (DPR) 和 Bias-Amplified Pooled Recovery (BAPR) 两种方法来破解Goldreich的PRG，并证明了在某些参数设置下，这些方法可以有效降低攻击成本。
>
> **达到效果:** 对于提出的\(\operatorname{MAJ}_7 \oplus \operatorname{XOR}_4\)挑战，作者估计DPR的成本为\(2^{109.71}\)次操作，低于声称的128位安全性。此外，BAPR方法在特定参数设置下可以以亚指数时间复杂度恢复任何固定种子。
>
> **技术梗概:** 研究采用了猜测和验证的技术，通过池化输出并利用线性方程组来破解PRG；同时引入了偏置放大池化恢复技术来进一步提高攻击效率。

---
### [2026/2157] Jolt-QED: Formally Verifying Bytecode Expansions In Lean

- **作者:** Ari Biswas, Quang Dao, Daniel Ross, Justin Thaler

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2157) | [PDF](https://eprint.iacr.org/2026/2157.pdf)


> **研究背景:** 本文利用Lean 4证明助手验证Jolt zk-VM扩展程序能忠实模拟RISC-V CPU，旨在为后续阶段的验证奠定基础。
>
> **主要贡献:** 贡献在于将官方Sail规范中的RISC-V翻译成Lean，并在此基础上定义Jolt指令集架构，生成并验证了51个Jolt字节码扩展与相应RISC-V指令的一致性。
>
> **达到效果:** 结果显示，大部分Jolt字节码扩展程序在逻辑上等同于其对应的RISC-V指令，为更高级别的验证工作提供了坚实的基础。
>
> **技术梗概:** 通过构建生成器运行Jolt的Rust字节码扩展器，并将输出渲染为Lean程序进行形式化验证。

---
### [2026/2158] Formally Modeling the Terrapin Attack on SSH

- **作者:** Jörg Schwenk, Fabian Bäumer, Marcus Brinkmann

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2158) | [PDF](https://eprint.iacr.org/2026/2158.pdf)


> **研究背景:** 本文针对SSH通道完整性中的Terrapin攻击，提出了一个基于部分选择状态的正式模型。现有的安全信道形式化模型多基于状态加密，但未明确定义通道状态及其在不同AEAD模式中的应用。
>
> **主要贡献:** 作者提出了一种抽象的状态加密接口，并为八种最常用的SSH AEAD模式提供了伪代码描述。通过改变SND或acles，可以模拟仅密文攻击、已知明文攻击和选择明文攻击，从而确定这些模式的安全边界。
>
> **达到效果:** 研究发现，在仅密文模型中，所有三种Encrypt-then-MAC (EtM) 模式和SSH中的ChaCha20-Poly1305均不安全；AES-GCM在所有三种模型变体中都是唯一的安全算法。此外，还展示了Encrypt-and-MAC (EaM) 与CBC模式的安全性，并描述了一种针对这种方案的新型选择明文攻击。
>
> **技术梗概:** 通过设计一个抽象的状态加密接口来建模通道状态，并使用不同的或acles来模拟不同类型的选择攻击。

---
### [2026/2159] Batch IT-MAC: Program the Randomness in Succinct VOLE

- **作者:** Liqiang Liu, Tianren Liu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2159) | [PDF](https://eprint.iacr.org/2026/2159.pdf)


> **研究背景:** 许多密码协议依赖于形式为\({\boldsymbol\sigma}=\Delta{\boldsymbol x}+{\boldsymbol k}\)的线性消息认证标签，批量版本的信息论MAC。生成长向量的消息标签通常需要与其长度成线性的通信量。
>
> **主要贡献:** 作者引入了batch-MAC，允许验证者的关键向量从短种子预先生成，并且能够在两轮消息中认证选定的消息向量。
>
> **达到效果:** 在决策性合成残余性假设下，构造的batch-MACs通信量为\(O(m^{2/3}\lambda)\)；在学习误差假设下，通信量为\(\operatorname{poly}(\log m,\lambda)\)，同时需要公共参考字符串。
>
> **技术梗概:** 该技术利用了决策性合成残余性和学习误差假设来减少通信量，并通过公共参考字符串实现关键向量的预先生成。

---
### [2026/2160] Accordion: Efficient Batch Proving via Algebraic Folding

- **作者:** David Balbás, Anca Nitulescu, Maxime Plançon

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2160) | [PDF](https://eprint.iacr.org/2026/2160.pdf)


> **研究背景:** 本文提出了一种新的证明系统$\mathsf{Accordion}$，旨在提高批量证明的效率，特别是在需要验证多个独立实例的有效性的场景中。
>
> **主要贡献:** $\mathsf{Accordion}$包括两个关键组件：Accumulate和Decide。Accumulate通过非递归方式高效地将$k$个不同实例的多项式关系合并为一个单一实例；Decide则提供了一种高效的验证机制。
>
> **达到效果:** 实验结果表明，对于64个实例，$\mathsf{Accordion}$比单独计算64个Plonk证明快2.5倍以上。该方案显著减少了递归带来的开销。
>
> **技术梗概:** Accumulate基于Protostar和Protogalaxy的工作进一步优化了验证者效率；Decide则利用了Plonk基的证明以及KZG多项式承诺打开机制来实现高效的验证过程。

---
### [2026/2161] Extracting CNNs in the Unknown-Architecture and Feedback-Agnostic Setting

- **作者:** Jiashuo Liu, Ruijie Ma, Manman Li, Yi Chen, Shaozhen Chen

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2161) | [PDF](https://eprint.iacr.org/2026/2161.pdf)


> **研究背景:** 研究背景：现有针对卷积神经网络（CNN）的密码分析提取攻击假设网络架构已知，旨在恢复模型参数。本文首次证明，在具有最大池化和平均池化的CNN中可以移除架构假设。
>
> **主要贡献:** 主要贡献：作者发现通过现有参数恢复攻击获得的权重向量的空间几何结构自然泄露了架构信息，并建立了这些几何结构与卷积层架构知识之间的对应关系，从而提出了一种反馈无关的架构恢复方法。
>
> **达到效果:** 达到的效果：结合参数恢复攻击，本文提出了一个在黑盒设置下同时恢复架构和参数的完整密码分析提取框架，实验结果表明该方法有效。
>
> **技术梗概:** 技术梗概：通过分析权重向量的空间几何结构来推断CNN的架构信息，并与参数恢复攻击相结合，实现了无反馈的架构和参数双重恢复。

---
### [2026/2162] A Survey of Pseudorandom Codes

- **作者:** Miranda Christ

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2162) | [PDF](https://eprint.iacr.org/2026/2162.pdf)


> **研究背景:** 伪随机码（PRC）结合了伪随机加密和纠错码的特性，能够容忍恒定比率的比特翻转错误，并且任何多项式数量的编码词都是伪随机的。这种对象在理论上是新颖的，与许多标准性质分离，如单向函数、公钥加密甚至虚拟黑盒混淆等。
>
> **主要贡献:** 本文综述了伪随机码的研究现状，介绍了当前定义、技术和应用，并提出了新的技巧来增强伪随机码的功能。
>
> **达到效果:** 通过这些新技巧的应用，伪随机码在水印和纠错编码等领域取得了显著效果，特别是在保持模型质量的同时抵抗内容修改方面表现出色。
>
> **技术梗概:** 文章详细阐述了如何通过现有技术的组合与创新来提升伪随机码的安全性和实用性，并为未来的研究提供了工具和指导。

---
### [2026/2169] Hardware-Adapted SIMD Optimization of AIMer v3 on x86 and Cortex-M55

- **作者:** Seung-Won Lee, Si-Woo Eum, Min-Seo Kim, Su-Been Cho, Hwa-Jeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/2169) | [PDF](https://eprint.iacr.org/2026/2169.pdf)


> **研究背景:** 研究旨在优化AIMer v3在x86和Cortex-M55上的硬件适应性SIMD实现，以验证先前为AIM2设计的SIMD方法是否能加速AIM3并保持其结果的一致性。
>
> **主要贡献:** 贡献在于成功将AVX-512和AVX2的field-arithmetic、affine、party-parallel及SHAKE方法移植到AIMer v3，并针对Cortex-M55 MVE实现优化，显著提升了签名和验证的速度。
>
> **达到效果:** 结果表明，与参考实现相比，在不同平台上的加速比分别为x86 AVX2 3.14–4.41倍、AVX-512 4.59–6.55倍以及Cortex-M55 MVE 1.28–1.91倍。所有参数集的输出均与官方已知答案测试和差异测试结果一致。
>
> **技术梗概:** 技术上，通过使用16位多项式部分乘积、分组对应32位词并矢量化 affine层来优化Cortex-M55 MVE实现，并保持AIMer v3的数学结构和证明过程不变。

---
### [2026/2171] Improved Quantum Linear Key-Recovery Attacks with Walsh-Spectrum Puncturing

- **作者:** Qihang Ding, Jiongjiong Ren, Shaozheng He, Shaozhen Chen

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2171) | [PDF](https://eprint.iacr.org/2026/2171.pdf)


> **研究背景:** 量子傅里叶变换（QFT）基于相关态准备的线性密钥恢复攻击可以实现超二次加速，但其复杂度随着活跃子键维度的增长而迅速增加，可能抵消了相对于Grover搜索的优势。
>
> **主要贡献:** 作者提出了一种穿孔感知框架，并开发了Q-Puncture自动化优化方法，针对量子密钥恢复的复杂性进行了优化。
>
> **达到效果:** 该方法在Serpent、RECTANGLE-128和PIPO-128上应用后，提出了新的穿孔量子线性密钥恢复攻击家族，其建模的量子复杂度优于通用Grover搜索。
>
> **技术梗概:** 通过使用频谱穿孔近似较小维度的关键恢复映射，并结合Q-Puncture自动化优化方法来降低活跃子键维度，从而改进了量子线性密码分析。

---
### [2026/2172] A New Prime-Norm Ideal Sampling for SQIsign

- **作者:** Li-Jie Jian

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/2172) | [PDF](https://eprint.iacr.org/2026/2172.pdf)


> **研究背景:** SQIsign通过在每次密钥生成和签名时采样具有奇素范数的理想来工作，但这一过程存在效率问题。
>
> **主要贡献:** 作者提出了一种新的基于理想采样的方法，从几何角度重新审视了随机理想的选取过程。
>
> **达到效果:** 新算法相比参考实现，仅需一次模除逆运算和少量加法与乘法操作，实现了NIST安全级别I下13.76倍的速度提升，并保持输出均匀性。
>
> **技术梗概:** 通过固定点绘制直线的方式将单一随机参数转化为理想，确保每个理想恰好被选中一次。

---
### [2026/2174] Blind Spots in Blind Signatures: A System-Level Security Analysis of Deployed Chaumian Ecash

- **作者:** Huaifeng Chen, Yuchang Zhang, Yu Cheng

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2174) | [PDF](https://eprint.iacr.org/2026/2174.pdf)


> **研究背景:** 文章对主流部署的Chaumian电子现金系统Cashu进行了首次系统级安全分析，该系统已发展为包含31份规范文档、58个铸币点和七个互操作钱包。
>
> **主要贡献:** 贡献在于构建了五层模型，提出了九项属性（三项新属性），并证明了一个状态化的‘一个更多不可伪造性’定理，揭示了部署攻击仅通过窃取而不伪造即可完成。
>
> **达到效果:** 研究结果表明，所有公开披露的盗窃事件均源于密钥集轮换、钱包恢复和多铸币身份组合；58个铸币点中，每个都声明支持DLEQ证明作为唯一剩余防御手段，但这些声明未经过验证。
>
> **技术梗概:** 采用纯标准库Python实现，并通过49个官方测试向量验证了所有工具，揭示了恢复性并非免费的教训。

---
### [2026/2175] $\mathsf{Rotor}$: SNARKs for Power-of-Two Rings

- **作者:** Chongrong Li, Yun Li, Pengfei Zhu, Zhechen Li, Michael Dong, Alan Li, Yuncong Hu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2175) | [PDF](https://eprint.iacr.org/2026/2175.pdf)


> **研究背景:** 在实际应用中，模幂二环上的算术简洁证明对于验证机器字计算至关重要。然而，设计基于此类环的SNARKs具有挑战性，因为它们缺乏其他常见SNARKs所依赖的代数性质。
>
> **主要贡献:** $\mathsf{Rotor}$首次提出了一种直接在$\mathbb Z_{2^k}$上工作的高效SNARK方案，解决了现有方法中由于使用高成本的伽罗瓦环或随机大素域带来的效率问题。
>
> **达到效果:** 通过引入具有张量查询的交互式oracle证明、弱化线性码邻近间隙以及兼容承诺机制，$\mathsf{Rotor}$实现了在保持计算效率的同时达到可忽略的安全误差。
>
> **技术梗概:** 该方案首先构建一个基于更大环$\mathbb Z_{2^m}$的张量查询sumcheck风格交互式oracle证明，然后通过引入弱化线性码邻近间隙确保矢量在减少到$\mathbb Z_{2^m}$后接近于代码。

---
### [2026/2176] Budgeted Threshold Signatures: Protocol-Native Spending Limits with Zero On-Chain Footprint

- **作者:** Huaifeng Chen, Yuchang Zhang, Yu Cheng

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2176) | [PDF](https://eprint.iacr.org/2026/2176.pdf)


> **研究背景:** 现有的托管策略中的支出限制存在中心化、链上痕迹或可信绕过表面等问题，研究旨在解决这些问题。
>
> **主要贡献:** 提出了一种协议内固有的支出限制方案BTS，实现了预算在签名过程中的秘密共享和内部验证。
>
> **达到效果:** 证明了线性门组合协议下的每种成员都会签署超出预算的交易，并提出了一个认证门来防止攻击，实际成本可接受。
>
> **技术梗概:** 通过引入认证门技术，在不增加链上足迹的情况下实现了支出限制功能，确保了诚实多数配置的安全性。

---
### [2026/2177] LEANY: A Hardware-Friendly Wide-Block Block Cipher

- **作者:** Gökçe Düzyol, Nida Fidan, Kamil Otal, Selman Pınar

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2177) | [PDF](https://eprint.iacr.org/2026/2177.pdf)


> **研究背景:** 鉴于高吞吐量加密需求及NIST的Accordion模式进程，研究者们对宽块大小的分组密码产生了浓厚兴趣。Rijndael-256（即WAES）是此类密码的主要选择之一。
>
> **主要贡献:** 本文提出了LEANY，一种基于8×8矩阵的256位密钥和256位块大小的SPN结构分组密码，通过独特的四层圆周操作确保了高安全性。
>
> **达到效果:** 经过分析，七轮后的最佳差分和线性传播概率上限低于$2^{-256}$。FPGA实现证明LEANY在硬件上既紧凑又高效。
>
> **技术梗概:** LEANY采用4比特字节的SPN结构，并通过特定的四层操作（密钥添加、S盒层、行内字节移位和GF(2^4)上的8×8 MDS矩阵乘法）实现了宽扩散特性。

---
### [2026/2179] Capacity-Approaching Pseudorandom Code for Stochastic Edit Channels

- **作者:** Leshui Wang, Kenji Yasunaga

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2179) | [PDF](https://eprint.iacr.org/2026/2179.pdf)


> **研究背景:** 研究背景：现有伪随机码（PRC）主要针对汉明错误信道，而对同时包含汉明和同步错误的随机记忆缺失二进制置换-插入-删除（BSID）信道优化不足。
>
> **主要贡献:** 主要贡献：提出了一种基于多数表决的伪随机码构造方法，并设计了一个独特的可公钥多比特伪随机码，能够在给定信道参数下实现接近容量的表现。
>
> **达到效果:** 达到的效果：证明了该伪随机码在BSID信道上具有鲁棒性，在特定条件下能够恢复超过一半的原始坐标，并且对于足够大的字母表，可以实现接近容量的性能。
>
> **技术梗概:** 技术梗概：通过多数表决机制和控制信息与高率载荷的分离编码策略来增强伪随机码在BSID信道上的鲁棒性。

---
### [2026/2182] Lexicure: Efficient, Encrypted Lexical Search

- **作者:** Malvika Raj Joshi, Tal Wagner, Shai Halevi, Nina Mishra

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2182) | [PDF](https://eprint.iacr.org/2026/2182.pdf)


> **研究背景:** 在云环境中进行加密的词汇搜索以保护隐私成为一个关键问题，因为查询和文档都托管在数据所有者无法完全控制的基础设施上。现有的全隐私保证方法通常需要大量的客户端存储或通信开销。
>
> **主要贡献:** Lexicure通过将搜索转换为有利于密码学原语（如加密矩阵-向量乘积）的算术操作，提出了一个协议来解决这一问题。
>
> **达到效果:** 在十六个不同的数据集上，Lexicure保持了超过95%的明文BM25准确性，并且相比其他方法减少了2.6到9.9倍的通信开销，同时客户端存储需求低于1KB。
>
> **技术梗概:** Lexicure通过引入修改后的加密矩阵-向量乘积来安全地计算二次函数和h...

---
### [2026/2183] Vespa: Efficient Secure Aggregation with Integrity Defense under Server Privacy

- **作者:** Jia Hu, Hongwei Li, Meng Hao, Hanxiao Chen, Pengzhi Xing, Wenbo Jiang, Dongxiao Liu, Haiyang Xue, Robert H. Deng

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2183) | [PDF](https://eprint.iacr.org/2026/2183.pdf)


> **研究背景:** 现有安全聚合方案因依赖通用且计算密集的零知识证明（ZKPs）而效率低下，限制了其大规模部署；同时输入验证仅限于公开界限下的范数检查。
>
> **主要贡献:** 	extsf{Vespa} 提出了一个高效的安全聚合方案，能够同时实现实际效率和扩展输入验证能力，支持范数检查和基于私有服务器输入的相似性检查。
>
> **达到效果:** 该方案通过设计新颖的有效加权安全聚合协议，并结合矢量盲线性评估（VOLE）技术，实现了高效的输入验证，提高了系统的整体性能。
>
> **技术梗概:** 利用优化范围证明和基于 VOLE 的 ZKPs 实现了高效范数检查；并通过扩展框架支持基于私有服务器输入的高级相似性检查。

---
### [2026/2184] Static Group Signature with Post-Compromise Accountability

- **作者:** Kyosuke Yamashita

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/2184) | [PDF](https://eprint.iacr.org/2026/2184.pdf)


> **研究背景:** 研究背景：静态组签名为成员提供匿名签名和可追溯性，但在密钥泄露后无法区分合法与非法签名。本文旨在引入具有密钥泄露后问责性的组签名方案（GSPCA），以解决现有方案的不足。
>
> **主要贡献:** 主要贡献：提出了GSPCA的概念，并设计了一个基于公钥加密、签名和零知识证明系统的通用构造方法，同时定义了开签与重验证一致性要求。
>
> **达到效果:** 达到的效果：实现了在密钥泄露后仍能区分合法与非法签名的组签名方案，确保了匿名性和可追溯性的同时增加了密钥泄露后的问责机制。
>
> **技术梗概:** 技术梗概：通过引入适应式混合安全（AH-CCA）来处理高最小熵随机性的加密问题，并利用随机预言模型实现所需的随机性恢复加密。

---
### [2026/2185] Instantiating Microcrypt: Obstacles and opportunities via tailored state certification

- **作者:** Jose Carrasco, Jens Eisert, Soumik Ghosh, Dominik Hangleiter, Nicky Kai Hong Li, Ryan Sweke

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2185) | [PDF](https://eprint.iacr.org/2026/2185.pdf)


> **研究背景:** 研究探讨了Hamiltonian phase state (HPS)假设在构建伪随机和单向状态生成器中的应用，以及其与一维函数存在的关系。
>
> **主要贡献:** 工作通过证明如果HPS假设成立，则一维函数存在，从而否定了使用HPS假设直接实例化Microcrypt的可能性。
>
> **达到效果:** 研究展示了HPS假设提供了量子原生的假设来构建经典密码学，并提出了构造单向谜题的新方法。
>
> **技术梗概:** 技术上通过设计特定的“先测量后询问”状态认证协议，结合一维状态生成器构造了单向谜题，并分析了其性质与状态认证协议的关系。

---
### [2026/2187] Rethinking Zero-Knowledge TLS: Proof of Protocol Execution and the Reclaim Protocol

- **作者:** Andrey Bozhko, Kirill Kutsenok

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2187) | [PDF](https://eprint.iacr.org/2026/2187.pdf)


> **研究背景:** 现有零知识TLS(zkTLS)协议各自拥有独立的安全模型，缺乏统一定义。本文旨在提出一个适用于zkTLS的跨传输层安全模型，以解决这一问题。
>
> **主要贡献:** 作者首次提出了Zero-Knowledge Proof of Protocol Execution (zkPoPE)，这是一种应用层的概念，不涉及TLS或任何特定构造，为zkTLS提供了统一的安全框架。
>
> **达到效果:** 通过引入Reclaim协议，该研究实现了在符合部署信任模型的条件下，UC安全地实现F_{zkpope}功能，从而验证了私有输入和回复满足特定属性，并确保回复来自真实执行过程。
>
> **技术梗概:** 该研究采用了一种新的安全模型，从应用层而非传输层出发定义零知识证明的概念，以此来区分真正的协议执行与伪造的输入-回复对。

---
### [2026/2188] SPRUCE: Scalable Multiparty Private Set Union in Constant Rounds

- **作者:** Zhengwei Tong, Saba Eskandarian, Jonathan Katz, Kartik Nayak

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2188) | [PDF](https://eprint.iacr.org/2026/2188.pdf)


> **研究背景:** MPSU允许多个互不信任的方各自持有私有集合，通过安全方式学习这些集合的并集而不泄露额外信息。
>
> **主要贡献:** 研究提出了一种新的半诚实MPSU协议，消除了传统上需要进行的加密值混洗步骤，实现了常数轮次运行，并支持不平衡输入大小。
>
> **达到效果:** 在不平衡场景下，新协议显著优于现有方案PULSE和DZBC25；在平衡场景中，与PULSE相比，在大多数情况下性能提升一个数量级，在100ms延迟的广域网部署中也超越了DZBC25。
>
> **技术梗概:** 通过引入新的半诚实MPSU协议框架，利用信息论在线阶段和并行处理技术实现了上述效果。

---
### [2026/2189] SafeCast: End-to-End Security for Ultra-High-Bitrate Broadcast Media

- **作者:** Arman Kolozyan, Christian Knabenhans, Zayd Maradni, Carmela Troncoso

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2189) | [PDF](https://eprint.iacr.org/2026/2189.pdf)


> **研究背景:** 针对广播制作中点对点专用电缆的可扩展性和灵活性限制，广播界开发了一套通过IP组播传输媒体的标准。然而，这移除了物理保护，使流媒体容易受到篡改和窃听威胁。
>
> **主要贡献:** SafeCast系统旨在为超高比特率多播媒体提供端到端的安全性，它基于现有的标准化协议并进行了扩展与适应，以满足广播环境的需求。
>
> **达到效果:** SafeCast能够提供强大的机密性和完整性保证，并利用广播网络的精确时序同步实现低成本的发送者认证。评估表明，该系统可以满足生产设施严格的部署要求（吞吐量、延迟、大动态组）。
>
> **技术梗概:** SafeCast通过结合MLS组密钥协议和SRTP媒体保护协议来构建安全传输机制，并通过利用广播网络的精确时序同步实现低成本的发送者认证。

---
### [2026/2191] Building Quantum Circuits with Qarton

- **作者:** André Schrottenloher

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2191) | [PDF](https://eprint.iacr.org/2026/2191.pdf)


> **研究背景:** 为了准确评估大规模计算设备的影响，需要对量子算法所需的量子电路资源进行精确估计。Qarton是一个用于表示逻辑量子电路的Python库，专注于量子密码分析领域。
>
> **主要贡献:** 该论文贡献了一个名为Qarton的Python库，旨在简化量子电路的设计与模拟过程，并支持研究从经典到几乎经典的电路转换。
>
> **达到效果:** 通过使用Qarton，研究人员能够分析不同实现算术操作之间的权衡，并研究大型量子搜索等复杂电路。
>
> **技术梗概:** Qarton采用了模块化的设计方法，允许用户构建和模拟各种规模的量子电路，并提供了一个包含约300个电路和示例的库。

---
### [2026/2192] Distribution-Aware Encryption with Optimal Recovery and Semantic Security

- **作者:** Haijie Su, Haibo Cheng, Ping Wang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2192) | [PDF](https://eprint.iacr.org/2026/2192.pdf)


> **研究背景:** 该研究针对保护数据的概率分布，提出了适应概率定律的加密方法，并在不假设明文和密钥独立的情况下，同时实现了消息恢复、密钥恢复及目标分布语义安全。
>
> **主要贡献:** 作者通过分解联合概率矩阵并采用容量控制随机舍入构造了此分解，进而设计了一步步条件采样器将舍入过程转化为正确的加密算法。
>
> **达到效果:** 该方法达到了消息恢复的最优值$\max\{p_\star,q_\star\}$，密钥恢复成功率低于$2q_\star$，并提供了针对固定布尔消息谓词预测的$O(\sqrt{q_\star})$优势。
>
> **技术梗概:** 加密通过分解联合概率矩阵和采用容量控制随机舍入实现，并利用步步条件采样器将舍入过程转化为正确的加密算法，无需枚举输出分布或拒绝完整样本。

---
### [2026/2193] Square-Root Log-Rank Standard Deviations for the Higher-Rank Kadison–Singer Problem: Polynomial-Time Algorithms via Schatten-Norm Potentials

- **作者:** Zhao Song, Song Yue

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2193) | [PDF](https://eprint.iacr.org/2026/2193.pdf)


> **研究背景:** 研究了正定矩阵的符号分配问题，即给每个原始矩阵一个固定的符号，而不考虑其秩。该工作特别关注高阶Kadison-Singer分区问题。
>
> **主要贡献:** 证明了存在一种符号分配方法，使得矩阵和的最大范数不超过特定值，给出了多项式时间算法，并改进了依赖于秩的常数因子。
>
> **达到效果:** 通过使用Schatten范数构建势函数，实现了秩平方根对数的依赖性。在额外假设下，得到了最优的离散度上界，证明了该方法的有效性和理论上限。
>
> **技术梗概:** 利用Lee-Sidford障碍技术，设计了一种基于势函数的方法来优化符号分配问题，并通过分析Schatten范数来降低算法复杂度。

---
### [2026/2196] Revisiting Fuzzy Password-Authenticated Key Exchange: Unified Leakage and Improved Efficiency

- **作者:** Sihang Pu, Li Yao, Yu Yu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2196) | [PDF](https://eprint.iacr.org/2026/2196.pdf)


> **研究背景:** 模糊密码身份认证密钥交换(fuzzy PAKE)允许双方在使用近似或低熵的密码时，安全地建立共享密钥，特别适用于处理误输密码或生物特征等噪声样本。然而，现有定义和协议存在泄漏问题，并且通常局限于汉明距离。
>
> **主要贡献:** 研究者展示了当密码可能在多个会话中重复使用时，任何功能中的泄漏都会导致相同的最终泄露后果。他们提出了支持L_p距离的高效协议，并统一了多种泄漏概念。
>
> **达到效果:** 通过构造高效的理想世界对手，证明了对于常见距离度量，在密码重用情况下，各种泄漏概念实际上是一致的。同时，提出了一种更有效的协议来支持Minkowski距离。
>
> **技术梗概:** 研究采用了理想世界模型和构造高效对手的技术来分析模糊PAKE的安全性，并通过优化协议提高了效率。

---
### [2026/2197] MultLA: A Framework of Fast Correlation Attack Exploiting Multiple Linear Approximations and Its Application to SNOW 5G

- **作者:** Jiang Wan, Lin Ding, Jiekun Sun, Xinhai Wang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/2197) | [PDF](https://eprint.iacr.org/2026/2197.pdf)


> **研究背景:** 快相关攻击的复杂性主要由所使用的线性逼近的相关性决定。现有方法要么依赖单一主导线性逼近，要么使用下界评估多个具有相同输出掩码和不同输入掩码的线性逼近的结果相关性。
>
> **主要贡献:** 	exttt{MultLA}框架提出了一种利用多个具有不同输出掩码的线性逼近的新方法，解决了现有技术未解决的问题。
>
> **达到效果:** 对于SNOW 5G，该攻击的时间/内存/数据复杂度分别为$2^{279.25}/2^{265.03}/2^{263.62}$，优于现有工作；当时间和内存复杂度略好于现有工作时，数据复杂度显著降低至$2^{261.21}$。
>
> **技术梗概:** 	exttt{MultLA}框架通过使用线性逼近绝对相关性的算术平均值来评估多个具有不同输出掩码的线性逼近，从而提高了攻击效率。
...
### [推荐] [2026/2021] Silk: Computation-Efficient and Post-Quantum-Friendly Randomness Beacon

- **匹配关键字:** post-quantum

- **作者:** Binyang Hou, Jiamian Yan, Liang Zhang, Borui Chen, Haibin Kan

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2021) | [PDF](https://eprint.iacr.org/2026/2021.pdf)


> **研究背景:** 现有的随机性信标依赖于秘密共享机制，通常需要昂贵的配对或群组操作来验证份额，并且其底层假设容易受到量子攻击。此外，为每个输出运行单独的拜占庭容错（BFT）共识实例会导致重复的计算和通信成本。
>
> **主要贡献:** Silk通过引入Mulberry协议实现了批处理异步可验证秘密共享带部分输出（bAVSS-PO），并进一步在一批输出上共用BFT共识，仅需一次BFT实例即可。
>
> **达到效果:** 实验结果显示，在广域网环境下使用121个副本分布在八个AWS区域中，Silk的吞吐量达到5.414次/秒，分别是Rondo和Spurt的1.53倍和4.18倍。
>
> **技术梗概:** Mulberry协议利用哈希验证份额来降低计算成本，并在部分同步环境下确保了bAVSS-PO属性；Silk则通过共用BFT共识实例来减少重复的成本开销，同时保持了信标的确定性。

---
### [推荐] [2026/2026] A unified reduction from RLWE to MP-LWE

- **匹配关键字:** LWE

- **作者:** Alice Pellet-Mary, Wenwen Xia

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2026) | [PDF](https://eprint.iacr.org/2026/2026.pdf)


> **研究背景:** 该研究旨在统一从环学习误差问题（RLWE）到中间产品学习误差问题（MP-LWE）的约简方法，这两种问题是密码学中重要的数学基础。
>
> **主要贡献:** 作者提出了一个参数化为一对字段元素$(y_a,y_s) \in K^2$的统一约简方法，该方法能够覆盖先前的所有实例，并且优化了选择以适用于所有符合条件的数域。
>
> **达到效果:** 通过这种方法，首次实现了对所有满足特定条件的数域K = ℚ[X]/f(X)中的RLWE到MP-LWE的约简，这在以前的研究中是不存在的。
>
> **技术梗概:** 该研究使用了多项式和数论的技术来优化参数选择，并且证明了新的约简方法的有效性。

---
### [推荐] [2026/2028] Dimension Reduction for Theta Series of Ideal Lattices

- **匹配关键字:** lattice

- **作者:** Runkun Liu, Zhaoyang Liang, Dan Ding

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2028) | [PDF](https://eprint.iacr.org/2026/2028.pdf)


> **研究背景:** Theta系列是记录晶格向量长度的基本函数，连接了晶格几何与高斯质量等量纲。对于理想嵌入下的理想晶格，算术坐标通常耦合在一起，使得固定子集的theta系列计算和相关高斯计算在高维空间中变得困难。
>
> **主要贡献:** 我们展示了显式可验证的相对基条件可以将每个固定子集的n维theta系列分解为ℓ个独立的n₀维度因子计算。这种分解适用于所有高斯参数，无需近似或平滑假设。
>
> **达到效果:** 该方法显著降低了每种子集计算的维度，实现了最高335倍、215.45倍和7.9倍的速度提升，分别对应于固定子集theta系列评估、数值平滑参数评估和顺序高斯采样。
>
> **技术梗概:** 通过验证特定条件下的相对基，将高维问题分解为低维度因子的组合计算。这种方法适用于多种循环和非循环扩展，并提供了具体的公式表示。

---
### [推荐] [2026/2033] Efficient Single-Server Online-Offline PIR without Periodic Preprocessing

- **匹配关键字:** homomorphic encryption

- **作者:** Hoang-Dung Nguyen, Jorge Guajardo, Thang Hoang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2033) | [PDF](https://eprint.iacr.org/2026/2033.pdf)


> **研究背景:** 传统的私有信息检索（PIR）协议要求服务器在每次客户端查询时进行昂贵的计算，这使得在线成本很高。为降低这一成本，提出了在线-离线PIR（OO-PIR），允许客户端预先计算一个与查询无关的提示表，从而实现子线性时间的在线查询复杂度。然而，现有的OO-PIR协议要么需要非合谋的双服务器设置，要么在单服务器设置中需要昂贵的周期性预处理，这导致了极高的带宽或计算开销。
>
> **主要贡献:** 我们提出了ESCAPE，一种针对单服务器设置的新颖OO-PIR协议，完全消除了昂贵的周期性预处理，支持无限在线查询并在子线性时间内以低常数响应带宽运行。
>
> **达到效果:** ESCAPE通过结合新的提示采样策略与线性同态加密（LHE）来隐藏任何提示和任何在线查询之间的相关性，并允许在子线性时间内在客户端上实时刷新消耗的提示，从而实现了这一目标。我们全面实现并评估了ESCAPE协议。
>
> **技术梗概:** ESCAPE的核心创新在于结合了一种新的提示采样策略与线性同态加密（LHE），以隐藏任何提示和任何在线查询之间的相关性，并允许在子线性时间内在客户端上实时刷新消耗的提示，同时设计了一个随机采样结构，使其能够利用高效LHE实例化的简化预处理。

---
### [推荐] [2026/2034] Barely Doubly-Efficient PIR from Quadratic Residuosity
...
- **链接:** [论文](https://eprint.iacr.org/2026/2034) | [PDF](https://eprint.iacr.org/2026/2034.pdf)


> **研究背景:** DEPIR方案允许客户端从远程服务器托管的数据库中检索数据而不泄露访问位置，现有的DEPIR方案大多基于格理论，尚未有非格基础的DEPIR方案。
>
> **主要贡献:** 本文提出了一种基于二次剩余假设的CRS模型下的DEPIR方案，并在每次查询时实现亚线性时间响应。
>
> **达到效果:** 该方案通过利用Williams的矩阵-向量乘法预处理技术，结合Legendre符号的伪随机性来保证隐私，同时解决了每次查询使用新模数的问题。
>
> **技术梗概:** 采用二次剩余假设和Legendre符号的伪随机性证明，结合矩阵-向量乘法预处理技术和子线性时间模数缩减的数据结构。

---
### [推荐] [2026/2035] Maliciously Secure Shuffled Distributed OPRF with Applications to Private Set Operations

- **匹配关键字:** post-quantum

- **作者:** Aron van Baarsen, Aarushi Goel, Lisa Kohl, Peihan Miao, Phuoc Van Long Pham, Peter Scholl, Satvinder Singh

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2035) | [PDF](https://eprint.iacr.org/2026/2035.pdf)


> **研究背景:** 现有私有集合交集（PSI）协议在恶意设置下效率显著低于半诚实设置，主要瓶颈在于缺乏高效且安全的混排分布式不可区分伪随机函数（SH-DOPRF）。
>
> **主要贡献:** 本文提出了首个线性复杂度的恶意安全SH-DOPRF构造，并应用于实现多种私有集合操作。
>
> **达到效果:** 基于双线性对和暗物质PRF变体构建了两种实例，实现了高效且安全的多查询逆私有成员测试协议，进而支持私有集合操作。
>
> **技术梗概:** 该技术主要依赖于对称密钥技术，仅在评估伪随机函数输出时使用少量公钥运算。

---
### [2026/2019] The Residue Is the Resource: Exact Fresh-State Complexity, Reconstruction Limits, and Optimal Challenge Design in Folding

- **作者:** Xiang Wang

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2019) | [PDF](https://eprint.iacr.org/2026/2019.pdf)


> **研究背景:** 研究背景：折叠协议必须在公开展示挑战之前保留足够的输入相关信息，以支持所有必要的后挑战输出。本文探讨了这种边界处的精确保留宽度，并分析了不同挑战支持下的重建限制和最优挑战设计。
>
> **主要贡献:** 主要贡献在于确定了残余目标的空间维度作为所需新鲜标量信息的确切保留宽度，并通过秩来表征有限域上坐标乘法的最佳重构低于此宽度的情况。
>
> **达到效果:** 达到的效果包括：明确了不同挑战支持下的精确宽度，揭示了基于Reed-Solomon域的直接宽度差异；并为多项式乘法提供了前三个缺陷的精确几何确定。
>
> **技术梗概:** 技术梗概：通过分析残余目标的空间维度和秩来表征最优重构，并考虑了双线性侧信息引入的兼容性约束，从而确定不同挑战支持下的重建轮廓。

---
### [2026/2020] Removing the Human, Keeping the Doors Unlocked: A Cross-Tool Formal Analysis of GSMA SGP.32 eSIM IoT Provisioning

- **作者:** Rong Qian, Yu Cheng, Hongbo Zhao, Yuchang Zhang, Zengli Guo

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2020) | [PDF](https://eprint.iacr.org/2026/2020.pdf)


> **研究背景:** 研究背景：GSMA SGP.32通过远程管理器的签名替代了消费者eSIM配置中的本地用户确认，以实现无人值守物联网设备的安全配置。
>
> **主要贡献:** 主要贡献：首次对SGP.32进行了形式化安全分析，并揭示了RSP家族三代产品中的漏洞。
>
> **达到效果:** 达到的效果：发现了六个规范级攻击，包括一个涉及两次未认证本地操作的10步管理平面接管攻击；验证了协议在网络对手面前具有抵抗力；提出了两种修复措施并证明其有效性。
>
> **技术梗概:** 技术梗概：使用Tamarin和ProVerif对SGP.32 v1.3管理平面进行建模，通过形式化方法分析安全性和可行性。

---
### [2026/2022] A Partial Injection from Unsatisfiable to Uniquely Satisfiable Quadratic Systems

- **作者:** Gabriele Radici, Massimiliano Sala

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2022) | [PDF](https://eprint.iacr.org/2026/2022.pdf)


> **研究背景:** 研究背景：本文探讨了从无解的二次方程系统到唯一解的二次方程系统的部分映射问题，这是密码学中一个重要但未解决的问题。
>
> **主要贡献:** 主要贡献在于提出了一个可计算且无需预处理或提示的映射τ，该映射在特定子集上是单射，并且其逆过程等价于求解多项式方程组的零点问题。
>
> **达到效果:** 达到的效果是在无解二次方程系统中找到了一个大于74.5%的概率保证下可单射到唯一解系统的子集，这为构建基于多项式方程的安全方案提供了新的可能性。
>
> **技术梗概:** 技术梗概：映射τ通过在系数表示上进行三次点译码支持的平坦空间添加修正项来实现，并证明了其随机自约简性质。

---
### [2026/2023] Robust Side Channel Analysis with Blocking and Quantification of Leakage

- **作者:** Alicja Kario, George Pantelakis

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2023) | [PDF](https://eprint.iacr.org/2026/2023.pdf)


> **研究背景:** 现有侧信道分析(SCA)方法尽管理论基础强大，但在实际应用中往往忽视了重要的现实因素。
>
> **主要贡献:** 作者提出了一种新的SCA方法BLAQ，旨在弥合理论模型与不完美的测试环境之间的差距。
>
> **达到效果:** 通过使用BLAQ方法，可以更准确地量化泄漏并提高分析的鲁棒性。
>
> **技术梗概:** BLAQ方法通过阻塞和量化的手段来处理数据点，并提供正式边界以衡量测量精度。

---
### [2026/2024] Optimistic History-Independent Sleepy Atomic Broadcast

- **作者:** Kwok-Yan Lam, Zhenliang Lu, Qiang Tang, Xinrui Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2024) | [PDF](https://eprint.iacr.org/2026/2024.pdf)


> **研究背景:** 现有睡眠共识协议的延迟已显著提高，但大多数依赖于历史消息访问的假设或未在乐观条件下优化性能。
>
> **主要贡献:** 作者设计了一种新的历史无关型睡眠原子广播协议，并提出了一个乐观条件下的原子广播协议。
>
> **达到效果:** 新协议在最多1/5故障情况下实现了确认延迟为$2\Delta$和最优延迟$\Delta$，并在领导者诚实时实现了最好情况延迟$2\Delta$并容忍至多1/6的故障。
>
> **技术梗概:** 通过引入乐观分级协议$OptimisticGA$，实现动态乐观半数集，并据此构建了新的原子广播协议。

---
### [2026/2025] Optimal Amplification via Bias-Resilient Combiners

- **作者:** Benny Applebaum, Nathan Geier

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2025) | [PDF](https://eprint.iacr.org/2026/2025.pdf)


> **研究背景:** 研究背景：密码学中的组合器通过将多个候选方案结合，确保至少有k个是安全的即可实现整体的安全性。然而，在标准安全性概念下，弱候选者可能持续泄露部分信息而非完全失败，这使得直观的安全性放大假设失效。
>
> **主要贡献:** 主要贡献：作者提出了一个新的专门框架——抗偏差区分组合器，克服了现有方法在误差率和放大阈值上的不足，并且不依赖于$\delta < 0.5$的限制。
>
> **达到效果:** 达到的效果：通过这种新框架，研究人员能够实现更优的安全性放大效果，并且在弱候选者的错误参数上减少了大约$2^{n-k}$的损失。
>
> **技术梗概:** 技术梗概：该研究采用了抗偏差的技术来设计组合器，确保即使输入中有偏差或部分信息泄露的情况下也能保持整体安全性。

---
### [2026/2027] Leopard: A High-Performance Framework for Updatable Private Set Intersection

- **作者:** Xiaowei Li, Shi-Feng Sun, Yanxue Jia, Hao Li, Dawu Gu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2027) | [PDF](https://eprint.iacr.org/2026/2027.pdf)


> **研究背景:** UPS Intersection (UPSI)扩展了传统的PSI协议，使其适用于动态场景，在这种场景中，输入集会随时间发生变化。现有的UPS方案虽然允许在不完全重新执行整个协议的情况下进行更新，但仍然存在实用性问题。
>
> **主要贡献:** Leopard框架通过利用不平衡的PSI设计来实现无需完整重执行即可处理插入和删除操作的UPSI，同时兼容任何不平衡的PSI协议，并通过跨更新重用内部数据结构进一步提高性能。
>
> **达到效果:** 实验结果表明，与Badrinarayanan等人（ASIACRYPT 2024）和Alborch等人（ACNS 2026）相比，Leopard实例将计算成本最多减少了111.28倍，并至少减少了1.48倍；通信成本最多减少了118.97倍，并最多减少了522.01倍。
>
> **技术梗概:** Leopard框架通过在不平衡的PSI中增量更新某些数据结构而不是完全重建它们来实现性能提升，提供了一个基于FHE的实例以展示通过这种数据结构重用获得的显著性能改进。

---
### [2026/2029] It's Not Just a Phase: Understanding Function Dependence in MPC Preprocessing

- **作者:** Andreas Brüggemann, Kasra Edalatnejad, Jan Filipp, Philipp Kusterer, Thomas Schneider

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2029) | [PDF](https://eprint.iacr.org/2026/2029.pdf)


> **研究背景:** 安全多方计算(MPC)是实现隐私保护应用的关键技术。为了在输入提供后实现低延迟，许多高效协议利用了预处理，并且预处理方法的景观正在变得多样化和复杂化。
>
> **主要贡献:** 研究系统化了不同类型的函数依赖性，并将其与效率之外的实际需求联系起来，填补了现有文献在这方面关注不足的问题。
>
> **达到效果:** 通过区分不同的函数依赖类型及其实际影响，为设计更高效的MPC预处理方案提供了理论基础和指导。
>
> **技术梗概:** 通过对已有工作的分析和总结，识别并分类了多种函数依赖性，并探讨了它们在实际应用中的意义。

---
### [2026/2030] New Pseudorandom Correlation Functions and Exponent VRFs from DCR

- **作者:** Archisman Dutta, Lisa Kohl, Lawrence Roy, Peter Scholl

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2030) | [PDF](https://eprint.iacr.org/2026/2030.pdf)


> **研究背景:** 该研究旨在设计一种新的伪随机关联函数(PCFs)，以实现两个参与者之间基于短相关密钥生成大量可局部评估的关联随机性。
>
> **主要贡献:** 作者首次提出了在任意模数$M$下针对协盲传输(COT)和向量协盲线性评价(VOLE)关联性的PCF构造，并且引入了扩展幂DDH问题的新家族及其与标准DCR假设之间的关系。
>
> **达到效果:** 通过这些构造，研究者实现了高效的PCFs，并证明其安全性基于DCR假设。此外，还提出了一种基于离线/在线同态秘密共享的公开可验证eVRF。
>
> **技术梗概:** 该技术基于DCR假设，并且包含一个轻量级的两轮分布式设置协议。

---
### [2026/2031] Mind Small Integers in MDS Matrix: Collision Attacks on Round-Reduced Reinforced Concrete

- **作者:** Jiamin Cui, Fukang Liu, Jianqiang Ni, Willi Meier

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2031) | [PDF](https://eprint.iacr.org/2026/2031.pdf)


> **研究背景:** 针对零知识证明(ZKP)的快速发展，设计了ZK友好的哈希函数，并采用小查找表(LUTs)构建非线性变换(称为Bars层)，以增强其安全性和性能。
>
> **主要贡献:** 作者发现由MDS矩阵包围的Bars层可能导致更弱的差分特性，从而提出了一种针对Reduced Reinforced Concrete的新碰撞攻击方法。
>
> **达到效果:** 该研究成功实现了对4.5轮Reduced Reinforced Concrete的碰撞攻击，并验证了3.5轮的具体碰撞实例。
>
> **技术梗概:** 通过分析Bars层和随后的Concrete层以及Bricks层之间的差分特性，作者提出了一种新颖的技术来绕过这些复杂结构。

---
### [2026/2032] Tight Constants in Talagrand's Boolean Convolution Inequality

- **作者:** Zhao Song

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2032) | [PDF](https://eprint.iacr.org/2026/2032.pdf)


> **研究背景:** 研究旨在确定Talagrand布尔卷积不等式中的最优维度无关常数$C_*(a)$，该不等式描述了噪声算子$T_a$作用于非负函数$f$在均匀测度$\mu$下的概率上界。
>
> **主要贡献:** 作者提出了每个$a\in(0,1)$的具体上界，并证明了当$a$接近1时，常数$C_*(a)/\sqrt{\kappa_a}$的极限上界小于0.3577。
>
> **达到效果:** 同时，通过构建特定函数序列，作者还给出了该常数的下界估计，表明其极限下界大于等于0.3570。
>
> **技术梗概:** 研究采用了构造性方法来建立这些边界，并结合了概率论和凸分析中的技术。

---

## 更新: 2026-09-15 07:17

*新增 34 篇论文 (编号 1984--2018)*

### [推荐] [2026/1985] Post-Quantum Private Set Intersection for Small Sets

- **匹配关键字:** post-quantum

- **作者:** Junxin Liu, Mike Rosulek, Ni Trieu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1985) | [PDF](https://eprint.iacr.org/2026/1985.pdf)


> **研究背景:** 当前的私有集合交集（PSI）协议在处理小型数据集时表现出色，但它们面临量子计算带来的安全挑战。现有协议如RT21虽然适用于小规模集合，但在后量子时代存在兼容性问题。
>
> **主要贡献:** 本文通过修改RT21协议以适应后量子密钥封装机制（ML-KEM），并提出了一种直接扩展理想置换的方法，从而解决了其在后量子安全方面的局限性。
>
> **达到效果:** 实验结果显示，在标准的后量子原语支持下，该协议实现了低至0.25毫秒/项的延迟和1.67千字节/项的通信量，性能损失在1.25倍到5.92倍之间。
>
> **技术梗概:** 通过引入新的盲键值存储抽象，并结合Keccak置换家族构建大规模理想置换，本文提出的方法使得PSI协议能够在后量子环境中更加实用。

---
### [推荐] [2026/1988] Wasp: Succinct Non-Interactive Zero-Knowledge Proofs from VOLE

- **匹配关键字:** lattice

- **作者:** Zhanpeng Guo, Zhelei Zhou, Yun Li, Chenkai Weng, Cheng Hong, Tao Wei

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1988) | [PDF](https://eprint.iacr.org/2026/1988.pdf)


> **研究背景:** 基于向量模糊线性评估（VOLE）的零知识证明（ZKP）在验证者效率方面表现出色，但通常需要线性通信和验证。现有技术如Antman和Antman++虽然减少了通信成本，但仍保持交互性和非精简的验证者。
>
> **主要贡献:** 本文提出了一个名为Wasp的简洁非交互式零知识证明系统，通过增强信息论多项式认证码（IT-PAC）为通用函数提供完整的多项式承诺方案支持，并构建了基于Antman的SIMD电路和基于Plonkish约束系统的通用zkSNARK。
>
> **达到效果:** Wasp实现了非交互式的ZKP，验证者时间和证明大小均为常数级。与现有技术相比，Wasp显著减少了通信成本并提高了验证效率。
>
> **技术梗概:** 通过引入支持通用评估开口的多项式承诺方案（PCS），优化了SIMD到通用编译器，并利用稀疏矩阵表示和扩展预处理技术来实现上述目标。

---
### [推荐] [2026/1991] Towards Practical Iterative Rejection Sampling: A Compact and Efficient Signature over Module Lattices
...
- **链接:** [论文](https://eprint.iacr.org/2026/1991) | [PDF](https://eprint.iacr.org/2026/1991.pdf)


> **研究背景:** 现有的基于模块格的签名方案在紧凑性、实现简单性和依赖标准格假设之间存在严格权衡。
>
> **主要贡献:** SHUTTLE通过引入三项关键技术，实现了更小的签名尺寸和更快的执行速度：确定性的迭代采样循环替换秘密相关的拒绝机制；将过渡逻辑重新表述为简单的整数区间比较；以及采用不对称拉伸-压缩机制来抵消MLWE参数扩展。
>
> **达到效果:** SHUTTLE在NIST Level I实现了1175字节的签名尺寸（合并后2167字节），分别比ML-DSA-44和HAETAE-120减少了51%和20%，同时实现了常数时间执行，签名速度约为HAETAE-120的3倍，并且验证速度优于ML-DSA-44。
>
> **技术梗概:** 通过确定性的迭代采样循环替换秘密相关的拒绝机制；将过渡逻辑重新表述为简单的整数区间比较；以及采用不对称拉伸-压缩机制来抵消MLWE参数扩展。

---
### [推荐] [2026/1992] Bridging the Cryptographic Transition: A Unified Framework for Post-Quantum Key Exchange

- **匹配关键字:** post-quantum

- **作者:** Benjamin Dowling, Bhagya Wimalasiri

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1992) | [PDF](https://eprint.iacr.org/2026/1992.pdf)


> **研究背景:** 鉴于Google宣布了2029年完成后量子密码学(PQC)迁移的内部目标，比NIST和美国政府设定的2035年期限提前了六年，本文旨在通过构建统一的安全框架来简化现有依赖经典密钥交换机制的互联网协议向后量子密钥交换的过渡。
>
> **主要贡献:** 作者提出了一种统一的框架，将经典的密钥交换构造迁移到其对应的后量子版本，并进行了形式化分析以确保安全性。
>
> **达到效果:** 该框架能够有效地支持现有协议的安全迁移至后量子时代，从而缓解紧迫的时间压力。
>
> **技术梗概:** 通过形式化验证和安全分析技术，本文设计了一种统一的框架来实现经典密钥交换向后量子密钥交换的平滑过渡。

---
### [推荐] [2026/2003] NP-Hardness of Ideal Lattice Problems
...
- **链接:** [论文](https://eprint.iacr.org/2026/2003) | [PDF](https://eprint.iacr.org/2026/2003.pdf)


> **研究背景:** 研究背景：本文探讨了理想格问题（如最短向量问题SVP和最近向量问题CVP）在$\ell_2$范数下的最坏情况复杂性。
>
> **主要贡献:** 主要贡献：作者通过提供一个保持维度的确定多项式时间归约，将这些理想格问题与通用格问题联系起来。
>
> **达到效果:** 达到的效果：证明了理想格问题在最坏情况下的难度等同于通用格问题，从而为基于格的密码系统提供了理论支持。
>
> **技术梗概:** 技术梗概：通过构造嵌入数域的理想格来近似输入格，并利用其判别式和整环性质实现归约。

---
### [推荐] [2026/2009] On Succinct Obfuscation via Propositional Proofs

- **匹配关键字:** LWE

- **作者:** Abhishek Jain, Zhengzhong Jin, Surya Mathialagan, Omer Paneth

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2009) | [PDF](https://eprint.iacr.org/2026/2009.pdf)


> **研究背景:** 研究旨在最小化混淆程序的大小，特别是在输入等价程序中去除输入大小依赖性。
>
> **主要贡献:** 提出了完全简洁的IO新概念，并基于特定类别的程序构建了此类混淆。
>
> **达到效果:** 成功实现了与公有部分和输入大小无关的增长的混淆程序大小，并通过简洁密码学原语将其扩展到通用IO安全。
>
> **技术梗概:** 利用Cook理论PV中的等价性证明，结合超级多项式硬电路和LWE，构造了完全简洁的pv-IO，并基于这些原语进行进一步的构建。

---
### [推荐] [2026/2010] Beasley: Efficient Zero-Knowledge Proofs for Lattice-based Round-Optimal Oblivious Pseudorandom Functions

- **匹配关键字:** lattice

- **作者:** Baiyu Li, Ting-Yuan Wang, Jiapeng Zhang

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/2010) | [PDF](https://eprint.iacr.org/2026/2010.pdf)


> **研究背景:** 现有可验证的不可知态伪随机函数(VOPRF)主要依赖于易受量子攻击的安全假设。尽管存在理论上同时实现轮最优和恶意安全性的格基构造，但缺乏实际实施，因为证明基于格基伪随机函数正确评估的零知识证明极其昂贵。
>
> **主要贡献:** Beasley首次实现了基于标准格假设的轮最优、恶意安全的VOPRF原型。通过引入两个核心优化：一是扩展Boneh-Lewi-Montgomery-Raghunathan (BLMR13)伪随机函数，二是使用环切换多项式检查协议替代LaBRADOR风格证明，并高效实现OPRF计算的算术化。
>
> **达到效果:** Beasley显著降低了评估深度和运行时间，同时保持了安全性和性能。通过采用较大的环维数（512），实现了快速的环算术、紧凑的密钥以及高效的精确范围证明。
>
> **技术梗概:** 该设计结合了BLMR13伪随机函数的并行处理技术和环切换多项式检查协议，有效降低了评估深度和运行时间，并通过优化算术化提高了性能。

---
### [推荐] [2026/2011] CauchyFold: Residue-Optimal High-Arity Lattice Folding via Scaled Cauchy Challenges
...
- **链接:** [论文](https://eprint.iacr.org/2026/2011) | [PDF](https://eprint.iacr.org/2026/2011.pdf)


> **研究背景:** 研究背景：现有的直接高元二次折叠协议在处理大量输入时，暴露了过多的混合交互项，这限制了其效率和安全性。CauchyFold旨在通过优化挑战机制来减少这些交互项，从而提高折叠协议的效果。
>
> **主要贡献:** 主要贡献：提出了CauchyFold协议，利用缩放柯西挑战实现最优余数高元二次折叠，减少了混合状态的必要宽度，提高了折叠效率和安全性。
>
> **达到效果:** 达到的效果：通过优化后的挑战机制，CauchyFold成功实现了仅需线性关系的折叠过程，并且在特定假设下保证了节点级的知识健全性。
>
> **技术梗概:** 技术梗概：CauchyFold利用多项式载体进行承诺，在柯西挑战后确定混合部分的二次关系，并通过字段级别的检查将折叠的关系转化为承诺的线性关系，从而实现高效安全的折叠协议。

---
### [推荐] [2026/2012] Universally Composable Reverse Firewalls for Oblivious Linear Evaluation and Applications

- **匹配关键字:** homomorphic encryption

- **作者:** Suvradip Chakraborty, Kiran Deep Ghosh, Satrajit Ghosh, Subha Kar, Divyam Katiyar, Sougata Mandal, Soumit Pal, Amlan Sinha

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2012) | [PDF](https://eprint.iacr.org/2026/2012.pdf)


> **研究背景:** 传统的多方计算(MPC)假设各方在可信赖的机器上执行协议，但在设备被黑的情况下，这一假设会失效。为应对这种威胁，引入了反防火墙(RF)，通过外部方式安全地处理消息，而不依赖于内部执行环境的信任。
>
> **主要贡献:** 研究提出了首个适用于半诚实对手的、基于通用可组合性框架下的抗篡改算术MPC协议，并针对Oblivious Linear Evaluation(OLE)功能进行了形式化定义和实现。
>
> **达到效果:** 通过设计并证明了两种不同安全模型下的OLE协议，实现了在加性同态加密(AHE)基础上的半诚实对手下通用可组合安全性。
>
> **技术梗概:** 采用了基于类群组的关键可塑AHE方案，并结合了包装化去污OLE技术，在保证性能的同时增强了协议的安全性和实用性。

---
### [推荐] [2026/2017] On the eCK Security of MQV-Style LWE-Based Authenticated Key Exchange

- **匹配关键字:** post-quantum, LWE

- **作者:** Ruoyu Ding, Tianrun Yu, Yanbin Pan, Chi Cheng, Jian Weng

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2017) | [PDF](https://eprint.iacr.org/2026/2017.pdf)


> **研究背景:** 研究旨在设计直接基于学习误差(LWE)假设的认证密钥交换(AKE)协议，以克服现有通用构造的安全性不足，并解决在密钥重用情况下可能引入额外泄漏的问题。
>
> **主要贡献:** 作者提出了新的信号泄漏攻击方法，能够突破ZZDSD-AKE和GDLL-KE等MQV风格AKE协议的安全性限制，通过几何视角的方法揭示了这些协议在强扩展Canetti-Krawczyk(eCK)模型下的安全性问题。
>
> **达到效果:** 研究结果表明，即使采取随机化噪声对策以抵御密钥重用攻击，GDLL-KE协议仍然存在秘密依赖泄漏，从而证明了现有对策的有效性不足。
>
> **技术梗概:** 通过分析协议中的临时密钥泄露，并采用几何方法推导出静态密钥，作者成功地设计了一种新的信号泄漏攻击策略。

---
### [2026/1984] Improving GIJS Key Recovery for Classic McEliece

- **作者:** Stephen A. Weis

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1984) | [PDF](https://eprint.iacr.org/2026/1984.pdf)


> **研究背景:** 该研究改进了GIJS密钥恢复方法，针对经典McEliece公钥提出了更有效的攻击手段。
>
> **主要贡献:** 作者提出了一种新的密钥提取方法，并降低了GIJS区分器的成本估计。
>
> **达到效果:** 通过两种方法成功从区分器计算中提取出秘密密钥，降低了解密成本并提高了攻击效率。
>
> **技术梗概:** 第一种方法利用了梯度子码的特性进行线性代数运算；第二种方法直接从单次运行中读取支持信息。

---
### [2026/1986] Two-Anchor Holdout/Hermite: Solving the TII-254 McEliece Key Recovery Challenge

- **作者:** Markku-Juhani O. Saarinen

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1986) | [PDF](https://eprint.iacr.org/2026/1986.pdf)


> **研究背景:** 该研究解决了TII-254 McEliece密钥恢复挑战，这是目前在原始暴力破解度量标准下最难解决的挑战。
>
> **主要贡献:** 作者提出了一种十三步方法来解决此问题，并详细描述了其中的启发式和非启发式选择。
>
> **达到效果:** 通过这种方法，研究人员成功地识别出了87个可见定位器，并最终确定了完整的支持和多项式，显著提高了破解效率。
>
> **技术梗概:** 研究采用了计算两个121维关系核、结合隔离核心对、去除公共干扰空间以及识别剩余维度为有限几何的方法。

---
### [2026/1987] Bonsai: Scalable Private Payments

- **作者:** Patrick O'Grady, Lúcás Críostóir Meier, Guru-Vamsi Policharla

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1987) | [PDF](https://eprint.iacr.org/2026/1987.pdf)


> **研究背景:** 现有的私密支付系统通过公开每个交易的nullifier来防止双重支付，但随着交易量激增，nullifier集的增长对存储和隐私管理提出了挑战。
>
> **主要贡献:** Bonsai提出了一种基于账户的私密支付方案，通过验证者仅存储单个账户承诺并允许用户私有地维护收到的nullifiers，从而解决了大规模吞吐量下的隐私保护问题。
>
> **达到效果:** 该系统能够在不增加证明大小和几乎无额外证明者开销的情况下实现零知识证明，并结合批处理验证策略，原型在M5 MacBook Pro上每秒验证超过一百万次操作。
>
> **技术梗概:** Bonsai通过引入零知识证明技术并优化验证流程，实现了高效的大规模私密支付验证，同时保持了较低的资源消耗和较高的隐私保护水平。

---
### [2026/1989] Angular Watermarking: Sharp Leakage Bounds and Public Subspace Validation

- **作者:** Bin He

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1989) | [PDF](https://eprint.iacr.org/2026/1989.pdf)


> **研究背景:** 研究背景：该论文探讨了基于角度的水印技术，特别是在高维空间中精确度量隐藏信息泄漏边界的方法。
>
> **主要贡献:** 主要贡献在于推导出高斯隐秘模型下精确的四阶特征，并提出了公钥验证方向来估计候选载体的相关性。
>
> **达到效果:** 达到了在固定探针学习者下，观测数量与秩、准确性和置信度之间的关系，并提供了无密钥情况下对候选载体相关性的有限样本下界。
>
> **技术梗概:** 技术梗概：通过精确的四阶张量协方差边界来分析几何压缩和验证方差之间的权衡，并使用独立验证方向进行公钥验证。

---
### [2026/1990] Bounded Information: PAC Certification of Multivariate Side-Channel Traces

- **作者:** Kuheli Pratihar, Nimish Mishra, Debdeep Mukhopadhyay

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1990) | [PDF](https://eprint.iacr.org/2026/1990.pdf)


> **研究背景:** 侧信道泄漏认证旨在量化攻击者从观测到的泄漏中能学到多少关于秘密变量的信息，现有的信息论估计方法如感知信息、假设信息和非参数互信息，在高维迹线中变得不稳定且无法提供最佳攻击者的有限样本证书。
>
> **主要贡献:** 研究提出了Bounded Information (BI)，这是一种可能近似的有限样本认证方法，能够上界固定攻击者范围在未见迹线上的精确恢复成功率，并定义了$\mathrm{BI}^{\mathrm{loc}}(\varepsilon)$来限制与套件中模型得分差异不超过$\varepsilon$的攻击者的恢复成功。
>
> **达到效果:** 通过八个侧信道基准测试，BI提供了无需密度估计的稳定证书，并且其紧度诊断帮助评估者判断认证值是泄漏的真实测量还是保守边界。
>
> **技术梗概:** 该方法采用了KL-二项置信区间与固定套件上的并集界，将标准化攻击评价转化为可审计的证书，具有明确的攻击范围和置信水平。

---
### [2026/1993] Network-Agnostic VSS with Quadratic Communication in Computational Setting

- **作者:** Yusong Yao, Qi Feng, Cong Peng, Min Luo, Debiao He

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1993) | [PDF](https://eprint.iacr.org/2026/1993.pdf)


> **研究背景:** Verifiable Secret Sharing (VSS)是多方计算（MPC）中的核心机制，Bhimrajka等人提出了首个在计算模型下具有网络无关性的VSS和基于VSS的MPC方案。
>
> **主要贡献:** 本文重新审视了网络无关性VSS架构，并提出了一种更高效且通用的网络无关性VSS结构，引入了一种新的虚拟节点策略以提高效率并减少通信量。
>
> **达到效果:** 该方案在单次分享中通信成本为$O(n^2)$比特，在逐轮分享中使用争议控制技术通信成本为$O(n)$比特，实现了首个具有摊销线性通信复杂度的计算模型下的网络无关性VSS协议。
>
> **技术梗概:** 本文的关键技术是一种新的虚拟节点策略，通过引入更多节点进行重建以减少实际参与节点数，并结合争议控制技术优化了通信效率。

---
### [2026/1994] Data-strophy: When Your Integrity Goes Wild, So Does Your Data!

- **作者:** Ya-Nan Li, Yaqing Song, Qiang Tang, Moti Yung, Yuan Zhang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1994) | [PDF](https://eprint.iacr.org/2026/1994.pdf)


> **研究背景:** 研究背景：Proton Docs/Sheets 是一个支持实时协作编辑的文档服务，声称提供端到端的安全性。然而，本文揭示了其在实际应用中的安全漏洞。
>
> **主要贡献:** 主要贡献：作者发现了三种针对 Proton Docs/Sheets 的完整性攻击方法，这些攻击能够篡改历史记录、修改上下文和进行审查，并且能够在服务器行为正常或恶意的情况下实施。
>
> **达到效果:** 达到的效果：通过这些攻击，可以绕过现有的检测机制，对文档内容进行未授权的更改，从而威胁用户的数据安全与隐私。
>
> **技术梗概:** 技术梗概：作者通过对开源 Web 客户端和网页代码的分析，发现了 Proton Docs/Sheets 在设计上的缺陷，并提出了相应的缓解措施。

---
### [2026/1995] Symmetric Models for Syndrome Decoding

- **作者:** Elisa Gorla, Simone Trebiani

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1995) | [PDF](https://eprint.iacr.org/2026/1995.pdf)


> **研究背景:** 该研究旨在通过引入基于基本对称多项式的新型多项式模型，为二元情况下的 Syndrome Decoding 问题提供一种精确解法。
>
> **主要贡献:** 贡献在于提出了两种不同复杂度的多项式模型，并估计了求解这些模型所需的计算复杂性。
>
> **达到效果:** 研究表明，新模型在某些情况下比现有方法更高效，特别是在特定实例上。
>
> **技术梗概:** 技术上，通过分析相关理想上的正则度和解决度来估算计算复杂性。

---
### [2026/1996] One More A: Sharper Tails Without Scaling

- **作者:** Majid Khabbazian

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/1996) | [PDF](https://eprint.iacr.org/2026/1996.pdf)


> **研究背景:** RAA码在无随机坐标缩放的情况下，其最小距离与块长成线性关系。然而，对于特定的重复因子和块长度范围，未缩放的RAA码和缩放的RAA码之间存在可靠性上的权衡。
>
> **主要贡献:** 作者提出了一种新的三重累加器结构（RAAA），通过增加一个额外的累加器，实现了在不使用随机缩放的情况下，显著提高可靠性。
>
> **达到效果:** 对于特定重复因子和块长度范围内的RAAA码，其最小距离低于给定比例的概率与未缩放的RAA码相比有了更大幅度的减少，从而提高了编码效率和可靠性。
>
> **技术梗概:** 通过引入第三个累加器，作者证明了这种新的RAAA结构能够在保持固定特征域的基础上，显著提高编码的可靠性，并减少了所需的计算复杂度。

---
### [2026/1997] LibFWHT: From Exact Walsh Spectra to Key Dependence in Differential-Linear Correlations

- **作者:** Hosein Hadipour, Saleh Khalaj Monfared, Jens Alich, Jan Vorloeper

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1997) | [PDF](https://eprint.iacr.org/2026/1997.pdf)


> **研究背景:** LibFWHT旨在提供一种通用工具，用于加速大规模数组的批次操作，特别是在沃尔什-哈达玛变换（WHT）中，以支持差分线性密码分析等复杂任务。
>
> **主要贡献:** 该库通过结合矢量化、多核和图形处理器后端，提供了比现有工具更快的性能，并且首次实现了对SIMON-32、KATAN-32、BEANIE、KeeLoq和RC5-16等32位块密码体制的精确差分线性区分器。
>
> **达到效果:** LibFWHT在单个CPU核心上比FFTW快2.5到3.3倍，在NVIDIA H200图形处理器设备上实现了每秒1.8万亿次操作，并且能够显著提高针对特定密钥依赖性的分析速度。
>
> **技术梗概:** 该库利用沃尔什-哈达玛变换和矢量化技术，通过批处理操作优化了大规模数据的处理效率。

---
### [2026/1998] A Low-Communication Garbled RAM from Homomorphic Secret Sharing

- **作者:** Chase Fickes, Jinye He, Wei-Kai Lin

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1998) | [PDF](https://eprint.iacr.org/2026/1998.pdf)


> **研究背景:** 研究背景：门控电路是现代密码学和安全多方计算中的基本组成部分。对于自然以随机存取机（RAM）模型表示的实际程序，门控RAM是门控电路的RAM对应物，避免了将整个程序编译成电路的成本，通信复杂度成为主要效率指标。
>
> **主要贡献:** 主要贡献：本文构建了一个适用于运行时间T、内存大小为N个W位字的公有程序的门控RAM方案，实现了O(T·(W+λ)·log N)+poly(λ)比特的通信量。对于足够大的字长W≥λ和运行时间T≥poly(λ)，通信量达到O(T W log N)，几乎与最优的盲存取存储器带宽匹配。
>
> **达到效果:** 达到的效果：该方案在保持内存访问隐蔽性的同时，实现了接近最优的通信效率，为实际程序的安全执行提供了新的解决方案。
>
> **技术梗概:** 技术梗概：本文基于Ishai等人（Crypto'25）和Li等人（Eurocrypt'26）最近提出的简洁门控电路，并利用同态秘密共享技术构建了该方案。

---
### [2026/1999] Revisiting Single-Color Initial Structures in Meet-In-The-Middle Attacks

- **作者:** Shiyao Chen, Jian Guo, Wenjie Nan, Danping Shi, Tianyu Zhang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1999) | [PDF](https://eprint.iacr.org/2026/1999.pdf)


> **研究背景:** Meet-in-the-middle攻击框架是预象、密钥恢复和碰撞攻击中最具影响力的密码分析技术之一。
>
> **主要贡献:** 作者提出了一种改进的单色初始结构技术和扩展了部分目标前象到碰撞转换方法，用于MITM基碰撞攻击。
>
> **达到效果:** 这些技术被应用于AES-like构造的(伪)预象、(选择前缀)碰撞和引导攻击中，并取得了优于现有最佳结果的效果。
>
> **技术梗概:** 通过利用常量空间在MITM攻击中的自由度来改进单色初始结构，同时扩展了用于构建选择前缀碰撞和钻石结构的部分目标前象到碰撞转换方法。

---
### [2026/2000] Information-Theoretic Hiding in Quantum Marginal Fibers

- **作者:** Jaroslav Hruby

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2000) | [PDF](https://eprint.iacr.org/2026/2000.pdf)


> **研究背景:** 研究探讨了在对手拥有无限制量子处理能力但仅对声明子系统之一有物理访问权限的情况下，信息论意义上的隐藏机制。
>
> **主要贡献:** 贡献在于通过固定边际集组织迹距离直径、正交零错误填充和精确相干子空间隐藏，并区分基态边缘等价性与量子纠错的交叉项条件。
>
> **达到效果:** 结果包括对这些隐藏技术在特定条件下的效果进行了分析，揭示了经典编码字在纤维中的完美隐藏机制。
>
> **技术梗概:** 采用信息论方法和技术来研究量子边际纤维中的隐藏问题，并通过数学工具进行验证。

---
### [2026/2002] Derivatives of Quantum Randomness: Separating Pseudorandom Unitaries from Pseudorandom (Function-like) States

- **作者:** Minki Hhan

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2002) | [PDF](https://eprint.iacr.org/2026/2002.pdf)


> **研究背景:** 量子计算引入了新的伪随机态和酉算子原语，如伪随机态生成器（PRSG）、伪随机函数似态生成器（PRFSG）和伪随机酉算子（PRU）。本文探讨了这些原语之间的分离问题。
>
> **主要贡献:** 作者展示了在最强态观念与最弱酉观念之间存在全单位元Oracle分离：即使适应性安全、量子可访问的PRFSG也不能推导出非适应性安全、单向的PRU，即便其实现允许是非酉且使用任意数量辅助量子比特。
>
> **达到效果:** 这一分离揭示了量子态和量子酉算子伪随机性的根本区别。通过研究候选PRU构造对状态生成Oracle的映射及其导数，作者利用低秩结构区分了生成的单位元与真正随机的单位元。
>
> **技术梗概:** 作者将候选PRU构造视为从基础Oracle态到实现酉算子的映射，并研究该映射的导数。由于这些导数本质上是低秩的，作者通过这一特性来区分生成的单位元和真正的随机单位元。

---
### [2026/2004] Exact Error-Compensated Pruning in Matrix Multiplication Time

- **作者:** Xiaoyu Li

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2004) | [PDF](https://eprint.iacr.org/2026/2004.pdf)


> **研究背景:** 研究背景：本文探讨了在矩阵乘法运算中精确误差补偿剪枝的算术复杂性，旨在优化计算效率并保持精度。
>
> **主要贡献:** 主要贡献在于提出了一个递归执行方法，该方法能够以可控的方式进行权重移除和错误传播，同时保留关键决策信息。
>
> **达到效果:** 达到的效果是对于特定维度的矩阵层，通过精确误差补偿剪枝可以实现接近理论下界的操作复杂度，并且在平方维度情况下验证了多种选择规则的有效性。
>
> **技术梗概:** 技术梗概：利用源到目标因子消去机制和局部系数提取论证来比较指数，确保即使在包含比较和平方根操作的情况下也能精确计算。

---
### [2026/2005] SoK: Private Transformer Inference Across Systems, Models, and Cryptography

- **作者:** Andes Y. L. Kei, Sherman S. M. Chow

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2005) | [PDF](https://eprint.iacr.org/2026/2005.pdf)


> **研究背景:** 研究背景：私有推理旨在保护用户查询和模型权重，无需依赖可信硬件或统计隐私放松。然而，变压器结合了大规模的秘密矩阵乘法、昂贵的非线性操作以及顺序自回归执行，这使得实现私有推理面临挑战。
>
> **主要贡献:** 主要贡献在于系统化分析了2022年至2026年间58个加密私有变压器框架，并从三个交互层面（系统、模型和密码学）进行分类，揭示了跨后端适用性的两个重复约束条件。
>
> **达到效果:** 达到的效果是识别出四个类别的安全问题影响了五个框架，并综合了恶意安全设计中的组合边界。此外，发现16个框架依赖于经验校准或特定分布机制，21个需要额外训练，限制了一般化和跨框架可比性。
>
> **技术梗概:** 技术梗概涉及从系统执行与优化、模型的密码学-机器学习协同设计及适应到加密操作的安全实现三个层面进行分类分析，并通过对比不同框架来识别安全问题和组合边界。

---
### [2026/2006] Beyond DCR: HSS and PCFs from Subgroup Indistinguishability

- **作者:** Sebastian Hasler

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2006) | [PDF](https://eprint.iacr.org/2026/2006.pdf)


> **研究背景:** 该研究在群论框架下构建了同态秘密共享（HSS）和伪随机相关函数（PCFs），基于Brakerski和Goldwasser引入的子群不可区分性（SgI）假设，扩展了能够推导出HSS和PCFs的安全假设集合。
>
> **主要贡献:** 贡献在于提出了一种新的分布式离散对数算法（DDLog），并在特定条件下实现了在任意群中的高效计算，并且通过分而治之的优化提高了性能。此外，还引入了公共硬币子群不可区分性（PC-SgI）的概念，并基于此构建了多种类型的伪随机相关函数。
>
> **达到效果:** 研究成果包括：1) 提出了一种适用于任意群的新DDLog算法；2) 构建了OLE和二阶相关性的伪随机相关函数，同时依赖稀疏的线性近似噪声（LPN）假设；3) 扩展了能够推导HSS和PCFs的安全假设集合。
>
> **技术梗概:** 技术上采用了群论框架、子群不可区分性假设以及分布式离散对数算法，并通过分而治之的方法优化了DDLog的性能。

---
### [2026/2007] Sharp Minimum-Distance Tails for BAA Codes

- **作者:** Majid Khabbazian

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2007) | [PDF](https://eprint.iacr.org/2026/2007.pdf)


> **研究背景:** 研究背景：针对BAA码的最小距离尾部概率提供了定量保证，这对于伪随机相关生成器和基于代码的证明系统中的快速随机线性码至关重要。
>
> **主要贡献:** 主要贡献：确定了固定长度构成元素最小距离为d≥2时，全局最小距离下尾部的概率表达式，并证明了线性域增长是N^(1-d)定律的确切阶数。
>
> **达到效果:** 达到的效果：对于特定的MDS块矩阵，在大字段情况下，验证了已知标记的一块系数是实际失败概率的主要常数；并提供了接近精确的双侧有限证书，证实了发布的大型领域基准的有效性。
>
> **技术梗概:** 技术梗概：通过支持依赖加权间隔估计控制每个活动块计数和每层取消，并将全块间隙事件分组以限制其相关性来实现上述结果。

---
### [2026/2008] New Imperfect High Threshold and Slice Secret Sharing Schemes

- **作者:** Andrej Bogdanov, Yanbo Chen

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/2008) | [PDF](https://eprint.iacr.org/2026/2008.pdf)


> **研究背景:** 本文提出了针对短秘密的新颖的(n-t+1)-out-of-n门限和切片秘密共享方案，特别关注于不完美安全性。
>
> **主要贡献:** 贡献在于设计了ε-差异私密的秘密共享方案与ε-统计私密的位秘密共享方案，前者在有限域中因子分解猜想的支持下实现了更小的份额大小，后者提供了计算安全性的替代选择。
>
> **达到效果:** 结果是成功构建了满足特定门限条件且具有较小份额大小的安全秘密共享方案，并讨论了其与秘密共享对偶性相关联的应用。
>
> **技术梗概:** 技术上依赖于在有限域中因子分解的猜想，以及通过优化份额大小来实现不完美安全性的方法。

---
### [2026/2013] Revisiting Differential-Linear Cryptanalysis via a Walsh-Transform Perspective

- **作者:** Jiahui He, Kai Hu, Zhongfeng Niu, Bart Preneel, Meiqin Wang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2013) | [PDF](https://eprint.iacr.org/2026/2013.pdf)


> **研究背景:** 研究背景：差分-线性密码分析（SDL）已经发展成熟，包括SDL连接表（SDLCT）框架、多轮中间部分扩展以及基于CP的搜索模型。然而，旋转差分-线性（RDL）和内部差分-线性（IDL）变体尚未被充分研究。
>
> **主要贡献:** 主要贡献：作者统一了SDL、RDL和IDL，并提出了一个基于差异转换函数（DTF）的沃尔什变换框架，实现了这些方法之间的技术转移，提高了相关性计算和区分器搜索的有效性。
>
> **达到效果:** 达到的效果：对于ARX原语，作者推导出单模加法的$2\times 2$矩阵乘积公式，并解决了Niu等人提出的一个开放问题。在应用该框架时，作者为\(\xoodoo-p\)、\(\ascon-p\)、\(\alzette\)等提供了新的区分器。
>
> **技术梗概:** 技术梗概：通过沃尔什变换视角重新审视差分-线性密码分析，统一了SDL、RDL和IDL，并利用差异转换函数（DTF）进行相关性计算和区分器搜索。

---
### [2026/2014] Quantum Discrete Logarithms on Elliptic Curves with $5n/2+o(n)$ Logical Qubits and $\widetilde O(n^2)$ Toffoli Gates

- **作者:** Sunghyeon Jo, Gye Jin Lee

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/2014) | [PDF](https://eprint.iacr.org/2026/2014.pdf)


> **研究背景:** 该研究旨在提出一种量子算法，用于解决椭圆曲线上基于素数字段的离散对数问题，以提高量子计算效率。
>
> **主要贡献:** 作者通过优化逻辑量子比特和Toffoli门的数量，降低了空间复杂度，并改进了中间余数和系数存储的方法。
>
> **达到效果:** 研究实现了在近二次时间复杂度下使用较少逻辑量子比特解决椭圆曲线离散对数问题的算法。
>
> **技术梗概:** 采用精确就地模逆运算和基于测量的未计算技术，以及Hua恒等式减少变量平方乘法需求，从而优化了点加操作。

---
### [2026/2015] A Unified Analysis of Refresh Gadgets in the Random Probing Model

- **作者:** Sonia Belaïd, Victor Normand, Matthieu Rivain

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/2015) | [PDF](https://eprint.iacr.org/2026/2015.pdf)


> **研究背景:** 研究背景：掩码技术是一种标准的对抗侧信道攻击的方法，其安全性通常在随机探针模型下进行分析。近年来，基于通用/基数随机探针可组合性（RPC）框架出现了多种掩码编译器，但大多数方法依赖于特定设计的刷新小部件，这些小部件与安全分析紧密相关。
>
> **主要贡献:** 主要贡献：作者正式定义了两种刷新小部件家族——直接和零编码基底刷新，并在显式随机性和均匀性条件下定义了理想构造；引入了一个原子刷新函数来统一表达和分析现有构造，并推导出它们的通用/基数RPC边界，从而获得多个构造的解析公式。
>
> **达到效果:** 达到的效果：通过系统比较理想的和实际的刷新小部件，作者为AES和Raccoon加密实现提供了系统的对比结果，增强了对掩码技术安全性的理解。
>
> **技术梗概:** 技术梗概：采用形式化方法定义理想构造，并利用原子刷新函数统一分析现有构造，推导出其边界条件，从而提供了一种系统化的评估框架。

---
### [2026/2016] Refining Leakage Diagram Analyses for Random Probing Security

- **作者:** Sonia Belaïd, Ghozlane Boukacem, Gaëtan Cassiers, Victor Normand, Mélissa Rossi

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/2016) | [PDF](https://eprint.iacr.org/2026/2016.pdf)


> **研究背景:** 研究背景：掩码技术被广泛用于抵御侧信道攻击，通过将敏感变量拆分为随机份额来防止中间计算的信息泄露。然而，在随机探针模型下构建实现随机探针安全性（RPS）的大规模电路具有挑战性。
>
> **主要贡献:** 主要贡献：本文重新审视并改进了TCHES 2023年发表的泄漏图方法，形式化并泛化证明以表明泄漏图中轨道的概率上限了攻击者的RPS优势，并细化了乘法小部件中边包含概率的分析，从而获得了更紧的安全性边界。
>
> **达到效果:** 达到的效果：通过详细研究小型案例和较大的结构属性，改进了轨道的组合分析，最终提高了安全性的证明强度。
>
> **技术梗概:** 技术梗概：本文提出了对泄漏图方法进行形式化和泛化的证明，并通过细致的研究和新的结构性质来优化乘法小部件中边包含概率的分析。

---
### [2026/2018] On the (Im)possibility of Asynchronous Proactive Secret Sharing

- **作者:** Erica Blum, Yingzi Gao, Julian Loss, Yuan Lu, Zhenliang Lu, Qiang Tang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/2018) | [PDF](https://eprint.iacr.org/2026/2018.pdf)


> **研究背景:** 本文系统研究了在完全异步认证网络中主动秘密共享（APSS）的可能性和不可能性，特别是在缺乏全局时钟的情况下，不同的敌手模型带来的复杂性。
>
> **主要贡献:** 作者首次形式化了一级自然移动敌手模型，用于异步PSS，并通过展示通用攻击建立了新的不可能结果，从而确定了相应的抗毁上限。
>
> **达到效果:** 此外，作者还展示了在每个模型中实现最优抗毁性的显式APSS协议，证明了这些界限的紧致性。
>
> **技术梗概:** 研究采用了敌手建模、协议设计和安全性分析等技术来探讨异步网络中的主动秘密共享问题。

---

## 更新: 2026-09-14 07:15

*新增 29 篇论文 (编号 1955--1983)*

### [推荐] [2026/1956] PRISM: Efficient zkSNARKs for RNS-Based Homomorphic Encryption

- **匹配关键字:** homomorphic encryption

- **作者:** Zhelei Zhou, Yun Li, Zhaomin Yang, Cheng Hong, Tao Wei

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1956) | [PDF](https://eprint.iacr.org/2026/1956.pdf)


> **研究背景:** 现有技术在实现基于RNS的HE方案的有效vHE时面临挑战，主要在于如何高效地进行跨域一致性验证而不增加证明大小和验证者成本。
>
> **主要贡献:** $\mathsf{PRISM}$提出了首个适用于标准RNS基HE方案的zkSNARK，通过引入多项式承诺方案和交互式Oracle证明来解决跨域一致性验证难题。
>
> **达到效果:** 该方法实现了高效且零知识的vHE验证，同时保持了较低的证明大小和验证者成本，并在实际应用中展示了良好的性能。
>
> **技术梗概:** 采用了多项式承诺方案（MF-PCS）和多项式交互Oracle证明（PIOP），并通过向量盲线性评估（VOLE）关联实现零知识验证。

---
### [推荐] [2026/1958] Arithmetic for Large-Characteristic Finite Fields in CKKS

- **匹配关键字:** lattice

- **作者:** Daehyun Jang, Junho Lee

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1958) | [PDF](https://eprint.iacr.org/2026/1958.pdf)


> **研究背景:** 研究背景：在大素数特征的有限域上进行计算时，整数提升的大小限制了支持的乘法深度。现有方法如Seur\'e和Suvanto的编码需要控制放大误差，但对大特征域的支持不足。
>
> **主要贡献:** 主要贡献：结合Peikert等人提出的明文位分解技术和Seur\'e和Suvanto的有限域编码，支持在$\mathbb F_{p^r}$上进行算术运算，并通过多项式减少保持字段操作。
>
> **达到效果:** 达到的效果：分析误差放大并通过限制乘法操作符的幂来控制评估范数，使用携带晶格确定每个消息可用提升的评估范数界限。对于secp256k1素数和扩展度$1, 2, 4$，评估了边界和支持深度。
>
> **技术梗概:** 技术梗概：通过将字段元素表示为两个变量的多项式，并利用多项式减少来保持字段操作，同时使用携带晶格限制提升的评估范数，从而支持在大特征域上的高效算术运算。

---
### [推荐] [2026/1962] Compare Before Clearing: Exact Integral-Comparison Frontiers for Lattice Extraction

- **匹配关键字:** lattice

- **作者:** Xiang Wang, Shihui Fu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1962) | [PDF](https://eprint.iacr.org/2026/1962.pdf)


> **研究背景:** 该研究聚焦于改进Lattice提取过程中的直接整数比较方法，以提高协议的安全性和效率。
>
> **主要贡献:** 作者提出了一个通用的多项式块系统直接整数比较的形式化定义，并界定了最坏情况下的系数度数。
>
> **达到效果:** 通过精确条件重采样技术，研究达到了无需相互成功即可获得所需部分同步成功的执行效果。
>
> **技术梗概:** 采用分支分离的多项式先整数后比较架构，结合条件重采样方法优化了Lattice提取过程。

---
### [推荐] [2026/1964] Three-Round Weak Non-Malleable Zero-Knowledge Argument

- **匹配关键字:** homomorphic encryption

- **作者:** Xinxuan Zhang, Yuanju Wei, Zhichao Wang, Zhongliang Zhang, Ming Yang, Ruida Wang, Yi Deng, Hailong Wang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1964) | [PDF](https://eprint.iacr.org/2026/1964.pdf)


> **研究背景:** 非可塑零知识论证(NMZK)是一种强零知识论证概念，确保在中间人(MIM)攻击下的安全性。尽管存在各种弱零知识论证的三轮构造，但在标准假设下所有已知(弱)NMZK协议在普通模型中至少需要四轮。
>
> **主要贡献:** 本文首次提出了基于标准密码学假设的第一种三轮弱非可塑零知识论证。
>
> **达到效果:** 该协议满足弱零知识和$\epsilon$-非可塑性，其中后者允许MIM实验与独立实验之间的多项式逆概率差距$\epsilon$。
>
> **技术梗概:** 我们的构造仅依赖于成熟的原语，如两消息盲传输协议、延迟输入一致性论证、非交互式承诺以及电路隐私全同态加密。

---
### [推荐] [2026/1965] Lattice-based Threshold Traitor Tracing with Public Traceability
...
- **链接:** [论文](https://eprint.iacr.org/2026/1965) | [PDF](https://eprint.iacr.org/2026/1965.pdf)


> **研究背景:** 现有阈值追踪方案大多缺乏后量子安全保证，而Das等人提出的基于格的私有追踪方案虽具备后量子安全性但存在单点故障问题。
>
> **主要贡献:** 本文首次提出同时实现后量子安全和公开可追踪性的阈值追踪方案，并构建了Q-Partite Threshold Public Key Encryption (QTPKE)作为核心组件。
>
> **达到效果:** 该方案能够在不依赖任何秘密追踪密钥的情况下，由任何人对盗版解码器进行追踪，显著提高了系统的安全性与可靠性。
>
> **技术梗概:** 通过在Regev加密方案的基础上应用{0,1}-线性秘密共享和关键位移机制构建QTPKE，并证明其在标准模型下的安全性能。

---
### [推荐] [2026/1966] Design and Analysis of Isogeny-Based Strong Designated Verifier Signature
...
- **链接:** [论文](https://eprint.iacr.org/2026/1966) | [PDF](https://eprint.iacr.org/2026/1966.pdf)


> **研究背景:** 强指定验证签名提供身份认证同时限制验证给选定的验证者，并保护签名人免受可转移证据的影响。在后量子环境中设计此类签名极具挑战性，需要实现身份认证、签名人隐私、模拟和效率等多方面的要求。
>
> **主要贡献:** 作者分析了CSI-SDVS方案中响应设计的安全性问题，并提出了一种基于类群作用的强指定验证签名方案，证明了其正确性、不可转让性、签名人隐私性和强伪造性。
>
> **达到效果:** 新方案在随机预言模型下被证明是安全的，在$\eta=1$的情况下，签名大小为113字节，比CSI-SDVS增加了64字节，但签名人密钥大小保持不变。
>
> **技术梗概:** 该研究采用了基于类群作用的密码技术，并通过改进响应设计来增强安全性。

---
### [推荐] [2026/1968] The Closest-Vector Problem over Cyclotomics and its Application to Homomorphic Encryption

- **匹配关键字:** lattice, homomorphic encryption

- **作者:** Natalie Lang, Dana Dachman-Soled

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1968) | [PDF](https://eprint.iacr.org/2026/1968.pdf)


> **研究背景:** 研究围绕在任意阶数m的cyclotomic格上最近向量问题（CVP）中的舍入误差，特别是在近似同态加密（HE）中，精确的关键操作直接影响噪声和精度。
>
> **主要贡献:** 贡献在于推导了最坏情况下的新的覆盖半径上限，并研究了Babai最近平面算法的平均情况解法及其误差上界。
>
> **达到效果:** 结果表明，在任意格中，对于均匀分布的目标值，Babai舍入的误差在Gram–Schmidt基下具有独立均匀坐标，从而确定其均方误差；对于cyclotomic格，进一步证明了平方误差集中在均值附近。
>
> **技术梗概:** 技术上利用cyclotomics的张量分解来表达边界，并以质数幂分解的形式表示m，揭示非2的幂次cyclotomics相对于2的幂次cyclotomics具有可证明改进的舍入效果，同时保持广泛的指数家族中的高效算术。

---
### [推荐] [2026/1969] Efficient Polynomial System Solving via Dixon Resultants: Applications to AO Primitives

- **匹配关键字:** lattice

- **作者:** Haohai Suo, Jiamin Cui

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1969) | [PDF](https://eprint.iacr.org/2026/1969.pdf)


> **研究背景:** 多变量多项式求解在代数攻击AO原语中至关重要，但现有方法如Gröbner基和Sylvester结果式存在局限性。
>
> **主要贡献:** 作者重新审视了Dixon结果式的高效性和灵活性，并提出了改进的上限估计和复杂度分析。
>
> **达到效果:** 实验表明，在低变量高次情形下，该方法优于最先进的Gröbner基求解器Magma和msolve。
>
> **技术梗概:** 通过格路径计数获得细化的Dixon矩阵大小上界，并分析多种行列式计算模型下的复杂度。

---
### [推荐] [2026/1974] Exact-Coset Response Existence in SQIsign-like Protocols: Beyond Additive Hom Geometry

- **匹配关键字:** lattice

- **作者:** Ti-Hong Qin, Hong-Yu Tang, Zong-Bin Wang, Wen-Lun Pan

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1974) | [PDF](https://eprint.iacr.org/2026/1974.pdf)


> **研究背景:** 研究探讨了在SQIsign-like协议中，精确余子响应的存在性问题，特别是在超奇异曲线特征p下的端点碰撞、精确层级结构约束和采样依赖性的影响。
>
> **主要贡献:** 证明了对于每个独立于端点的二进制度数滤波器以及$1 \le D < p$中的每一个D，存在一个均方端点偏差界$\mathcal{V}_a \ll_\delta p^\delta(D^2 + D^{7/2}/p) + P_a^2/p^2$。
>
> **达到效果:** 证明了对于独立均匀分布的端点在$p^{1/2+\gamma}$以上，存在概率为$1-o(1)$；而在$p^{1/2-\gamma}$以下，则存在概率为$o(1)$。Weil配对将核的二次决定性字符的两个余子转换为度数滤波器。
>
> **技术梗概:** 通过结合平方除数反转、经典Brandt–Hecke估计和谐振加权Petersson估计来证明上述结果，并利用精确余子发生率界限，得出了在相同度量界下的相反存在概率：$1-o(1)$对于该子群，而对分裂和非分裂卡坦正规化为$o(1)$。

---
### [推荐] [2026/1975] Oblivious Signaling

- **匹配关键字:** homomorphic encryption

- **作者:** Mirza Kamrul Bashar Shuhan, Foteini Baldimtsi, Giuseppe Ateniese

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1975) | [PDF](https://eprint.iacr.org/2026/1975.pdf)


> **研究背景:** 传统的匿名消息服务通过广播所有密文来隐藏目的地，但增加了每个接收者的扫描负担。Oblivious Message Retrieval (OMR) 使用全同态加密（FHE）让不可信服务器为接收者代劳检索过程而不泄露相关信息，但仍存在成本问题。
>
> **主要贡献:** 本文提出了Oblivious Signaling，将检索成本转移到发送端，通过统一的同态更新机制，使消息传递成本严格依赖于匿名集大小而非全局流量。
>
> **达到效果:** 该方案实现了接收者隐私保护，并在与不可信服务器和其他用户合谋的情况下仍然有效。实验原型表明，在某些实际场景中，这种成本模型的转变优于传统方法。
>
> **技术梗概:** 通过为每个接收者维护固定大小的加密收件箱，并使用统一的同态更新机制来吸收消息，Oblivious Signaling 实现了高效的消息传递和低成本的检查过程。

---
### [推荐] [2026/1977] Lattice-based Secret-Key Functional Encryption for Constant-Degree Polynomials

- **匹配关键字:** lattice, post-quantum, LWE

- **作者:** Valerio Cini, Russell W. F. Lai, Akin Ünal, Ivy K. Y. Woo

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1977) | [PDF](https://eprint.iacr.org/2026/1977.pdf)


> **研究背景:** 该研究旨在构建基于格的密钥功能加密方案，用于低范数常数级多项式，并扩展至$\mathsf{NC}^{0}$电路。
>
> **主要贡献:** 贡献在于提出了新的针对特定度数张量结构矩阵的陷阱门和预象采样算法，以及一种新型假设，证明了在该假设下可实现选择性安全的秘密密钥功能加密方案。
>
> **达到效果:** 研究结果表明，在上述新假设、标准LWE及多项式扩展伪随机生成器存在性的条件下，可以构建后量子安全的不可区分混淆（iO）。同时提供了一种基于标准LWE的选择性安全公共密钥线性函数加密方案。
>
> **技术梗概:** 技术上依赖于特定度数张量结构矩阵的新陷阱门和预象采样算法，并提出了一个新型假设，以证明秘密密钥功能加密方案的安全性。

---
### [推荐] [2026/1983] Akita: A High-Performance Lattice-Based Polynomial Commitment Scheme

- **匹配关键字:** lattice, post-quantum

- **作者:** Quang Dao, Omid Bodaghi, Amirhossein Khajehpour, Giuseppe Vitto, Mohammadtaghi Badakhshan, Markos Georghiades, Fengrun Liu, Jiapeng Zhang, Justin Thaler

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1983) | [PDF](https://eprint.iacr.org/2026/1983.pdf)


> **研究背景:** Akita旨在解决现有基于格的多项式承诺方案在证明大小、验证时间和安全性之间的权衡问题，特别是针对Twist和Shout等应用场景的需求。
>
> **主要贡献:** Akita通过引入新的设置卸载技术实现了所有三个关键属性：小证明大小、快速验证时间和标准假设下的安全性。
>
> **达到效果:** Akita将验证时间减少到$\widetilde O_{k,\lambda}(N^{1/k})$，同时保持了$\widetilde O_{k,\lambda}(\log N)$的证明大小和$\widetilde O_{k,\lambda}(N)$的证明者时间，并且安全性基于标准假设。
>
> **技术梗概:** Akita通过承诺公共设置矩阵并在验证过程中延迟处理这些矩阵来改进Hachi的平方根时间验证器，从而实现上述性能提升。

---
### [2026/1955] Maverick: Private and Verifiable LLM Inference Made Practical via Matrix-Vector Multiplication Delegation

- **作者:** Ben Merbaum, Mohammad Amin Raeisi, Wenhao Wang, Charalampos Papamanthou, Katerina Sotiraki, Fan Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1955) | [PDF](https://eprint.iacr.org/2026/1955.pdf)


> **研究背景:** 现有开源大语言模型虽然提供了透明性，但仍需依赖第三方服务提供商以利用计算资源，这引发了隐私和正确性的担忧。
>
> **主要贡献:** Maverick 提出了基于矩阵-向量乘法委托协议的私密且可验证的大语言模型推理方法。
>
> **达到效果:** Maverick 实现了首个信息论上稳健的、带有透明预处理的矩阵-向量乘法委托验证协议，客户端配置下实现了高达17倍的吞吐量提升。
>
> **技术梗概:** 通过结合基于LPN的伪随机掩码技术提供输入隐私，并有效减少服务器端开销。

---
### [2026/1957] More Efficient Secret-Shared Joins with Multiplicity via  Oblivious Sort Expansion

- **作者:** Xiaoxin Du, Xiaojie Guo, Pinzhi Chen, Tong Li, Zheli Liu

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1957) | [PDF](https://eprint.iacr.org/2026/1957.pdf)


> **研究背景:** 秘密共享SQL风格的连接是安全协作数据分析中的基本构建块，但在实际操作中经常涉及重复键，导致一对多(Join-OM)和多对多(Join-MM)关系。
>
> **主要贡献:** 本文提出了一种高效的常数轮次协议Oblivious Sort Expansion (OSE)，用于秘密共享数据库上的Join-OM和Join-MM操作，并且这种技术可能具有独立的研究价值。
>
> **达到效果:** 实验结果表明，与现有协议相比，我们的协议在运行时间和通信成本上分别降低了约27%和65%，特别是在Join-MM操作中去掉了聚合树的使用。
>
> **技术梗概:** OSE通过仅对原始输入进行排序并在扩展后推导出排序顺序来减少冗余的排序开销，并结合局部线性运算以实现无聚合树的Join-MM协议。

---
### [2026/1959] Information-theoretic two-server PIR requires $(6-o(1))\log n$ bits of communication

- **作者:** Keewoo Lee

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1959) | [PDF](https://eprint.iacr.org/2026/1959.pdf)


> **研究背景:** 该研究致力于证明信息论意义上的双服务器私有信息检索方案在处理n位数据库时所需的最低通信量，并改进了先前的研究结果。
>
> **主要贡献:** 作者通过引入新的读取策略，即允许实值输出并仅支付其二次矩，从而提高了下限估计的精度。
>
> **达到效果:** 研究最终证明了每种信息论意义上的双服务器私有信息检索方案都需要至少$(6-o(1))\log n$比特的通信量，并改进了先前的界。
>
> **技术梗概:** 通过将数据库编码为量子态，从足够多的副本中恢复一个条目，并应用Nayak关于量子随机访问码的边界来证明这一结果。

---
### [2026/1960] Unbounded Broadcast and KP-ABE with Sublinear Ciphertext from Pairings

- **作者:** Junichi Tomida, Hoeteck Wee

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1960) | [PDF](https://eprint.iacr.org/2026/1960.pdf)


> **研究背景:** 该研究旨在设计一种基于对偶的广播加密和密钥策略属性基加密(KP-ABE)方案，以实现未绑定用户数量、属性长度或策略大小的广播加密，并且具有亚线性密文大小。
>
> **主要贡献:** 贡献在于首次提出了基于对偶的广播加密和KP-ABE方案，支持未绑定的用户数，并证明了广播加密的自适应安全性和KP-ABE的选择性安全性。
>
> **达到效果:** 研究结果表明，在标准模型下无需随机预言机的情况下，该方案的公钥大小为O(1)，密文大小为O(\u221An)，私钥大小分别为O(\u221An)和O(\u221An \cdot |f|)，其中N表示广播集的最大尺寸，|f|表示策略大小。
>
> **技术梗概:** 技术上采用了布尔公式和区间程序来支持KP-ABE，并基于k-Lin假设证明了方案的安全性。

---
### [2026/1961] A Formal Security Analysis of a MACsec Key Agreement Protocol Using Tamarin

- **作者:** Halil İbrahim Kaplan

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1961) | [PDF](https://eprint.iacr.org/2026/1961.pdf)


> **研究背景:** MACsec Key Agreement (MKA) 是 IEEE 802.1X 协议的一部分，用于建立和维护 MACsec 部署的安全关联。尽管 MKA 广泛部署，但对其核心密钥协商逻辑的机器验证分析仍然稀缺。
>
> **主要贡献:** 作者使用 Tamarin 证明器对简化版的两方 MKA 交换进行了形式化分析，并验证了机密性、认证、协议一致性、顺序性和新鲜性属性。
>
> **达到效果:** 该分析在预共享连接关联密钥 (CAK) 未被破坏的情况下，确认这些保证在 Dolev--Yao 敌手模型下成立。同时揭示了一个结构上的弱点：恶意或受攻击的密钥服务器可以注入任意安全关联密钥 (SAK)，而服务器会接受它。
>
> **技术梗概:** 研究采用了形式化方法和技术，通过 Tamarin 证明器对 MKA 协议进行了建模和验证。

---
### [2026/1963] Impossible Polytopic Attack Revisited: Low-Data Distinguishers and Attacks

- **作者:** Yongqiang Li

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1963) | [PDF](https://eprint.iacr.org/2026/1963.pdf)


> **研究背景:** 研究了具有大于块大小的密钥长度的分组密码的安全性，如AES及其变体、Deoxys-TBC和ChiLow等，这些设计提供了高于块大小的安全性。
>
> **主要贡献:** 提出了改进的Impossible Polytopic Attack (IPA)，构建了更长轮次的区分器，并在已知明文设置下展示了其应用。
>
> **达到效果:** 成功对AES、Deoxys-BC、Joltik-BC、LED-128和ChiLow-32等密码算法进行了新攻击，其中针对10轮AES的攻击尤为显著。
>
> **技术梗概:** 提出了分组明文技术和分区猜测筛选技术，降低了构造复杂性，并建立了IPA的第一个框架。

---
### [2026/1967] Azkaban: A Zero-Knowledge Abstract Analysis for Neural Networks

- **作者:** Sankha Das, Lucien L. K. Ng, Yibin Yang, Vladimir Kolesnikov, Teodora Baluta

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1967) | [PDF](https://eprint.iacr.org/2026/1967.pdf)


> **研究背景:** 针对深度神经网络(DNN)在敏感应用中的使用，需要确保其属性如对抗鲁棒性和公平性。现有工作提出使用零知识证明(ZKP)进行DNN认证，但直接将实数域算法转化为有限精度实现可能导致声学不准确，特别是在固定点算术模拟中更为显著。
>
> **主要贡献:** 本文提出了AZKABAN系统，这是一种基于零知识抽象解释的分析方法，确保了从头到尾的准确性。该系统引入了在有限域上进行精确区间分析的操作，并提供了适用于内积和除法等ZKP友好的高效算法，同时防止非线性激活引起的隐私泄露。
>
> **达到效果:** AZKABAN显著提高了端到端证明时间，比最先进的零知识个体公平认证协议快两个数量级。此外，它能够处理比现有技术更大的模型规模。
>
> **技术梗概:** AZKABAN通过引入有限域上的精确区间分析操作和ZKP友好的高效算法实现了其目标，并在保持准确性的同时防止了隐私泄露。

---
### [2026/1970] Multi-Party Distributed Point Functions, Revisited

- **作者:** Elaine Shi, Tianyao Gu, Xuanye Zheng, Yue Yang, Yiping Liu, Yucheng Fu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1970) | [PDF](https://eprint.iacr.org/2026/1970.pdf)


> **研究背景:** 本文重新审视了多方分布式点函数（DPFs）的设计，并在安全放大等基本工具上取得了多项新进展，这些成果推动了该领域的前沿研究。
>
> **主要贡献:** 作者提供了一个新的安全放大定理证明，填补了先前理论中的空白，并在此基础上开发了多种改进多方DPFs通信成本的技术。
>
> **达到效果:** 研究成果包括提出了一种基于单向函数的新方案，在所有但一个被破坏的情况下实现了约$\widetilde{O}_\lambda(N^{\frac12 + \epsilon} \cdot \sqrt{m})$的份额大小，显著优于现有最佳结果。
>
> **技术梗概:** 通过改进安全放大技术，作者开发了新的方法来优化多方DPFs在诚实多数和被破坏多数情况下的通信成本。

---
### [2026/1971] Component-Dual Compression and the Exact Characteristic-Two Contribution Region of the Relaxed Non-Fano Port

- **作者:** Shahram Khazaei, Maghsood Parviz

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1971) | [PDF](https://eprint.iacr.org/2026/1971.pdf)


> **研究背景:** 研究旨在通过引入组件对偶压缩（CDC）方法，结合星形和树状结构的方法来完善线性秘密共享方案中特定访问结构的特征二贡献区域分析。
>
> **主要贡献:** 贡献包括提出组件对偶压缩技术，并将其与星形和树状结构方法相结合，以确定松弛非Fano港口$\widehat N$的完整特征二线性贡献区域。
>
> **达到效果:** 通过这些方法，研究团队成功地确定了$\widehat N$的最大和平均线性信息比率分别为5/4，并且实现了该贡献区域的完全确定。
>
> **技术梗概:** 技术上，CDC通过选择最小重构中的组件并将其压缩到公共坐标系中来实现对偶空间，从而填补了先前方法未覆盖的部分。

---
### [2026/1972] Criminology: Refined Techniques for Compression Side-Channel Attacks

- **作者:** Yuanming Song, Lenka Mareková, Kenneth G. Paterson

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1972) | [PDF](https://eprint.iacr.org/2026/1972.pdf)


> **研究背景:** 早在二十年前，就已指出在加密之前进行压缩存在安全风险，因为这会导致通过密文长度泄露关于明文的信息。尽管知道这些危险，但在实践中仍广泛使用压缩-然后-加密的模式。
>
> **主要贡献:** 本文提出了新的技术来放大压缩过程中产生的小长度差异，并展示了这种侧信道比以前认为的要强大得多。
>
> **达到效果:** 通过使用新颖的技术，研究者能够显著增强密文长度差异的影响，从而提高了对明文恢复攻击的有效性。
>
> **技术梗概:** 研究采用了缩放和链式放大技术来利用DEFLATE算法中常见字符串被较短回溯替换的方式，并基于碰撞进行放大处理。

---
### [2026/1973] Addition-Efficient MDS Matrices from Superconcentrators (Full Version)

- **作者:** Jooyoung Lee, Seungmin Park, Mincheol Son

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1973) | [PDF](https://eprint.iacr.org/2026/1973.pdf)


> **研究背景:** MDS矩阵是提供对称原语中最佳扩散的关键结构，但其成本理论分析有限，特别是在面向算术化置换的设计中，通常要么使用昂贵的MDS矩阵，要么牺牲MDS性质以减少约束数量。
>
> **主要贡献:** 论文研究了实现MDS矩阵所需的扇入为二加法的数量，并通过构建基于超级集中器的递归算法，提高了$ t \times t $矩阵（$ t\leq 8 $）的上下界估计。
>
> **达到效果:** 结果表明，对于$ t=3,4,5,6,7,8 $，分别需要5,8,12,16,21,26个加法，并且这些界限在$ t\leq 6 $时是紧的。此外，使用相同的超级集中器图作为模板构建了$ k $位字节的MDS矩阵，在大多数参数选择中比现有最佳方案所需的XOR更少。
>
> **技术梗概:** 通过将加法约束表示为有向无环图，并利用已有的结果证明任何实现MDS矩阵的计算图必须是超级集中器，从而推导出下界；基于超级集中器大小缩减引理提出递归算法以改进上界和下界。

---
### [2026/1976] Properties of the Me Operation and Me-Scalar Multiplication on Elliptic Curves over Finite Fields

- **作者:** Masaaki Shirase

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1976) | [PDF](https://eprint.iacr.org/2026/1976.pdf)


> **研究背景:** 研究介绍了M操作的椭圆曲线版本Me操作及其相关的算术运算，旨在为有限域上的盒球系统（BBS）构造提供替代方案，并探索其在密码学中的应用潜力。
>
> **主要贡献:** 论文定义了1st和2nd Me-标量乘法，并证明了它们具有特定的性质，这些性质可能对构建加密协议有用。
>
> **达到效果:** 研究揭示了Me操作及其相关问题（如MeDLP、MeCDH）在经典计算机上的难度特性，以及非周期序列的存在性。
>
> **技术梗概:** 通过形式化定义和严格证明，论文分析了Me操作的性质，并探讨了其在密码学中的潜在应用。

---
### [2026/1978] Succinct Two-Round Two-Party Signing from PCFs

- **作者:** Lennart Braun, Geoffroy Couteau, Kelsey Melissaris, Mahshid Riahinia, Elahe Sadeghi

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1978) | [PDF](https://eprint.iacr.org/2026/1978.pdf)


> **研究背景:** 本文提出了高效且安全的两方门限签名方案，特别适用于Schnorr和ECDSA两种主流签名算法。
>
> **主要贡献:** 贡献在于设计了一种新的向量OLE伪随机相关函数(PCF)，并基于此构建了恶意攻击下可同时模拟的安全分布式密钥生成协议(DKG)。
>
> **达到效果:** 该方法实现了两轮、两方、无状态和确定性的签名过程，计算时间仅为4-12毫秒，通信量低至96字节（Schnorr）或128字节（ECDSA），并提供了全面的并发模拟安全性。
>
> **技术梗概:** 技术上采用了高效的向量OLE伪随机相关函数(PCF)，并通过设计恶意攻击下可同时模拟的安全分布式密钥生成协议(DKG)来实现高效签名。

---
### [2026/1979] From Specs to Apps: Verifying and Monitoring Models of Signal and WhatsApp

- **作者:** Moustafa Said, Aurora Naska, Kevin Morio, Robert Künnemann

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1979) | [PDF](https://eprint.iacr.org/2026/1979.pdf)


> **研究背景:** 研究背景：Signal协议作为一款安全消息传输协议，广泛应用于包括WhatsApp在内的多个应用中。尽管已有大量关于该协议形式化验证的研究工作，但在实际运行时的行为与理论模型之间仍存在差距。
>
> **主要贡献:** 主要贡献：通过使用SpecMon实时监控工具，将WhatsApp Web和Signal Desktop的网络交互及加密组件行为转化为可验证的形式化模型，并首次对WhatsApp Web的Signal协议实现进行了形式化建模，同时揭示了原libsignal库与WhatsApp版本之间的差异。
>
> **达到效果:** 达到的效果：验证结果显示实际运行中的执行符合理论模型（相对于可信事件提取和符号抽象），并确认了Signal协议核心组件的安全属性，如认证性和保密性。此外，还发现了现有文献中未记录的细节差异。
>
> **技术梗概:** 技术梗概：通过在两个应用中嵌入监控代码来捕获网络交互及加密操作，并基于此开发了与Tamarin兼容的多集重写模型进行验证。

---
### [2026/1980] Better Security Proofs for X3DH and XHMQV

- **作者:** Jiawei Bao, Jiaxin Pan, Runzhi Zeng

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1980) | [PDF](https://eprint.iacr.org/2026/1980.pdf)


> **研究背景:** Signal协议因其端到端加密特性被广泛使用，其初始握手协议X3DH依赖于离散对数难题的假设来保证安全性。然而，之前的非紧致安全证明导致了会话数量的巨大损失，并且实际的安全保障远低于预期的128位安全保障水平。
>
> **主要贡献:** 作者通过在随机预言模型中提供基于多用户Diffie-Hellman假设的紧致安全归约，改进了X3DH和其增强版XHMQV的安全分析。
>
> **达到效果:** 这种改进使得会话数量损失显著减少，并且为使用Curve25519提供了更强的安全依据。
>
> **技术梗概:** 通过在多测试设置中进行证明，作者能够提供更紧致的安全性保证，并将这些结果应用于Signal协议的开发中。

---
### [2026/1981] Parallelized Authenticated Encryption with Tag Combiners

- **作者:** Christoph Dobraunig, Charlotte Lefevre

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1981) | [PDF](https://eprint.iacr.org/2026/1981.pdf)


> **研究背景:** 研究了在认证加密方案中，如何通过并行调用底层构建块来优化性能，并探讨了将串行方案并行化时的不同选择及其安全性。
>
> **主要贡献:** 提出了多种可能的标签组合方法，并提供了这些方法的安全性证明，揭示了串行方案并行化后的巨大安全差异。
>
> **达到效果:** 通过分析不同标签组合策略，确定了最安全且高效的并行认证加密实现方式。
>
> **技术梗概:** 采用了形式化的安全性证明技术来评估各种标签组合方案的性能和安全性。

---
### [2026/1982] Cryptanalysis of the Alternative Mod-2/Mod-3 Weak PRF

- **作者:** Augustin Bariant, Christina Boura, Baptiste Germon, Rachelle Heim, Charles Meyer-Hilfiger, Tyge Tiessen

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1982) | [PDF](https://eprint.iacr.org/2026/1982.pdf)


> **研究背景:** 研究背景：模-2/模-3函数是现代密码协议中最常用的弱伪随机函数（Weak Pseudorandom Function, WPRF）之一，尽管其应用广泛，但对其安全性关注较少。
>
> **主要贡献:** 主要贡献：作者通过分析固定汉明重量的密钥和输入下的输出分布，发现了新的统计偏差，并提出了分拆策略以增强这些偏差。
>
> **达到效果:** 达到的效果：新方法实现了对n=384参数实例的第一个实用攻击，并且是当前针对该构造的最佳已知攻击，具有亚线性的时间、空间和内存复杂度。
>
> **技术梗概:** 技术梗概：通过固定或猜测部分密钥来放大偏差，并利用快速傅里叶变换类似的技术处理额外的计算成本。
...
### [推荐] [2026/1803] Adaptive Multi-Algorithm Key Exchange for Quantum-Resilient Secure Communication: Dynamic Switching among QKD, Post-Quantum, and Classical Key Establishment with Entropy Fusion

- **匹配关键字:** post-quantum

- **作者:** Ogbodo Tochukwu Hillary, Bilkisu Larai Muhammad-Bello, Saleh El-Yakub Abdullahi

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1803) | [PDF](https://eprint.iacr.org/2026/1803.pdf)


> **研究背景:** 面对量子计算机的威胁，经典密钥交换协议如RSA、椭圆曲线和有限域Diffie-Hellman易受未来解密攻击。量子密钥分发提供信息论安全性但对信道噪声、损耗和距离敏感，而后量子密码学在传统硬件上提供量子抵抗力但需要更大密钥并依赖于硬件计算能力。
>
> **主要贡献:** 该研究提出了一种适应性多算法密钥生成与交换框架，能够根据实时监测的量子位错误率和网络延迟动态选择BB84、Kyber512及经典Diffie-Hellman，并通过基于HMAC的关键材料融合阶段整合所有活跃来源。
>
> **达到效果:** 该框架在模拟环境中实现了100%的密钥生成成功率，且量子抵抗力模式在256位密钥长度下保持约3 kbps的秘密密钥吞吐量。
>
> **技术梗概:** 通过实时监控和动态切换算法选择，结合HMAC进行关键材料融合，确保了不间断的关键可用性与安全性。

---
### [推荐] [2026/1804] Dealing Haystack: Towards Trustless Haystack in the Optimistic Setting

- **匹配关键字:** lattice, post-quantum

- **作者:** Isabel Muñoz, Isaac Agudo, Marco López, Daniel Morales

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1804) | [PDF](https://eprint.iacr.org/2026/1804.pdf)


> **研究背景:** 该研究聚焦于基于哈希的签名方案，这类方案的安全性依赖于哈希函数的成熟属性，而非更新的假设。尽管阈值签名可以分散信任并减轻一次性密钥重复使用的问题，但构建此类方案对于缺乏同态结构的哈希基方案来说具有挑战性。
>
> **主要贡献:** 研究实现了Haystack协议，并通过网络感知评估验证了其实用性；同时提出了一种基于乐观轻量级MPC的部分DKG变体设置，以减轻对经销商的信任依赖。
>
> **达到效果:** 实验结果显示，该实现版本在实际部署中的性能与其它后量子阈值方案相当；新型设置减少了对完全可信设置的依赖，并降低了共同参考值的成本。
>
> **技术梗概:** 研究采用了Java语言实现了Haystack协议，并通过MPC技术构建了一种轻量级的部分DKG变体设置，以提高系统的灵活性和安全性。

---
### [推荐] [2026/1805] On Module Lattices with Galois-Symmetries: What You See Is Not What You Get

- **匹配关键字:** lattice

- **作者:** Ronald Cramer, Daniël van Gent, Andrea Lesavourey, Alice Pellet-Mary

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1805) | [PDF](https://eprint.iacr.org/2026/1805.pdf)


> **研究背景:** 研究探讨了在具有Galois对称性的模格中寻找短向量的难度，指出这些格可以被视为较低维度的子域上的模格，从而简化了问题。
>
> **主要贡献:** 论文扩展了先前关于理想格的研究，提出了适用于更高秩模格的新理论框架。
>
> **达到效果:** 通过考虑Galois对称性，研究证明了在具有特定对称性的高维模格中寻找短向量的实际难度低于预期。
>
> **技术梗概:** 利用代数结构的对称性分析模格，并将其视为较低维度上的子域上的模格来简化问题。

---
### [推荐] [2026/1809] PikkuFold: Efficient Folding in a Few Kilobytes
...
- **链接:** [论文](https://eprint.iacr.org/2026/1809) | [PDF](https://eprint.iacr.org/2026/1809.pdf)


> **研究背景:** PikkuFold 是一种高效的折叠协议，特别适用于流式计算的简洁证明系统。
>
> **主要贡献:** 该研究提出了 PikkuFold 协议，相比现有方案减少了通信量，并且无需额外的内部承诺。
>
> **达到效果:** 通过使用分层随机投影技术，PikkuFold 实现了高效验证和较短的消息传递长度，同时保持证明者时间和验证者响应时间在可接受范围内。
>
> **技术梗概:** PikkuFold 利用分层随机投影来加速验证过程，并直接发送最终图像给验证者以减少通信成本。

---
### [推荐] [2026/1812] Communication-Efficient Private Join and Compute over Distributed Input Sets

- **匹配关键字:** homomorphic encryption

- **作者:** Yunqing Sun, Xinran Cai, Hanlin Liu, Xiao Wang, Wei Dong

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1812) | [PDF](https://eprint.iacr.org/2026/1812.pdf)


> **研究背景:** PJC协议允许双方在保持数据隐私的前提下，对私有数据集中的匹配记录进行计算。然而，现有方案假设每个参与方持有整个数据集，这在实际场景中往往不现实，特别是在多个数据拥有者分散持有相关数据的情况下。
>
> **主要贡献:** 本文提出了一种适用于分布式输入集的高效近似PJC协议，在半诚实设置下工作，并使用两个非协作服务器进行计算，最终仅学习到近似结果。核心技术创新在于重新设计了Gödel奖获奖的AMS草图，使其能够在全同态加密下有效评估。
>
> **达到效果:** 该协议实现了在输入大小为亚线性的情况下高效通信，客户端可以将每个输入元素插入到加密草图中，所需时间为30毫秒，且草图大小与输入无关，仅为250KB。
>
> **技术梗概:** 通过引入新的结构化随机性，可以在仅使用3级乘法生成从短种子出发的同态密文的同时保持最佳明文精度边界。

---
### [推荐] [2026/1813] Atom: Single-Server Private Information Retrieval with Low Communication and Fast Computation

- **匹配关键字:** LWE

- **作者:** Baoyu Li, Binwu Xiang, Kang Yang, Yu Yu, Xiaogang Zhou

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1813) | [PDF](https://eprint.iacr.org/2026/1813.pdf)


> **研究背景:** 私有信息检索（PIR）允许客户端检索记录而不泄露索引。现有数据库独立预处理的低通信PIR协议通常需要几秒到几十秒，而更快的协议则需要数百千字节的通信量。
>
> **主要贡献:** 本文提出了三种不同的密文转换技术：生成二轨道SIMD选择器、将打包的$\mathsf{RLWE}$密文转换为对齐单项式$\mathsf{RGSW}$密文，以及从加密位生成任意单项式$\mathsf{RGSW}$密文。
>
> **达到效果:** 基于这些技术，设计了新的PIR协议Atom，实现了低通信和快速计算的最佳平衡。对于256B记录和不同大小的数据库，Atom在线通信量仅为3.0-3.8KB，并且每次查询耗时0.4-5.0秒，相比CCS'24的KsPIR，在线通信成本降低了40.5倍至51.3倍。
>
> **技术梗概:** 通过生成二轨道SIMD选择器、将打包的$\mathsf{RLWE}$密文转换为对齐单项式$\mathsf{RGSW}$密文，以及从加密位生成任意单项式$\mathsf{RGSW}$密文的技术，实现了Atom协议。

---
### [推荐] [2026/1816] From Lattices to Tensor Cores: Accelerating Private Information Retrieval

- **匹配关键字:** lattice

- **作者:** Sidaarth Sabhnani, David J. Wu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1816) | [PDF](https://eprint.iacr.org/2026/1816.pdf)


> **研究背景:** 该研究旨在设计一种加速私有信息检索（PIR）的协议，以提高服务器处理能力和通信效率。
>
> **主要贡献:** SandwichPIR是首个主要依赖GPU张量核心进行密集8位整数矩阵乘法计算的单服务器PIR协议。
>
> **达到效果:** SandwichPIR在处理单个查询和批量查询时均表现出显著性能提升，分别实现了488 GB/s和24 TB/s的有效吞吐率。
>
> **技术梗概:** 通过优化GPU上的密集矩阵运算，并减少通信开销，SandwichPIR提高了私有信息检索的速度和效率。

---
### [推荐] [2026/1817] ATLAS: Automated Approximation of Transformers for Efficient Homomorphic Inference in One Hour
...
- **链接:** [论文](https://eprint.iacr.org/2026/1817) | [PDF](https://eprint.iacr.org/2026/1817.pdf)


> **研究背景:** 在全同态加密(FHE)环境下运行Transformer模型以实现高效安全的推理是一个挑战，因为其非线性操作需要被CKKS方案支持的多项式近似替代，且这些近似的深度会显著增加计算成本。现有的FHE Transformer方法依赖手动调参，效率低下且容易出错。
>
> **主要贡献:** ATLAS框架通过将每个层的近似设置视为延迟和准确性的多目标优化问题来自动化这一过程，从而解决了上述挑战。
>
> **达到效果:** ATLAS能够在不到一小时内完成搜索，并显著减少了Transformer模型在FHE环境下的推理成本。
>
> **技术梗概:** ATLAS采用了两阶段优化策略和代理模型来处理大规模决策空间和长时间的评估时间，并成功地完成了搜索任务。

---
### [推荐] [2026/1822] Non-Malleable Reductions of Knowledge

- **匹配关键字:** lattice

- **作者:** Antonio Faonio, Lili Tong

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1822) | [PDF](https://eprint.iacr.org/2026/1822.pdf)


> **研究背景:** 非交互式零知识证明的不可篡改性要求，给定一个陈述的有效证明后，无法通过计算手段获得相关陈述的有效证明而无需知道相应的证人。
>
> **主要贡献:** 作者提出了一种模块化框架来分析知识还原中的不可篡改性，并识别了两种非交互式知识还原在特定条件下的组合可以保持模拟可提取性和不可篡改性。
>
> **达到效果:** 通过该框架，作者证明了基于格的R1CS证明系统LaBRADOR的模拟可提取性，并设计了一个零知识变体以保持其实用效率和亚线性证明大小。
>
> **技术梗概:** 该研究采用了一种模块化的方法来隔离每个组件所需的简单且具体的属性，包括新颖形式的知识性和新的安全概念。

---
### [2026/1799] Two Constructions of Rotation-Symmetric Bent Functions Outside the Completed Maiorana--McFarland Class with Any Possible Algebraic Degree

- **作者:** Deng Tang

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1799) | [PDF](https://eprint.iacr.org/2026/1799.pdf)


> **研究背景:** 旋转对称布尔函数是密码学中重要的布尔函数类，Su和Tang在2017年提出了一类具有所有可能代数度的旋转对称Bent函数。
>
> **主要贡献:** 本文提出了两种构造方法，用于生成当变量为$n=30\cdot 7^j$或$n=70t$时，完全位于Maiorana--McFarland类之外的Bent函数，并且这些函数具有从3到$n/2$的所有可能代数度。
>
> **达到效果:** 通过这两种构造方法，作者成功地扩展了已知旋转对称Bent函数的范围，填补了之前在该领域中的空白。
>
> **技术梗概:** 作者利用特定变量数量下的构造技巧，确保生成的Bent函数具有从3到$n/2$的所有可能代数度，并且完全位于Maiorana--McFarland类之外。

---
### [2026/1800] Separating Quantum Indistinguishability Obfuscation from Falsifiable Assumptions

- **作者:** Mohammed Barhoush, Tomoyuki Morimae, Ramis Movassagh

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1800) | [PDF](https://eprint.iacr.org/2026/1800.pdf)


> **研究背景:** 量子不可区分混淆（qIO）旨在使量子电路变得难以理解，同时保持其功能。尽管其重要性，从标准假设构建qIO仍然是一个主要的开放问题。
>
> **主要贡献:** 作者证明了基于受限类量子黑盒归约，任何可验证的加密假设都无法支持QMA的证人加密（WE）安全性，从而将null-qIO与可验证假设区分开来。
>
> **达到效果:** 这一结果为基于标准密码学假设构建qIO设定了障碍。然而，该不可能性仅适用于具有经典密文的WE，并不排除输出为量子态的混淆器。
>
> **技术梗概:** 证明依赖于存在一个QMA-QCIP[2]差距问题，这是一个平均情况假设，提出了一个无法用两消息的经典通信验证的语言。

---
### [2026/1802] VERIF: An Efficient Zero-Knowledge Proof System for Verifying IVF-Flat Retrieval in RAG Services

- **作者:** Zhiwen Zhang, Yuao Zhou, Ge Chang, Cong Li, Yuejian Fang, Qingni Shen

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1802) | [PDF](https://eprint.iacr.org/2026/1802.pdf)


> **研究背景:** RAG服务外包向量搜索，但客户端无法验证返回的内容是否符合承诺的索引、参数和快照。
>
> **主要贡献:** VERIF是首个针对完整且服务一致的IVF-Flat检索的专用零知识多项式交互式预言证明系统。
>
> **达到效果:** VERIF实现了最高86.5倍的证明者加速，并将峰值内存减少高达99.1%；验证时间在0.62至1.48秒之间，展示了RAG-as-a-Service中实用的可验证IVF-Flat检索。
>
> **技术梗概:** 通过消除承诺查询依赖性分数并直接将选择声明缩减为经过身份验证数据上的内积来实现技术。

---
### [2026/1806] Practical Differential Fault Attacks on the GPRS Standard Ciphers

- **作者:** Zhengting Li, Lin Ding, An Wang, Haotong Xu, Zheng Liu, Zheng Wu, Xinhai Wang, Jiang Wan

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1806) | [PDF](https://eprint.iacr.org/2026/1806.pdf)


> **研究背景:** 针对GPRS标准中的GEA-1和GEA-2流密码，首次提出了一种基于随机故障模型的差分故障攻击方法。
>
> **主要贡献:** 提出了一个高效的专用算法来识别精确的故障位置，并成功应用于实际攻击中以恢复密钥。
>
> **达到效果:** 通过该攻击方法，能够在较短时间内（约16分钟）破解GEA-1和GEA-2标准流密码，验证了其有效性。
>
> **技术梗概:** 利用差分故障分析技术，在随机故障模型下定位故障位置，并结合模拟实验进行验证。

---
### [2026/1807] A (6, 4) Vectorial Boolean Function With Nonlinearity 26 and the Maximum Nonlinearity for Six-Bit Permutations

- **作者:** Yufei Yuan, Lei Zhang, Wenling Wu

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1807) | [PDF](https://eprint.iacr.org/2026/1807.pdf)


> **研究背景:** 研究探讨了高输出维数矢量布尔函数的非线性度问题，特别是在输入维度大于输出维度一半的情况下，最佳可实现分量非线性度未知。
>
> **主要贡献:** 作者构造了一个六输入四输出函数，其非线性度为26，打破了之前24的下限，并解决了六位置换的最大非线性度问题。
>
> **达到效果:** 通过计算机辅助评估和布尔函数分类，确定了六位置换的最大绝对沃尔什系数至少为16，从而限制了其非线性度不超过24。同时，给出了非线性度超过24的六输入六输出函数的编码条件。
>
> **技术梗概:** 研究利用了矢量第四矩恒等式和计算机辅助评估方法来确定最佳分量非线性和沃尔什系数的最大值。

---
### [2026/1808] Cross-Signature Signing-Key Recovery and Domain-Separation Repair for SDitH v2

- **作者:** José Luis Delgado

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1808) | [PDF](https://eprint.iacr.org/2026/1808.pdf)


> **研究背景:** 该研究针对SDitH v2协议中的交叉签名密钥恢复攻击，揭示了隐藏的VOLE叶节点中存在的安全漏洞。
>
> **主要贡献:** 作者首次提出了一种基于公共选择消息转录的跨签名密钥恢复攻击方法，并修复了共享流域分离问题。
>
> **达到效果:** 实验结果表明，在六组参数设置中，该攻击比AES-128/192/256的穷举搜索成本低11.23–11.67位比特数；多密钥实验进一步验证了其有效性。
>
> **技术梗概:** 通过组织公共掩码在字典树中的非线性项，并沿Gray编码遍历更新电路，作者实现了攻击的有效实施；修复方案则通过为每个扩展添加签名盐、全局叶序号和块位置标签来实现。

---
### [2026/1810] An Algebraic-Geometry Lower Bound against the ePrint:2026/1747 McEliece Key-Recovery Attack

- **作者:** Daniel Apon

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1810) | [PDF](https://eprint.iacr.org/2026/1810.pdf)


> **研究背景:** 该研究针对Vedenev提出的基于Goppa码的McEliece公钥恢复攻击方法进行了深入分析，指出其假设独立性在实际实例中并不成立。
>
> **主要贡献:** 作者通过代数几何的方法揭示了这种依赖性的根本原因，并提出了一个具体的非退化条件来量化所需样本点的数量。
>
> **达到效果:** 研究证明，在关键步骤中需要的样本点数量超过$2t + 3$，从而为攻击的有效性设定了理论上限。
>
> **技术梗概:** 利用代数几何中的线性映射和导数空间等技术手段，分析了Goppa多项式的依赖关系，并通过构造特定维度的家庭来量化这种依赖。

---
### [2026/1811] Efficient Soft Analytical Side-Channel Attacks on Large-Scale Cryptographic Computations

- **作者:** Yiteng Sun, Zhuo Huang, Yan Zhuang, Shuo Sun, Xinyu Li, Yu Yu, Weijia Wang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1811) | [PDF](https://eprint.iacr.org/2026/1811.pdf)


> **研究背景:** 针对大规模密码计算，传统的软分析侧信道攻击（SASCA）由于涉及大量变量和复杂图结构导致内存和计算开销巨大。
>
> **主要贡献:** 提出了一种新的框架GRWP-SASCA，通过区域化剪枝策略将全局因子图分解为可管理的局部区域，从而实现高效推理。
>
> **达到效果:** 该方法显著降低了内存需求，并提高了攻击效率，在ML-DSA中6层子NTT组件下仅需约4GB内存，比标准SASCA减少约30倍。
>
> **技术梗概:** GRWP-SASCA通过逐步合并局部区域的信念传播，保留关键依赖关系的同时避免了短环路和耦合更新问题。

---
### [2026/1814] LetoPIR: Fast Keyword Private Information Retrieval with Logarithmic Communication

- **作者:** Baoyu Li, Kang Yang, Qi Liu, Binwu Xiang, Xiaogang Zhou, Xiang Xie, Yu Yu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1814) | [PDF](https://eprint.iacr.org/2026/1814.pdf)


> **研究背景:** 现有关键词私有信息检索(PIR)协议在单服务器标准设置下，通信和计算成本较高。
>
> **主要贡献:** 提出了生成同态加密形式的k-热向量的方法，并结合桶合并技术减少了最大桶大小，从而构建了LetoPIR协议。
>
> **达到效果:** LetoPIR相比SparsePIR在256 MB到4 GB数据库中实现了12.4至17.0倍的通信成本降低和超过3倍的计算成本减少；与KPIR相比，在相同甚至更低的计算成本下，实现了51.4至184.8倍的通信成本降低。
>
> **技术梗概:** 通过生成同态加密形式的k-热向量和桶合并技术优化了关键词PIR协议的性能。

---
### [2026/1815] Novel SMT Encoding for Quantum Circuit Optimization

- **作者:** Youbo Guo, Fengrong Zhang, Lei Liao, Yongzhuang Wei, Baocang Wang, Xiaogang Zhou

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1815) | [PDF](https://eprint.iacr.org/2026/1815.pdf)


> **研究背景:** 量子电路优化作为近年来的研究热点，针对量子门作用于固定物理线并仅修改目标线的特点，本文提出了一种新颖的SMT编码方法。
>
> **主要贡献:** 该研究贡献了两种SMT编码：精确G编码和最多G编码（含空门），显著提升了4位S盒实例的速度，并实现了5位S盒量子电路的自动化合成。
>
> **达到效果:** 在无辅助比特设置下，对于KECCAK S盒，模型获得了17个NCT门、深度51的具体实现，以及16个NCT门、深度52的实现，改进了EUROCRYPT 2025的结果；还发现了具有最小门数且深度为55的13个门实现。
>
> **技术梗概:** 通过SMT编码技术，本文方法能够加速量子电路优化过程，并自动合成实用的量子电路，特别是在无辅助比特和有限辅助比特的情况下找到了最优或接近最优的门级实现。

---
### [2026/1818] Blood MERIDIAN: a blockcipher that is not a blockcipher

- **作者:** JP Aumasson

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1818) | [PDF](https://eprint.iacr.org/2026/1818.pdf)


> **研究背景:** MERIDIAN被提出作为AES的轻量化替代方案，但其Directional Substitution层非注入性揭示了其安全性的缺陷。
>
> **主要贡献:** 作者通过构造明确的碰撞展示了MERIDIAN的关键实例不是置换，并且发现了一轮差分超过了声称界限13.37倍。
>
> **达到效果:** 这些发现导致了对MERIDIAN作为块密码和伪随机函数安全声明的失败，证明其不能被视为传统的块密码。
>
> **技术梗概:** 通过分析Directional Substitution层的非注入性，作者使用构造碰撞的方法揭示了MERIDIAN的安全弱点。

---
### [2026/1819] On Memory Effects in PWXL variants

- **作者:** Jintai Ding, Hao Guo, Bo-Yin Yang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1819) | [PDF](https://eprint.iacr.org/2026/1819.pdf)


> **研究背景:** 研究了在Ran Wedge攻击和Furue--Ikematsu交点攻击中，自由内存访问和Macaulay系数按需RAM建模的固有欠计数问题。
>
> **主要贡献:** 贡献在于量化了这些攻击中的内存效应，并表明UOV方案对于Ip、Is和III足够安全。
>
> **达到效果:** 结果是提出了uov-Ip\# (256,116,44)，uov-III\# (256,186,72) 和 uov-V\# (256,250,96) 等参数调整，以保持压缩公钥不变。
>
> **技术梗概:** 使用了Parallelized Wiedemann-based XL算法进行分析，并基于乐观但可行的假设来评估攻击效果。

---
### [2026/1820] Practical Silent Threshold Signatures and Silent Threshold Encryption for Dynamic Committees

- **作者:** Yifei He, Zheng Zhou, Yu Chen, Zhi Guan, Zhong Chen

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1820) | [PDF](https://eprint.iacr.org/2026/1820.pdf)


> **研究背景:** 现有沉默门限签名和加密方案要么不支持动态委员会设置，要么存在严重的可扩展性问题。针对此挑战，本文旨在设计适用于动态委员会的沉默门限签名与加密方案。
>
> **主要贡献:** 作者提出了两种新的沉默门限签名和加密方案，分别在聚合时间上实现了严格依赖于活跃委员会大小n的目标，并且优化了前期设置过程。
>
> **达到效果:** 新方案显著降低了聚合成本，使得效率大幅提升，特别适用于如以太坊等需要频繁调整小规模活跃委员会的现代分布式系统。
>
> **技术梗概:** 通过重新设计Dyna-hinTS框架并采用线性配对检查和新的多项式承诺来表示委员会，实现了聚合时间从O(NlogN)降至O(nlog^2n)，同时引入了首个适用于动态委员会的沉默门限加密方案。

---
### [2026/1821] Concrete Security Assessment of Isogeny-based Cryptography with the new Isogeny-Path algorithm

- **作者:** Maher Mamah

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1821) | [PDF](https://eprint.iacr.org/2026/1821.pdf)


> **研究背景:** 文章评估了Wesolowski提出的解决超奇异Isogeny路径问题的新算法对基于Isogeny的密码方案，特别是SQIsign的具体安全性影响。
>
> **主要贡献:** 研究了该攻击的时间-内存权衡，并探讨了基于van Oorschot-Wiener技术的优化方法。
>
> **达到效果:** 结果显示，在实际的内存范围内，即使经过优化，全列表攻击及其vOW变体也不如之前的低内存算法来计算超奇异自同态环。量子爪查找改进也未能显著提高运行时间。
>
> **技术梗概:** 研究采用了Wesolowski的新Isogeny-Path算法，并结合van Oorschot-Wiener技术进行了优化分析，同时考虑了Grover搜索和Tani算法的量子改进方法。

---
### [2026/1823] HyperSolver: Asymptotically and Concretely Accelerating the Delfs–Galbraith Attack using Isogeny Ladders

- **作者:** Lorenz Panny, Ryan Rueger, Alessandro Sferlazza, Aleksei Udovenko

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1823) | [PDF](https://eprint.iacr.org/2026/1823.pdf)


> **研究背景:** 该研究旨在加速Delfs-Galbraith攻击，针对超奇异椭圆曲线间的isogeny路径寻找问题，这是后量子密码学中核心的难题。
>
> **主要贡献:** 研究人员提出了一种新的Delfs-Galbraith算法变体HyperSolver，并通过理论分析和实际计算成本估算展示了其在多种特征下的优越性。
>
> **达到效果:** HyperSolver在任意特征下理论上比之前的最佳Delfs-Galbraith变体快一个对数因子；对于有利结构的特征，其实际计算成本低于所有先前的方法。
>
> **技术梗概:** 研究通过引入isogeny梯子和分析图探索中的“杰出”子图概率来优化算法，并在GPU上实现加速计算瓶颈部分。
...
### [推荐] [2026/1730] New Techniques for Fast and Shallow FHE Bootstrapping and Beyond

- **匹配关键字:** homomorphic encryption, LWE

- **作者:** Aayush Jain, Huijia Lin, Zeyu Liu, Sagnik Saha

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1730) | [PDF](https://eprint.iacr.org/2026/1730.pdf)


> **研究背景:** 全同态加密(FHE)的瓶颈在于其解密刷新(bootstrapping)步骤的高延迟和成本，这限制了其在实际应用中的可行性。
>
> **主要贡献:** 作者提出了两种创新方法：一种是利用稀疏小范数LWE秘密假设减少算术复杂度；另一种是引入新的RLWE变体，通过无重新线性化乘法技术显著降低电路深度。
>
> **达到效果:** 这些新方法在实际参数设置中实现了4.5至7.5倍的门级和功能解密加速，并将FHEW/TFHE的解密刷新所需的NTT/INTT层从超过500减少到仅3层。
>
> **技术梗概:** 通过优化LWE秘密假设和设计新的RLWE变体，作者减少了全同态加密中解密刷新步骤的计算复杂度和电路深度。

---
### [推荐] [2026/1734] Key Recovery from Residue-Confined Errors in the Pradhan CRT-RLWE Construction
...
- **链接:** [论文](https://eprint.iacr.org/2026/1734) | [PDF](https://eprint.iacr.org/2026/1734.pdf)


> **研究背景:** 该研究揭示了Pradhan等人的CRT-FHE方案在假设的误差分布范围内存在安全漏洞，特别是在处理特定类型的错误时。
>
> **主要贡献:** 作者证明了当公钥乘数为单位时，可以通过单次环逆运算从公钥恢复秘密密钥，并且可以在没有密钥的情况下从任意密文恢复明文，从而获得选择明文攻击的优势。
>
> **达到效果:** 研究结果表明，普通Ring-LWE到CRT-RLWE的转换并未保持误差分布的一致性，这使得CRT-RLWE的安全性不能保证至少与Ring-LWE相当。
>
> **技术梗概:** 通过分析中国剩余定理（CRT）函数在特定模数下的行为，揭示了误差被限制在一个子空间内的机制，从而破坏了原有的安全假设。

---
### [推荐] [2026/1735] Adapting AES-Oriented Optimizations to Rijndael-256: Cortex-M4, ARMv8-A, and CUDA

- **匹配关键字:** post-quantum

- **作者:** Siwoo Eum, Minho Song, Minjoo Sim, Anupam Chattopadhyay, Hwajeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1735) | [PDF](https://eprint.iacr.org/2026/1735.pdf)


> **研究背景:** Rijndael-256 (R256)作为AES的256位块变体，在NIST标准讨论和后量子签名候选方案中具有实际意义，其独特的状态布局和非标准ShiftRows偏移挑战了现有AES优化方法的有效性。
>
> **主要贡献:** 研究提出了针对Cortex-M4、AArch64和CUDA平台的具体适应策略，分别为更宽的经典位切片、重用与重新排列以及错配的摊销。
>
> **达到效果:** 实现了在Cortex-M4上具有秘密无关控制流和内存访问模式的位切片实现（每块4,962周期），AArch64预混洗设计可达到最高6.52 GB/s，CUDA共享内存T表实现达到了81.16 GiB/s；通过在四个NIST PQC参考代码库中替换R256核心实现了签名加速1.18倍至114.3倍和验证最多155.6倍。
>
> **技术梗概:** 针对不同平台，分别采用更宽的经典位切片、重用与重新排列以及错配的摊销等策略进行优化。

---
### [推荐] [2026/1736] Copy-Protection with Correlated Challenges: Point Functions and More via Decisional Coset Monogamy
...
- **链接:** [论文](https://eprint.iacr.org/2026/1736) | [PDF](https://eprint.iacr.org/2026/1736.pdf)


> **研究背景:** 研究背景：论文致力于解决量子信息在密码学中的一项主要应用——版权保护问题，特别是在面对两个同时使用同一挑战的对手时的安全性。
>
> **主要贡献:** 主要贡献在于提出了一个新的安全概念——相关挑战安全性，并证明了其等价于所有先前的安全定义，包括相同的挑战安全性。
>
> **达到效果:** 达到的效果是首次在无假设模型中实现了具有相同挑战的安全单解密者加密，并为点函数的版权保护提供了解决方案。
>
> **技术梗概:** 技术梗概：通过引入相关挑战安全性和假设不可区分混淆（iO）和单向函数，证明了其在版权保护中的应用。

---
### [推荐] [2026/1738] Noisy Subset Product
...
- **链接:** [论文](https://eprint.iacr.org/2026/1738) | [PDF](https://eprint.iacr.org/2026/1738.pdf)


> **研究背景:** 研究背景：该论文延续了对子集乘积问题的研究，这是一个自1978年以来就已知的NP完全问题。本文在前人工作的基础上，探讨了一个带噪声的模运算变体，并将其应用于构建后量子非交替密钥交换方案。
>
> **主要贡献:** 主要贡献：作者证明了几种相关变体之间的等价关系，并基于此提出了一个无需额外校正明文的后量子非交替密钥交换方案，直接将安全性归约到噪声子集乘积问题的搜索版本上。
>
> **达到效果:** 达到的效果：该方案提供了一种自然的一方解释作为密钥封装机制的新方法，并且不依赖于其他基于噪声的方案如格基方案中所需的额外校正明文。
>
> **技术梗概:** 技术梗概：通过引入一个抽象框架，作者系统地研究了未知系数多元指数系统的难度问题，并将其应用于具体设置中的带噪声子集乘积问题，从而构建了一个后量子非交替密钥交换方案。

---
### [推荐] [2026/1743] Notes on Short-Limb Modular Multiplication Techniques: Barrett, Montgomery, Plantard, and the Explicit CRT
...
- **链接:** [论文](https://eprint.iacr.org/2026/1743) | [PDF](https://eprint.iacr.org/2026/1743.pdf)


> **研究背景:** 该论文总结了在实施基于格的密码学中使用的短模数位大小的模乘技术，包括Barrett、Montgomery和Plantard方法及其变体。
>
> **主要贡献:** 作者整理并详细分析了这些技术的具体实现细节，涵盖了误差范围、正确性和规范性等关键方面。
>
> **达到效果:** 通过详细的数值验证和随机搜索，确保了所有给出的常数和范围的准确性，并纠正了一些传播的错误。
>
> **技术梗概:** 论文采用了压缩形式，详细描述了Barrett、Montgomery和Plantard算法及其变体的技术细节。

---
### [推荐] [2026/1750] Threshold Lattice-Based Zero-Knowledge Proofs

- **匹配关键字:** lattice, homomorphic encryption, LWE

- **作者:** Scott Griffy, Victor Youdom Kemmoe, Ngoc Khanh Nguyen, Tjerand Silde

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1750) | [PDF](https://eprint.iacr.org/2026/1750.pdf)


> **研究背景:** 现有的基于格的零知识证明虽然已足够高效，但所有已知构造中单一证明者持有全部见证信息，成为系统单点故障。
>
> **主要贡献:** 本文首次构建了适用于精确关系的基于格的阈值零知识证明系统，使得任何t个参与者可以联合生成证明且保持验证不变性。
>
> **达到效果:** 通过阈值化Attema等人和Esgin等人的证明系统，并利用Hint-MLWE实现无拒绝抽样的验证过程，同时在阈值同态加密上进行评估。
>
> **技术梗概:** 定义了基于承诺与证明的阈值协议及其相应的零知识性和模拟可提取性概念，并证明了这些构造在被动对手对抗下的安全性。

---
### [推荐] [2026/1752] L-BAS: A Lattice-Based Blind Adaptor Signature Scheme

- **匹配关键字:** lattice

- **作者:** Amin Mohammadali, Riham AlTawy

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1752) | [PDF](https://eprint.iacr.org/2026/1752.pdf)


> **研究背景:** 随着数字货币的兴起和对隐私保护数字交互的需求增加，基于格的盲签名引起了广泛关注。然而，在不损害隐私的前提下实现公平性仍然是一个挑战。
>
> **主要贡献:** 本文提出了一种基于格的盲适配器签名方案（L-BAS），该方案结合了公平性和盲签密的隐私保证。
>
> **达到效果:** 与基础的基于格的盲签名相比，L-BAS 的构建仅增加了约 5.2 KB 的签名大小，并且在效率上保持了原有系统的大部分特性。
>
> **技术梗概:** 通过利用 ABDLOP 提出的承诺和证明框架中的二分结构来实现适配器功能。

---
### [推荐] [2026/1755] QuaILLL: Quaternion Ideal LLL and BKZ

- **匹配关键字:** lattice

- **作者:** Joshua Limbrey, Cong Ling, Christian Porter

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1755) | [PDF](https://eprint.iacr.org/2026/1755.pdf)


> **研究背景:** 当前针对LIP方案的通用破译技术仅在实数嵌入中调用SVP算子，忽视了通过将秩为2模块LIP归约到 quaternion 高斯整环上的主理想问题（nrd-PIP）所获得的四元结构。
>
> **主要贡献:** 首次提出并描述了基于四元环的格规约算法，并扩展了经典的LLL算法以利用四元序的代数性质和新颖的后处理步骤。进一步将块大小为2的BKZ算法推广到任意块大小。
>
> **达到效果:** 通过这些方法，研究人员设计了一种在子格中寻找最优边界的算法以及一种牺牲输出质量但能提供原始格真基的算法。这种方法被应用于理想格，特别是来自nrd-PIP实例如Hawk等。
>
> **技术梗概:** 利用四元序的独特性质和后处理技术改进了LLL算法，并通过调整标准嵌入方法来应用这些算法于特定的理想格中。

---
### [推荐] [2026/1756] Fully Homomorphic Encryption with Chosen-Ciphertext Security from LWE

- **匹配关键字:** homomorphic encryption, LWE

- **作者:** Rupeng Yang, Zuoxia Yu, Willy Susilo

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1756) | [PDF](https://eprint.iacr.org/2026/1756.pdf)


> **研究背景:** 本文构建了首个基于LWE假设在标准模型下具备选择密文安全性的全同态加密方案，填补了该领域的重要空白。
>
> **主要贡献:** 该工作首次将循环安全的LWE假设与全同态加密相结合，并设计了一种特殊的简洁论证来验证FHE密文的有效性。
>
> **达到效果:** 所提出的方案不仅实现了严格强于CCA1的安全性，而且在安全性证明上更为简洁和高效。
>
> **技术梗概:** 采用Naor-Yung双层加密范式，结合批处理论证和新颖的谓词可提取承诺等技术来实现全同态加密。

---
### [推荐] [2026/1761] Lightweight Lattice-based Single-Party Public-Key Authenticated Key Exchange

- **匹配关键字:** lattice, post-quantum

- **作者:** Alex Aïdan, Sébastien Canard, Emmanuel Fouotsa, Nyiang Melchisedech Mbeng

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1761) | [PDF](https://eprint.iacr.org/2026/1761.pdf)


> **研究背景:** 在资源受限的物联网环境中，轻量级且后量子安全的认证密钥交换（AKE）协议至关重要。现有的后量子AKE协议通常追求更强的安全性，如IND-CCA加密方案，这与物联网设备的需求不匹配。
>
> **主要贡献:** 本文通过改进确定性的 OW-CPA 方案并将其应用于 HBE 帕累托中，证明了其OW-CCA安全性，并修改了 Bai 等人的高效后量子 OW-CPA 方案以支持 OW-CCA 构造。此外，作者还扩展了 SPAKE 框架以支持提出的 OW-CCA 构造。
>
> **达到效果:** 通过上述贡献，本文提出了一种轻量级的、基于格的单方公钥认证密钥交换协议，能够在不牺牲安全性的情况下显著减少计算开销和通信成本。
>
> **技术梗概:** 研究采用了 HBE 帕累托和 OW-CPA 方案，并结合了 Fujisaki–Okamoto 变换来实现 OW-CCA 安全性。此外，还对 SPAKE 框架进行了扩展以支持新的构造方法。

---
### [2026/1724] Data-Dependent Memory-Hard Functions: Sustained Space and Cumulative Complexity Trade-offs in the Parallel Random Oracle Model

- **作者:** Jeremiah Blocki, Blake Holman

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1724) | [PDF](https://eprint.iacr.org/2026/1724.pdf)


> **研究背景:** 数据依赖的内存硬函数（dMHF）旨在提供密码学保护，防止低熵秘密如密码被暴力破解。理想情况下，需要确保并行攻击者必须在整个过程中维持$\Theta(N)$的内存，并且函数可以在顺序时间内计算为$\Theta(N)$。然而，这一目标在动态分片模型中难以实现。
>
> **主要贡献:** Blocki和Holman提出了DEGSample，这是一种新的数据依赖MHF，并首次直接在平行随机预言模型（PROM）中证明了dMHFs的SSC/CMC权衡。
>
> **达到效果:** 在动态分片模型中，DEGSample实现了与先前工作相同的理想权衡：任何动态分片策略要么在整个过程中维持$\Omega(N)$内存，要么面临最大累计记忆复杂度惩罚$\Omega(N^{3-\epsilon})$。在PROM中，我们证明了任何攻击者要么在整个过程中维持$\Omega(N)$内存……
>
> **技术梗概:** DEGSample的设计利用了数据依赖性来实现SSC和CMC之间的权衡，并通过形式化分析证明了其在PROM中的安全性。

---
### [2026/1726] Jacobi Signatures, Revisited

- **作者:** Yansong Feng

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1726) | [PDF](https://eprint.iacr.org/2026/1726.pdf)


> **研究背景:** 研究重新审视了Jacobi签名在模$N=p^2q$下的单向性证明，移除了Boneh--Lipton的猜想依赖。
>
> **主要贡献:** 贡献在于直接将Damgård提出的伪随机生成器的一方性与标准数论问题联系起来。
>
> **达到效果:** 结果是简化了Jacobi签名的安全性证明，并提高了其理论基础的可靠性。
>
> **技术梗概:** 技术上采用了一种新的方法，不依赖于复杂的猜想，而是基于基本的数论难题。

---
### [2026/1727] Enhancing Capital Efficiency in DeFi Lending and Liquidity Provision

- **作者:** Adam Smehyl, Ivan Homoliak

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1727) | [PDF](https://eprint.iacr.org/2026/1727.pdf)


> **研究背景:** DeFi领域正经历快速增长，但大量资本在基于池的借贷储备中闲置或流动性提供位置不活跃，导致效率低下。
>
> **主要贡献:** 提出了两种扩展方案以提高DeFi协议中的资本效率：一种是通过增加分配层将未使用的流动性部署到外部收益生成策略；另一种是通过自动化范围迁移管理流动性位置，解决集中流动性位置的不活动问题。
>
> **达到效果:** 评估结果显示，在特定外部收益率假设下，借贷分析表明供应商收益率显著提升；而范围迁移分析则关注执行时间和成本敏感性。
>
> **技术梗概:** 通过设计机制、预期效果、实施方法和实际限制来探讨解决方案，并基于原型基准测试验证了这两种机制可以作为模块化扩展实现，但也揭示了额外的gas开销和特定的风险因素。

---
### [2026/1728] A Controlled Case Study of Design Trade-offs in DeFi Lending Protocols

- **作者:** Jan Findra, Zdenek Lapes, Ivan Homoliak

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1728) | [PDF](https://eprint.iacr.org/2026/1728.pdf)


> **研究背景:** 研究探讨了去中心化金融（DeFi）借贷协议的设计权衡，包括gas成本、资本效率、清算行为和风险分配。
>
> **主要贡献:** 贡献在于通过对比Aave V3, SparkLend, Compound III 和 Morpho Blue四种协议，在匹配的以太坊分叉测试中评估了它们在特定条件下的表现。
>
> **达到效果:** 结果表明，Morpho Blue在交互调用路径gas消耗最少且借款容量最高，但风险选择被转移到隔离的去许可市场；Aave V3和SparkLend提供了更广泛的市场抽象但gas成本较高。
>
> **技术梗概:** 技术上采用了以太坊分叉测试套件进行重复测量，并通过Foundry工具比较借贷设计权衡。

---
### [2026/1729] A Unified Framework for Contract-Validated Benchmarking of Zero-Knowledge Proving Systems

- **作者:** Matej Hulek, Martin Peresini, Ivan Homoliak

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1729) | [PDF](https://eprint.iacr.org/2026/1729.pdf)


> **研究背景:** 本文旨在提供一个统一的框架来基准测试零知识证明系统，以解决现有基准测试结果难以解读的问题。
>
> **主要贡献:** 该框架通过隔离容器化的代理和方案适配层实现了异构证明系统的统一执行与报告方法。
>
> **达到效果:** 评估了多种基于电路、透明、GPU加速、折叠及zkVM的系统，并使用六个具有相似语义的主要工作负载进行测试，揭示了不同系统在各种部署标准下的表现差异。
>
> **技术梗概:** 通过公共执行和报告机制确保后端在相同声明的公开声明下进行比较，即使它们的电路、轨迹或客机管道存在差异。

---
### [2026/1731] Generic Ring-Signature Transforms for Fiat-Shamir with Aborts and Hash-and-Sign with Retry

- **作者:** Haruhisa Kosuge, Koutarou Suzuki

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1731) | [PDF](https://eprint.iacr.org/2026/1731.pdf)


> **研究背景:** 研究背景：环签名提供了一种匿名性，适用于任意集合的公钥。尽管Abe-Ohkubo-Suzuki (AOS)转换对普通的 Fiat--Shamir 和 hash-and-sign 签名有很好的理解，但对于它们的拒绝采样变体Fiat--Shamir with aborts (FSwA)和hash-and-sign with retry (HSwR)，则缺乏相应的研究。
>
> **主要贡献:** 主要贡献：本文首次形式化了AOS环签名转换对于 FSwA 和 HSwR，并在量子随机预言模型中分析了其安全性；证明了在适应性环签名查询下的不可伪造性，同时处理适应性作恶需要模拟属性的有效性；对于强不可伪造性，FSwA 需要响应唯一性和特殊诚实性，而 HSwR 则需要第二原像抗性和非可逆性。
>
> **达到效果:** 达到的效果：通过形式化和分析，本文填补了Fiat--Shamir with aborts (FSwA) 和 hash-and-sign with retry (HSwR) 的环签名转换研究空白；证明了这些方案在量子随机预言模型下的安全性和适用条件。
>
> **技术梗概:** 技术梗概：采用了形式化方法和安全性分析，特别是在适应性查询和作恶处理上使用了模拟属性，并通过案例研究验证了方案的有效性。

---
### [2026/1732] Indifferentiability of Public-Key Encryption: Theory Meets Practice

- **作者:** Taiyu Wang, Cong Zhang, Hong-Sheng Zhou, Jiayi Ai, Zhihong Jia, Wenli Wang, Jian Liu, Xin Wang, Li Lin, Kui Ren, Chun Chen

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1732) | [PDF](https://eprint.iacr.org/2026/1732.pdf)


> **研究背景:** 公钥加密（PKE）是现代密码学中的基本构建块，许多PKE方案已被标准化并广泛部署。然而，现有理论在复杂和高度组合的环境中验证其安全性时存在局限性。
>
> **主要贡献:** 研究识别了理想PKE模型中的一个障碍，并重新定义了PKE的理想化模型以去除伪随机性的要求；提出了实现公钥加密非等价性的标准化方案。
>
> **达到效果:** 通过修订后的理想PKE模型，证明稍微增强的基于组的标准化PKE方案可以实现可证明的非等价性。
>
> **技术梗概:** 研究重新审视了Zhandry和Zhang的理想PKE定义，并提出了去除伪随机性要求的新模型；设计并分析了两个实际可行的公钥加密方案以验证其非等价性。

---
### [2026/1733] TEE Server-Assisted Aggregated Offline Deployment Scheme for Multiplication Triples

- **作者:** Puyu Wang, Ruidan, Zhenshen Liu, Ruiqi Yang, Hui Li

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1733) | [PDF](https://eprint.iacr.org/2026/1733.pdf)


> **研究背景:** 随着数据密集型应用中安全多方计算（MPC）的部署增长，生成和分发认证乘法三元组的离线过程已成为关键的可扩展性瓶颈。现有的软件预处理协议如MASCOT和LowGear，在参与者数量或需求增加时会带来显著的交互和计算成本。
>
> **主要贡献:** 本文提出了一种基于可信执行环境（TEE）服务器辅助聚合离线部署方案，将昂贵的交互式预处理过程移至远程验证的保护区域内，并通过认证加密通道分发参与者的特定认证三元组份额。
>
> **达到效果:** 实验结果表明，该方案在生成速率和可扩展性方面优于MASCOT和LowGear的MP-SPDZ实现。具体而言，与MASCOT相比，生成速率达到其三倍，并且具有线性可扩展性直至10万个参与者。此外，通过采用流式处理策略，实现了高效的分发过程。
>
> **技术梗概:** 该方案设计了从远程验证、单个参与者的会话建立到加密分发的端到端工作流程。保护区域充当TEE支持下的伪随机关联随机数生成器（PCG），输出用于MPC的一次性交付认证相关随机数。

---
### [2026/1737] From Mechanical Lock-Picking to Autonomous Driving Deception: A Comprehensive Review of Vehicle Attacks, Cyberattacks, and Defenses

- **作者:** Karim Lounis

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1737) | [PDF](https://eprint.iacr.org/2026/1737.pdf)


> **研究背景:** 随着现代汽车演变成高度互联的软件定义的物理信息系统(CPS)，它们集成了嵌入式电子设备、无线通信、人工智能和高级驾驶辅助系统(ADAS)。虽然这些技术进步显著提高了车辆的安全性、效率和自动驾驶水平，但也引入了日益扩大的网络安全威胁面。
>
> **主要贡献:** 该论文通过统一且独立的分类法对现代汽车攻击与防御机制进行了全面综述，将文献划分为四个研究方向：车身及物理系统攻击、车辆访问与启动系统攻击、电子与软件系统攻击以及自动驾驶系统攻击。
>
> **达到效果:** 综述系统地回顾和分析了每个方向的安全问题、代表性攻击和现有对策，并识别了当前的研究趋势，指出了现有的研究空白并讨论了未来挑战。
>
> **技术梗概:** 采用统一且独立的分类法对现代汽车攻击与防御机制进行了全面综述，通过系统性地审查和分析，为汽车网络安全提供了深入见解。

---
### [2026/1739] Prop RFQ: Proprietary Request for Quote as Pressure-Aware Exit Pricing for Redeemable Real-World Asset Tokens

- **作者:** Daniel Rolnik, Theodore Georgas, Ivan Homoliak

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1739) | [PDF](https://eprint.iacr.org/2026/1739.pdf)


> **研究背景:** 研究背景：可赎回现实世界资产代币可以在区块链上更快地交易，但其背书资产的销售或结算速度较慢。立即退出机制不能将报告的净资产价值（NAV）视为完全流动。
>
> **主要贡献:** 主要贡献：提出了一种Proprietary Request for Quote (Prop RFQ)，这是一种链上的退出报价设施，通过结合NAV、订单大小曲线、资金流动性墙、衰减卖压和重复出售的节奏响应来定价退出，并减轻但不消除拆分订单激励。
>
> **达到效果:** 达到的效果：在参考参数下，节奏减少了9.67%的整体拆分优势，改善了40种常见工作负载中的34种，并将最坏情况下的剩余储备从10.49%提高到17.66%。没有简单的策略或Prop RFQ的消融版本在其比较中在常用指标上优于参考配置。
>
> **技术梗概:** 技术梗概：该设计通过考虑NAV、订单大小曲线、资金流动性墙、衰减卖压和重复出售的节奏响应来保持孤立的小退出有用，同时限制储备耗尽并减少集中退出带来的收益。

---
### [2026/1740] Non-Local Search-to-Decision Reduction over $\mathbb{F}_2$

- **作者:** Prabhanjan Ananth

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1740) | [PDF](https://eprint.iacr.org/2026/1740.pdf)


> **研究背景:** 研究背景：该论文探讨了非局部搜索到判定问题，即两个不通信的实体能否通过共享编码信息预测随机值而不进行本地测量恢复原信息。
>
> **主要贡献:** 主要贡献：证明了在最优本地测量概率为p的情况下，双方正确回答共同挑战的概率最多为1和0.5+5p^(1/22)中的较小者。
>
> **达到效果:** 达到的效果：该结果对非局部问题的理解有重要影响，并促进了不可克隆密码学的应用研究，如不可克隆加密和量子复制保护领域的发展。
>
> **技术梗概:** 技术梗概：证明方法基于信息论，未提供有效提取器，且在开发过程中借助了高级AI助手进行辅助。

---
### [2026/1741] One Proof to Rule Them All: Practical, Sublinear Verification for Actively Secure MPC on $\mathbb{Z}_{2^k}$ with Dishonest Majority and a Dealer (Full Version)

- **作者:** Andreas Brüggemann, Ariel Nof, Thomas Schneider

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1741) | [PDF](https://eprint.iacr.org/2026/1741.pdf)


> **研究背景:** 研究旨在弥合被动安全与主动安全多方计算(MPC)之间的差距，特别是在具有恶意多数派和经销商的情况下，在环$\mathbb{Z}_{2^k}$上实现高效且子线性验证。
>
> **主要贡献:** 提出了一种新颖的单个分布式零知识(DZK)证明方法，能够在保持被动安全协议的同时，实现主动安全性，并特别适用于环$\mathbb{Z}_{2^k}$上的计算。
>
> **达到效果:** 该方法将通信复杂度从$\mathcal{O}(n \cdot \log m)$降低到$\mathcal{O}(n + \log m)$环元素，显著提高了验证效率。
>
> **技术梗概:** 通过设计一种新型DZK证明机制，确保所有参与方共同行为的正确性，从而实现单个证明覆盖整个验证过程。

---
### [2026/1742] Unclonable encryption from BB84 states: a simultaneous Goldreich-Levin reduction

- **作者:** Andrea Coladangelo, Qipeng Liu, Ziyi Xie

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1742) | [PDF](https://eprint.iacr.org/2026/1742.pdf)


> **研究背景:** 研究背景：Goldreich-Levin约简在密码学中广泛使用，能够将猜测隐藏字符串的能力转化为提取该字符串的完全信息。本文探讨了如何利用量子纠缠态增强加密方案的安全性。
>
> **主要贡献:** 主要贡献：提出了一个同时的Goldreich-Levin约简方法，适用于两个具备特定挑战响应能力的实体，从而提升了基于BB84状态的最简单候选不可复制加密方案至满足黄金标准的不可复制区分安全性。
>
> **达到效果:** 达到的效果：证明了基于BB84态的最简单不可复制加密方案达到了不可复制的区分安全性标准。
>
> **技术梗概:** 技术梗概：通过设计一种新的同时Goldreich-Levin约简，结合量子纠缠态和特定的安全性假设，实现了上述目标。

---
### [2026/1744] Improved Collision Attack on RIPEMD-160

- **作者:** Zhengrong Lu, Hongbo Yu, Yingxin Li, Xindi Zhang, Xiaoen Lin

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1744) | [PDF](https://eprint.iacr.org/2026/1744.pdf)


> **研究背景:** RIPEMD-160作为一种基于Merkle-Damgård结构的ISO/IEC哈希函数标准，尽管已有多种针对其减少轮数的攻击方法，但之前的最佳成果仅实现了40步的实际碰撞攻击。
>
> **主要贡献:** 本文提出了一种新的差分特征结构，通过在每条分支中使用一个连续的差分特征覆盖第1到2轮，从而减少了需要注入差异的消息字数量，并成功地识别出了三个42步RIPEMD-160哈希函数碰撞消息对。
>
> **达到效果:** 该方法显著提高了攻击的有效性，将针对RIPEMD-160的最佳实际碰撞攻击步骤数从40步提升至42步。
>
> **技术梗概:** 研究利用了基于高性能并行SAT求解器PRS的自动搜索工具，通过更严格的条件控制、差异注入和传播概率来识别合适的差分特征。

---
### [2026/1745] Round-Preserving Compilers for Super-Rushing Secure MPC

- **作者:** Michele Ciampi, Divya Ravi, Mingrui Zou

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1745) | [PDF](https://eprint.iacr.org/2026/1745.pdf)


> **研究背景:** 研究背景：现有的同步多方计算（MPC）协议在实践中要求各方在接收到所有预期消息后立即进入下一通信轮次，这与理论模型中设定的每轮次需等待超时时间不同。为了弥合这一差距，提出了超级冲刺型敌手模型。
>
> **主要贡献:** 主要贡献：本文首次探讨了计算复杂性MPC协议在面对超级冲刺型敌手时的安全性，并设计了一种保持通信轮次不变的编译器，将标准同步MPC协议转换为能够抵御超级冲刺型敌手攻击的协议。
>
> **达到效果:** 达到的效果：通过这种编译方法，实现了在不增加通信轮次的情况下提高MPC协议的安全性，确保了即使在网络中存在超级冲刺型敌手时也能保持协议的有效性和安全性。
>
> **技术梗概:** 技术梗概：采用了一种新颖的圆保留编译策略，通过对标准同步MPC协议进行改造，在理论模型和实际应用之间建立桥梁，确保在面对超级冲刺型敌手攻击时仍能保持通信效率。

---
### [2026/1746] Chasing QuOCCAs in a Quantum World: Type-2 Oracles for CCA-Secure PKE

- **作者:** Barbara Jiabao Benedikt, Tommaso Gagliardoni, Patrick Struck

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1746) | [PDF](https://eprint.iacr.org/2026/1746.pdf)


> **研究背景:** 研究背景：针对公钥加密(PKE)方案的安全性，Gagliardoni等人提出了基于量子超位置的qIND-qCPA安全性概念，但该概念仅适用于特定类型的PKE方案，并且在扩展到选择密文攻击(CCA)场景时面临挑战。
>
> **主要贡献:** 主要贡献：本文通过引入类型2算子理论，定义了适用于所有PKE方案（包括强解码可处理的）的量子超位置安全性概念，在CPA和CCA设置下无需使用Zhandry压缩oracle技术。
>
> **达到效果:** 达到的效果：成功扩展了基于量子超位置的安全性定义至更广泛的PKE方案，并通过引入‘净化’变换，为所有PKE方案提供了一种通用方法来实现这种安全性。
>
> **技术梗概:** 技术梗概：首先证明了直接将Gagliardoni等人技术推广到一般情况的不可能性，然后定义了一个新的PKE方案子类（强解码可处理），并引入‘净化’变换以克服障碍。

---
### [2026/1747] Extending Distinguishing to Key Recovery for Subfield Subcodes of GRS codes

- **作者:** Kirill Vedenev

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1747) | [PDF](https://eprint.iacr.org/2026/1747.pdf)


> **研究背景:** 研究背景：Ghoshal等人最近提出了针对GRS子码（包括戈帕码）的新颖准多项式时间区分器，但尚未解决如何将其转化为完整的密钥恢复攻击的问题。
>
> **主要贡献:** 主要贡献：作者提出了一种方法，将上述区分器转化为全密钥恢复攻击，并验证了该方法在$\mathbb{F}_4$上的戈帕码上是有效的。
>
> **达到效果:** 达到的效果：实验表明，此方法适用于$\mathbb{F}_4$上的戈帕码，并推测其可能也适用于二元戈帕码的密钥恢复。
>
> **技术梗概:** 技术梗概：该攻击利用了区分器的结果进行多次执行来实现密钥恢复，从而克服了直接应用区分器无法完成密钥恢复的问题。

---
### [2026/1748] SafeHub: End-to-end encrypted Git hosting system

- **作者:** Easwar Vivek Mangipudi

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1748) | [PDF](https://eprint.iacr.org/2026/1748.pdf)


> **研究背景:** 现有Git托管系统在传输和静止状态下对代码库进行加密，但代码库内容仍可被托管服务提供商读取。SafeHub旨在提供端到端加密的Git托管解决方案，确保代码库及其元数据的安全性。
>
> **主要贡献:** SafeHub通过将仓库内容及语义元数据（如文件名、提交信息等）加密，使得托管方仅能看到密文和不透明标识符，从而保护用户隐私。每个仓库被设计为一个MLS组，提供管理员调解的成员资格管理等功能。
>
> **达到效果:** SafeHub实现了对Git对象哈希链接的保护，并通过加密设备签名的清单记录引用，确保了在恶意服务器和适应性成员篡改情况下的功能完整性。实验结果表明其性能良好。
>
> **技术梗概:** SafeHub采用了MLS协议组机制、量子随机预言模型假设下的安全擦除技术以及理想功能规格化方法来实现系统设计与安全性证明。

---
### [2026/1749] CAKE-HI - Compact Authenticated Key Exchange Hiding Identities

- **作者:** Uri Blumenthal, Gene Itkis, Roger Khazan, Brandon Luo, Sean O'Melia, Brian Proulx, David Stott, Gabriel Torres, David A. Wilson

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1749) | [PDF](https://eprint.iacr.org/2026/1749.pdf)


> **研究背景:** 面对量子计算对现代公钥密码学的威胁，研究转向了能抵抗量子攻击的算法。然而，这些算法导致密钥长度增加、加密数据和数字签名变大，这对带宽或功率有限的设备提出了挑战。
>
> **主要贡献:** 提出了一种名为CAKE-HI的新协议，旨在减少在资源受限环境下使用量子抗性算法时的开销，同时保持安全性。
>
> **达到效果:** 与传统的TLS双向认证相比，CAKE-HI显著减少了握手消息大小和计算成本，证明了其在符号和计算模型中的安全属性。
>
> **技术梗概:** 通过形式化分析框架Verifpal和CryptoVerif验证了CAKE-HI的安全性，并将其应用于评估协议的效率和安全性。

---
### [2026/1751] Efficient Dynamic Group Signatures with Forward Security

- **作者:** Amin Mohammadali, Riham AlTawy

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1751) | [PDF](https://eprint.iacr.org/2026/1751.pdf)


> **研究背景:** 动态群签名方案（GSS）需要确保新加入成员不能伪造过去时间的签名，并且即使管理者也无法将签名归咎于合法用户。然而，大多数现有方案要么缺乏非归咎性，要么在更新密钥以保证前向安全时效率低下。
>
> **主要贡献:** 作者提出了一种具有前向安全性的动态群签名方案并确保了非归咎性，并提供了一个更高效的替代方案。
>
> **达到效果:** 通过证明和实验结果表明，所提出的方案在计算和通信效率方面与现有文献相当或优于现有文献。
>
> **技术梗概:** 提出了有效的撤销机制以避免重新注册现有用户，并提出了一种批量验证技术来提高验证过程的效率。

---
### [2026/1753] Linear Distance for Fixed-Row-Weight Expand--Accumulate Codes over Arbitrary Fields

- **作者:** Majid Khabbazian

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1753) | [PDF](https://eprint.iacr.org/2026/1753.pdf)


> **研究背景:** 研究背景：Expand–accumulate (EA) 码是构建相关伪随机性和领域无关简洁论证的基础稀疏线性码。Block等人在CRYPTO 2024中提出了一个关于固定行重EA组件的猜想，认为其能实现恒定相对距离。
>
> **主要贡献:** 主要贡献在于证明了该猜想，并以更强的形式进行了证明：对于任意有限域，存在固定的行重EA组件，能够达到任意指定的可靠性指数。
>
> **达到效果:** 研究结果表明，单个固定行重EA组件在所有足够大的块长度上都是渐近良好的，并且其多项式可靠性指数可以通过增加行重常数来任意增大。
>
> **技术梗概:** 技术梗概：证明通过区分稀疏消息和高权重消息进行。稀疏消息通过扩展和累加器取消的紧凑分析处理，而高权重消息则通过大域上的线性约束过剩和有界域上的随机累加器分析控制。

---
### [2026/1754] SoK: Why Optimal Cryptographic Combiners Do Not Get Deployed: Security, Complexity, and Adoption of Hybrid KEMs

- **作者:** Merland Chrislain Chadrel BAFOUETILA, Anis BKAKRIA

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1754) | [PDF](https://eprint.iacr.org/2026/1754.pdf)


> **研究背景:** 研究探讨了为何最优密码组合器未被部署，分析了认证强度、标准成熟度和实现复杂性对加密采用的影响。
>
> **主要贡献:** 提出了(A, P, φ)框架来量化这些因素，并应用于七类组合器和44个库中，首次提供了可验证的定量模型预测加密技术的采纳情况。
>
> **达到效果:** 研究表明，实现复杂性是影响加密技术采纳的主要因素之一，最安全的构造未必是最易部署的。
>
> **技术梗概:** 通过引入(A, P, φ)框架，结合软件工程指标进行分析，并通过实际代码行数验证了实施复杂性的影响力。

---
### [2026/1758] $\textsf{Sluice}$: Prove-Phase Bounded-Memory Groth16 via Read-Write Streaming

- **作者:** Kyeongtae Lee, Jihye Kim, Hyunok Oh

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1758) | [PDF](https://eprint.iacr.org/2026/1758.pdf)


> **研究背景:** 本文提出了一种基于读写流式计算的Groth16证明者$\textsf{Sluice}$，旨在减少证明阶段的随机访问工作内存需求，同时保持标准接口不变。
>
> **主要贡献:** $\textit{Split-Butterfly-Merge}(\mathsf{SBM})$算法是本文的关键技术贡献，该算法在读写流式模型中实现了$\mathcal{O}(\log N)$的内存使用。
>
> **达到效果:** 实验结果表明，在相同的证明规模下，$\textsf{Sluice}$能够在较低的工作内存限制（8GB）下成功运行，而传统方法则需要更高的内存支持或更长的时间来完成证明。
>
> **技术梗概:** 通过结合SBM算法、稀疏R1CS评估和分块Pippenger MSM技术，$\textsf{Sluice}$实现了验证器兼容的证明路径，并在存储需求与时间消耗之间进行了权衡。

---
### [2026/1759] Revisiting the Transferability of Chosen- to Known-plaintext Attacks and Applications to Round-reduced AES

- **作者:** Xiaomeng Sun, Eik List, Wenying Zhang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1759) | [PDF](https://eprint.iacr.org/2026/1759.pdf)


> **研究背景:** 针对许多分组密码的最佳已知攻击方法通常是基于差分的，但这些攻击通常要求对手能够选择明文或密文以满足差分轨迹。然而，在实际应用中，对手往往无法直接选择输入。
>
> **主要贡献:** 本文研究了差分攻击在随机已知明文模型(RKP)下的应用，并针对AES的所有版本提出了最佳现有鉴别器和攻击方法，至少比之前的结果多了一轮。
>
> **达到效果:** 研究表明，Demirci-Selcuk的中间相遇攻击（MITM）结合差异枚举，在不需要接近完整码本的情况下也能有效工作。对于某些攻击，展示了在选择数据模型下最优的攻击复杂度下的路径可能在RKP模型中表现不佳，并可被更优的路径替代。
>
> **技术梗概:** 通过分析差分轨迹和改进现有技术，本文提出了适用于随机已知明文模型的新攻击方法。

---
### [2026/1760] Midpoint Reset: A Full-Round Poseidon Collision from an Adaptively Chosen MDS Matrix

- **作者:** Sunghyeon Jo

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1760) | [PDF](https://eprint.iacr.org/2026/1760.pdf)


> **研究背景:** 研究背景：本文针对KoalaBear Poseidon实例中的28轮进行了全轮次的碰撞攻击，探讨了固定轮常数与后续选择线性层之间的适应性相关性。
>
> **主要贡献:** 主要贡献在于提出了一个明确的压缩碰撞构造方法，适用于所有28轮，并且该方法揭示了一种仅通过矩阵检查无法捕捉到的固定轮常数和随后选择线性层之间的适应性关联。
>
> **达到效果:** 达到的效果是成功在KoalaBear实例的所有16个输出坐标中找到了碰撞，验证了所提出的方法的有效性。
>
> **技术梗概:** 技术梗概：通过追踪中间值和半差分，在每两个轮次块中利用特定的线性层图像来抵消中间值，并重用额外的图像将半差分恢复到同一一维子空间，最终确定轨迹约束并使用标量递归完成差异传播。

---
### [2026/1762] Pilaf: Fully Tight Two-Round Threshold Signatures with Adaptive Corruptions

- **作者:** Chen Qian, Xingyu Zhao, Hao Cheng, Zengpeng Li, Puwen Wei, Quan Yuan

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1762) | [PDF](https://eprint.iacr.org/2026/1762.pdf)


> **研究背景:** 针对适应性恶意节点攻击的阈值签名方案，现有方法要么增加轮次、依赖代数或知识假设，要么在猜测破坏模式、关键会话或最终传输时付出非紧致的安全损失。
>
> **主要贡献:** 提出$\mathsf{TPilaf}$，首个结合部分非交互式签名和完全适应性攻击紧致证明的两轮阈值签名方案。
>
> **达到效果:** 该方案在随机预言模型中基于$\mathsf{MDDH}$假设构建，无需配对运算，任意阈值集合的签名人可以聚合其第二轮份额生成单一公开可验证签名。
>
> **技术梗概:** 通过引入线性同态双模式承诺并结合目标可开启特性，以及使用按配置零和掩码后完成技术来实现上述成果。

---
### [2026/1763] Multidimensional Hill Cipher Substitution– Permutation Network

- **作者:** Porter E. Coggins, III

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1763) | [PDF](https://eprint.iacr.org/2026/1763.pdf)


> **研究背景:** 该研究提出了一种结合多级扩散混排层、记忆硬KDF和同时多重度量经验评估的Hill密码变体——MD-Hill-SPN。
>
> **主要贡献:** 其创新性地将这些组件整合，首次实现了从理论上构造到实际通过定义明确的经验评估套件的转变。
>
> **达到效果:** 实验结果显示了全面的雪崩效应、差异概率采样、线性偏差探针、代数度下界饱和以及在单一密钥和已知明文条件下满足Singleton（MDS）分支数量，所有这些都达到了理论上限。
>
> **技术梗概:** 研究采用了Hill密码的基本原理，并通过多级扩散混排层、记忆硬KDF和多重度量经验评估的技术来增强其安全性。

---
### [2026/1764] Two Novel Multidimensional Affine Variations of the Hill Cipher

- **作者:** Porter Eldridge Coggins

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1764) | [PDF](https://eprint.iacr.org/2026/1764.pdf)


> **研究背景:** 文章提出了希尔密码的一种新颖的多维仿射变体，旨在增强加密方案的安全性。
>
> **主要贡献:** 作者通过引入不同阶数的关键矩阵和独或操作，扩展了希尔密码的应用范围，并增强了其安全性。
>
> **达到效果:** 这两种新方法显著提高了加密强度，但同时也存在一些潜在弱点需要进一步研究。
>
> **技术梗概:** 技术上，文章采用了2×2、4×4和8×8阶的可逆密钥矩阵，并结合独或操作来处理明文块。

---
### [2026/1765] Exact linear correlations and the cost of Walsh-transform key recovery, with application to SPEEDY

- **作者:** Guoqiang Liu, Bing Sun

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1765) | [PDF](https://eprint.iacr.org/2026/1765.pdf)


> **研究背景:** 研究了在SPEEDY密码算法中，当两个S盒层之间没有密钥添加时，圆周相关性的计算方法与传统乘积法则的不同之处。
>
> **主要贡献:** 提出了精确的一轮算法，并开发了依赖图分解、覆盖数界以及沃尔什支持标准等工具。
>
> **达到效果:** 修正了对SPEEDY的线性密码分析结果，提高了五轮攻击的重量上限，并指出全圆攻击所需的密文数量增加。
>
> **技术梗概:** 通过精确计算内部圆周和，改进了线性路径权重边界，并引入沃尔什支持标准来确定密钥恢复变换的成本。

---
### [2026/1766] Eavesdropper-Blind Remote State Preparation and Applications to Quantum Public-Key Encryption

- **作者:** Kaniuar Bacho, Alexandru Cojocaru

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1766) | [PDF](https://eprint.iacr.org/2026/1766.pdf)


> **研究背景:** 远程状态准备（RSP）是量子密码学中的核心机制，允许经典方通过经典通信远程构建量子态。
>
> **主要贡献:** 作者引入了一种较弱形式的远程状态准备——旁观者盲远程状态准备（EB-RSP），并证明了其在构造具有经典公钥和量子密文的量子公钥加密方面的应用价值。
>
> **达到效果:** 通过基于特定的一方向群操作构建两消息EB-RSP协议，作者为基于假设不依赖于陷阱门的RSP型原语迈出了第一步。
>
> **技术梗概:** 该研究采用旁观者仅需对公开协议转录内容保持盲目的模型，并从具体的一方向群操作中构造了两消息EB-RSP协议。

---
### [2026/1767] Circle-Linear Cryptanalysis: Bibrace Characters and Weak-Key Linear Distinguishers for CRAFT

- **作者:** Roberto Civino

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1767) | [PDF](https://eprint.iacr.org/2026/1767.pdf)


> **研究背景:** 研究背景：线性密码分析衡量了一种密钥流与加密算法中使用的字符之间的相关性。当使用二元bibrace结构替代传统的元素阿贝尔群时，可接受的掩码不再是普通的标量乘积，而是被限制为半数是线性的，另一半是非线性的。
>
> **主要贡献:** 主要贡献：作者提出了Circle-Linear密码分析方法，并应用于Craft算法，揭示了Midori/Craft S盒在特定bibrace群上的四个概率性关系，从而发现了弱密钥区分器。
>
> **达到效果:** 达到的效果：通过完全枚举而非启发式方法解决了最小重量码字问题，成功找到了18轮以内的弱密钥区分器，并证明Craft的圆常数对密钥无任何限制。
>
> **技术梗概:** 技术梗概：作者使用贝恩斯几何方法在任意有限阿贝尔群上发展了线性密码分析，并将其应用于特定bibrace群，通过最小重量码字问题精确求解来确定最佳路径。

---
### [2026/1768] Constant-round MPC protocols with Fall-back Security

- **作者:** Anasuya Acharya, Aditya Patankar, Arpita Patra, Divya Ravi, Raghavendra Vernekar

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1768) | [PDF](https://eprint.iacr.org/2026/1768.pdf)


> **研究背景:** 研究背景：本文探讨了在阈值恶意攻击设置下，具有降级安全性的多方计算（MPC）协议的轮次复杂度。此前的工作大多要求轮次复杂度与参与方数量和计算规模成线性关系。
>
> **主要贡献:** 主要贡献在于提出了适用于最优阈值的常数轮次协议：在无认证字符串模型中，对于$t < rac{n}{2}$恶意攻击者情况，提出了一种3轮半诚实降级安全协议；在有认证字符串模型中，同样针对$t < rac{n}{2}$恶意攻击者情况，提出了4轮恶意行为降级安全且满足一致终止的协议，并进一步扩展为5轮协议以实现公平性。
>
> **达到效果:** 达到的效果是显著降低了常数轮次MPC协议在特定阈值下的通信复杂度和计算效率，特别是在无认证字符串模型中实现了前所未有的低轮次数。
>
> **技术梗概:** 技术梗概：通过引入新的设计技术和安全证明方法，在保持降级安全性的同时大幅减少了协议的轮次复杂度。

---
### [2026/1769] Rank Measures and Exponential Lower Bounds for Multilinear Secret Sharing

- **作者:** Shahram Khazaei

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1769) | [PDF](https://eprint.iacr.org/2026/1769.pdf)


> **研究背景:** 研究背景：多线性秘密共享方案通过分摊秘密维度上的密份额大小，可能推翻一维线性方案中的已知下界。此前，对于多线性方案的最佳显式下界为准多项式级别。
>
> **主要贡献:** 主要贡献：证明了Razborov--G\'al秩度量在分摊后仍然有效，并结合Pitassi和Robere的秩证据，给出了一个完美多线性方案族，其平均和最大信息比率为$2^{\Omega(n)}$。
>
> **达到效果:** 达到的效果：对于任意重构算法且在对下界隐私低于一的情况下，该结果扩展到具有多项式级秘密维度的方案，从而得到每个固定重构度数下的指数下界。
>
> **技术梗概:** 技术梗概：通过利用秩度量和秩证据，结合分摊分析，证明了多线性秘密共享方案的信息比率下限，并推广至更广泛的场景。

---
### [2026/1770] How Many Traces Suffice? PAC Guarantees for Profiled and Non-profiled Side-Channel Analysis

- **作者:** Seyedmohammad Nouraniboosjin, Fatemeh Ganji

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1770) | [PDF](https://eprint.iacr.org/2026/1770.pdf)


> **研究背景:** 侧信道分析（SCA）通常通过报告减少正确密钥排名所需的踪迹数量来进行评估，但这种评估仍具有经验性，并未解释足够踪迹的数量、单独的建模和攻击数据如何贡献以及额外的踪迹是否能克服较弱的密钥区分度。
>
> **主要贡献:** 本文通过概率近似正确（PAC）形式化方法解决了上述问题，将候选密钥评分作为共同的密码分析对象，并分离有限样本估计与正确的密钥和竞争假设之间的固有差异。
>
> **达到效果:** 实验表明，在ASCAD-f和ASCAD-r上，建模攻击在数十个攻击踪迹中实现了精确恢复，而非建模单攻击排名证书保证了约1000个踪迹的精确恢复。这些结果与最近的ASCAD攻击具有竞争力，并且在非建模设置下显著低于先前研究中识别出的最小踪迹数量，同时提供了有限样本保证。
>
> **技术梗概:** 本文采用代表性的建模和非建模攻击实例进行分析，通过PAC框架分离了候选密钥评分作为密码分析对象，并将有限样本估计与固有区分度分开处理。

---
### [2026/1771] New Lower Bounds for Rows of $d$-Disjunct Matrices via Recursive Potentials

- **作者:** Xiaopeng Zhao

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1771) | [PDF](https://eprint.iacr.org/2026/1771.pdf)


> **研究背景:** 研究背景：非适应性组合组测试中，目标是在不超过d个阳性的情况下，通过最少的组合测试来识别n个项目。
>
> **主要贡献:** 主要贡献：作者通过引入列删除递归方法，并使轻重阈值随递归状态z变化，改进了先前的下界证明。
>
> **达到效果:** 达到的效果：新的下界为$T(d) \ge 0.9283d^2 - O(d)$，比之前的结果更加精确。
>
> **技术梗概:** 技术梗概：通过分析一阶微分方程，并利用区间算术验证方法确保解满足所需接触点条件。

---
### [2026/1772] Multi-PGBF: Efficient Oblivious Key-Value Store and Application to Private Set Intersection

- **作者:** Mingli Wu, Tsz Hon Yuen, Man Ho Au, Siu-Ming Yiu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1772) | [PDF](https://eprint.iacr.org/2026/1772.pdf)


> **研究背景:** 研究背景：密文委托存储（Oblivious Key-Value Store）是一种能够安全地编码和解码$n$个键值对的数据结构，且在编码后无法区分这些键值对与其他输入域中的键值对。
>
> **主要贡献:** 主要贡献是提出了Peelable Garbled Bloom Filter (PGBF) 和 Multi-PGBF 技术，以提高密文委托存储的效率和安全性。
>
> **达到效果:** 达到的效果是在编码时间和解码速度上显著提升，其中Multi-PGBF将RR（CCS’22）的编码时间提高了65.1%至77.6%，而C-Multi-PGBF则进一步提升了编码效率。
>
> **技术梗概:** 技术梗概：通过递归地组合多个PGBFs进行分层编码和解码，以及将大集合聚类为小集合以加快编码过程。

---

## 更新: 2026-08-21 06:57

*新增 1 篇论文 (编号 1719--1719)*

### [2026/1719] Matrix Hoeffding and Bernstein Bounds with Sharp Constants for Markov Chains

- **作者:** Zhao Song

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1719) | [PDF](https://eprint.iacr.org/2026/1719.pdf)


> **研究背景:** 研究背景：针对马尔可夫链上的矩阵集中性问题，现有文献提供的常数较为宽松，因此有必要探索更紧致的证明方法以达到与独立随机变量相同的效果。
>
> **主要贡献:** 主要贡献：作者提出了一个紧致的Hoeffding指数，并改进了Chernoff和Bernstein不等式的常数，解决了先前研究中的松散问题。
>
> **达到效果:** 达到的效果：通过引入新的技术手段，论文在马尔可夫链矩阵集中性方面取得了显著进展，提供了更精确的概率上界估计。
>
> **技术梗概:** 技术梗概：作者利用了标量障碍物证明Hoeffding指数的紧致性，并通过精细分析改进了Chernoff和Bernstein不等式的常数。
...
### [推荐] [2026/1650] D-James: Ultra Short Multivariate Signatures

- **匹配关键字:** post-quantum

- **作者:** Jacques Patarin, Alexandre Roullet

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1650) | [PDF](https://eprint.iacr.org/2026/1650.pdf)


> **研究背景:** 多变量签名方案是少数几个能够提供超短签名的后量子候选方案之一，但设计安全构造极具挑战性。
>
> **主要贡献:** D-James通过引入Dragon项，实现了在经典128位和256位安全性水平下分别仅有156比特和348比特的最短签名，并且其签名验证成本与G$e$MSS相当。
>
> **达到效果:** 该方案不仅显著减小了签名大小，还在非二元有限域上实现了公共密钥尺寸最多减少十倍的效果，同时保持了签名大小和计算成本接近于二元情况。
>
> **技术梗概:** D-James通过Dragon项技术解耦公方程数量与哈希输出长度，并结合HFE$_	ext{IP}^-$框架来抵抗已知的MinRank攻击和差分攻击。

---
### [推荐] [2026/1653] Lumora: A Family of Permutation-Based Wide-Block Ciphers for Post-Quantum zkSNARK Applications

- **匹配关键字:** post-quantum

- **作者:** Susanta Samanta, Martin Grenouilloux, Guang Gong, Chunlei Li

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1653) | [PDF](https://eprint.iacr.org/2026/1653.pdf)


> **研究背景:** 针对零知识证明（ZKPs）等高级加密协议的应用需求，需要优化的对称密码原语以加快验证速度。在基于R1CS框架中，乘法约束的数量显著影响证明者的性能和证明大小。
>
> **主要贡献:** Lumora家族通过采用置换为基础的宽块密码设计，实现了针对ZKSNARK电路及后量子数字签名应用的有效性，并统一了AES结构定义于二元扩展域$\mathbb{F}_{2^n}$上。
>
> **达到效果:** Lumora在保持证明者工作量仅限于评估公共置换的同时，提高了验证效率并减少了证明大小。安全分析和实现结果表明其性能优越。
>
> **技术梗概:** 通过Even-Mansour范式实例化底层置换作为块密码，避免了单独密钥调度的R1CS约束开销，确保了公钥密码的安全性与高效性。

---
### [推荐] [2026/1655] Hardness of Euclidean Closest Vector within $n^{1/8-\epsilon}$ and Binary Nearest Codeword within $n^{1/4-\epsilon}$
...
- **链接:** [论文](https://eprint.iacr.org/2026/1655) | [PDF](https://eprint.iacr.org/2026/1655.pdf)


> **研究背景:** 本文证明了两个确定性的近似难解性结果，旨在提高格最近向量问题和二元码最近编码词问题的已知难度界限。
>
> **主要贡献:** 作者通过多项式时间归约方法，将欧几里得间隙最短向量问题(Euclidean GapCVP)和二进制最近编码词问题的近似难解性分别提高到了$n^{1/8-\epsilon}$和$n^{1/4-\epsilon}$。
>
> **达到效果:** 这些结果显著提高了对这两个问题已知难度界限的理解，为相关领域的研究提供了新的理论基础。
>
> **技术梗概:** 通过使用多项式时间归约方法，作者证明了上述近似难解性结果，该方法在格理论和编码理论中具有广泛应用。

---
### [推荐] [2026/1658] Upper bounds for failure probabilities of reductions from low density subset sum problems to lattice problems on linearly independent vectors
...
- **链接:** [论文](https://eprint.iacr.org/2026/1658) | [PDF](https://eprint.iacr.org/2026/1658.pdf)


> **研究背景:** 研究引入了一种新的格问题$l$-SIVP，并估计了从低密度子集和问题到$l$-SIVP的约简失败概率上界，特别地当$l=1$时改进了先前的结果。
>
> **主要贡献:** 贡献在于通过Ehrhart理论提供了更精确的失败概率上界估计，并分析了一般情况下主导项的特点。
>
> **达到效果:** 结果表明，在$l=1$的情况下，新方法给出的上界比之前的方法更为紧致，并为一般情况下的主导项提供了一些见解。
>
> **技术梗概:** 技术上采用了Ehrhart理论来处理格问题和子集和问题之间的约简失败概率估计。

---
### [推荐] [2026/1660] Transient Quantum Resistance, with Application to Ethereum Consensus

- **匹配关键字:** post-quantum

- **作者:** Pranay Anchuri, Matteo Campanelli, Rosario Gennaro

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1660) | [PDF](https://eprint.iacr.org/2026/1660.pdf)


> **研究背景:** 后量子迁移的候选方案相比其前量子对应物具有额外成本，特别是在签名方面，并且会失去如BLS方案中的一些吸引人的属性，例如同态性，进而影响直接签名聚合。
>
> **主要贡献:** 提出了一种方法论，使预量子原语可以在预设的时间段后（即在量子计算机出现之后）仍然安全使用，前提是防止在此时间段内对签名或加密证明的伪造，从而实现临时量子安全性。
>
> **达到效果:** 该研究为以太坊共识机制提供了后量子迁移解决方案，保持了BLS签名聚合的同时，减少了验证者揭示流量，并使关键共识摘要保持在约96字节，远小于当前基于SNARK聚合哈希方案所需的数百千字节。
>
> **技术梗概:** 通过绑定一个临时的普通预量子密钥和长期的后量子身份来实现临时量子安全性，确保暴露临时密钥不会泄露长期秘密，并且设计了一种解决方案以保持BLS签名聚合性。

---
### [推荐] [2026/1671] Verified Pythagorean Composition for Adaptive Cryptographic Games: Noise Flooding in Homomorphic Encryption
...
- **链接:** [论文](https://eprint.iacr.org/2026/1671) | [PDF](https://eprint.iacr.org/2026/1671.pdf)


> **研究背景:** 研究背景：噪声泛滥是抵御近似同态加密解密攻击的标准防御措施，但其安全证明对组合特别敏感。现有方法在处理多个查询时效率低下，导致性能损失。
>
> **主要贡献:** 主要贡献：提出了一种新的关系程序逻辑，并通过SSProve验证了该逻辑，实现了在不转换为统计距离的情况下组合条件KL预算，从而提高了多项式查询IND-CPAD敌手的安全证明效率。
>
> **达到效果:** 达到的效果：对于任何近似正确的完全同态加密方案和IND-CPA安全的方案，给出了一个参数化的安全归约，并证明了其在多项式查询下的安全性边界。
>
> **技术梗概:** 技术梗概：使用Rocq和SSProve进行机器验证，构造了一个新的关系程序逻辑来组合条件KL预算，并通过验证的跟踪编译器将局部Oracle规则提升为任意适应性程序。

---
### [推荐] [2026/1676] Simple and Efficient SKL-IBE with Classical Revocation from LWE

- **匹配关键字:** LWE

- **作者:** Ho Nguyen Pham, Duong Hieu Phan, Quoc-Huy Vu, Weiqiang Wen

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1676) | [PDF](https://eprint.iacr.org/2026/1676.pdf)


> **研究背景:** 研究背景：安全密钥租赁(SKL)是一种量子密码学原语，允许将解密密钥出租给被授权用户，并保证一旦撤销，租户将无法恢复解密能力。经典撤销功能使得SKL在实践中具有重要意义。
>
> **主要贡献:** 主要贡献在于提出了基于标准学习误差(LWE)假设的高效且具备经典撤销功能的安全密钥租赁方案，特别适用于身份基加密(IBE)，并改进了现有方法的效率。
>
> **达到效果:** 达到的效果是实现了首个基于多项式难度LWE的经典撤销安全密钥租赁公钥加密(SKL-PKE)方案，并提出了一种直接扩展自Dual-Regev SKL-PKE的高效选择性安全SKL-IBE方案，避免了使用门控电路和混淆假设。
>
> **技术梗概:** 技术梗概：通过改进Dual-Regev SKL-PKE的安全分析方法，结合资源效率高的Dual-Regev构造，以及直接在Agrawal等人的IBE框架内扩展Dual-Regev SKL-PKE的方法来实现高效性。

---
### [推荐] [2026/1677] Fair-Weather No More: Guaranteed Efficiency in Secure Group Messaging

- **匹配关键字:** post-quantum, LWE

- **作者:** James Bartusek, Nir Bitansky, Yevgeniy Dodis, Rachit Garg, David J. Wu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1677) | [PDF](https://eprint.iacr.org/2026/1677.pdf)


> **研究背景:** 安全的群组消息协议，如IETF标准化的消息层安全性（MLS），为数十亿用户提供端到端加密。关键挑战在于连续组密钥协商（CGKA）协议在最坏情况下的效率问题，特别是分组操作需要亚线性复杂度。
>
> **主要贡献:** 本文通过引入新的技术路径，解决了CGKA协议在最坏情况下无法实现亚线性复杂度的问题，提出了一个新的理论框架。
>
> **达到效果:** 该研究证明了即使在最坏的情况下，也可以设计出具有亚线性复杂度的CGKA协议，从而填补了实践与理论之间的空白。
>
> **技术梗概:** 本文采用了一种新颖的方法，结合了随机化技术和概率论证，绕过了现有的障碍，并提出了一个高效的CGKA协议构造方案。

---
### [推荐] [2026/1682] Incomplete Ciphertext Comparison in ML-KEM: From an IND-CCA2 Break to Key Recovery
...
- **链接:** [论文](https://eprint.iacr.org/2026/1682) | [PDF](https://eprint.iacr.org/2026/1682.pdf)


> **研究背景:** 研究背景：ML-KEM的安全性依赖于解封装过程中的一项检查，即Fujisaki–Okamoto (FO) 检查。wolfSSL的实现存在漏洞，未能比较所有密文字节，导致IND-CCA2安全性的减弱。
>
> **主要贡献:** 主要贡献在于揭示了这些未比较的字节实际上携带了解密噪声，并且这种噪声与秘密密钥呈线性关系，从而通过最小二乘法解密出完整密钥。
>
> **达到效果:** 达到的效果是提出了一种新的攻击方法，即使在u完全验证的情况下也能成功恢复密钥，扩大了攻击面。
>
> **技术梗概:** 技术梗概：利用未比较的字节读取解密噪声，并通过最小二乘法构建并求解线性方程组以恢复密钥。

---
### [推荐] [2026/1683] Fully-Succinct Multi-Key FHE & Rate-1 Simulatable Threshold Decryption from LWE
...
- **链接:** [论文](https://eprint.iacr.org/2026/1683) | [PDF](https://eprint.iacr.org/2026/1683.pdf)


> **研究背景:** 现有研究中，多密钥全同态加密（MKFHE）方案的密文大小、公钥和私钥大小随用户数量线性增长，限制了其广泛应用。
>
> **主要贡献:** 本文提出了首个在标准假设下实现常数密文大小的多密钥全同态加密方案，并设计了一种单轮分布式解密协议。
>
> **达到效果:** 该方案使得每个用户的部分解密份额大小与明文长度相同，且能够模拟诚实用户的部分解密过程，显著提高了多方计算协议中的通信效率。
>
> **技术梗概:** 通过利用Learning with Errors（LWE）假设，构建了层次化的多密钥全同态加密方案，并结合分布式算法实现了高效的解密机制。

---
### [推荐] [2026/1689] A Hybrid Post-Quantum Encryption Architecture with Self-Hosted Key Management for SME Cloud Data Protection
...
- **链接:** [论文](https://eprint.iacr.org/2026/1689) | [PDF](https://eprint.iacr.org/2026/1689.pdf)


> **研究背景:** 当前云存储中的数据保护主要依赖于RSA或ECDH等经典加密算法，但这些算法在未来量子计算机的攻击下将变得脆弱。中小企业由于缺乏安全预算和技术支持，难以进行数据迁移和重新加密以应对这一挑战。
>
> **主要贡献:** 作者提出了一种名为Quantum Cloud Guard (QCG) 的软件架构，它结合了客户端侧混合后量子加密、自我托管密钥保管以及应用层滥用防护网关三个元素，旨在为中小企业提供一种无需额外硬件投入的数据保护解决方案。
>
> **达到效果:** 实验结果显示，在STM32F407微控制器上进行的ML-KEM-1024密钥生成和解封装操作分别仅需40.8毫秒和44.0毫秒，而在服务器端，所有后量子加密操作均保持在亚毫秒级，签名操作每请求增加约0.24毫秒。该服务可以在每月4.49欧元的虚拟服务器上运行。
>
> **技术梗概:** QCG架构通过将密钥保管与数据保管分离，并利用ML-KEM-1024和ML-DSA-87等后量子算法确保了数据的安全性和完整性，同时通过客户端验证机制防止了密钥被篡改。

---
### [推荐] [2026/1690] Concurrently Secure Compact Blind Signatures from Module-SIS

- **匹配关键字:** lattice

- **作者:** Olivier Blazy, Lola-Baie Mallordy, Weiqiang Wen

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1690) | [PDF](https://eprint.iacr.org/2026/1690.pdf)


> **研究背景:** 传统的基于零知识证明的环签名方案导致了较大的盲签名称字，而本文旨在设计一种更紧凑且具备并发安全性的盲签名方案。
>
> **主要贡献:** 作者通过精心设计，去除了传统盲签名中依赖于零知识证明的部分，从而实现了更小的签名尺寸，并确保了并发安全性。
>
> **达到效果:** 最终，该方案成功将盲签名大小压缩至4.7KB，并在模块-SIS假设下达到了并发安全。
>
> **技术梗概:** 作者采用了经典的格基Σ协议，并巧妙地让最后一个组件自然成为消息哈希值的短前象，从而避免了零知识证明的使用。

---
### [推荐] [2026/1701] DTRU: A Versatile, Compact, Simple, and Robust NTRU KEM with Double $E_8$ Encoding

- **匹配关键字:** lattice

- **作者:** Hengchuan Zou, Songlin Li, Jieyu Zheng, Xiaowen Hu, Hanyu Wei, Weizhi Ao, Yifan Dong, Wenbo Guo, Yunlei Zhao

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1701) | [PDF](https://eprint.iacr.org/2026/1701.pdf)


> **研究背景:** 针对中国2025战略对商用密码标准的要求，提出了基于NTRU的DTRU密钥封装机制，支持128位、256位和512位安全（可选384位），以适应不同应用场景的需求。
>
> **主要贡献:** 主要贡献在于双$E_8$编码技术，该技术构建了低解码复杂度的16维格码，并详细分析了解密失败概率。
>
> **达到效果:** DTRU设计在灵活性、紧凑性、简单性和鲁棒性之间达到了平衡，支持多种环结构，实现了高效错误校正和带宽节省。
>
> **技术梗概:** 通过选择合适的参数确保安全性冗余，利用LPPNF规避了cyclotomic环的风险，并推荐了无稀疏噪声分布的参数集以增强安全性。

---
### [2026/1647] Information-Theoretic SFE and PFE with Reduced Communication

- **作者:** Shuaishuai Li, Cong Zhang, Juntong Lin, Anyu Wang, Xiaoyun Wang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1647) | [PDF](https://eprint.iacr.org/2026/1647.pdf)


> **研究背景:** 安全函数评估(SFE)和隐私函数评估(PFE)是多方计算中基本的原语，前者允许多方在私有输入上联合计算公共函数而不泄露额外信息，后者则涉及私人函数仅由指定方知晓。
>
> **主要贡献:** 作者提出了一种简单技术来减少SFE中的通信开销，并开发了新的PFE技术以避免使用通用电路，从而提高了效率。
>
> **达到效果:** 对于SFE，新方法实现了严格线性O(m^* n)的通信复杂度；对于PFE，在诚实多数的情况下达到了O(m^2n)的通信复杂度，并在特定情况下进一步优化。
>
> **技术梗概:** 通过引入简单技术减少乘法门的二次通信开销，同时开发了新的PFE方法以避免使用通用电路。

---
### [2026/1651] Single-Server Verifiable PIR with Updates and Universally Composable Security

- **作者:** Julia Guskind, Ariel Hamlin, Ryan Little, Daniel S. Roche, Mayank Varia

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1651) | [PDF](https://eprint.iacr.org/2026/1651.pdf)


> **研究背景:** 私有信息检索（PIR）是一种基本的密码学原语，允许客户端从服务器获取数据库中的条目而不泄露所访问的具体条目。近期研究引入了验证性私有信息检索（VPIR），以确保服务器无法篡改数据库内容或误导客户端。
>
> **主要贡献:** 本文首次在单服务器场景下提出了验证性私有信息检索的通用可组合安全（UC）定义，并通过提供基于简单PIR和VeriSimplePIR两种实现来展示了该功能的普适性。
>
> **达到效果:** 研究结果表明，传统的游戏化VPIR安全性在顺序和并发协议组成中会失效。本文提出的UC VPIR定义确保了即使在复杂的应用场景下也能保持安全性和正确性。
>
> **技术梗概:** 通过设计理想化的VPIR功能，并基于现有技术构建其实现方案，本文为验证性私有信息检索的安全性研究提供了新的视角和技术基础。

---
### [2026/1652] On the Security of In-band Verification in End-to-End Encrypted Video Calls

- **作者:** Daniel Jones, Melissa Chase, Esha Ghosh, Kim Laine

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1652) | [PDF](https://eprint.iacr.org/2026/1652.pdf)


> **研究背景:** 研究背景：在端到端加密视频通话中，客户端软件通过展示给参与者一个代码进行人工驱动的密钥验证仪式，以保护会议免受恶意服务提供商攻击。然而，先前的安全分析假设了经过身份认证的离带通道来进行比较，这在实际应用中通常是不现实的。
>
> **主要贡献:** 主要贡献：作者提出了人类到人类组密钥协商协议的形式化概念，并定义了一种基于人们一致识别彼此能力的认证分组密钥交换模型。
>
> **达到效果:** 达到的效果：研究证明了该构造的安全性，推导出了具体的边界条件，并讨论了一些非示例以说明定义的细微之处。结果强调了此方法在可用性和对人类认证视频流不可伪造性的依赖上的局限性。
>
> **技术梗概:** 技术梗概：通过形式化分析和理论证明来评估人类驱动的密钥验证仪式的安全性，识别出其安全性依赖于人们一致地相互识别的能力，并探讨了该假设面临的挑战。

---
### [2026/1654] Verifpal Seven Years Later: Can a Toy Become an Instrument?

- **作者:** Nadim Kobeissi

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1654) | [PDF](https://eprint.iacr.org/2026/1654.pdf)


> **研究背景:** Verifpal是一个在2019年提出的符号协议验证器，旨在通过牺牲分析的通用性来提供一种工程师可以理解的建模语言。尽管其最初的论文称其证明是不完整的半形式化论证，但该工具仍被视为教学辅助而非研究工具。
>
> **主要贡献:** 本文详细描述了Verifpal引擎的替换版本，新引擎采用目标导向的方法，从查询出发推导子目标，并在协议结构限定范围内进行搜索。此外，论文还首次正式阐述了新引擎的语义、等式理论、知识闭包和目标导向求解方法。
>
> **达到效果:** 通过改进后的验证器，研究者能够更准确地检测攻击而不会产生误报，并且验证过程更加高效。同时，新的语言支持公钥加密机制的表达以及协议组件的弱化声明。
>
> **技术梗概:** 新引擎采用目标导向的方法，从查询出发推导子目标，并在协议结构限定范围内进行搜索。通过结合语义、等式理论和知识闭包的概念，实现了对攻击的有效验证。

---
### [2026/1657] Ideal Pseudorandom Code, Revisited

- **作者:** Ganyuan Cao

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1657) | [PDF](https://eprint.iacr.org/2026/1657.pdf)


> **研究背景:** 研究背景：论文重新审视了伪随机纠错码（PRC），这是一种结合了伪随机性和纠错能力的编码方式，适用于生成式AI模型中的鲁棒水印和隐写术。
>
> **主要贡献:** 主要贡献在于通过适应性UC安全处理PRC，并识别出在适应性攻击下实现NC-PRC的关键障碍。
>
> **达到效果:** 达到的效果是证明了LDPC基的PRC不具备NC-PRC特性，同时提出了两种编译器来提升纠错码以支持NC-PRC。
>
> **技术梗概:** 技术梗概包括UC安全处理、适应性攻击下的安全性分析以及利用可编程接受机制绕过障碍的方法。

---
### [2026/1659] Efficient Transaction Traceability for Auditable Privacy-Preserving Ledgers

- **作者:** Elli Androulaki, Angelo De Caro, Kaoutar Elkhiyaoui, Rebekah Mercer, Elina van Kempen

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1659) | [PDF](https://eprint.iacr.org/2026/1659.pdf)


> **研究背景:** 隐私保护分布式账本允许用户在不泄露身份或交易细节的情况下提交交易。然而，监管和机构环境要求授权方能够高效地追踪交易的源头而不破坏整体系统的隐私性。现有的方法存在并发问题、高计算开销以及审计员对用户秘密过度访问的问题。
>
> **主要贡献:** 作者提出了一种高效的追溯框架，解决了并发性和审计员访问限制的问题，并通过形式化安全需求和基于伪随机函数及匿名凭证的黑盒构造提供了两种具体实现方案。
>
> **达到效果:** 实验评估表明该方法具有实用性，在添加追踪功能的同时仅增加了几毫秒的开销。
>
> **技术梗概:** 技术上，作者利用了伪随机函数和零知识证明等密码学工具来构建追溯机制，并通过哈希基伪随机函数及零知识简短论证系统（zk-SNARKs）或代数伪随机函数及Sigma协议的具体实例进行了验证。

---
### [2026/1661] MKA Meets Multidrop: Optimizing Time-to-Key-Agreement on 10BASE-T1S Ethernet

- **作者:** Jonathan Ndop, Isaac Molina, Guillermo Oliver, Friedrich Wiemer, Axel Sikora

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1661) | [PDF](https://eprint.iacr.org/2026/1661.pdf)


> **研究背景:** MACsec Key Agreement (MKA)协议用于管理并分发以太网链路的临时安全关联密钥，但在共享介质Ethernet多滴链接上可能扩展性不佳，且现有形式化最坏情况边界远超汽车启动目标。
>
> **主要贡献:** 本文提出了针对共享介质网络中安全启动时间优化的新型MKA算法，并通过详细的网络仿真评估了这些优化的效果。
>
> **达到效果:** 研究结果量化了所提优化对密钥协商时间的影响，提供了在建模启动假设下的保守仿真衍生实用启动时间边界，补充了形式化最坏情况分析中的分布信息。
>
> **技术梗概:** 该工作通过对实际启动场景下优化协议的完整启动时间分布进行研究，利用网络仿真来评估和验证这些技术的有效性。

---
### [2026/1662] Degree-Sum-Freedom Is Not EA Invariant: Exact Profiles in a 4-Uniform Permutation Family

- **作者:** Jingchuan Ma, Yanhua Liu, Qiaoyun Huang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1662) | [PDF](https://eprint.iacr.org/2026/1662.pdf)


> **研究背景:** 研究背景：度和自由度是评估置换家族中分部性质传播的局部标准，而已有的研究表明该标准在扩展仿射等价下保持不变。本文旨在验证并量化这一结论在特定置换家族中的有效性。
>
> **主要贡献:** 主要贡献在于证明了在4-均匀置换族中，自然金德APN对和特定完全映射族的度和自由度行为与已知理论存在差异，并精确计算了这些置换的具体性质。
>
> **达到效果:** 达到的效果是揭示了度和自由度在某些维度上的确切值，并展示了这些值之间的差距，从而为理解分部性质传播提供了新的视角。
>
> **技术梗概:** 技术梗概：通过分析特定的APN对和完全映射族的性质，结合关联级数约简、经典连续摩尔行列式以及替换子式的非零性等方法，精确确定了这些置换的具体度和自由度配置。

---
### [2026/1663] Five-Bit Axial Subspace Differential Uniformity: Exact Optima and a DDT-Support Realizability Gap

- **作者:** Jingchuan Ma, Yuqing Shao, Xin Wei, Qiaoyun Huang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1663) | [PDF](https://eprint.iacr.org/2026/1663.pdf)


> **研究背景:** 研究了五维空间中轴向仿射子空间差异均匀性（SDU）的具体优化值，并探讨了局部可接受支持设计与几乎完美非线性（APN）置换实现之间的严格差距。
>
> **主要贡献:** 证明了每个16元集在某些仿射3-平坦中有至少六点交集，且仅当极秩为四的平衡二次指标取等时成立。分类了所有有序单项式迹候选函数，并确定了最优类型的具体实现限制。
>
> **达到效果:** 尽管所有最优类型满足常规性和零向量和约束，但它们违反了全局必要条件：其二维特征变换包含一个负系数，而该位置需要正的矢量沃尔什平方。
>
> **技术梗概:** 通过证明特定集合与仿射子空间的关系，并对多项式迹函数进行分类来确定SDU的具体优化值。

---
### [2026/1664] Trace-Moment Canonicalization for Average-Case Matrix Code Conjugacy

- **作者:** Jingchuan Ma, Yanhua Liu, Qiaoyun Huang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1664) | [PDF](https://eprint.iacr.org/2026/1664.pdf)


> **研究背景:** 研究背景：矩阵码共轭问题涉及判断两个矩阵子空间是否通过同时基变换相关联。现有算法在特定条件下达到$\Theta(1/q)$的正确率，但存在未知系数空间作用的问题。
>
> **主要贡献:** 主要贡献：提出了一种基于迹矩元的经典化方法，绕过了未知系数空间作用，直接处理生成器$A$和迹矩元对$(A,M_2)$的变换问题，从而在特定条件下提高了算法的正确率。
>
> **达到效果:** 达到的效果：对于奇素数幂$q$、$n\ge 5$且$2\le m\le n^2-2$的情况，该算法在至少$\frac{1}{35q}$的比例上对随机$m$维第一码是正确的，并且其比特复杂度为$\operatorname{poly}(n,m,\log q)$。
>
> **技术梗概:** 技术梗概：通过迹函数和迹对偶性构造基不变的矩阵，利用迹核选择非退化补空间，从而实现基于迹矩元的经典化处理。

---
### [2026/1665] Nullity-One Canonicalization for Average-Case 4-Tensor Isomorphism

- **作者:** Jingchuan Ma, Yanhua Liu, Qiaoyun Huang

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1665) | [PDF](https://eprint.iacr.org/2026/1665.pdf)


> **研究背景:** 研究在有限域上四阶张量同构的平均情况模型中，探讨了第一个张量均匀且第二个张量任意的情况。现有方法依赖于高维展平核，但在处理秩一奇异集时存在缺陷。
>
> **主要贡献:** 作者通过将左、右核矩阵归一化为单位阵，揭示了残余作用是两个伴随作用，并证明了Gram算子的共中心化标量边界条件的概率恒定值，从而恢复了四个张量因子的剩余共轭。
>
> **达到效果:** 该算法在奇数素幂 \(q \geq 5\) 和 \(n \geq 5\) 且特征不整除 \(n\) 的情况下，提供了期望多项式时间的随机部分算法，正确率至少为 \(c/q\)。此外，还针对某些张量给出了大型域的结果。
>
> **技术梗概:** 通过归一化展平并利用伴随作用和Gram算子，作者证明了共中心化标量边界条件，并设计了一种无需枚举字段元素即可恢复残余共轭的方法。

---
### [2026/1667] Resolving the Complexity of Linear Secret Sharing

- **作者:** Oded Nir

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1667) | [PDF](https://eprint.iacr.org/2026/1667.pdf)


> **研究背景:** 研究背景：线性秘密共享方案允许经销商将秘密s分发给n个参与方，只有预定义的‘授权’集合才能重构秘密，而其他‘未授权’集合则学不到任何关于s的信息。
>
> **主要贡献:** 主要贡献：作者展示了如何通过一个线性秘密共享方案实现所有n-参与者的访问结构，并且对于一位秘密，最大份额大小为$2^{\lceil n/2\rceil-1}+1$位。
>
> **达到效果:** 达到的效果：该方案的上界接近于计数下界$2^{n/2-o(n)}$，并且通过结合非线性方案，作者还提出了一个适用于所有访问结构的线性构造方法，份额大小为$2^{0.496n+o(n)}$。
>
> **技术梗概:** 技术梗概：该研究利用了单调泛函程序和组合数学技巧来设计更简单的线性秘密共享方案，并通过结合非线性方案进一步优化了分享的规模。

---
### [2026/1668] A Key-Recovery Attack on TALUS-MPC in TALUS v4

- **作者:** Sunghyeon Jo

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1668) | [PDF](https://eprint.iacr.org/2026/1668.pdf)


> **研究背景:** 该研究针对TALUS v4版本中的TALUS-MPC协议提出了一个双重信标的关键恢复攻击。
>
> **主要贡献:** 作者通过控制协调者和部分参与方，成功利用了双重签名机制中的漏洞来恢复密钥。
>
> **达到效果:** 此攻击使得攻击者能够完全获取s1和s2的秘密份额，并伪造签名。
>
> **技术梗概:** 研究基于对协议中非随机多项式的评估以及挑战响应方程的分析，通过两个不同的挑战精确地恢复了s1。

---
### [2026/1669] Quantum Advantage for Two-Party Differential Privacy

- **作者:** Daniel Alabi, Emil T. Khabiboulline

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1669) | [PDF](https://eprint.iacr.org/2026/1669.pdf)


> **研究背景:** 该研究探讨了在两方哈明距离计算中实现信息论私密性量子协议的可能性，旨在超越经典通信下的误差限制。
>
> **主要贡献:** 作者提出了一个基于量子通信的协议，在保持纯ε量子差分隐私的同时实现了O(1)的信息论误差，并且对于近似(ε, δ) QDP，误差更小。
>
> **达到效果:** 该研究证明了在Klauck模型下，量子通信可以实现与经典计算假设相当的准确性，但在信息保留方面提供了更强的安全性保证。
>
> **技术梗概:** 通过使用受保护的相干往返和等Gram刚性原理来防止诚实玩家保留输入相关的互补信息，从而实现了上述目标。

---
### [2026/1670] RSS: Robust Signing Service using Threshold Signatures and TEEs

- **作者:** Filip Rezabek, Kilian Glas, Eber Christer, Xinxin Fan, Georg Carle

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1670) | [PDF](https://eprint.iacr.org/2026/1670.pdf)


> **研究背景:** 传统的阈值签名减少了单一密钥泄露的风险，但每个密钥份额仍可能因执行协议的软件和基础设施被攻破而受到威胁。本文提出RSS，旨在通过在可信执行环境（TEE）中运行密钥生成和签名来增强安全性。
>
> **主要贡献:** 作者将GG20阈值ECDSA、FROST和阈值BLS集成到EnGINE实验框架中，并评估了本地和Google云部署情况，使用AMD SEV-SNP和Intel TDX。
>
> **达到效果:** 实验结果显示，在良性操作假设下，执行阈值签名工作负载在受保护的虚拟机（CVM）内具有性能可行性。分布式密钥生成是主要的扩展瓶颈，而签名则相对快速完成。
>
> **技术梗概:** RSS利用TEE技术隔离并保护密钥份额和签名过程，同时通过EnGINE框架支持多种阈值签名方案的集成与评估。

---
### [2026/1672] AFS: A Family of ARX-Based Large-State S-boxes with Exceptional Properties

- **作者:** Zhiguang Yan, Yongzhuang Wei, René Rodríguez-Aldama, Enes Pasalic

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1672) | [PDF](https://eprint.iacr.org/2026/1672.pdf)


> **研究背景:** 大型状态的ARX基S盒已成为现代轻量级密码设计的关键组件，但对其差分和线性性质的安全界限进行严格的分析仍然具有挑战性。
>
> **主要贡献:** 作者提出了一种名为SMCS的混合搜索策略，并基于此策略为Alzette S盒提供了更紧致的边界值。此外，他们还提出了ARX-Feistel结构的新家族AFS。
>
> **达到效果:** 通过使用SMCS方法，作者不仅细化了现有对Alzette的界限，而且还首次为15轮（差分）和14轮（线性）建立了紧致的界限。同时，AFS实例在单迹差分和线性密码分析中表现出更好的抗攻击能力。
>
> **技术梗概:** 该研究采用了结合混合整数线性规划（MILP）优化与模型检查（SMT）的方法来计算最大预期差分概率和线性相关性的紧致界限。

---
### [2026/1673] Linearly Homomorphic Secret Sharing and Multi-Party Computation with Unanimously Verifiable Deletion

- **作者:** Yilei Chen, Liheng Ji, Han Luo

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1673) | [PDF](https://eprint.iacr.org/2026/1673.pdf)


> **研究背景:** 研究旨在构建一种信息论安全的线性同态门限秘密共享方案，结合了私有验证和公开验证的优点，并引入了一种新的验证方式——一致验证删除。
>
> **主要贡献:** 该工作首次提出了共识验证删除的概念，并构造了一个具有适应性共识验证删除的线性同态门限秘密共享批量方案。
>
> **达到效果:** 通过此方案，可以在不依赖加密假设的情况下实现敏感信息的删除证明，并且能够进行线性同态操作以保持已认证的删除特性。
>
> **技术梗概:** 利用BB84类型状态的线性同态结构，在编码值上执行线性运算的同时保留了认证删除的功能。

---
### [2026/1674] MinMandate: Private Task-Scoped Payment Authorization for Adaptive Agent Workflows

- **作者:** Ge Gao, Haining Yu, Zhichao Liu, Dongyang Zhan, Yuanxiao Zhu, Zhongyun Hua

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1674) | [PDF](https://eprint.iacr.org/2026/1674.pdf)


> **研究背景:** 现有的代理支付框架通过商家准入授权凭证支持用户代理执行付费工作流，但要求用户在执行前指定商家，这限制了代理的适应性或迫使用户重新介入。
>
> **主要贡献:** MinMandate创新地提供了一种机制，在用户批准的任务范围内进行动态的商家选择，并为每次调用生成独立的支付视图，无需使用跨商家稳定的标识符。
>
> **达到效果:** 实验表明，当50%的商家不可用时，与AP2基准相比，MinMandate在四个测试规划器中平均提高了32.7个百分点的任务成功率。重新引入可重复使用的公共支付层句柄会导致攻击者恢复任务的成功率提高27.4个百分点。
>
> **技术梗概:** 该技术通过动态授权和一次性支付视图生成来实现隐私保护，确保了用户意图的隐匿性，并且在AgentDojo任务上的实验验证了其有效性。

---
### [2026/1675] SparseMPC: Secure Sparse Operations using Multi-Party Computation

- **作者:** Marc Damie

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1675) | [PDF](https://eprint.iacr.org/2026/1675.pdf)


> **研究背景:** 现有的多方计算(MPC)协议在处理高维稀疏数据时效率低下，而稀疏线性代数算法通过Scatter和Gather两种基本操作在明文环境下有效解决了这一问题。
>
> **主要贡献:** SparseMPC提出了一种外包的MPC协议，安全地实现了Scatter和Gather，并利用它们来执行稀疏矩阵乘法。
>
> **达到效果:** 该协议支持任意数量的数据所有者，并提供低内存占用、恒定轮次复杂性和低通信成本。
>
> **技术梗概:** SparseMPC通过设计特定的Scatter和Gather操作实现，为在外包MPC中高效实现更广泛的稀疏计算奠定了基础。

---
### [2026/1678] Cryptanalytic Extraction of Multi-Head Softmax Attention Models

- **作者:** Sunan Wang, Hao Lei, Longxiang Wei, Qun Liu, Kai Hu, Meiqin Wang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1678) | [PDF](https://eprint.iacr.org/2026/1678.pdf)


> **研究背景:** 针对Carlini等人在CRYPTO 2020提出的工作，现有攻击主要基于分段线性现象，而多头softmax注意力模型因其非线性特性和序列依赖性使得这些方法失效。
>
> **主要贡献:** 首次提出了针对多头注意力模型的攻击方法，并给出了提取参数的多项式时间算法。
>
> **达到效果:** 成功在有限精度下从一个具有8个词元维度和6个头的softmax注意力模型中以$2^{-51}$的准确度提取了参数，解决了现有单层单头Transformer参数提取算法在包含偏置项的ReLU前馈网络中的局限性。
>
> **技术梗概:** 通过形式化多头注意力的可提取代表，并设计多项式时间算法来提取标准表示模型的参数实现上述贡献。

---
### [2026/1679] Critical-Round Special Soundness for Multi-Round Proofs

- **作者:** Masayuki Abe, David Balbás, Dung Bui, Miyako Ohkubo, Zehua Shang, Akira Takahashi, Mehdi Tibouchi

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1679) | [PDF](https://eprint.iacr.org/2026/1679.pdf)


> **研究背景:** 该研究重新审视了多轮公共硬币证明系统的模拟器和提取器在其他密码协议中的应用，填补了与三轮（如Sigma协议）设置在接口和行为上的差异。
>
> **主要贡献:** 作者引入了关键轮次特殊诚实性这一概念，使得多轮公共硬币证明系统中的提取器可以在协议设计中以类似于三轮特殊诚实性的方法被利用。
>
> **达到效果:** 研究展示了多个现有的多轮公共硬币证明系统满足该属性，并提出了三个应用实例：无交互的秘密共享方案、离线双开门即时提取陷阱门承诺方案以及参数改进等。
>
> **技术梗概:** 通过结合MPC-in-the-Head与秘密分享技术实现了非交互式秘密共享方案，填补了多轮证明系统中离线双开门即时提取陷阱门承诺方案的空白。

---
### [2026/1680] New Results on the Density of Irreducible NFSRs

- **作者:** Daoyuan Zhang, Dongdai Lin

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1680) | [PDF](https://eprint.iacr.org/2026/1680.pdf)


> **研究背景:** 非线性反馈移位寄存器（NFSRs）是现代流密码结构中的基本构建块，其不可约性对于确保序列的随机性和安全性至关重要。
>
> **主要贡献:** 研究通过精确计算进一步缩小了不可约NFSRs密度的上下界差距，提高了对这一关键参数的理解。
>
> **达到效果:** 研究成果将原本0.04的界限差值减小至仅8×10^-6，显著提升了不可约NFSRs密度估计的准确性。
>
> **技术梗概:** 研究采用了高级组合数学和概率论方法来分析NFSR的输出序列特性，并通过复杂的计算验证了新的边界条件。

---
### [2026/1681] Aegon: Self-Auditable Key Transparency

- **作者:** Hossein Hafezi, Alireza Shirzad, Benedikt Bünz, Kevin Lewi, Dillon George, Joseph Bonneau

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1681) | [PDF](https://eprint.iacr.org/2026/1681.pdf)


> **研究背景:** 现有系统依赖第三方全球审计者来检测密钥服务器的恶意行为，但缺乏同时支持轻量级用户高效审计和大规模用户下的短周期审计能力。
>
> **主要贡献:** $\mathsf{Aegon}$通过减少每周期的工作量并引入分片字典设计，实现了对大规模加密消息服务的有效自我审计。
>
> **达到效果:** 实验表明，$\mathsf{Aegon}$能够处理包含40亿条记录的字典，并将当前周期的延迟降低至不到一分钟。
>
> **技术梗概:** 该方案基于$\mathsf{IronDict}$的设计理念，通过减少每周期的工作量和使用证明缓存来控制长期存储需求，实现了高效和可扩展性。

---
### [2026/1685] Pruning Merkle-Tree Consistent Accumulator

- **作者:** Anna Mendonca, Hudson Shi, Ivan Pryvalov, Amir Herzberg

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1685) | [PDF](https://eprint.iacr.org/2026/1685.pdf)


> **研究背景:** 认证数据结构广泛用于计算随时间变化的数据集合的紧凑摘要，并支持元素包含的有效验证。然而，这些认证集合通常不应无限增长：较旧的元素可能会过期而不再需要验证。现有的只读累加器方法在处理这种情况时存在局限性。
>
> **主要贡献:** 本文提出了一种剪枝累加器，这是一种状态机，能够同时支持新元素的增量添加和对先前累积元素前缀的剪枝操作。这种设计使得认证摘要仅包含未被剪枝的序列，并且剪枝后的元素从认证状态中移除但仍可验证其包含性。
>
> **达到效果:** 通过两种基于Merkle树的剪枝累加器构造，本文实现了高效的累积、前缀剪枝、证明生成与更新以及验证过程。这些构造保留了活动元素的标准Merkle树风格验证方式，并减少了长期存储需求。
>
> **技术梗概:** 文中提出了两种Merkle树为基础的剪枝累加器构造方法，通过巧妙地结合累加和剪枝操作，实现了认证摘要的有效生成与维护。

---
### [2026/1686] A Unifying Umbrella for Circular-Secure Cryptographic Primitives

- **作者:** Fuyuki Kitagawa, Takahiro Matsuda

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1686) | [PDF](https://eprint.iacr.org/2026/1686.pdf)


> **研究背景:** 该研究探讨了循环安全密码学基础中看似不同的几种原始形式之间的等价性。
>
> **主要贡献:** 作者引入了一个新的安全性概念——键依赖偏移（KDS）安全弱伪随机函数，并展示了其作为连接这些不同形式的枢纽作用。
>
> **达到效果:** 通过这一新概念，作者证明了循环安全的各种形式本质上是等价的，从而为构建具有循环安全性的密码学原语提供了一个统一框架。
>
> **技术梗概:** 研究的关键技术贡献在于从任何键依赖消息（KDM）安全密钥加密方案中构建KDS安全弱伪随机函数的通用构造方法。

---
### [2026/1687] Theoretical Open Problems in Symmetric Cryptography: Verifiable LLM-Guided Analysis

- **作者:** Yufei Yuan, Yaoda Hu, Yixin Zhang, Lei Zhang, Wenling Wu

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1687) | [PDF](https://eprint.iacr.org/2026/1687.pdf)


> **研究背景:** 本文提出了Pilot--Sailor框架，这是一种利用LLM指导的系统，用于研究对称密码学中的理论开放问题。
>
> **主要贡献:** 贡献在于通过布尔函数理论和对称密码分析的应用，解决了多项数学案例，并证明了Tu--Deng猜想等重要结果。
>
> **达到效果:** 研究成果包括完全解决某些问题、修正表述、提供反例以及获得范围内的定量进展。特别地，证明了原点的Tu--Deng猜想对于所有字长和允许余数都成立。
>
> **技术梗概:** 技术上采用了Pilot提出中间陈述和证明计划，Sailor尝试形式化证明，而证明助手仅接受验证声明到验证上下文中的检查声明。

---
### [2026/1688] Hell’s Bells: A Neural Network Pipeline for Ternary Fast Matrix Multiplication Algorithms

- **作者:** Erik Mårtensson, Paul Stankovski Wagner, Joshua Stapleton

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1688) | [PDF](https://eprint.iacr.org/2026/1688.pdf)


> **研究背景:** 研究旨在利用神经网络高效生成适用于小尺寸但任意维度的矩阵乘法算法，并特别关注三元性、稀疏性和加法优化等特性。
>
> **主要贡献:** 提出了一种基于神经网络的方法，能够自动生成具有特定属性的快速矩阵乘法方案，并通过大量实验验证了其有效性。
>
> **达到效果:** 实现了多种尺寸组合下的算术复杂度新低记录，特别是在(2,2,k)设置下表现尤为突出，且优化过程受维度影响显著。
>
> **技术梗概:** 采用神经网络构建算法生成管道，结合性能评估和可视化技术来指导优化过程。

---
### [2026/1691] Equivalence Between Average-Case Hardness of Learning and Cryptography for Mixed Quantum States

- **作者:** Alexandru Cojocaru, Laura Lewis

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1691) | [PDF](https://eprint.iacr.org/2026/1691.pdf)


> **研究背景:** 研究背景：在理论计算机科学的基础中，密码学与学习理论之间的关系一直是核心主题。近期工作开始探索量子环境下的类似联系，即混合态的学习平均情况难度（AHL）与不可验证的一次性状态生成器（IV-OWSG）之间的关联。
>
> **主要贡献:** 主要贡献：证明了混合态AHL的存在性等价于IV-OWSG的存在性，并将这种关系与EFI对相关联，从而填补了该领域的空白。
>
> **达到效果:** 达到的效果：通过这一结果，研究者们可以更好地理解混合态学习问题在量子密码学中的角色，并为构建新的量子安全协议提供了理论依据。
>
> **技术梗概:** 技术梗概：采用形式化的方法和复杂的数学证明来建立AHL与IV-OWSG之间的等价性，并利用现有的研究成果进一步分离了IV-OWSG与OWSG的相对关系。

---
### [2026/1692] From Round Skipping to S-Box Skipping: Attacking Poseidon's Partial Layer via Subspace Restriction

- **作者:** Amit Singh Bhati, Sundas Tariq, Tomer Ashur

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1692) | [PDF](https://eprint.iacr.org/2026/1692.pdf)


> **研究背景:** Poseidon是一种专为零知识应用设计的算术化导向哈希函数，旨在提高效率。然而，该研究揭示了一种攻击方法，通过限制子空间来削弱其安全性。
>
> **主要贡献:** 作者提出了GSR工具，这是一种通用的S-box跳过技巧，能够在不增加多项式度的情况下吸收多个轮次，从而为Poseidon的安全性分析提供了新的视角。
>
> **达到效果:** 利用GSR技术，研究人员成功构建了一个概率为1的区分器，并在此基础上开发了基于插值的攻击方法。实验结果表明，在特定条件下，可以解决部分轮次的问题。
>
> **技术梗概:** 该研究通过限制子空间的方法来约束解的空间，从而减少内部状态转换中的多项式度，将原本复杂的多项式系统简化为参数化自由变量的理想形式。

---
### [2026/1693] The ePrint:2026/1591 Quantum Algorithm Does Not Solve DCP

- **作者:** Aparna Gupte, Seyoon Ragavan, Mark Zhandry

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1693) | [PDF](https://eprint.iacr.org/2026/1693.pdf)


> **研究背景:** 本文旨在正式证明Simon在ePrint:2026/1591中提出的算法无法以非微不足道的优势提取Dihedral coset problem (DCP)的秘密最小子显著位，因此未能解决DCP问题。
>
> **主要贡献:** 研究贡献在于直接展示了该算法不可能成功，并且其结论适用于更广泛的算法类别，强调了解决DCP的算法必须在未计算阶段大量使用经典傅里叶标签。
>
> **达到效果:** 结果表明Simon的算法仅能以$\mathsf{poly}(n)2^{-n/3}$的误差实现最显著三分之一的经典傅里叶标签的使用，并因此无法成功解决DCP问题。
>
> **技术梗概:** 研究通过形式证明和Lean 4代码验证，直接分析了算法的工作原理及其局限性。

---
### [2026/1694] Relations Between the Uniform MQ Assumption and Other Multivariate Assumptions

- **作者:** Zijun Zhuang, Yingjie Zhang, Jintai Ding

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1694) | [PDF](https://eprint.iacr.org/2026/1694.pdf)


> **研究背景:** 研究背景：探讨了均匀多变量二次（UMQ）假设与其他多变量假设之间的关系，以增强公钥密码学中相关问题的理论基础。
>
> **主要贡献:** 主要贡献在于证明了UMQ和MQSPR假设之间存在紧密联系，并且UMQ假设在特定条件下能够推导出MQOW假设。
>
> **达到效果:** 达到的效果是为多变量多项式问题提供了更坚实的平均情况硬度假设支持，特别是在密钥长度较短的情况下。
>
> **技术梗概:** 技术梗概：通过数学证明和复杂性分析，研究了不同假设之间的逻辑关系，并提出了适用于特定条件下的推导方法。

---
### [2026/1695] On the Impossibility of Robust Combiners for Cryptographic Groups

- **作者:** Cong Zhang, Wenli Wang, Taiyu Wang, Hong-Sheng Zhou, Pengfei Chen, Zhihong Jia, Jian Liu, Jinfei Liu, Moti Yung, Kui Ren

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1695) | [PDF](https://eprint.iacr.org/2026/1695.pdf)


> **研究背景:** 研究背景：在密码学中，组合器被广泛用于增强多个候选实例的安全性。尽管对于哈希函数、公钥加密和秘密转移等基本原语的研究较为丰富，但在密码组的背景下进行此类研究却相对较少。
>
> **主要贡献:** 主要贡献在于首次探讨了在Maurer通用群模型下如何将多个候选密码组安全地组合成一个单一的安全方案。证明了对于决策性Diffie-Hellman假设，在多项式限制下的$(k,n)$-稳健组合器无法保持DDH安全性。
>
> **达到效果:** 研究结果表明，直接产品构造虽然可以保留搜索难度，但在决策性假设下会失效，并且会产生显著的表示开销。此外，还确定了在固定常数$n$和$k$的情况下，离散对数问题下的安全稳健组合阈值。
>
> **技术梗概:** 技术梗概：通过形式化定义$(k,n)$-稳健组合器并使用Maurer通用群模型进行分析，证明了特定假设下稳健组合的不可能性。

---
### [2026/1696] Toward Secure Compilation: Leakage Detection for Masked Implementations in Jasmin

- **作者:** Nicolai Schmitt, Sven Wroblewski, Fabio Campos, Andreas Heinemann

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1696) | [PDF](https://eprint.iacr.org/2026/1696.pdf)


> **研究背景:** 现有方法要么依赖于特定功率模型的泄漏模拟，计算成本高；要么通过形式验证源程序来检测泄漏，但无法捕捉后续编译阶段引入的影响。
>
> **主要贡献:** 提出了一种集成在Jasmin语言正式验证流水线中的泄漏检测过程，该过程基于可配置且面向微架构的泄漏模型，在寄存器和堆栈分配之前的操作中间表示上运行。
>
> **达到效果:** 通过60个专门设计的Jasmin测试片段验证了此过程的有效性，并能够明确标识出检测到的泄漏的根本原因以及识别出掩码顺序降低的问题。
>
> **技术梗概:** 该技术采用了一种基于微架构的可配置泄漏模型来跟踪共享、秘密、随机值和公共值之间的接触，从而在编译器内部实现泄漏检测。

---
### [2026/1697] Actively Secure Two-Party Function Secret Sharing with Dynamic Cross-Phase Verification

- **作者:** Yujie Xue, Lin Liu, Rongmao Chen, Yizhen Jiang, Yuchuan Luo, Bing Sun, Shaojing Fu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1697) | [PDF](https://eprint.iacr.org/2026/1697.pdf)


> **研究背景:** 该研究旨在解决函数秘密共享(FSS)中去中心化处理和恶意安全验证的问题，特别是在动态交叉阶段绑定方面。
>
> **主要贡献:** 提出了VeriFSS方案，这是一种无需经销商的双方FSS方案，并且具有主动安全性，在标准模拟框架下对抗单一静态恶意攻击。
>
> **达到效果:** 该方案通过双重平面密钥设计实现了在线报告绑定，避免了向量承诺、可提取哈希或每点交互的需求，同时证明了关于密钥系数的引理，提高了验证的准确性。
>
> **技术梗概:** VeriFSS采用了配对函数$x\mapsto(f_\theta(x),\Lambda f_\theta(x))$的设计，并通过证明时刻准确性和形状识别引理来确保恶意方无法篡改其密钥报告。

---
### [2026/1698] MamaBearZKP: A Holistic Co-design of Prime Fields and Proving Stacks for High-Throughput ZKP on Modern CPUs

- **作者:** Jipeng Zhang, Yanpei Guo, Tao Lu, Hao Cheng, Jiaheng Zhang

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1698) | [PDF](https://eprint.iacr.org/2026/1698.pdf)


> **研究背景:** 现代零知识证明系统如HyperPlonk和基于FRI的DeepFold方案中，求和检查和快速傅里叶变换（FFTs）占据了主要计算成本，尽管已有诸多优化但依然分散在算法、协议和实现层面上，未能充分利用CPU性能。
>
> **主要贡献:** MamaBearZKP提出了一种综合设计框架，通过结合49位素域算术与AVX-512IFMA向量化技术，实现了对现代CPU上高吞吐量零知识证明的优化。
>
> **达到效果:** 在HyperPlonk-DeepFold验证器中，MamaBearZKP分别实现了ZeroCheck、ProductCheck等操作高达42倍至81倍的速度提升，并且整体端到端证明生成速度提升了18倍以上。
>
> **技术梗概:** 该框架通过统一优化素域算术、协议结构和低级硬件原语，特别针对AVX-512IFMA执行模型设计了系统向量化方法。

---
### [2026/1699] DumboMix: Robust Asynchronous Anonymous Broadcast Made Practical

- **作者:** Wei Tang, Hanwen Feng, Jiliang Li, Yuan Lu, Qiang Tang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1699) | [PDF](https://eprint.iacr.org/2026/1699.pdf)


> **研究背景:** 研究背景：针对异步环境下匿名广播协议，特别是在存在 Byzantine 服务器恶意行为和不可预测的网络延迟时，确保消息最终以随机顺序被安全揭示的需求。
>
> **主要贡献:** 主要贡献在于提出了一种名为 DumboMix 的框架，结合了高效的算术电路设计，实现了在 Shamir 秘密共享多方计算（MPC）中异步匿名广播，并保证了即使在部分服务器失效或网络延迟的情况下也能实现消息的均匀混排。
>
> **达到效果:** 达到的效果是：DumboMix 在线阶段仅需常数深度乘法、预期 O(N^2) 模乘操作和最多 O(N) MPC 乘法，确保所有揭示的消息被均匀混合，同时支持多达 n/3 的 Byzantine 服务器恶意行为。
>
> **技术梗概:** 技术梗概：DumboMix 包含 DumboMix1 和 DumboMix2 算术电路及其优化版本，用于 Shamir 秘密共享 MPC 中的混排操作，并通过随机化机制确保消息的安全性和匿名性。

---
### [2026/1700] Qlapoty: Improved analysis and eﬃciency for quaternionic ideal to isogeny transformation

- **作者:** Max Duparc, Antonin Leroux, Sina Schaeﬄer

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1700) | [PDF](https://eprint.iacr.org/2026/1700.pdf)


> **研究背景:** SQIsign是一种基于理想到isogeny变换的公钥加密方案，而Qlapoti算法简化并加速了这一关键步骤，但仍存在分析不充分和技术细节处理不足的问题。
>
> **主要贡献:** 研究团队改进了Qlapoti算法，解决了其失败概率分析不准确和实现与文档不符的问题，并提出了新的无失败概率的范数方程求解算法。
>
> **达到效果:** 新算法在C语言实现中的速度显著提升，相较于Qlapoti的范数方程求解器提高了6到9倍的速度，在SQIsign NIST2签名生成中也表现出1.3到2.1倍的加速效果。
>
> **技术梗概:** 研究采用了更加严谨的数学分析方法，并优化了算法流程以提高计算效率，确保了新算法在实际应用中的稳定性和高效性。

---

## 更新: 2026-08-14 08:06

*新增 3 篇论文 (编号 1642--1644)*

### [推荐] [2026/1642] A Torus-Structured Generalisation of NTRU: the NTC Assumption, its Cryptanalysis, and a Compact KEM

- **匹配关键字:** lattice, LWE

- **作者:** Sidoine Djimnaibeye, Djiby Sow, Mahamat Borgou Hassan

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1642) | [PDF](https://eprint.iacr.org/2026/1642.pdf)


> **研究背景:** 本文提出了一种基于Noisy Torus Conjugation (NTC)假设的密钥封装机制（KEM），该假设扩展了NTRU，并改变了其几何结构，以增强安全性。
>
> **主要贡献:** 作者引入了NTC假设及其相关的数学理论，并构建了一个IND-CCA安全的关键封装方案，证明其安全性依赖于NTC假设和一个孤立的决策性假设。
>
> **达到效果:** 该KEM在NIST标准下的1, 3和5类中达到了与Kyber相当或更优的安全性和效率指标。
>
> **技术梗概:** 通过将秘密密钥限制在一个非分裂的最大torus内，并利用伴随矩阵进行加密，作者设计了一种新的假设和技术框架。

---
### [推荐] [2026/1644] Recursive Common Information for Group-Characterizable Random Variables
...
- **链接:** [论文](https://eprint.iacr.org/2026/1644) | [PDF](https://eprint.iacr.org/2026/1644.pdf)


> **研究背景:** 研究背景：共信息（CI）在基于熵的秘密共享中作为安全参数的下界估计中有重要作用。本文针对群可表征随机变量（GC），探讨了其共信息特性。
>
> **主要贡献:** 主要贡献在于提出了一个精确对子准则，即两个陪集随机变量具有共信息当且仅当它们对应的子群互换。
>
> **达到效果:** 达到的效果是确定了GC元组的1-CI和递归CI条件，并通过实例展示了不同深度的递归CI扩展对于特定访问结构的信息比率下界估计。
>
> **技术梗概:** 技术梗概：基于Kaboli-Khazaei-Parviz提出的充分条件，通过研究子群互换性来判断共信息的存在与否，并设计了决定递归CI的有限算法。

---
### [2026/1643] SoK: Encrypted Mempools Through the MEV Lens

- **作者:** Jonathan Passerat-Palmbach

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1643) | [PDF](https://eprint.iacr.org/2026/1643.pdf)


> **研究背景:** 研究背景：随着公共内存池的透明性增加，矿工提取价值（MEV）和审查行为对以太坊及其二层扩展生态系统构成了重大威胁。加密内存池旨在保护交易内容直至排序最终确定，但其可行性依赖于复杂的加密、经济和部署权衡。
>
> **主要贡献:** 主要贡献：本文系统地梳理了阈值加密内存池的发展历程，从早期的Shutter和Ferveo方案到最新的研究进展，并分析了各阶段如何解决瓶颈问题，如委员会通信开销、未确认交易隐私不足及位置相关加密等问题。
>
> **达到效果:** 达到的效果：通过设计轴向的收敛，包括批量解密以减轻延迟、静默设置以消除分布式密钥生成的复杂性、无周期加密以去除位置依赖性和冲突免费编码以防止区块碰撞审查，本文为加密内存池的设计提供了全面视角，并对比了当前部署方案。
>
> **技术梗概:** 技术梗概：文章采用系统化方法回顾了加密内存池的发展历程和技术改进，强调了四个关键设计轴的重要性，并通过映射实际需求与现有加密研究之间的关系来评估这些方案的优劣。
...
### [推荐] [2026/1591] A Polynomial-Time Quantum Algorithm for the Dihedral Coset Problem
...
- **链接:** [论文](https://eprint.iacr.org/2026/1591) | [PDF](https://eprint.iacr.org/2026/1591.pdf)


> **研究背景:** 本文提出了一种多项式时间量子算法，用于解决Dihedral Coset Problem (DCP)，这与Regev关于Dihedral Subgroup Problem (DSP)到模子集和问题的多项式时间归约有关。
>
> **主要贡献:** 该贡献在于开发了不同于使用子集和预言机的方法来消除样本位，从而结合Regev和其他改进，为多种格问题提供了多项式时间量子算法。
>
> **达到效果:** 通过这种方法，可以高效解决如最短向量问题(SVP)和学习带误差问题(LWE)，即使在存在故障样本率的情况下也能实现。
>
> **技术梗概:** 该技术涉及使用不同的方法来消除样本位，并结合改进的格问题归约，以提高算法的鲁棒性和效率。

---
### [推荐] [2026/1593] HAWK-$n$ Key Recovery Reduces to SVP in Dimension $n/2 + 1$

- **匹配关键字:** lattice, post-quantum

- **作者:** Zygimantas Straznickas, Stephen A. Weis

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1593) | [PDF](https://eprint.iacr.org/2026/1593.pdf)


> **研究背景:** HAWK是一种基于格的签名方案，作为NIST后量子签名竞赛中的第三轮候选者，其安全性受到广泛关注。
>
> **主要贡献:** 本文提出了一个无条件、确定性的多项式时间攻击，将HAWK-$n$密钥恢复问题归约到维度为$n/2+1$的精确最短向量问题（SVP）求解上。
>
> **达到效果:** 该攻击显著降低了HAWK-512和HAWK-1024的密钥恢复成本，分别为从$2^{150}$降至$2^{108}$和从$2^{288}$降至$2^{182}$。
>
> **技术梗概:** 通过利用伽罗瓦对合自同构，并结合Ducas的块缩减技术和van Gent与Pulle斯的下降方法，实现了密钥恢复。

---
### [推荐] [2026/1597] Solving the Shortest Vector Problem in $2^{0.6039n}$ Time via Mid-point Hessian
...
- **链接:** [论文](https://eprint.iacr.org/2026/1597) | [PDF](https://eprint.iacr.org/2026/1597.pdf)


> **研究背景:** 研究背景：本文旨在改进求解最短向量问题（SVP）的算法，提高经典和量子计算下的效率。
>
> **主要贡献:** 主要贡献是提出了一种新的随机化算法，能够在$2^{0.6039n+o(n)}$时间和$2^{0.5411n+o(n)}$量子时间及$2^{0.5n+o(n)}$空间内解决$n$维格网的SVP问题。
>
> **达到效果:** 达到的效果是显著提高了求解最短向量问题的时间复杂度，相较于之前的算法有了指数级的改进。
>
> **技术梗概:** 技术梗概：该算法利用周期高斯函数在半最短向量处Hessian矩阵的性质，通过离散高斯采样和随机子格陪集等优化技术来搜索最短向量的类。

---
### [推荐] [2026/1599] One Discrete Gaussian Sample in $2^{n/2+o(n)}$ Time
...
- **链接:** [论文](https://eprint.iacr.org/2026/1599) | [PDF](https://eprint.iacr.org/2026/1599.pdf)


> **研究背景:** 该研究旨在优化离散高斯分布的采样效率，特别是在有限时间内生成高质量样本的问题上有所突破。
>
> **主要贡献:** 作者证明了在特定时间和空间复杂度下可以高效地生成一个参数任意的离散高斯样本，回答了先前工作中的开放问题。
>
> **达到效果:** 通过改进算法，研究达到了与之前相同甚至更优的效果，即在统计意义上接近完美匹配的情况下显著减少了所需的时间和空间资源。
>
> **技术梗概:** 该技术基于随机超晶格采样，并结合了高斯质量比较方法来确保生成的样本具有所需的概率分布特性。

---
### [推荐] [2026/1601] Power side-channel leakage distinguishers on LESSv2.0 - Exploiting sparse columns in Gaussian Elimination

- **匹配关键字:** post-quantum

- **作者:** Maciej Czuprynko, Rishub Nagpal, Tobias Schneider, Sujoy Sinha Roy

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1601) | [PDF](https://eprint.iacr.org/2026/1601.pdf)


> **研究背景:** 本文研究了LESSv2.0的侧信道泄漏，这是一种在NIST后量子数字签名方案征集中进入第二轮候选的算法。针对其核心操作高斯消元法中的稀疏列和密集列处理差异导致的信息泄露问题进行了分析。
>
> **主要贡献:** 首次提出了一个被动侧信道区分器，该方法利用了常时间实现中也存在的稀疏与稠密列操作差异，并通过区分零值计算与随机计算来实施攻击。
>
> **达到效果:** 实验结果显示，攻击需要观察300到2357个签名样本，尽管存在噪声，但仍然能够有效恢复密钥信息。此外，研究还验证了常见防护措施的有效性。
>
> **技术梗概:** 通过模拟和物理测试，作者利用高斯消元过程中稀疏列与密集列操作的差异来区分零值计算与随机计算，并展示了攻击在有无防泄漏措施下的效果。

---
### [推荐] [2026/1604] Algebraic Analysis of Homomorphic Trace Evaluation and Its Applications
...
- **链接:** [论文](https://eprint.iacr.org/2026/1604) | [PDF](https://eprint.iacr.org/2026/1604.pdf)


> **研究背景:** 研究背景：字段迹评估作为全同态加密中的强工具，广泛应用于从启动算法到隐私保护协议的多个领域。然而，现有分析依赖于均匀噪声边界，未能准确捕捉不同系数的实际噪声行为，导致理论估计与实际观察之间存在显著差距。
>
> **主要贡献:** 主要贡献：作者提出了一种精细的代数分析方法，揭示了迹映射在子扩展中的结构化消减效应，并展示了除常数项外，每个输出噪声系数的方差依赖于其指数的2-进估值，从而改进了先前均匀估计的因子为$O(\log n)$。
>
> **达到效果:** 达到的效果：对于密文打包，作者推导出了一种非递归公式，具有更清晰的结构和略紧的噪声估算；在系数提取方面，作者的系数分析比之前的统一方差边界改进了从...到...的不同因子。
>
> **技术梗概:** 技术梗概：通过结合塔基评估策略与缩放操作，并深入研究迹映射在子扩展中的结构化消减效应，作者提出了更精确的噪声行为估计方法。

---
### [推荐] [2026/1608] Efficient Large-Integer Arithmetic for FHE

- **匹配关键字:** homomorphic encryption, LWE

- **作者:** Ahmad Al Badawi, Andreea Alexandru, Gurgen Arakelov, Charles Gouert, Sergey Gomenyuk, Valentina Kononova, Yarkın Doröz, Yuriy Polyakov

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1608) | [PDF](https://eprint.iacr.org/2026/1608.pdf)


> **研究背景:** 研究背景：全同态加密（FHE）技术为隐私保护计算提供了关键手段，允许在加密数据上执行任意计算。基于环学习误差（RLWE）的向量化FHE方案如BFV、BGV和CKKS常用于大规模数据分析场景，但其高维度多项式环上的大整数运算超出了现代处理器的能力范围。
>
> **主要贡献:** 主要贡献：文章详细探讨了在RLWE基FHE库中大整数算术的发展，并重点介绍了残余数系统（RNS）技术的运用。通过对比BEHZ和Halevi等人的算法方法，为理解和优化这些操作提供了理论基础。
>
> **达到效果:** 达到的效果：通过对RNS基本构建块——基数扩展和缩放的操作进行形式化处理，文章揭示了这两种主要算法方法之间的差异，并为进一步研究和实现高效的大整数算术提供了指导。
>
> **技术梗概:** 技术梗概：文章分析了基于BEHZ的纯整数方法和Halevi等人的浮点方法在处理RNS中的基数扩展和缩放操作时的不同策略，强调了信息量对这些操作的重要性，并讨论了它们之间的优缺点。

---
### [推荐] [2026/1609] Distributed Monotone Policy Encryption with Stronger Security for DNFs and Threshold Policies from Lattices
...
- **链接:** [论文](https://eprint.iacr.org/2026/1609) | [PDF](https://eprint.iacr.org/2026/1609.pdf)


> **研究背景:** 研究背景：分布式单调策略加密（DPE）允许每个用户生成并发布自己的密钥，之后任何人都可以在满足单调访问策略下对数据进行加密，该策略决定了哪些联盟能够解密。
>
> **主要贡献:** 主要贡献：首次提出了基于可验证格假设的静态安全DPE方案，特别适用于DNF和阈值策略，并且达到了更强的安全性模型。
>
> **达到效果:** 达到的效果：对于DNF策略，同态加密的大小为$\mathsf{poly}(\lambda, \log N)$，与子句的数量无关；对于$t$-out-of-$N$阈值策略，在随机预言机模型下证明了安全性，并且在某些情况下提高了阈值参数。
>
> **技术梗概:** 技术梗概：通过分解LWE假设和简洁LWE假设下的安全证明，实现了紧凑的同态加密方案，适用于动态选择的访问策略。

---
### [推荐] [2026/1610] Algorithmic Optimization of the Gaussian Sampler in the FN-DSA Post-Quantum Signature Scheme

- **匹配关键字:** post-quantum

- **作者:** Nicolas HOULÈS, Thibaut Heckmann

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1610) | [PDF](https://eprint.iacr.org/2026/1610.pdf)


> **研究背景:** FN-DSA（Falcon）作为后量子签名方案，正由NIST标准化为FIPS 206，在此背景下，优化其关键瓶颈函数fpr_expm_p63的实现变得尤为重要。
>
> **主要贡献:** 该研究通过系统地应用分段多项式逼近方法来优化fpr_expm_p63，并结合Remez交换算法、固定点算术和霍纳法则，实现了针对不同硬件平台的两种配置。
>
> **达到效果:** 针对ARM Cortex-M3 IoT设备，实现了一种16段度3次逼近方案，在不依赖硬件浮点单元的情况下，达到了与x86-64平台相当的效果，并在30次独立运行中取得了平均1.28倍的速度提升。
>
> **技术梗概:** 研究使用了50位精度的Remez交换算法计算分段多项式系数，结合固定点算术和霍纳法则进行评估与实现。

---
### [推荐] [2026/1616] Flip a Failure into a Success: Improved Bit Flipping Decoding for QC-MDPC Codes

- **匹配关键字:** post-quantum

- **作者:** Paolo Santini, Davide De Zuane, Alessio Baldelli, Marco Baldi

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1616) | [PDF](https://eprint.iacr.org/2026/1616.pdf)


> **研究背景:** QC-MDPC码被用于设计BIKE，一个在NIST后量子密码标准竞赛中入围的候选算法。由于参数限制，这些代码的解码通常会失败，导致非零的解码失败率（DFR），这会影响其安全性。
>
> **主要贡献:** 作者提出了一种改进的位翻转解码方法，通过识别并纠正与近似码字相关的综合症状，显著降低了解码失败率。
>
> **达到效果:** 该方法使得在极低的DFR值下也能成功解码，从而提高了QC-MDPC代码的安全性，并满足了IND-CCA2安全性要求。
>
> **技术梗概:** 通过修改位翻转算法以识别并纠正与近似码字相关的综合症状，这种方法仅增加了轻微的计算开销。

---
### [推荐] [2026/1617] Verifiable SelfMix
...
- **链接:** [论文](https://eprint.iacr.org/2026/1617) | [PDF](https://eprint.iacr.org/2026/1617.pdf)


> **研究背景:** 现有匿名通信系统旨在隐藏发送者与消息之间的关联，但通常依赖于至少一个诚实的混洗服务器或需要更多交互的分布式协议。
>
> **主要贡献:** \emph{Verifiable SelfMix} (VSM) 提出了一种新的匿名架构，通过分离盲分配插槽和私密放置消息的过程来实现更高的隐私性和可验证性。
>
> **达到效果:** 该方案在静态恶意模型下证明了无条件的个人可验证性、完整性和匿名性，并且当表被解密后，每个用户都能验证自己的信息位置。
>
> **技术梗概:** VSM 使用 \emph{Unique Number Selection} (UNS) 和 \emph{Secure Mapping of Private Permutation} (SMPP) 两种技术分别分配唯一编号和安全地映射私有置换，同时利用可信硬件或加密机制来实现方案的高效性和安全性。

---
### [推荐] [2026/1618] Two-Limb CRT Ring-LWE Encryption with Exact Decryption and Public Re-randomization

- **匹配关键字:** lattice, LWE

- **作者:** Damir Vodenicarevic, Andrei Fleiser, Pierre Seznec, Karen Mayen Naranjo, Lucas Foucher, Léo Besançon, Thybault Alabarbe, Jean-François Morcillo, Benjamin Reynes, Lilian Urvoy

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1618) | [PDF](https://eprint.iacr.org/2026/1618.pdf)


> **研究背景:** 现有匿名基础设施如混洗网络、匿名存储和隐私保护复制依赖于公共重随机化，但经典ElGamal方案在量子攻击下失效，而现有的基于格的替代方案则存在密文膨胀、噪声增长未分析或解密不精确等问题。
>
> **主要贡献:** 提出了一种基于Ring-LWE的公钥加密方案，支持无密文膨胀的公共重随机化，并通过两肢CRT模数设计和特定嵌入机制确保了解密精度。
>
> **达到效果:** 该方案在多次重随机化后仍能保持正确的解密结果，且证明了其安全性并实现了精确的消息恢复。
>
> **技术梗概:** 采用LPR/BFV风格的环学习误差加密，并通过两肢CRT模数设计和特定嵌入机制实现了解密精度和公共重随机化的结合。

---
### [推荐] [2026/1619] Relect: Single Secret Leader Election via FHE with Reduced Computation and Communication and Transparent Setup

- **匹配关键字:** lattice, homomorphic encryption

- **作者:** Haofei Liang, Zeyu Liu, Yunhao Wang, Xiang Xie, Yu Yu, Fan Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1619) | [PDF](https://eprint.iacr.org/2026/1619.pdf)


> **研究背景:** 单秘密领导者选举(SSLE)协议允许所有参与者共同且不知情地选举一位领导者，未被选中的参与者除非由领导者公开，否则不应得知领导者的身份。此前的工作如$\mathsf{Qelect}$虽然实现了这一目标，但在效率和环境假设上存在不足。
>
> **主要贡献:** $\mathsf{Relect}$协议基于环学习误差假设构建，通过利用底层同态加密的代数结构并设计定制化的同态电路，显著提高了效率，并去除了$\mathsf{Qelect}$中的强环境假设，支持动态领导者选择。
>
> **达到效果:** 与$\mathsf{Qelect}$相比，在32至2048个参与者的情况下，$\mathsf{Relect}$的本地FHE计算运行时间分别快了7.15到42.4倍（单线程）和7.10到48倍（多线程），通信成本也降低了1.14到2倍。
>
> **技术梗概:** 通过利用同态加密的代数结构设计定制化电路，$\mathsf{Relect}$实现了高效且无信任设置的SSLE协议。

---
### [推荐] [2026/1624] Code Generation of Faster Formally Verified NTT with Plantard Reduction

- **匹配关键字:** lattice

- **作者:** Donnie Y. Xu, Rajeev Gore, Amin Sakzad, Ron Steinfeld, Raymond K. Zhao

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1624) | [PDF](https://eprint.iacr.org/2026/1624.pdf)


> **研究背景:** 本文旨在通过形式验证的方法实现基于Plantard算术的ML-KEM Number-Theoretic Transform (NTT)，以提高其在密码学应用中的性能和可靠性。
>
> **主要贡献:** 作者开发了一个代码生成器，能够针对不同的密钥封装机制自动生成结构相同的高性能实现，并且首次实现了从形式化规格到实际代码的端到端验证。
>
> **达到效果:** 实验结果表明，所生成的代码在前向NTT和逆向NTT上的性能分别比标准C代码提高了1.5至1.8倍和1.7至2.5倍，并且相对于现有的形式化验证基准提高了1.26至2.19倍。
>
> **技术梗概:** 该方法通过静态边界分析器在编译时确定模数减少的位置，避免了运行时分支，并使用Jasmin进行形式化验证以确保代码的正确性。

---
### [2026/1589] The Role of Regular Integers Modulo n in RSA Cryptography

- **作者:** Klaus Dohmen, Mandy Lange-Geisler

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1589) | [PDF](https://eprint.iacr.org/2026/1589.pdf)


> **研究背景:** 本文探讨了RSA密码系统的一种多素数高次幂的一般化版本，适用于任意模数$n>1$。基于对Carmichael定理的新锐化，并针对模$n$下的正则整数进行了调整，该一般化版本在合理假设下几乎对所有消息$m<n$都能正确工作。
>
> **主要贡献:** 作者通过证明该一般化版本的正确性仅限于用模$n$下的正则整数表示的消息，从而扩展了原始RSA正确性定理的应用范围。
>
> **达到效果:** 研究结果表明，在合理假设下，此多素数高次幂的一般化RSA方案几乎对所有消息都能正确工作，并且可以通过中国剩余定理加速解密过程，进而推广了CRT-RSA。
>
> **技术梗概:** 通过利用针对模$n$下的正则整数的新锐化Carmichael定理，作者证明了该一般化版本的正确性。

---
### [2026/1590] Updatable Oblivious Key Value Stores with Access Control and Application to Multi Key Searchable Encryption

- **作者:** Benjamin Fuller, Ariel Hamlin, Arinjita Paul, Maryam Rezapour, Ronak Sahu, Amey Shukla, Mason Stuart

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1590) | [PDF](https://eprint.iacr.org/2026/1590.pdf)


> **研究背景:** 在安全计算应用中，如私有集合交集和多密钥可搜索加密中，不可区分性对键和随机值至关重要。现有Oblivious Key-Value Stores（OKVS）支持静态数据的存储与检索，但缺乏动态更新能力。
>
> **主要贡献:** 本文提出了具有访问控制的可更新Oblivious Key-Value Store（UOKVS），并首次分析了Cuckoo哈希的不可知插入方法。该方案为多密钥可搜索加密提供了新的实现路径。
>
> **达到效果:** 通过结合UOKVS和私有信息检索技术，实现了无需跨授权用户复制数据即可分享部分多映射的功能。实验表明，在处理大规模数据集时，查询响应时间显著缩短。
>
> **技术梗概:** 该研究采用了细粒度访问控制机制，并对Cuckoo哈希进行了不可知插入的分析与优化，从而支持动态更新和高效检索。

---
### [2026/1592] Sub-Cubic Homomorphic Matrix Multiplication via Self-Dual Normal Bases

- **作者:** Efe İzbudak, Kubra Kaytanci, Ferruh Ozbudak, Erkay Savas

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1592) | [PDF](https://eprint.iacr.org/2026/1592.pdf)


> **研究背景:** 研究背景：该论文旨在通过自对偶规范基，优化单乘法双线性多项式嵌入矩阵代数到交换cyclotomic环中的方法，以降低同态矩阵乘法的渐近复杂度。
>
> **主要贡献:** 主要贡献在于通过自对偶规范基和Strassen张量秩分解，将单乘法的渐近复杂度从$\widetilde{\Omega}(N^3)$优化至$\widetilde{\Omega}(N^{\log_2 7})$，并确保同态踪深度减半。
>
> **达到效果:** 达到的效果是实现了更高效的同态矩阵乘法算法，降低了计算成本，并通过自对偶规范基减少了所需的基生成器数量，从而提高了同态加密方案的效率和实用性。
>
> **技术梗概:** 技术梗概：论文采用Cohn-Umans方法，结合Strassen张量秩分解和Chinese Remainder Theorem理想，优化了嵌入过程中的多项式提升，并通过自对偶规范基确保了高效的基转换操作。

---
### [2026/1594] Breaking ADP-Based Witness Encryption

- **作者:** Muhammad El Gebali, Yaroslav Rebenko, Markus Schofnegger, Lev Soukhanov

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1594) | [PDF](https://eprint.iacr.org/2026/1594.pdf)


> **研究背景:** Witness加密(WE)允许一方在任意可满足电路下加密消息，持有满足输入的人可以解密。ADP和AADP构造基于秩属性的安全性，但缺乏形式化的安全证明。
>
> **主要贡献:** 作者提出了两种针对ADP及其扩展AADP的攻击方法，首次验证了这些候选方案的安全性问题。
>
> **达到效果:** 第一种攻击通过线性代数直接从公钥中恢复加密消息；第二种攻击则线性化NSS变体，揭示出加密随机性和消息内容，成功破解了两种构造。
>
> **技术梗概:** 利用矩阵的秩性质和线性代数方法进行攻击，展示了对稀疏电路的有效性。

---
### [2026/1595] Budget Allocation in Neural Differential Distinguishers

- **作者:** Alireza Gholizadeh Shahrbejari, Reza Ebrahimi Atani

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1595) | [PDF](https://eprint.iacr.org/2026/1595.pdf)


> **研究背景:** 研究背景在于神经差分区分器通常在固定标记样本数量下进行比较，但不同的输入表示可能需要不同数量的密文样本，这使得固定样本量的比较可能存在从密码分析数据复杂性角度来说误导性的结论。
>
> **主要贡献:** 主要贡献是提出了共享基多差分表示法，并将其与标准单差值基线和独立对控制表示进行对比研究。
>
> **达到效果:** 结果表明，在固定密文字典预算下，单差值基线并不总是最佳分配策略；增加更多的差分行通常可以提高区分器的效果，尽管这会减少独立训练样本的数量。最优的差分数目并非普遍适用：逻辑回归往往受益于更大的表示形式，而多层感知机则经常偏好中间值以避免样本饥饿和过拟合。
>
> **技术梗概:** 技术梗概包括引入共享基多差分表示法，并通过在GIFT-64、PRESENT-64、RECTANGLE-64和SPECK-64/128上的实验来评估其性能。

---
### [2026/1598] zk-Cinema: Proving Video Provenance in Zero Knowledge

- **作者:** Alexander Frolov, Jianfeng Guo, Xinyi Zhao, Trisha Datta, Dan Boneh, Ian Miers

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1598) | [PDF](https://eprint.iacr.org/2026/1598.pdf)


> **研究背景:** 视频来源验证是现代互联网中的一个重要问题，现有的标准如C2PA虽然能够验证未编辑的视频和图像，但面对已编辑和重新尺寸的视频时却无法直接应用。
>
> **主要贡献:** 本文提出了一种新的zk-Cinema系统，通过优化零知识证明技术来验证编辑过的视频来源，并开发了sfvr视频表示法以减少证明者的工作量。
>
> **达到效果:** 该系统能够高效地生成视频编辑的零知识证明，并在实际应用中达到了与现有方法相当甚至更好的性能和规模。
>
> **技术梗概:** 通过将常见视频编辑操作表示为矩阵乘法，优化了零知识证明过程；设计了sfvr视频表示法以适应SNARK证明的需求；并改进了NeutronNova折叠方案以支持高阶折叠，从而提高了系统的效率和可扩展性。

---
### [2026/1600] Paras: Actively Secure Two-Server Private Histograms

- **作者:** Dimitris Mouris, Lucas Piske, Pratik Sarkar, Ni Trieu, Mehmet Ugurbil

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1600) | [PDF](https://eprint.iacr.org/2026/1600.pdf)


> **研究背景:** 私有直方图计算是许多数据分析任务的基本构建块，能够进行频率分析而不泄露个体输入。现有的协议通常需要三个服务器来实现恶意客户端和服务器的鲁棒性，并且具有有限的恶意行为容忍度，限制了其实用性。
>
> **主要贡献:** Paras 是第一个在任意多个恶意客户端与恶意服务器合谋的情况下仍能提供私有直方图计算的双服务器协议。该工作引入了基于分布式点函数的方法并结合矢量模糊线性评估（VOLE）来确保输入正确性和输出完整性，设计了两种新的加密原语以实现这些检查。
>
> **达到效果:** Paras 协议在客户端和服务器端都表现出高效性和可扩展性：客户端的成本与域大小无关，而每个服务器的运行时、通信量和存储需求即使在大规模场景下也保持较低水平。例如，在8192个客户端和128个输入域的情况下，每台服务器仅需14毫秒的运行时间和24KB的通信量。
>
> **技术梗概:** Paras 基于分布式点函数，并引入了基于矢量模糊线性评估（VOLE）的一致性检查来确保输入正确性和输出完整性。设计了两种新的加密原语：一种是aBV，用于验证VOLE承诺份额是否对应有效位；另一种是adIPA，用于在两个不同的VOLE会话之间实现安全一致性检查。

---
### [2026/1602] New Designs of Multivariate-Polynomial Universal Hash Functions

- **作者:** Jean Paul Degabriele, Jan Gilcher, Jérôme Govinden, Kenneth G. Paterson

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1602) | [PDF](https://eprint.iacr.org/2026/1602.pdf)


> **研究背景:** 通用哈希函数(UHF)作为密码学中的基本构建块，设计安全且快速的UHF一直是长久以来的研究兴趣。本文探讨了多变量多项式UHF的设计空间，即通过有限域上多项式的评估来实现的UHF。
>
> **主要贡献:** 研究提出了基于两层设计的新方案，其中一层哈希函数生成中间值供另一层使用，并且两者均基于一元或多元多项式。这种方法使得设计方案能够从两个组件的优点中受益，从而在密钥大小、安全级别和优化技术的适用性之间找到新的权衡点。
>
> **达到效果:** 通过扩展Degabriele等人(IEEE S&amp;P 2024)最近提出的UHF代码生成和基准测试框架来支持多变量设计，并研究了一系列新两层设计方案的表现。通过对性能进行统计因子分析，确定了哪些设计特征及其组合对性能影响最大，并据此识别出具有特别好的性能/安全权衡的设计组合。
>
> **技术梗概:** 使用扩展后的框架对大量新的两层设计方案进行了性能研究，首先通过统计因素分析来确定关键设计特征的影响，然后利用这些发现来选择最佳的低层和高层哈希函数组合。

---
### [2026/1603] Design and Analysis of Quantum Designated Verifier Signature Scheme

- **作者:** Shanu Poddar, Vikas Srivastava

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1603) | [PDF](https://eprint.iacr.org/2026/1603.pdf)


> **研究背景:** 量子指定验证签名(DVS)方案确保只有特定的验证者可以验证签名，同时保持不可转让性。随着量子计算的发展，提出了多种量子DVS方案以实现量子安全性。
>
> **主要贡献:** 作者重新审视了Xin等人提出的量子DVS协议，并对其设计进行了结构性分析，揭示了一个存在伪造漏洞。
>
> **达到效果:** 研究展示了该方案在选择明文攻击下存在可被利用的伪造风险，即给定一个消息的有效量子签名，攻击者可以高效地将其转换为另一消息的有效签名，而无需知道签名人私钥。
>
> **技术梗概:** 通过基于量子密钥分发(QKD)的密钥和量子一次性垫加密提出了一种最小化对策措施。

---
### [2026/1605] UC, Categorically: Rigorous Diagrammatic Proofs

- **作者:** Pooya Farshim, Martti Karvonen, Andre Knispel, Markulf Kohlweiss, Philip Wadler

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1605) | [PDF](https://eprint.iacr.org/2026/1605.pdf)


> **研究背景:** 文章利用范畴论对Canetti的通用可组合性（UC）框架进行了形式化处理，旨在为静态数量的参与方和会话提供安全组合理论。
>
> **主要贡献:** 通过引入弦图进行图形化的证明，同时保持严格的数学验证，扩展了UC框架的应用范围，并修正了一些技术上的不足。
>
> **达到效果:** 该研究不仅增强了对UC框架的理解，还展示了其在不同计算形式下的适用性，如量子计算或特定领域语言。
>
> **技术梗概:** 采用范畴论中的弦图技术进行图形化证明，使得验证过程更加直观且易于转换为方程进行正式验证。

---
### [2026/1607] UFOs: A Very Efficient Multivariate Public Key Signature Scheme

- **作者:** Gilles Macario-Rat

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1607) | [PDF](https://eprint.iacr.org/2026/1607.pdf)


> **研究背景:** UFOs是一种基于Unbalanced Oil and Vinegar (UOV)家族的多变量公钥签名方案，通过使用Frobenius型二次形式替代通用的二次多项式，实现了紧凑的公钥表示并保持了高效的UOV签名过程。
>
> **主要贡献:** 该研究贡献了一种新的结构化二次形式，并将其应用于UOV签名方案中，从而提高了签名效率和安全性。
>
> **达到效果:** 实验结果显示，UFOs在密钥生成、签名验证方面的成本较低，同时公钥和签名的大小也得到了优化。
>
> **技术梗概:** 通过设计基于Frobenius型二次形式的结构化多项式，并详细描述了从紧凑的秘密描述中推导出公共系统的算法过程。

---
### [2026/1611] Private Identity-based Bulletin Boards for Anonymous Messaging and Other Online Services

- **作者:** Karim Eldefrawy, Stanislaw Jarecki, Ben Terner, Gene Tsudik

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1611) | [PDF](https://eprint.iacr.org/2026/1611.pdf)


> **研究背景:** 针对手头受限、操作中断、审查和间歇性网络故障等复杂网络环境，研究如何实现安全且匿名的消息传递和其他在线服务。
>
> **主要贡献:** (1) 正式定义了PIB^3的功能性和安全性要求；(2) 提出了并构建了一个层次结构的PIB^3方案。
>
> **达到效果:** 该方案允许用户在分布式数据库中匿名发布和检索消息，且无需预先建立联系或交换密钥。即使存在一个被篡改但不协同的服务器，服务器也无法获取接收者身份及记录索引信息（超出预设模型泄漏的信息）。若需保护检索次数隐私，可对检索进行填充处理。
>
> **技术梗概:** 通过设计基于身份的加密机制和多服务器结构，确保了消息的安全性和匿名性，并提供了针对特定联盟的隐私保护。

---
### [2026/1612] DYNAFIX: Dynamic Fixed‑Point Encoding for Arbitrary‑Range MPC

- **作者:** Yuntian Chen, Tianpei Lu, Zhanyong Tang, Bingsheng Zhang, Wenjing Yang, Zhuzhu Wang, Kui Ren

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1612) | [PDF](https://eprint.iacr.org/2026/1612.pdf)


> **研究背景:** 传统的隐私保护计算中，实数运算通常采用浮点或定点方法实现。虽然定点方法效率高，但在处理宽动态范围时存在局限性；而浮点方法虽能支持更广泛的数值范围，但性能开销远大于定点方法。
>
> **主要贡献:** 本文提出DYNAFIX方案，该方案结合了浮动和定点方法的优势，在保持与传统定点计算相当的性能的同时，能够支持任意数值范围的运算。
>
> **达到效果:** 实验结果表明，当评估高精度函数（如指数函数）时，该方法相较于最先进的技术实现了24.1倍的速度提升。
>
> **技术梗概:** DYNAFIX通过动态调整定点表示的位宽和小数点位置来适应不同的数值范围，在保证计算准确性的前提下提高了效率。

---
### [2026/1613] DuetORAM: Two-Server Distributed ORAM with Constant Rounds and O(log N) Communication

- **作者:** Feng Li, Xiangfu Song, Yingying Li, Lisha Yao, Guomin Yang, Tianwei Zhang, Robert H. Deng

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1613) | [PDF](https://eprint.iacr.org/2026/1613.pdf)


> **研究背景:** 分布式Oblivious RAM（DORAM）是隐私保护云数据库和外包存储系统的重要构建块，但现有双服务器设计往往依赖于缓慢的线性扫描或复杂的加密原语，难以在效率与带宽之间取得平衡。
>
> **主要贡献:** 我们提出了DuetORAM，这是一种实现常数轮访问并具有$O(\log N)$通信量的双服务器DORAM方案，避免了上述计算瓶颈。
>
> **达到效果:** 我们的研究表明，DuetORAM在局域网设置中检索延迟最多可减少170倍，并且与三服务器设计S$^3$ORAM相比，在局域网和广域网中分别将移除操作加速7倍和5倍。
>
> **技术梗概:** 我们通过复制到共享块编码，使服务器能够保持相同的密文以实现高效的PIR检索，并通过轻量级的洗牌本地解释这些密文作为秘密份额来实现无知识的驱逐。

---
### [2026/1614] LFSRs and Boolean Masking: An In-depth Security Analysis

- **作者:** Anna Guinet, Jan Schoone, Niklas Höher, Dina Hesse, Tim Güneysu

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1614) | [PDF](https://eprint.iacr.org/2026/1614.pdf)


> **研究背景:** 文章深入分析了线性反馈移位寄存器（LFSRs）与布尔掩码在侧信道攻击防护中的联合安全性，旨在填补现有研究中关于随机数生成器选择的空白。
>
> **主要贡献:** 作者首次提出了一种验证框架，该框架同时分析了特定类型的伪随机数生成器（如线性反馈移位寄存器）和掩码方案在d探针模型下的安全性。
>
> **达到效果:** 通过应用沃尔什-哈达玛变换并扩展到稳健的探针模型，作者对4比特和8比特S盒进行了详细分析，并通过FPGA实践验证了理论结果的有效性。
>
> **技术梗概:** 该研究采用了线性密码分析中的沃尔什-哈达玛变换技术来构建验证框架，从而实现了LFSRs与掩码方案的安全性联合分析。

---
### [2026/1615] KORD: Breaking the Key-Generation Bottleneck in Dealerless Function Secret Sharing via Protocol–Hardware Co-Design

- **作者:** Yijing Peng, Lin Liu, Yujie Xue, Shaojing Fu, Shaoqing Li, Yaohua Wang, Rongmao Chen, Yang Guo

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1615) | [PDF](https://eprint.iacr.org/2026/1615.pdf)


> **研究背景:** 功能秘密共享(FSS)是隐私保护计算的核心机制，但每次FSS调用都需要生成一对函数密钥，通常由可信的经销商完成，这扩展了系统的信任边界并阻碍了实际部署。
>
> **主要贡献:** KORD通过协议-硬件协同设计大幅降低了无经销商FSS密钥生成的成本，实现了无需经销商即可重建FSS密钥，并将交互次数从之前的线性增长减少到单轮次。
>
> **达到效果:** KORD减少了每次操作的密钥生成通信量，最高达7633至70274倍；在ZCU102 FPGA上，跨密钥交错提高了AES车道利用率至99%，实现了每秒11.6百万个32位DPF密钥。
>
> **技术梗概:** KORD通过建立共同的信任根并通过互认证实现，进一步在信任边界内重构生成协议，并采用交叉密钥调度方案交错独立的GGM树遍历以维持高计算吞吐量。

---
### [2026/1620] Extending the Applicability of Algebraic Key Recovery Attacks on the UOV Signature Scheme

- **作者:** Yasuhiko Ikematsu, Hiroki Furue

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1620) | [PDF](https://eprint.iacr.org/2026/1620.pdf)


> **研究背景:** UOV签名方案因其紧凑的签名大小和对多种攻击的抵抗力，成为多变量公钥密码学中的主要候选者。然而，Ran提出的代数密钥恢复攻击在特定条件下降低了其安全性。
>
> **主要贡献:** 本文提出的方法克服了现有攻击方法在某些参数设置下的局限性，扩展了这些攻击的有效范围。
>
> **达到效果:** 通过应用该方法并结合Nakamura等人的提升技术，作者展示了SNOVA中一些参数集的安全级别存在风险。
>
> **技术梗概:** 研究提出了一个新的方法来处理额外的内核元素问题，并使用了Nakamura等人提出的提升技术。

---
### [2026/1622] Formal Security Analysis of the Olvid Messenger

- **作者:** Noemi Terzo, Cas Cremers, Ruben Gonzalez, Peter Schwabe, Yuval Yarom, Zhiyuan Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1622) | [PDF](https://eprint.iacr.org/2026/1622.pdf)


> **研究背景:** 本文是对法国政府官员使用的Olvid加密即时通讯应用的首个形式化安全分析，旨在填补其在敏感环境部署下的独立安全性评估空白。
>
> **主要贡献:** 研究者开发了详细的模型来验证Olvid的认证密钥交换和连续密钥协议的安全性，并通过形式化验证确保了核心安全属性。
>
> **达到效果:** 然而，研究表明尽管设计符合基本的安全保证，但并未达到如Signal等现代先进安全消息协议所具有的强健安全性标准，例如eCK模型下的安全性未能实现。
>
> **技术梗概:** 研究采用了Dolev-Yao网络对手模型，并考虑了主动攻击者的可能行为来构建和验证协议模型。

---
### [2026/1623] Triple Cryptanalysis of Isogeny-Based VRFs from Asiacrypt 2025

- **作者:** Yi-Fu Lai, Yu Yu, Xiaogang Zhou

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1623) | [PDF](https://eprint.iacr.org/2026/1623.pdf)


> **研究背景:** 该研究针对Asiacrypt2025会议上提出的基于CGL类哈希函数的新型可验证随机函数(VRF)进行了分析，指出其设计存在潜在安全漏洞。
>
> **主要贡献:** 作者提出了两阶段攻击方法，首先揭示了公钥表示中的不一致性问题，然后利用此信息恢复了VRF的秘密密钥。
>
> **达到效果:** 通过1536次查询，在30分钟内成功破解了256位的秘密密钥，并展示了仅使用公共密钥有时也能泄露秘密路径的部分信息。
>
> **技术梗概:** 攻击技术包括识别并利用公钥表示中的不一致性，以及通过有限的查询恢复完整的秘密密钥。
...
### [推荐] [2026/1526] On the Suitability of Syndrome Decoding for Proof-of-Work under Quantum Adversaries: Design and Analysis
...
- **链接:** [论文](https://eprint.iacr.org/2026/1526) | [PDF](https://eprint.iacr.org/2026/1526.pdf)


> **研究背景:** 文章探讨了在量子对手攻击下， Syndrom 解码问题作为 Proof-of-Work (PoW) 基础的适用性，旨在克服哈希函数 PoW 构造面临的二次量子加速问题。
>
> **主要贡献:** 作者分析了 Syndrom 解码问题的经典和量子复杂度，并提出了一个基于 Syndrom 解码问题（SDP）且具有结构化特征约束的 PoW 构造方案。
>
> **达到效果:** 该研究提供了在随机实例假设下，预期解的数量及成功挖矿概率的表达式，为参数选择提供理论依据。
>
> **技术梗概:** 通过比较基于 Syndrom 解码问题、哈希函数和格基构造的量子优势，作者设计了一种具有可控解密度和难度的 PoW 构造方法。

---
### [推荐] [2026/1540] Lattice-Based Shuffle Arguments using Subset Checking

- **匹配关键字:** lattice, LWE

- **作者:** Behzad Abdolmaleki, Prastudy Fauzi, Jiaqi Gu, Toomas Krips, Nahid Roustaeifar

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1540) | [PDF](https://eprint.iacr.org/2026/1540.pdf)


> **研究背景:** 现有的通信高效混排论证依赖于经典假设，而基于格的构造则效率较低。本文旨在通过使用Abdolmaleki等人在格设置中的子集检查方法来提出一种基于格的混排论证方案，以提供更短的证明并解决现有问题。
>
> **主要贡献:** 该研究贡献了一种基于格的混排论证方案，适用于Ajtai承诺，并结合了Lyubashevsky等人的格基零知识框架。
>
> **达到效果:** 该协议在随机预言模型下安全，并通过重新随机化方法恢复输入承诺所需的分布属性，从而提高了证明通信、时间和验证时间。
>
> **技术梗概:** 技术上，本文利用Abdolmaleki等人提出的子集检查方法，在格设置中构建了混排论证方案，并结合了Ajtai承诺和Lyubashevsky等人的零知识框架。

---
### [推荐] [2026/1543] Efficient Private Filtering and Aggregation for Weighted Set Intersection via Oblivious Encrypted Weight Transfer

- **匹配关键字:** homomorphic encryption

- **作者:** Xiaodong Wang, Shengzhe Meng, Zijie Lu, Bei Liang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1543) | [PDF](https://eprint.iacr.org/2026/1543.pdf)


> **研究背景:** 现有研究在加权集合交集中隐私计算方面存在碎片化问题，且多为针对特定应用设计。
>
> **主要贡献:** 作者提出了一种统一框架PFA-WSI，通过参数化的联合评分函数和谓词支持过滤输出和聚合输出两种模式。
>
> **达到效果:** 该框架能够涵盖已部署的任务如PI-Sum、内积PJC以及IMWS，并能适应实践中出现的更多指标。
>
> **技术梗概:** 通过Oblivious Encrypted Weight Transfer技术实现高效私密计算。

---
### [推荐] [2026/1547] Silent Distributed Cryptography for DNFs and Threshold Policies from Lattices

- **匹配关键字:** lattice, post-quantum, homomorphic encryption, LWE

- **作者:** Abtin Afshar, Rishab Goyal, Saikumar Yadugiri

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1547) | [PDF](https://eprint.iacr.org/2026/1547.pdf)


> **研究背景:** 研究旨在解决基于格的门限密码学中的两个核心问题：一是针对一般门限$t \geq 2$的无后量子设置的秘密共享方案，二是具有亚线性参数的门限全同态加密（TFHE），这是自Boneh等人(CRYPTO 2018)的工作以来的一个开放问题。
>
> **主要贡献:** 贡献在于提出了$(\alpha,\beta)$-缩放线性秘密共享方案(LSSS)，并基于此提供了首个后量子门限加密构造，支持DNF公式和阈值策略，并显著减小了密文大小。
>
> **达到效果:** 结果是实现了分布式单调政策加密的无后量子设置，并证明在随机预言模型下安全；同时提出了具有亚线性参数的门限全同态加密方案，进一步减少了密文大小。
>
> **技术梗概:** 技术上采用了$(\alpha,\beta)$-缩放线性秘密共享方案，通过调整授权集合的重建系数和集依赖的缩放因子来保持未经授权份额的均匀分布。

---
### [推荐] [2026/1550] Algorithms for Sparse LWE and LPN with Small Secrets
...
- **链接:** [论文](https://eprint.iacr.org/2026/1550) | [PDF](https://eprint.iacr.org/2026/1550.pdf)


> **研究背景:** 研究背景：针对稀疏LWE和LPN问题，特别是在秘密向量受限于小$l_{\infty}$范数的情况下，探讨了在系数矩阵稀疏时攻击者能利用这些限制的程度。
>
> **主要贡献:** 主要贡献：提出了新的样本-运行时间权衡方法，并将区分任务归约为松弛的SIS问题，设计了一种基于Kikuchi图上近端非平凡路径采样的算法。
>
> **达到效果:** 达到的效果：对于LWE，该方法直接区分了种植实例和随机实例；对于LPN，通过不同的归约和扩展抗集中框架，证明了结果的有效性。
>
> **技术梗概:** 技术梗概：采用Kikuchi图上近端路径采样的算法来解决松弛的SIS问题，并结合不同的归约方法处理LWE和LPN问题。

---
### [推荐] [2026/1551] Fine-Grained and Runtime-Configurable Precision for Exact FHE Inference

- **匹配关键字:** homomorphic encryption

- **作者:** Wun-Ting Lin, Ja-Ling Wu

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1551) | [PDF](https://eprint.iacr.org/2026/1551.pdf)


> **研究背景:** 现有全同态加密(FHE)下的隐私保护机器学习框架中，精度固定且难以调整，这限制了混合精度策略的应用。
>
> **主要贡献:** $\mathtt{Tailor}$框架提出了一个后端无关的解决方案，允许在单一密钥生成下动态配置每层网络的精度，并精确计算非线性操作。
>
> **达到效果:** 通过$\mathtt{Tailor}$框架，研究人员能够在保持加密完整性的前提下灵活调整精度，显著提高了FHE中神经网络推理的效率和灵活性。
>
> **技术梗概:** 该框架通过将有符号整数表示为独立加密位向量，并构建所有算术和神经网络操作符从具有精度参数的布尔电路，实现了比特宽度与密码状态解耦。

---
### [推荐] [2026/1554] On the Hardness of some Vandermonde Knapsack problems

- **匹配关键字:** lattice

- **作者:** Dipayan Das, Arindam Mukherjee

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1554) | [PDF](https://eprint.iacr.org/2026/1554.pdf)


> **研究背景:** Vandermonde Knapsack问题是一类基于代数的背包问题变种，包括Partial Vandermonde $(\mathsf{PV})$ Knapsack问题和Vanishing $\mathsf{SIS}$ $(\mathsf{vSIS})$-based承诺问题等，这些问题是构建高效格基密码系统的关键。
>
> **主要贡献:** 作者提出了针对power-of-two cyclotomic环上$\mathsf{PV}$ Knapsack问题的新攻击方法，并结合Das和Joux的方法，展示了更多密钥实例是弱的。同时扩展了攻击范围至整数变体的$\mathsf{vSIS}$承诺问题。
>
> **达到效果:** 通过新方法，作者证明了一种更广泛的参数设置下，$\mathsf{PV}$ Knapsack问题和$\mathsf{vSIS}$承诺问题是弱的，这为相关密码系统的设计提供了新的见解。
>
> **技术梗概:** 研究结合了代数攻击技术和格基加密分析方法，通过巧妙地利用多项式性质来揭示密钥的弱点。

---
### [推荐] [2026/1559] Revisiting Shamir Secret Sharing for Threshold Fully Homomorphic Encryption

- **匹配关键字:** lattice, homomorphic encryption

- **作者:** Jiseung Kim, Seunghu Kim, Hyung Tae Lee

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1559) | [PDF](https://eprint.iacr.org/2026/1559.pdf)


> **研究背景:** 研究背景：近年来，基于格的阈值全同态加密（ThFHE）和阈值公钥加密（ThPKE）等方案普遍采用环上的Shamir秘密共享机制。然而，随着参与方数量N增加，用于秘密重建的分母清除因子迅速增长，导致密文模数增大及复杂的重建过程。
>
> **主要贡献:** 主要贡献：本文重新审视了基于环的Shamir秘密共享中的减法集概念，并提出了一种改进框架以在循环环上实现整数可重构的秘密分享。通过几何分析拉格朗日系数，我们展示了在特定设置下可以显著减少重建因子。
>
> **达到效果:** 达到的效果：该方法使得$(t,N)$-阈值设置下的密文大小更小，并且当应用于任何使用循环环上的Shamir秘密共享的阈值构造时，提高了正确性和效率。特别是在基于格的一轮$(t, N)$-ThFHE方案中，我们的方法将密文模数的位长从$O(N)$减少到$O(t\log (N/t^2))$，同时确保有效的分母处理。
>
> **技术梗概:** 技术梗概：通过几何分析拉格朗日系数，我们设计了一种新的减法集构造方法，并应用于基于环的阈值方案中，从而减少了秘密重建所需的因子大小。

---
### [推荐] [2026/1560] Dimension Reduction for SVP in Hawk: A Trace-Zero Approach

- **匹配关键字:** lattice

- **作者:** Guilhem Mureau, Alice Pellet-Mary

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1560) | [PDF](https://eprint.iacr.org/2026/1560.pdf)


> **研究背景:** 本文研究了在Hawk签名方案中，通过维度降低技术从高秩模格问题到低秩模格问题的归约方法，以提高攻击效率。
>
> **主要贡献:** 作者提出了两种改进：一是将秩为2的模格问题归约为秩为3的问题；二是针对特定模块结构给出了几何形状简单的模格问题。
>
> **达到效果:** 最终，通过这种方法，作者设计了一种算法，在维度最多为$3m/8+1$的格中进行多项式次数的确切最短向量问题调用，从而提高了攻击Hawk密钥恢复的速度。
>
> **技术梗概:** 该研究采用了Chevignard等人关于模理想问题和规范理想问题之间归约的方法，并结合了Ducas的结果来适应特定模块结构。

---
### [推荐] [2026/1562] Power Analysis and Countermeasures on the MiMC Block Cipher

- **匹配关键字:** homomorphic encryption

- **作者:** Elena Andreeva, Stefan Mangard, Rishub Nagpal, Arnab Roy, Stefano Trevisani

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1562) | [PDF](https://eprint.iacr.org/2026/1562.pdf)


> **研究背景:** 随着零知识（ZK）、全同态加密（FHE）和多方计算（MPC）协议的研究兴趣增加，对基于算术化导向（AO）的密码原语的需求也在增长。然而，在嵌入式平台上使用这些协议时，需要考虑保护措施以抵御侧信道分析（SCA），包括时间攻击和功率攻击。
>
> **主要贡献:** 本文针对MiMC块密码进行了侧信道分析，并提出了两种侧信道对策：经典的ISW掩码方法和适应大素数域的冗余数表示法（RNR）。
>
> **达到效果:** 研究表明，使用仅约32000个功率测量值，DPA攻击者可以将密钥猜测空间减少到\(2^{30}\)候选者；而通过100次测量即可实现完整的密钥恢复。此外，实验表明大约64位的冗余度足以降低信息泄露程度，使DPA攻击变得不切实际。
>
> **技术梗概:** 研究采用了常数时间实现MiMC在BN254素域上的x86和ARM-v7目标，并通过ARM Cortex-M4微控制器展示了基于特征值（SASCA）和线性回归的功率分析方法。

---
### [推荐] [2026/1569] Tensor Encodings for SIMD HSS
...
- **链接:** [论文](https://eprint.iacr.org/2026/1569) | [PDF](https://eprint.iacr.org/2026/1569.pdf)


> **研究背景:** 研究了基于格的同态秘密共享方案在维度-$N$循环环上的SIMD打包方法，特别是Boyle-Kohl-Scholl (BKS)方案。
>
> **主要贡献:** 提出了两种交替张量编码构造，一种使用加法封闭模式，另一种利用无三元等差数集来优化密文打包数量和计算成本。
>
> **达到效果:** 通过这两种构造，显著提高了SIMD-HSS的效率，实现了接近$\sqrt{N}$的密文打包，并且在两方执行中验证了算法的有效性。
>
> **技术梗概:** 采用交替张量编码和特定数集来优化同态操作，减少所需的认证自同构数量和其他常数乘法次数。

---
### [推荐] [2026/1572] SHARMONY: Composing SHA-2 and SHA-3 Hardware for Crypto-Agile PQC

- **匹配关键字:** post-quantum

- **作者:** Liga Anwar, Carlos Andres Lara-Nino, Jong-Yeon Park, Michael Hutter

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1572) | [PDF](https://eprint.iacr.org/2026/1572.pdf)


> **研究背景:** 本文旨在通过将SHA-2和SHA-3硬件统一设计，以适应后量子密码学（PQC）的需求，特别是在不同的标准化方案中依赖于不同哈希函数的情况下。
>
> **主要贡献:** 该工作提出了一个混合架构，通过选择性共享关键组件实现两者的结合，并引入了双执行模式来提高效率。
>
> **达到效果:** 实验结果表明，SHARMONY在Artix-7 FPGA上实现了SHA-256每秒1,656兆字节的吞吐量，相比OpenTitan、Caliptra、SPHINCSLET和SLotH等现有引擎分别提高了89%、72%和50%。
>
> **技术梗概:** 设计采用了共享寄存器银行、圆常数存储以及统一的数据路径组织，并通过双重执行模式优化了硬件利用率。

---
### [推荐] [2026/1576] A Systematic Literature Review on Optimising CRYSTALS-Dilithium (ML-DSA) Performance for IoT Devices via Lightweight Hashing

- **匹配关键字:** lattice, post-quantum

- **作者:** Ceasar Njuguna Ngunu, Edward Ombui

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1576) | [PDF](https://eprint.iacr.org/2026/1576.pdf)


> **研究背景:** 针对物联网(IoT)设备的资源限制，CRYSTALS-Dilithium作为模块化晶格基数字签名算法，在迁移至后量子密码学时面临显著性能成本。
>
> **主要贡献:** 综述了在受限平台上优化ML-DSA性能的研究，并分类总结了文献中采用的各种优化策略。
>
> **达到效果:** 通过使用轻量级扩展输出函数替代标准的SHAKE函数，实现了签名和验证过程的显著加速。
>
> **技术梗概:** 采用了Kitchenham指南和PRISMA 2020声明的方法进行系统性文献回顾，并在ARM Cortex-M4和Cortex-M7平台上进行了软件实现优化。

---
### [推荐] [2026/1581] Post-Quantum Internet Key Exchange via Authenticated Forward-Secure KEM

- **匹配关键字:** post-quantum, LWE

- **作者:** Yunlei Zhao, Biming Zhou, Zhixiang Zhao, Yifan Dong, Cheng Huang, Haodong Jiang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1581) | [PDF](https://eprint.iacr.org/2026/1581.pdf)


> **研究背景:** 该研究旨在提出一种无需签名、后量子安全的认证密钥交换（AKE）框架，以解决当前文献中的几个基本开放问题。
>
> **主要贡献:** 贡献在于设计了一种新的加密原语——认证前向安全KEM（AFS-KEM），它在单一KEM抽象中统一了认证和前向安全性。
>
> **达到效果:** 该框架实现了最多交换两次标准密钥封装机制（KEM）的密文，同时满足计算对称性、完美前向保密性和其他多项安全属性，并且在随机预言模型（ROM）和量子访问随机预言模型（QROM）下可证明安全。
>
> **技术梗概:** 技术上，通过引入AFS-KEM来统一认证与前向安全性，确保了在ML-KEM结构下的实例化以及对解密错误攻击的强抵抗性。

---
### [推荐] [2026/1585] Proving Threshold Regev PKE from Adaptive Hint-MLWE: Efficient, Non-interactive, and CCA Secure

- **匹配关键字:** lattice, post-quantum, LWE

- **作者:** Yisol Hwang, Shuichi Katsumata, Seonhong Min, Guilhem Niot, Yongsoo Song

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1585) | [PDF](https://eprint.iacr.org/2026/1585.pdf)


> **研究背景:** 针对NIST多 parties阈值密码学的呼吁，现有的后量子阈值公钥加密（tPKE）仍存在不足。特别是如何同时满足多项式模数、非交互解密和CCA安全性的证明尚未解决。
>
> **主要贡献:** 首次证明了在MLWE假设下，阈值Regev PKE能够同时满足上述三个要求，并且提供了一个更强大的模拟安全性证明。
>
> **达到效果:** 该工作不仅解决了理论上的开放问题，还为实际部署提供了可能的解决方案。
>
> **技术梗概:** 通过引入适应性提示-MLWE（AHMLWE）问题及其与标准MLWE之间的紧致归约来实现上述目标。

---
### [推荐] [2026/1586] Perturbation of Hankel moment singular values and supersingular endomorphism rings via CVP: a $p$-adic super-resolution law and a fully computed pipeline
...
- **链接:** [论文](https://eprint.iacr.org/2026/1586) | [PDF](https://eprint.iacr.org/2026/1586.pdf)


> **研究背景:** 本文探讨了通过CVP扰动Hankel矩阵的奇异值和超奇异椭圆曲线端射影环，以实现$p$-adic超分辨率定律，并提供了一个完全可再现的管道。
>
> **主要贡献:** 作者证明了在特定噪声下，可以通过Vandermonde条件数确定性地恢复节点，并提出了一个精确的超分辨率定律来计算$p$-adic精度损失。同时，通过CVP减少$\mathbb{Z}$-基并利用Weil配对离散对数解出规范矩阵，从而实现节点类型的读取。
>
> **达到效果:** 该研究提供了两个严谨的结果：一是确定性地恢复节点的噪声边界条件；二是通过计算精确的Gram矩阵来恢复节点类型，并在实数上完全实现了这些步骤。
>
> **技术梗概:** 技术上，作者使用了Vandermonde条件数、Teichmüller提升和Weil配对离散对数等方法，结合LLL算法和CVP减少基的方法，最终确定性地恢复了节点并读取了理想到isogeny和平滑步骤。

---
### [推荐] [2026/1587] Solving the Shortest Vector Problem in $2^{0.7314n+o(n)}$ Time via Discrete Gaussian Sampling on Superlattices
...
- **链接:** [论文](https://eprint.iacr.org/2026/1587) | [PDF](https://eprint.iacr.org/2026/1587.pdf)


> **研究背景:** 该研究针对的是计算复杂性中的最短向量问题（SVP），这是密码学中一个关键的难题，尤其是在 lattice-based cryptography 中。
>
> **主要贡献:** 作者提出了一种基于离散高斯采样的经典随机算法，显著改进了已知的经典和量子界的方法。
>
> **达到效果:** 该算法能够在 $2^{0.7314n+o(n)}$ 时间内找到最短向量，并且在空间复杂度上达到了 $2^{n/2+o(n)}$ 的水平。
>
> **技术梗概:** 通过构造一个随机主理想超格子 $\Gamma\supset L$ 并应用 ADRS 上述平滑诚实离散高斯采样，然后扫描结果样本并保留最短非零向量，该方法利用了 Kabatiansky--Levenshtein 球堆积界来控制超出 $pL^*$ 的向量贡献。

---
### [2026/1541] Batched Oblivious Transfer with Square-Root Communication

- **作者:** Yicheng Li, Claudio Orlandi, Lawrence Roy, Yizhou Yao

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1541) | [PDF](https://eprint.iacr.org/2026/1541.pdf)


> **研究背景:** Oblivious Transfer (OT)是密码学中的基本原语，广泛应用于多种多方加密协议中。现有的OT扩展技术在处理非常大的批量时表现出色，但在总OT数量适中的情况下性能下降，因为生成所需基础OT的成本无法有效摊销。
>
> **主要贡献:** 本文通过提出通信复杂度为平方根的批量OT构造方法来解决这一问题，实现了在生成\(\ell\)个随机OT时使用\(O(\lambda\sqrt{\ell})\)通信量的目标。
>
> **达到效果:** 该方案在典型参数下（\(\lambda=128\)），生成128个随机OT分别需要约2.5 KB和1 KB的通信量，对于批量大小\(\ell \leq 2^{15}\)的情况优于现有技术。
>
> **技术梗概:** 构造基于Damg{\aa}rd--Jurik密码系统和标准假设的第一种方案，并通过分布式离散对数计算技术探索设计空间；第二种方案则依赖于在素阶数群上的幂-DDH假设，无需任何一次性信任设置。

---
### [2026/1542] Passive Full-Key Recovery for the MQOM v2 Lineage from Saltless Root Expansion

- **作者:** José Luis Delgado

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1542) | [PDF](https://eprint.iacr.org/2026/1542.pdf)


> **研究背景:** 研究背景：MQOM v2签名方案通过固定PRG调用生成根函数，缺乏盐值，导致重复种子暴露线性方程，影响安全性。
>
> **主要贡献:** 主要贡献：提出了一种被动经典EUFCMA攻击方法，能够检测有效碰撞、恢复完整签名密钥并伪造新消息。
>
> **达到效果:** 达到的效果：在允许的\(Q=2^{64}\)签名校验边界内，生日阶段成功率为0.393395296381，误差为\(O(2^{-64})\)；通过扩展可恢复两个独立密钥。
>
> **技术梗概:** 技术梗概：利用固定根函数支持的全密钥恢复方法，并允许预计算跨目标和版本重用。

---
### [2026/1544] SoK: Confidential Transformer Inference and Retrieval-Augmented Generation

- **作者:** Timofey Yaluhin

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1544) | [PDF](https://eprint.iacr.org/2026/1544.pdf)


> **研究背景:** 研究背景：在处理保密数据时，Transformer推理和检索增强生成（RAG）面临两种选择：暴露提示词和文档给云运营商或将其保留在本地，后者限制了部署更弱的自托管模型。现有防御措施分为五类机制家族：安全计算（MPC和FHE）、可信执行环境（TEEs）、静态混淆、差分隐私以及混合TEES-混淆分割方案，但没有系统性地在统一的安全基础、威胁模型和部署成本框架下进行比较，并且未涵盖RAG检索层。
>
> **主要贡献:** 主要贡献：作者通过部署就绪度对领域进行了组织，评估了各种机制的性能、效用和威胁模型匹配程度，发现没有任何一家族能够全面满足所有标准，每个方案最多只能在两个标准上取得优势，而牺牲的标准则由其安全基础决定。
>
> **达到效果:** 达到的效果：研究揭示了没有一种设计能够在查询到答案的整个管道中保持保密性，并指出私有图RAG是目前最未被充分服务的场景。
>
> **技术梗概:** 技术梗概：通过评估不同机制在性能、效用和威胁模型匹配度上的表现，作者识别出了隐藏的成本，并提出了针对特定约束条件下的部署选择建议。

---
### [2026/1545] Zero-Knowledge Proofs of Isogeny Diamonds

- **作者:** Leonardo Colò, Maher Mamah, Youcef Mokrani, Bruno Sterner, Nicolas Swanson

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1545) | [PDF](https://eprint.iacr.org/2026/1545.pdf)


> **研究背景:** 研究背景：Isogeny钻石在基于曲率的密码学中扮演着重要角色，用于构造和分析。同时，零知识证明（ZKP）技术已被广泛研究并应用于多种场景。本文旨在结合这两方面，提出针对Isogeny钻石的零知识证明方法。
>
> **主要贡献:** 主要贡献：作者提出了四种不同的零知识证明方案，分别适用于不同条件下的Isogeny钻石验证，并详细描述了每种方案的具体实现方式。
>
> **达到效果:** 达到的效果：通过实验比较，展示了这些方案在安全性、效率和紧凑性之间的权衡关系，为实际应用提供了参考依据。
>
> **技术梗概:** 技术梗概：采用不同的假设条件来设计零知识证明协议，包括已知部分Isogeny的信息以及钻石的整体结构等，并通过实现验证了其有效性和可行性。

---
### [2026/1546] Note on Number-Theoretic Transforms for Implementers -- Butterflies, Twisting, Incompleteness, and Good's Trick

- **作者:** Bo-Yin Yang

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1546) | [PDF](https://eprint.iacr.org/2026/1546.pdf)


> **研究背景:** 本文旨在为后量子密码学实现者提供关于基数-2数论变换（NTT）及其蝴蝶操作、扭转技巧等关键概念的理解，填补了相关文献中的空白。
>
> **主要贡献:** 文章贡献在于详细阐述了NTT的实现细节，包括但不限于蝴蝶操作、扭转技巧和不完整NTT的应用，并解释了这些技术如何结合使用。
>
> **达到效果:** 通过本文，实施者能够更好地理解和应用NTT及其变体，特别是在后量子密码学中。
>
> **技术梗概:** 文章采用了一系列技术梗概，如蝴蝶操作的双向灵活性、扭转技巧的本质以及不完整NTT的应用等。

---
### [2026/1548] Revisiting the Wedge Attack on UOV problem

- **作者:** Jintai Ding, Peigen Li, Siyong Tao

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1548) | [PDF](https://eprint.iacr.org/2026/1548.pdf)


> **研究背景:** 本文重新审视了UOV问题中的楔形攻击，并在更清晰的代数几何框架下对其进行改革，扩展至任意特征下的多齐次多项式系统。
>
> **主要贡献:** 作者通过改进楔形攻击方法，提出了适用于多种情况的安全分析工具。
>
> **达到效果:** 该研究成功地将多齐次楔形攻击应用于SNOVA的安全性评估，揭示了潜在的弱点。
>
> **技术梗概:** 利用代数几何和多项式系统理论，开发了一种新的多齐次楔形攻击技术。

---
### [2026/1549] The Cross-ratio Property and Its Use for Cryptanalysis of Round-reduced AES

- **作者:** Zhenzhen Bao, Jian Guo, Eik List, Haoyang Wang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1549) | [PDF](https://eprint.iacr.org/2026/1549.pdf)


> **研究背景:** 本文旨在通过利用AES中未被充分研究的乘法逆运算和四元组结构特性，推进对圆减少AES的密码分析方法。
>
> **主要贡献:** 作者提出了三种技术：一是形式化了交叉比性质以追踪非线性方程在$F_{2^8}$上的解；二是揭示了乘法逆运算在特定输入情况下可以恢复输出差异或密钥应用前的状态；三是定义了混合四元组切换，展示了其在数据减少攻击中的应用。
>
> **达到效果:** 这些技术使得针对七轮AES-128的在线和离线阶段的时间和内存消耗均有所降低，并且能够构建新的三轮区分器。
>
> **技术梗概:** 研究利用了乘法逆运算和四元组结构，通过形式化交叉比性质来提高攻击效率。

---
### [2026/1553] Amortized Multi-Verifier Proofs from Reductions of Knowledge

- **作者:** Nikitas Paslis, Carla Ràfols, Alexandros Zacharakis

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1553) | [PDF](https://eprint.iacr.org/2026/1553.pdf)


> **研究背景:** 研究背景：在多验证者场景下，共享证明者为多个持有不同声明的独立客户端提供服务。目标是通过减少证明者的计算工作量来优化整体效率，同时保持每个验证者的本地验证能力。
>
> **主要贡献:** 主要贡献：提出了一种新的多验证者 Fiat--Shamir 变换方法，能够跨独立可验证的证明关联验证挑战，同时保留局部验证特性。此外引入了承诺局部折叠方案，推迟多项式约束检查并在后续阶段进行优化。
>
> **达到效果:** 达到的效果：该框架为本地验证下的多验证者证明系统的效率提升提供了通用解决方案，并应用于 R1CS 和 CCS 基础的 SNARK 证明者中，显著减少了证明者的计算负担。
>
> **技术梗概:** 技术梗概：通过设计一种新的 Fiat--Shamir 变换和承诺局部折叠方案，实现了在不牺牲验证本地化的情况下对多项式约束检查进行分批处理和后期优化，从而有效提升了多验证者场景下的证明效率。

---
### [2026/1555] Non-Interactive Secure Computation with Constant Communication Overhead

- **作者:** Yuval Ishai, Ziyang Jin, Naty Peter, Akshayaram Srinivasan

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1555) | [PDF](https://eprint.iacr.org/2026/1555.pdf)


> **研究背景:** 研究旨在探讨在恶意对手环境下，非交互式安全计算(NISC)协议的通信复杂性。
>
> **主要贡献:** 提出了一种通用的NISC协议，适用于由布尔电路C表示的任意两方函数，并且通信量仅为O(|C|λ)比特，其中λ是计算安全性参数。
>
> **达到效果:** 该协议在随机预言模型中是无条件安全的，假设存在标准随机位OT关联设置。与 Yao 的半诚实协议相比，本协议仅产生常数级别的通信开销，并且能够在没有额外交互的情况下实现对恶意方的安全性。
>
> **技术梗概:** 通过利用随机预言模型和标准随机位 OT 关联设置来实现无条件安全性，并设计了一种高效的通信机制以减少通信开销。

---
### [2026/1556] Breaking the Beyond-Birthday-Bound Security of ${\sharp}\textrm{Pencil}$

- **作者:** Léonard Assouline, Cécile Delerablée

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1556) | [PDF](https://eprint.iacr.org/2026/1556.pdf)


> **研究背景:** 论文针对Bhaumik等人在CRYPTO 2026会议上提出的${\sharp}\textrm{Pencil}$函数，该函数声称实现了接近$n$比特的安全性超越生日界限。然而，研究发现其存在生日界限内的碰撞攻击。
>
> **主要贡献:** 作者揭示了${\sharp}\textrm{Pencil}$的前端$\textsf{Sharp}$在处理输入后半部分时存在漏洞，导致了birthday-bound级别的碰撞攻击。
>
> **达到效果:** 通过该攻击，研究人员能够构造出一个恒定优势的伪随机函数区分器，并证明${\sharp}\textrm{Pencil}$-CAU模式也受到了影响。
>
> **技术梗概:** 研究采用了分析压缩函数和确定性计算过程的技术来识别碰撞发生的条件，并利用这些条件构建了攻击方案。

---
### [2026/1557] Certified in Theory, Broken in Practice: Assumption Gaps in Cryptographic Model Certification

- **作者:** Carter Luck, Olive Franzese-McLaughlin, Elisaweta Masserova, Akira Takahashi, Antigoni Polychroniadou, Nicolas Papernot

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/1557) | [PDF](https://eprint.iacr.org/2026/1557.pdf)


> **研究背景:** 隐私保护的机器学习审计协议允许审计者在不泄露模型内部结构或训练数据的情况下评估模型的属性，如准确性或公平性。然而，这些协议的实际效用取决于它们能否确保模型行为不仅限于审计期间的表现，还能在部署后保持一致。现有的安全性定义往往未能满足这一要求，导致了理论与实践之间的差距。
>
> **主要贡献:** 研究者提出了针对加密模型认证（CMC）框架的严格密码学安全概念，并设计了一个通用协议模板，证明其能够满足这些需求。这不仅为现有方法提供了警示性的证据，还构建了一种新的解决方案来填补这一理论与实践间的鸿沟。
>
> **达到效果:** 通过实验证明，攻击者可以利用精心设计的数据训练模型，在审计数据集上实现超过99%的准确率，但在相同分布的新样本中却表现不佳，准确率低于30%，从而揭示了现有CMC方案的安全性缺陷。
>
> **技术梗概:** 研究采用了形式化的方法来定义CMC框架下的安全概念，并设计了一个基于零知识证明（ZKP）的通用协议模板，以确保模型在实际部署中的行为与审计期间的表现一致。

---
### [2026/1561] DeepBrake: Efficient Row-Wise Reed-Solomon Commitments for Arbitrary Points

- **作者:** Zhongliang Zhang, Xinxuan Zhang, Yuanju Wei, Lang Qin, Yi Deng

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1561) | [PDF](https://eprint.iacr.org/2026/1561.pdf)


> **研究背景:** 现有透明多项式承诺方案如Brakedown依赖于小最小距离的编码，导致需要更多的列查询以保证正确性，从而增加了证明大小。
>
> **主要贡献:** DeepBrake通过结合Reed-Solomon编码和改进的验证机制，在任意评估点上同时验证接近性和绑定性，减少了查询次数。
>
> **达到效果:** 在n=2^20, 码率为1/2的情况下，DeepBrake显著降低了证明大小，提高了效率。
>
> **技术梗概:** 通过固定行评估并在验证者随机采样之前进行折迭，DeepBrake实现了单次接近性测试以同时验证两种属性。

---
### [2026/1563] A Generalized Framework for Conditional Linear Cryptanalysis and Its Application to AES-Like Ciphers

- **作者:** Cheng Che, Tian Tian, Jing Yang, Fan Yang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1563) | [PDF](https://eprint.iacr.org/2026/1563.pdf)


> **研究背景:** 研究背景：条件线性密码分析是一种线性密码分析的扩展方法，已被应用于DES和AES。然而，其关键挑战在于如何确定合适的条件以提高数据子空间内的线性相关性，而现有方法依赖于专家直觉，限制了其广泛应用。
>
> **主要贡献:** 主要贡献是提出了一种通用框架，通过构建条件线性近似表（CLAT）和定义条件线性重量来量化约束数据子空间内的线性相关性，并基于此开发了一个基于MILP的自动搜索模型以寻找条件线性路径，从而系统地进行区分攻击和密钥恢复攻击。
>
> **达到效果:** 达到的效果是显著提高了对AES及其类似密码体制的安全分析能力，为设计更安全的加密算法提供了新的视角和技术手段。
>
> **技术梗概:** 技术梗概包括构建CLAT来量化约束数据子空间内的线性相关性，并通过条件线性重量平衡增益与过滤开销之间的关系，进而开发了基于MILP的自动搜索模型以寻找有效的攻击路径。

---
### [2026/1564] Splitting Bilinear Groups: New Translations from Composite- to Prime-Order with Applications to Batch Arguments for NP

- **作者:** David Balbás, Dario Fiore, Duy Nguyen

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1564) | [PDF](https://eprint.iacr.org/2026/1564.pdf)


> **研究背景:** 研究背景：双线性群，尤其是具有合数阶的双线性群，在许多应用中提供了额外的代数结构优势，但效率远低于素数阶的双线性群。
>
> **主要贡献:** 主要贡献：作者提出了一种新的翻译框架，基于一种称为对偶编码的新抽象概念，成功将基于合数阶的方案转换为素数阶设置，并构建了首个具有线性大小公共参考字符串（CRS）的批论证系统（BARG），同时保持了先前构造的统计位置可提取性。
>
> **达到效果:** 达到的效果：该工作填补了从合数阶到素数阶双线性群转换框架中的空白，实现了第一个基于素数阶双线性群且具有线性大小CRS的批论证系统，代表了向实际高效方案的重要一步。
>
> **技术梗概:** 技术梗概：通过引入对偶编码和功能加密来实现隐藏子群嵌入，模拟合数阶双线性群，并利用这些对象的安全属性（不可区分性）进行更强大的转换。

---
### [2026/1565] How to Back Up High-Value Secret Keys

- **作者:** Sanjam Garg, Noemi Glaeser, Abhishek Jain, Michael Lodder, Hart Montgomery

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1565) | [PDF](https://eprint.iacr.org/2026/1565.pdf)


> **研究背景:** 研究背景：现有解决方案如阈值钱包和冷钱包在处理罕见使用的高价值密钥时存在不足，因此需要一种新的备份系统来满足特定需求。
>
> **主要贡献:** 主要贡献在于提出了一种新颖的设计方案，允许用户方便地备份其不常用但价值高的密钥，并确保这些密钥的安全存储和恢复过程。
>
> **达到效果:** 达到的效果是提供了一个既能保证高价值密钥安全又能支持持续监控和主动刷新的系统，使得恢复密钥的过程类似于生成签名。
>
> **技术梗概:** 技术梗概采用了将用户密钥在大量托管者之间进行门限秘密共享的方法，并确保冷部分独立于用户数量且仅在恢复时访问。

---
### [2026/1567] Zero Knowledge Barcode Decoding with Application to Private Online Attribute Verification

- **作者:** Kelsey Merrill, Anna Woo, Wenting Zheng, Sarah Scheffler

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1567) | [PDF](https://eprint.iacr.org/2026/1567.pdf)


> **研究背景:** 现有在线属性验证方法通常会泄露过多个人信息，而隐私保护方案虽存在但需要数字输入形式。C2PA虽然能够提供图像来源的加密认证，但在证明解码算法正确执行方面面临挑战。
>
> **主要贡献:** 本文提出了一种基于Dorian zkSNARK系统的端到端PDF417条形码解码零知识证明方案，并进行了多项技术改进以适应实际应用。
>
> **达到效果:** 该系统能够高效验证图像来源并确保解码过程的正确性，同时减少了证明大小和验证时间，提高了隐私保护水平。
>
> **技术梗概:** 通过调整Dorian的多项式承诺机制来更有效地验证C2PA签名，并结合集合不交性、数据依赖处理以及状态机等技术工具。

---
### [2026/1571] LAMP: Linear Verification of Matrix Multiplication via Proximity Testing

- **作者:** Kyeongtae Lee, Byeongkyu Han, Jihye Kim, Hyunok Oh

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1571) | [PDF](https://eprint.iacr.org/2026/1571.pdf)


> **研究背景:** 在可验证计算系统中，直接对\(k \times k\)矩阵乘法进行SNARK算术化需要\(\mathcal{O}(k^3)\)约束，而Freivalds的随机检查可以将代数运算减少到向量-矩阵乘积，但这些乘积在SNARK内部证明仍然需要\(\mathcal{O}(k^2)\)约束。
>
> **主要贡献:** \$\textsf{LAMP}\$结合了Freivalds的随机检查和线性纠错码上的邻近测试，提出了一个矩阵乘法验证协议，显著减少了证明所需的工作量。
>
> **达到效果:** 对于固定数量\(t\)个采样位置，在电路中的主要SNARK关系有\(\mathcal{O}(tk)\)约束，并且额外需要\(\mathcal{O}(t\log n)+E_{\textsf{link}}(k)\)后端工作用于Merkle打开和CP-Link检查。在矩阵基准测试中，当\(k=2^{12}\)，\$\textsf{LAMP}\$减少了证明成本。
>
> **技术梗概:** 通过承诺编码后的矩阵和中间向量，并在采样查询位置确定之后进行计算，仅检查电路中的采样码字位置并承诺电路内部使用的值，确保一致性通过Merkle打开和CP-Link证明。

---
### [2026/1573] The Non-Intersecting Codewords Problem and its Application to MPC-in-the-Head Signatures

- **作者:** Pierre Briaud, Philippe Gaborit, Romaric Neveu, Gilles Zémor

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1573) | [PDF](https://eprint.iacr.org/2026/1573.pdf)


> **研究背景:** 自1978年McEliece提出首个基于码的加密方案以来，大多数基于码的密码构造依赖于解随机线性码（或其变体）或码等价这类难题。近年来，MPC-in-the-Head范式被用于构建新的数字签名方案，如NIST提交的Mirath、PERK、RYDE和SDitH，以及最近由Huth和Joux提出的基于子域双线性碰撞问题的新方案。
>
> **主要贡献:** 本文通过引入非交集码字（NIC）问题，丰富了基于码的MPC-in-the-Head签名方案。该问题是要求在汉明度量下找到两个给定线性码的码字，使得它们的支持不相交。
>
> **达到效果:** 研究表明，NIC问题可以用于构建具有竞争力的MPC-in-the-Head签名方案。使用通用构造方法，获得了比SDitH和PERK更小的签名大小，达到NIST安全级别I所需的2934字节。
>
> **技术梗概:** 通过讨论如何生成硬实例并研究几种攻击方式，本文展示了NIC问题在构建MPC-in-the-Head签名方案中的应用。

---
### [2026/1574] Privacy-Preserving Multi-Signatures: Achieving Transcript-Aware Privacy

- **作者:** Yanzibo Zhou, Fuchun Guo, Willy Susilo, Nan Li

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1574) | [PDF](https://eprint.iacr.org/2026/1574.pdf)


> **研究背景:** 针对多签名为保护隐私而设计的机制，现有方案在公钥重用时仅提供有限的隐私保护。然而，在实际部署中，多签名协议可能通过公开渠道执行，导致外部可见的签名脚本泄露，从而揭示签署者身份。
>
> **主要贡献:** 本文提出了转录感知隐私的新框架，并基于MuSig2-H方案设计了一种新的构造方法，以实现更强的隐私保护。
>
> **达到效果:** 该方案在AGM+ROM模型下实现了UNF-3不可伪造性，并且在签名仅设置中保持了完整的隐私性。
>
> **技术梗概:** 通过引入转录感知隐私的概念，明确建模对手对签名脚本的访问，并允许有限的信息泄露，如签署者集合大小，从而实现更强的隐私保护。

---
### [2026/1575] Solving the supersingular isogeny problem in time $p^{2/5+o(1)}$ using bivariate multipoint evaluation

- **作者:** Aleksei Udovenko

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1575) | [PDF](https://eprint.iacr.org/2026/1575.pdf)


> **研究背景:** 该研究针对超奇异Isogeny问题提出了一种新的无条件攻击方法，改进了现有技术的复杂度。
>
> **主要贡献:** 贡献在于通过结合多变量多项式评估算法，提出了时间复杂度为$p^{2/5+o(1)}$的新攻击方式。
>
> **达到效果:** 结果表明，在理论分析中该攻击比先前的方法更为高效，但实际应用中并不构成威胁。
>
> **技术梗概:** 技术上依赖于Kedlaya-Umans和Bhargava-Ghosh等人的多变量多项式评估算法。

---
### [2026/1577] SONIC: Concurrent Oblivious RAM & Data Structures for Low-Latency and High-Throughput

- **作者:** Nihal Talur, Ioannis Demertzis

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1577) | [PDF](https://eprint.iacr.org/2026/1577.pdf)


> **研究背景:** 传统依赖加密的隐私保护计算容易遭受泄漏滥用和访问模式攻击，而可信执行环境（TEE）虽成本效益高但也面临侧信道攻击。因此，通过使用隐蔽内存访问模式和侧信道信息的机密原语（如ORAM和ODS），可以有效缓解这些风险。
>
> **主要贡献:** 本文提出了SONIC，一种结合了低延迟与高吞吐量特性的同时透明RAM及数据结构方案，旨在弥合低延迟设计与高吞吐量设计之间的差距。
>
> **达到效果:** 实验结果显示，SONIC在保持低延迟的同时显著提高了系统的吞吐量，并且能够在实际应用中支持大规模数据集的高效处理。
>
> **技术梗概:** SONIC采用了一种新颖的设计方法，通过动态调整ORAM和ODS的并行度来平衡延迟与吞吐量，从而实现了在不同应用场景下的灵活配置与优化。

---
### [2026/1578] OpenLLM: Modular and Scalable zkSNARKs for Verifiable LLM  Inference

- **作者:** Yunbo Yang, Yupeng Ren, Changtong Xu, Rui Zhang, Xuanming Liu, Jin Tan, Tao Wei, Bingsheng Zhang, Kui Ren

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1578) | [PDF](https://eprint.iacr.org/2026/1578.pdf)


> **研究背景:** 在远程服务中部署的大规模语言模型（LLM）引发了执行正确性的信任问题，而现有方法难以实现大规模的端到端验证。
>
> **主要贡献:** OpenLLM通过将大规摸LLM推理分解为可重用的基本算子，并为这些算子设计高效的零知识证明协议，实现了模块化和可扩展的验证。
>
> **达到效果:** OpenLLM能够在保持计算效率的同时，提供对LLM推理过程的有效验证，显著提升了系统的实用性和可靠性。
>
> **技术梗概:** 该系统基于原子操作级别的零知识证明构建，通过组合这些证明来实现复杂推理任务的端到端验证。

---
### [2026/1579] Slipway: Accessing Finite Subspace Trails in Poseidon

- **作者:** Giuseppe Vitto

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1579) | [PDF](https://eprint.iacr.org/2026/1579.pdf)


> **研究背景:** 研究背景：Poseidon是一种用于高效证明系统的代数置换，其非线性层由幂映射S盒组成。论文探讨了有限子空间路径的存在及其对密码学分析的影响。
>
> **主要贡献:** 主要贡献：作者通过构造受约束的输入家族和依赖轮常量的MDS矩阵，解决了如何使受约束的家庭在进入部分轮之前不首先获得高代数度的问题，并提出了“全轮吸收”效应的概念。
>
> **达到效果:** 达到的效果：证明了对于KoalaBear实例存在有限子空间路径，并展示了如何利用这些路径进行密码分析。
>
> **技术梗概:** 技术梗概：通过引入受约束的输入家族和依赖轮常量的MDS矩阵，论文实现了从全轮前缀到选择的有限轨迹的有效转换，从而避免了代数度的增长。

---
### [2026/1583] Cryptanalysis of a Candidate Witness Encryption Scheme for Aﬃne Determinant Programs

- **作者:** Sunghyeon Jo

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1583) | [PDF](https://eprint.iacr.org/2026/1583.pdf)


> **研究背景:** 该研究针对2020年ITCS会议上提出的一种基于仿射行列式程序的候选者证人加密方案进行了破译分析。
>
> **主要贡献:** 作者利用通utator技术，提出了一个确定性的多项式时间攻击方法，能够从公共密文矩阵中恢复隐藏的位信息。
>
> **达到效果:** 该攻击适用于所有$q \geq 1$的情况，并且在特定条件下可以几乎完全成功地破解加密方案。
>
> **技术梗概:** 研究采用了通utator技术来恢复隐藏的列空间，并通过这种方法实现了对候选证人加密方案的有效破译。

---
### [2026/1584] Beyond Affine Invariants: A Hamming-Weight Correlation Metric for Template-CPA Leakage in Key-Dependent S-boxes

- **作者:** Wiesław Maleszewski

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1584) | [PDF](https://eprint.iacr.org/2026/1584.pdf)


> **研究背景:** 传统的密码学S盒选择标准（如非线性度、差分均匀性和泡尔特定理均匀性）是仿射等价的不变量，这使得它们无法检测到一类侧信道弱点。
>
> **主要贡献:** 作者提出了Hamming权重模板相关性指标$\rho_{\mathrm{HW}}(\cdot,S_{\mathrm{AES}})$，并证明了它能够区分在密钥依赖S盒中基于模板的CPA攻击。
>
> **达到效果:** 通过引入新的度量标准，研究揭示了经典不变量无法捕捉到的信息，并展示了其对不同随机性来源的密钥依赖S盒的有效性。
>
> **技术梗概:** 研究利用了代数几何和统计学方法来分析密钥依赖S盒在模板CPA攻击下的行为差异。

---

## 更新: 2026-07-28 08:33

*新增 12 篇论文 (编号 1512--1524)*

### [推荐] [2026/1512] The McEliece Cryptosystem After Nearly Five Decades: A Survey of Security, Cryptanalysis, and Future Directions

- **匹配关键字:** post-quantum

- **作者:** Shabnam Jafarzade Mojaveri, Adel Khosravi

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1512) | [PDF](https://eprint.iacr.org/2026/1512.pdf)


> **研究背景:** 麦凯尔维密码系统自引入以来近五十年，尽管存在公钥大小和参数安全性等问题，其基于二元Goppa码的核心结构仍能抵抗已知的实际攻击。
>
> **主要贡献:** 文章回顾了该系统的演进，并对五十多年来的各种攻击进行了分类整理，强调了不同攻击之间的区别。
>
> **达到效果:** 研究表明，尽管存在多种攻击和参数更新，Classic McEliece仍然保持其安全性，这反映了其保守的数学基础。
>
> **技术梗概:** 通过将攻击分为通用解码、结构恢复、紧凑变体攻击等类别来系统地分析了该系统的安全性。

---
### [推荐] [2026/1523] Catching Many Traitors in Threshold Traitor Tracing: Lower Bounds and Constructions

- **匹配关键字:** LWE

- **作者:** Dan Boneh, Aditi Partap, Mark Zhandry

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1523) | [PDF](https://eprint.iacr.org/2026/1523.pdf)


> **研究背景:** 传统的$t$-out-of-$n$门限解密方案通过将解密密钥分发给$n$个参与方，使得任何$t$个参与方可以联合解密一个密文，而少于$t$个的参与方则无法获取明文信息。然而，这些传统方案缺乏追踪能力：一个包含$t$或更多参与方的联盟可以通过组合他们的密钥份额构建一个海盗解码器来解密任意有效的密文，而不必担心被追溯。
>
> **主要贡献:** - 该研究提出了门限追踪下界：所有现有追踪技术的密文大小必须足够大以允许接近追踪$t$个参与者。- 研究还提供了上界结果，并探讨了指纹编码在其中的作用。
>
> **达到效果:** 研究表明，要追踪$t-O(1)$个参与者，密文大小至少为$\Omega(t)$；要追踪$a \leq t-\omega(1)$个参与者时，密文大小需按比例缩放至$\Omega(\frac{a-1}{t-a+1})$。对于依赖指纹编码的方案，研究还给出了更强的下界结果。
>
> **技术梗概:** 该研究通过数学证明和理论分析来确定追踪门限下的密文大小限制，并探讨了不同技术在实现这些限制时的有效性。

---
### [推荐] [2026/1524] A Note on Single-Server QPIR from One-Way Functions

- **匹配关键字:** post-quantum

- **作者:** Prabhanjan Ananth, Divyanshu Bhardwaj, Aditya Gulati

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1524) | [PDF](https://eprint.iacr.org/2026/1524.pdf)


> **研究背景:** 研究背景：本文探讨了在假设存在后量子单向函数的情况下，如何实现具有多项对数通信量的单服务器量子私有信息检索（QPIR）系统。
>
> **主要贡献:** 主要贡献：作者通过结合[Kerenidis-Wolf STOC'03]中的编译技术和[Gilboa-Ishai EUROCRYPT'14]中的分布式点函数，证明了在后量子安全性的假设下可以实现单服务器QPIR。
>
> **达到效果:** 达到的效果：该研究提供了一种基于后量子单向函数的高效单服务器QPIR方案，通信复杂度为多项对数级。
>
> **技术梗概:** 技术梗概：本文的技术核心在于利用编译技术和分布式点函数来构造高效的QPIR协议，并确保其在量子计算环境下的安全性。

---
### [2026/1513] STEBR: A Timed-Erasure, Threshold-Gated Backup Ratchet

- **作者:** Shaurya Pratap Singh

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1513) | [PDF](https://eprint.iacr.org/2026/1513.pdf)


> **研究背景:** 现有的加密备份通常依赖单一静态密钥，一旦泄露会暴露所有历史数据，缺乏自我恢复机制。
>
> **主要贡献:** STEBR通过引入自毁哈希链、阈值秘密共享和交互式确认门控技术，解决了这一问题。
>
> **达到效果:** 该方案能够在密钥泄露时仅暴露有限的历史记录，并且需要多因素验证才能访问备份数据。
>
> **技术梗概:** STEBR采用自毁哈希链确保密钥泄露的影响范围受限，使用阈值秘密共享增强安全性，通过交互式确认机制防止单点攻击。

---
### [2026/1514] Distributed Vector Commitments and Their Applications

- **作者:** Rui Gao, Huaqun Wang, Zhiguo Wan, Yuncong Hu

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1514) | [PDF](https://eprint.iacr.org/2026/1514.pdf)


> **研究背景:** 现有的向量承诺（VC）方案主要用于中心化环境，无法应用于分布式系统，特别是在输入向量分布在多个节点的情况下。
>
> **主要贡献:** 提出了首个适用于分布式系统的向量承诺（DVC）方案HLE-DVC，并解决了在分布式环境中承诺和证明依赖于整个向量的问题。
>
> **达到效果:** HLE-DVC实现了紧凑的证明大小为O(log M)，并允许每个节点在一个通信轮次内生成其所有位置证明，具有较低的通信成本和计算成本。此外，该方案支持批量验证、证明聚合以及高效更新。
>
> **技术梗概:** 通过利用M个节点并行处理长度为N的分布式向量v，其中每个节点持有长度为N/M的子向量来实现上述目标。

---
### [2026/1515] Exponentially Fewer-Server PIR from Sparser $S$-Decoding Polynomials

- **作者:** Aparna Gupte, Seyoon Ragavan

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1515) | [PDF](https://eprint.iacr.org/2026/1515.pdf)


> **研究背景:** 本文研究了如何通过减少服务器数量来实现私有信息检索（PIR），特别是在指数级通信复杂度下的优化。
>
> **主要贡献:** 作者提出了一种新的$s$-server PIR协议，其通信复杂度为$\exp(O((\log n)^{1/s} (\log \log n)^{1-1/s}))$，显著减少了所需的服务器数量。
>
> **达到效果:** 该研究不仅在理论上证明了新协议的有效性，还通过实验证明了其可行性，并使结果在$s \leq 15$时无条件成立。
>
> **技术梗概:** 作者利用匹配向量家族和特殊稀疏的$S$-解码多项式构造方法，解决了先前提出的开放问题，并进一步优化了通信复杂度。

---
### [2026/1516] Complex-Multiplication Terminals for Supersingular Isogeny Path-Finding

- **作者:** Zheng Tao, Zhi Hu, Yijing Zhang, Changan Zhao

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1516) | [PDF](https://eprint.iacr.org/2026/1516.pdf)


> **研究背景:** 该研究针对超奇异伊瑟戈尼路径寻找中的子域搜索阶段提出了基于复乘法（CM）的补充停止策略，以解决Delfs-Galbraith/SuperSolver算法中的瓶颈问题。
>
> **主要贡献:** 作者通过引入预计算的CM超奇异顶点集来扩展可识别终端集合，并提出了一种新的停止策略，从而提高了路径寻找效率。
>
> **达到效果:** 该方法在不改变基本搜索复杂度的前提下，显著减少了子域搜索的时间和资源消耗，同时保持了算法的整体性能。
>
> **技术梗概:** 通过构造CM终端表并利用希尔伯特类多项式的根来识别额外的超奇异顶点，从而实现高效的路径寻找。

---
### [2026/1517] ConvertInput-Free Vector Homomorphic Secret Sharing and Its Applications

- **作者:** Fan Yang, Fucai Luo, Xingfu Yan, Haining Yang, Zheng Gong, Wing W. Y. Ng

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1517) | [PDF](https://eprint.iacr.org/2026/1517.pdf)


> **研究背景:** 该研究提出了一种基于决策复合残余性（DCR）假设的新颖向量同态秘密共享方案（Vector-HSS），旨在高效执行高维向量计算，同时避免昂贵的ConvertInput操作。
>
> **主要贡献:** 主要贡献在于引入了以向量为中心的计算范式，并通过Chinese Remainder Theorem (CRT)开发了一种批处理机制，支持并行跨多个向量的计算。
>
> **达到效果:** 实验结果表明，该方案在批量内积评估中分别比先前的HSS方案快280倍和70倍。应用于隐私保护图像检索时，实现了亚秒级检索时间，并表现出色。
>
> **技术梗概:** 技术梗概包括向量同态秘密共享的设计、批处理机制以及支持欧几里得/余弦距离计算、比较和矩阵-向量乘法的安全协议模块。

---
### [2026/1518] Practical Equivalent-Key Recovery in GRAFHEN

- **作者:** Remi Geraud-Stewart

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1518) | [PDF](https://eprint.iacr.org/2026/1518.pdf)


> **研究背景:** GRAFHEN是一种基于群的同态加密提案，其公钥为重写系统，而私钥则为置换表示。
>
> **主要贡献:** 作者提出了一种名为Maverick的等效密钥恢复攻击方法。
>
> **达到效果:** 该攻击成功破解了所有已发布的GRAFHEN挑战实例，并且在推荐参数下重建了一个等效密钥并正确解密了20,000个标签化的密文。
>
> **技术梗概:** Maverick通过将选定的公有重写规则转换为群关系，使用Todd-Coxeter枚举方法重构正则作用，识别置换表示，并利用公共标记加密来校准等效解密器。

---
### [2026/1519] Classic Full Plaintext Recovery Attacks on Low Round Generalized Feistel Networks

- **作者:** Yubing Zhu, Jianhong Shi, Yunteng Yang, Yonghui Yang

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1519) | [PDF](https://eprint.iacr.org/2026/1519.pdf)


> **研究背景:** 研究背景：通用分组Feistel网络（GFN）是广泛标准化的块密码的基础，但其对全明文恢复攻击的抵抗力尚未得到充分探索。
>
> **主要贡献:** 主要贡献：作者扩展了标准Feistel密码的全明文攻击框架至多分支场景，并针对Type-I和Type-II GFN提出了经典全明文恢复攻击方法。
>
> **达到效果:** 达到的效果：对于Type-I GFN，提出了一种在选择明文攻击（CPA）下的$d+1$次加密查询及在 chosen-ciphertext attack （CCA）下$d$次解密查询的攻击；对于Type-II GFN，在CPA下$3$次加密查询和CCA下$1$次加密加$2$次解密查询的攻击。
>
> **技术梗概:** 技术梗概：所有攻击将轮函数视为黑盒处理，表明弱点在于GFN拓扑而非具体的设计。

---
### [2026/1520] Quintus: Two-round Good-case Information Theoretic BFT for $n=5f+1$

- **作者:** Chenyang Liu, Dahlia Malkhi, Kartik Nayak, Nibesh Shrestha

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1520) | [PDF](https://eprint.iacr.org/2026/1520.pdf)


> **研究背景:** Quintus旨在为$n=5f+1$的多方环境提供信息论意义上的拜占庭容错（BFT）协议，以应对少于$f < n/5$个拜占庭故障。
>
> **主要贡献:** Quintus提出了两种协议：Quintus-Fixed和Quintus-Responsive，分别在固定视图和响应式视图下优化了消息复杂度和延迟。
>
> **达到效果:** Quintus-Fixed实现了在乐观情况下$O(n^2)$的消息复杂度和$2\delta$的时间延迟；而Quintus-Responsive则提供了$2\Delta + 2\delta$的最坏情况视图延迟，并保持了$O(n^2)$的消息效率。
>
> **技术梗概:** Quintus通过在固定视图中使用3$\Delta$时间周期来推进视图，以及在响应式视图中优化消息传递机制，实现了上述性能目标。
...
- **链接:** [论文](https://eprint.iacr.org/2026/1522) | [PDF](https://eprint.iacr.org/2026/1522.pdf)


> **研究背景:** 在BLS27曲线上计算最优Ate对偶对于实现256位安全级别至关重要，特别是在最近Number Field Sieve及其变体(exTNFS, SexTNFS)进展的背景下。传统的二进制方法未能充分利用\(\Fpk{27}\)的三级扩展塔。
>
> **主要贡献:** 作者提出了一种高效的三进制Miller循环版本，并将种子表示限制为稀疏的三进制数字\(\{0, 1\}\)，以简化点运算并消除昂贵的除法。此外，还生成了两个新的参数种子，分别针对exTNFS和SexTNFS安全级别。
>
> **达到效果:** 与Fouotsa等人(2020)提出的二进制方法相比，在总体最优Ate对偶计算成本上取得了22%的改进。同时，所提SexTNFS种子确保了更高的安全性，能够抵御最先进的NFS变体。
>
> **技术梗概:** 通过限制种子表示为稀疏三进制数字来简化点运算，并在Miller循环中消除昂贵的除法操作；生成两个新的参数种子以适应exTNFS和SexTNFS的安全级别要求。

---

## 更新: 2026-07-25 10:27

*新增 17 篇论文 (编号 1493--1511)*

### [推荐] [2026/1493] Bob DyLean: A Framework for the Symbolic Analysis of Cryptographic Protocols in Lean

- **匹配关键字:** post-quantum

- **作者:** Théophile Wallez, Cas Cremers

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1493) | [PDF](https://eprint.iacr.org/2026/1493.pdf)


> **研究背景:** 近年来，形式化的Dolev-Yao方法被广泛应用于安全协议的分析中，为诸如TLS 1.3、电子投票协议、EMV和MLS等广泛应用的安全系统提供了有效的验证手段。然而，现有的工具如Tamarin和ProVerif虽然提供了自动化支持用户自定义的等式理论，但缺乏灵活性；而DY*则较为灵活且模块化，但固定了威胁模型并不支持自定义等式理论。
>
> **主要贡献:** DyLean框架在Lean定理证明器中实现了通用形式化的安全协议分析方法，并允许用户自定义语义和等式理论。此外，它还提供了一种基于迹不变量的证明方法，继承了DY*的优点并进一步增强了其灵活性和可扩展性。
>
> **达到效果:** DyLean框架能够处理包含未绑定循环或数据结构的协议分析，并在多种场景下组合安全证明。通过允许用户定义等式理论和威胁模型，提高了协议分析的灵活性和适用范围。
>
> **技术梗概:** DyLean采用了灵活的一般性形式化语义，并结合了基于迹不变量的具体证明方法，使得其既具有DY*的优点又增强了对自定义理论的支持。

---
### [推荐] [2026/1494] On $k$-way split multiplication algorithms

- **匹配关键字:** lattice, post-quantum

- **作者:** Mehmet Özgün Cihangir, Oğuz Yayla

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1494) | [PDF](https://eprint.iacr.org/2026/1494.pdf)


> **研究背景:** 该研究聚焦于高效多项式乘法和Toeplitz矩阵向量乘积（TMVP）算法，这些算法在计算代数与现代密码学中至关重要。特别是在基于格的后量子密码学（PQC）方案中，利用Number Theoretic Transform (NTT)-不友好的环需要高度优化的亚二次时间复杂度乘法算法。
>
> **主要贡献:** 研究提出了广义k-方式分裂多项式乘法和TMVP算法的一般化数学框架，并构造了广义k-方式Schoolbook和Karatsuba乘法算法，引入了一种新颖的利用最优评估点和矩阵行反转技术的k-方式TMVP算法。
>
> **达到效果:** 研究证明了这种广义公式严格达到了理论插值下界，需要恰好2k-1个子问题，并且具有亚二次渐近复杂度O(n^{\log_k(2k-1)})。此外，确定了k-方式Karatsuba和Schoolbook算法的最佳连续应用序列，证明了峰值效率完全由n的质因数分解驱动。
>
> **技术梗概:** 研究通过构建广义k-方式乘法算法并引入新型TMVP算法，利用数学证明方法严格达到理论下界，并确定算法优化策略以实现最佳性能。

---
### [推荐] [2026/1495] Oblivious Sorting under Fully Homomorphic Encryption: A Comprehensive Survey and Performance Analysis

- **匹配关键字:** homomorphic encryption

- **作者:** Omar Ahmed, Rostin Shokri, Nektarios Georgios Tsoutsos

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1495) | [PDF](https://eprint.iacr.org/2026/1495.pdf)


> **研究背景:** 在外包计算至云服务提供商时，数据隐私成为主要问题，促使研究者转向全同态加密(FHE)以保护数据隐私。然而，在FHE环境中实现排序算法带来了显著的性能挑战。
>
> **主要贡献:** 本文综述了基于FHE的排序算法现状，并提出了一种新的复杂性度量标准——FHE-Effort，用于评估同态电路效率。此外，还对18种不同算法在三种主要FHE方案中的表现进行了基准测试。
>
> **达到效果:** 研究结果表明，在当前技术条件下，TFHE是最适合排序应用的FHE方案；同时，奇偶归并排序网络和Bitonic排序网络提供了最优的算法架构选择。
>
> **技术梗概:** 通过引入FHE-Effort度量标准，并使用统一代码库对多种算法进行跨方案基准测试，本文为FHE环境下的高效排序设计提供了指导。

---
### [推荐] [2026/1500] How to Define Expected Quantum Polynomial-Time Zero Knowledge Simulation

- **匹配关键字:** post-quantum

- **作者:** Zhengnan Lai, Nicholas Spooner, Max Tromanhauser

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1500) | [PDF](https://eprint.iacr.org/2026/1500.pdf)


> **研究背景:** 研究背景：在后量子计算时代，为了正确模拟量子验证者，需要将模拟器升级至量子多项式时间（QPT）。然而，Chia等人证明了一个负面结果，表明对于常数轮协议，既要求严格QPT又要求自然的期望量子多项式时间（EQPT）黑盒模拟是不可能实现非平凡零知识（ZK）的。因此，Lombardi等提出了一个新颖的EQPT概念——相干运行时EQPT（EQPT_c），并证明了[GMW86,FS90,GK96]协议允许EQPT_c模拟。
>
> **主要贡献:** 主要贡献：作者识别出EQPT_c定义中的根本问题，并提出了解决方案。具体而言，他们展示了EQPT_c计算不一定是高效的，并且实际上可以决定任何经典决策问题。通过谨慎限制选择有效的量子信道的酉扩张方式，作者提出了修订后的定义，并证明了该定义保留了[GMW86,FS90,GK96]协议的零知识特性。
>
> **达到效果:** 达到的效果：修订后的EQPT_c定义确保了在后量子计算环境中，经典零知识协议依然能够保持其零知识性质。这一工作为设计和实现安全有效的零知识证明系统提供了新的理论基础。
>
> **技术梗概:** 技术梗概：作者通过分析EQPT_c的自由度问题，并提出限制选择有效量子信道酉扩张方式的方法来修订定义。这种方法确保了在保留协议零知识特性的同时，提高了模拟器的效率。

---
### [推荐] [2026/1502] Efficient Privacy-Preserving LSTM Inference on Encrypted Sequential Data

- **匹配关键字:** homomorphic encryption

- **作者:** Qiang He, Jingwei Chen, Wenyuan Wu, Yong Feng

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1502) | [PDF](https://eprint.iacr.org/2026/1502.pdf)


> **研究背景:** 随着全同态加密(FHE)技术的发展，直接在加密数据上进行隐私保护的机器学习成为可能。然而，现有的基于FHE的LSTM推理方案仍然存在高延迟和可扩展性差的问题。
>
> **主要贡献:** 本文提出了一种基于FHE的高效隐私保护LSTM推理协议，通过引入轻量级归一化模块并结合混合Remez和最小二乘策略实现Sigmoid、Tanh和反平方根函数的低阶多项式近似，从而在保持准确性的前提下提高了效率。
>
> **达到效果:** 实验表明，在标准文本分类基准上，所提出的加密LSTM模型与明文模型相比具有竞争力，并且始终优于最先进的FHE方法，速度提升最高可达4.7倍。
>
> **技术梗概:** 该协议通过使用Lattigo库实现，并结合了密文打包优化、旋转最小化和SIMD并行化技术来提高性能。

---
### [推荐] [2026/1506] SM4th and uBlockith: VOLE-based Post-Quantum Signature Schemes from Chinese Block Ciphers

- **匹配关键字:** post-quantum

- **作者:** Weihan Li, Yuchen Wang, Zhelei Zhou, Cheng Hong, Tao Wei

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1506) | [PDF](https://eprint.iacr.org/2026/1506.pdf)


> **研究背景:** FAEST是一种基于VOLE-in-the-Head的后量子签名方案家族，是九个进入NIST额外数字签名过程第三轮候选名单的方案之一。
>
> **主要贡献:** 作者提出了SM4th和uBlockith两个后量子签名方案系列，分别针对128位和256位的经典安全性，并使用中国设计的块密码进行实例化。
>
> **达到效果:** 评估结果显示，在Intel平台上，带有内置SM4支持的SM4th变体性能与相应的FAEST-128变体相当；而在Hygon平台上，SM4th变体比FAEST-128慢约3倍。uBlockith系列方案则大约慢3到4倍于FAEST-256。
>
> **技术梗概:** 该方案基于FAEST框架，并针对中国设计的块密码（如SM4、uBlock和Ballet）定制了约束系统，提供了指令集意识优化实现。

---
### [2026/1496] Floor-IT: Information-Theoretic BFT in Partial Synchrony with Two Round Good Case Latency and Optimal Resilience

- **作者:** Ittai Abraham, Yuval Efron, Jovan Komatovic, Alejandro Ranchal-Pedrosa, Gilad Stern

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1496) | [PDF](https://eprint.iacr.org/2026/1496.pdf)


> **研究背景:** 研究背景：在信息论模型中，各方通过发送者认证的点对点信道进行通信，但不使用数字签名或其他可转移的加密证书；对手是计算能力无限制的。
>
> **主要贡献:** 主要贡献：Floor-IT协议是一个适用于部分同步环境的信息论拜占庭容错协议，在良好情况下延迟为两轮，并实现了在该设置下的最优鲁棒性界限$n = 5f - 1$。
>
> **达到效果:** 达到的效果：当实际网络延迟$\delta \leq \Delta$后，协议实现稳健的良好情况延迟$2\delta$；最坏情况下视图延迟不超过$2\Delta + 2\delta$。每个节点仅需$O(1)$大小的持久存储，每次视图涉及$O(n^2)$条消息，每条消息大小为$O(1)$。
>
> **技术梗概:** 技术梗概：Floor-IT协议通过在视图中操作和保证最坏情况下的视图延迟来实现其目标，同时优化了节点的存储需求和消息传递效率。

---
### [2026/1497] Updatable Private Set Union: Generic Construction with Efficient Instantiation

- **作者:** Seongbong Choi, Hyung Tae Lee

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1497) | [PDF](https://eprint.iacr.org/2026/1497.pdf)


> **研究背景:** 现有的私有集合并集（PSU）协议在任一参与方的数据集发生变化时，必须从头重新计算整个并集，这在数据频繁更新的情况下变得非常低效。
>
> **主要贡献:** 该研究提出了首个支持高效增量更新的标准双方可更新私有集合并集（uPSU）协议，并对其可能的更新场景进行了系统分类。
>
> **达到效果:** 通过这种分类和泄漏基线分析，研究人员提供了一种基于现有PSI和标记变体PSU构建块的通用构造方法，证明了其在模拟模型中的半诚实对手安全性，并确保增量更新不会泄露比标准PSU协议更多关于更新后集合的信息。
>
> **技术梗概:** 该研究通过分类所有可能的更新场景并建立泄漏基线来实现高效更新，同时利用现有的PSI和标记变体PSU作为构建块进行构造。

---
### [2026/1498] Multilevel Amortized Gaussian Elimination in Information-Set Decoding: Applications to HQC and PCG

- **作者:** Kévin Carrier, Valérian Hatey, Laura Luzzi, Jean-Pierre Tillich

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1498) | [PDF](https://eprint.iacr.org/2026/1498.pdf)


> **研究背景:** 针对依赖于亚线性解码难题的密码系统，已知的最佳攻击方法基于信息集解码(ISD)，但在特定应用如HQC和PCG中，高斯消元的成本变得不可忽略，影响了整体攻击复杂度。
>
> **主要贡献:** 作者改进并扩展了Kim和Lee的Reduce-and-Prange技术，并提出MAGE-Stern算法，这是一种Stern算法的多级摊销高斯消元变体。
>
> **达到效果:** MAGE-Stern显著降低了HQC的最佳已知攻击的时间复杂度约3位，并减少了内存复杂度约12位。对于标准化的HQC类别I参数集，估计安全性约为140位，低于NIST的安全目标约3位。
>
> **技术梗概:** 通过分支过程技术重新评估了部分重用主元的技术复杂性，并将其应用于Stern算法以提高效率。

---
### [2026/1499] BF²: A Bloom-Filtered Brute-Force Framework for Multi-Target Password Recovery

- **作者:** Cansu Karakuzu Aslan, Wenzel Pünter, Christian Dörr

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1499) | [PDF](https://eprint.iacr.org/2026/1499.pdf)


> **研究背景:** 针对大规模泄露哈希集的离线暴力破解攻击，尤其是多目标攻击，需要高效且可扩展的框架来评估实际密码安全性。
>
> **主要贡献:** BF²框架通过结合密码候选生成器、流水线NT哈希核心、布隆过滤器阶段以及主机侧多线程组件，实现了多目标密码恢复加速。
>
> **达到效果:** BF²在低成本NiteFury II板上实现，16个并行管道以100 MHz频率运行时，每秒生成$1.6	imes10^9$个哈希值，实验显示其吞吐量比John the Ripper高7.5倍，并将功耗降低了90%。
>
> **技术梗概:** BF²采用布隆过滤器快速排除不匹配的候选密码，利用完美哈希函数进行精确成员检查，同时在FPGA和CPU之间进行了模块化设计以提高灵活性和可扩展性。

---
### [2026/1501] Phishing in the Noise: Analysis of CT-based Phishing Detection Performance on Free Hosting Platforms

- **作者:** Maksymilian Nowak, Wojciech Mazurczyk, Ewa Syta

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1501) | [PDF](https://eprint.iacr.org/2026/1501.pdf)


> **研究背景:** 研究探讨了免费托管平台（FHP）上的证书透明度（CT）基于的钓鱼网站检测性能，分析了X.509、CT和URL特征在不同类型的网站之间的差异。
>
> **主要贡献:** 贡献在于揭示了托管提供商特性对钓鱼网站指标的影响，并指出现有CT基钓鱼检测方法在面对这种部署环境时存在的局限性。
>
> **达到效果:** 结果表明，托管平台状态比钓鱼状态更强烈地关联于提取的X.509、CT和URL特征，且相关性最高达到0.54，远超钓鱼相关的0.31。
>
> **技术梗概:** 研究通过分析不同类型的网站（FHP钓鱼、FHP良性、非FHP钓鱼及主流良性）的数据，并将证书级数据与域名级数据分开进行关联分析来实现上述目标。

---
### [2026/1503] The Consensus Number of Untraceable Cryptocurrencies

- **作者:** Christian Cachin, David Lehnherr, Juan Villacis, François-Xavier Wicht

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1503) | [PDF](https://eprint.iacr.org/2026/1503.pdf)


> **研究背景:** 研究了不可追踪加密货币中发送者不可追踪性对同步的影响，通过区分线性和常量状态方案来探讨其在共识层次中的位置。
>
> **主要贡献:** 提出了线性和常量状态的不可追踪资产转移对象（LUAT和CUAT），并分析了它们在不同不可追踪性假设下的共识数量。
>
> **达到效果:** 发现线性不可追踪资产转移对象具有2的共识数，而常量状态不可追踪资产转移对象在弱不可追踪性下共识数无界，在强不可追踪性下则与共享掩码集的账户历史相关。
>
> **技术梗概:** 通过形式化定义和冲突图来分析不同设计对同步的影响，并探讨了垃圾回收机制以优化存储效率。

---
### [2026/1505] Conditional-Affine Redundant Clauses for SHA-256 Differential SAT

- **作者:** Jiqiang Feng, Kun Gao

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1505) | [PDF](https://eprint.iacr.org/2026/1505.pdf)


> **研究背景:** 研究针对SHA-256非线性函数Ch和Maj的标准Tseitin编码，探讨如何通过冗余条件子句增强布尔约束传播（BCP）的效果，以揭示受控差分投影。
>
> **主要贡献:** 提出了一种基于条件仿射冗余子句的方法，用于表征所有控制下的差分线性形式，并生成了特定数量的Ch和Maj子句，旨在加强BCP过程。
>
> **达到效果:** 实验结果显示，这种机制在某些情况下确实提高了性能，但并未证明能普遍加速求解器或产生新的密码分析攻击。
>
> **技术梗概:** 通过应用一个定理来识别所有最小控制投影，并将其转化为具体的CNF子句，这些子句不引入新变量且严格增强了BCP过程。

---
### [2026/1507] Analyzing Cryptography in Context: A Cryptography-Native Approach to Threat Modeling

- **作者:** Ran Canetti, Julie Ha, Gabriel Kaptchuk

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1507) | [PDF](https://eprint.iacr.org/2026/1507.pdf)


> **研究背景:** 现有密码学规范未要求分析人员记录部署系统应具备的社会技术属性，本文旨在通过开发框架将社会技术维度引入对加密系统的分析中，特别是在基于广泛接受的密码学建模技术进行‘情境化’分析方面提供支持。
>
> **主要贡献:** 作者提出了一种新的框架，能够使社会技术因素在密码学系统分析中得到考虑，并以苹果2021年CSAM扫描提案为例验证了该框架的有效性。
>
> **达到效果:** 通过使用该框架，研究团队揭示了许多对苹果提案的批评意见，并识别出了提案中的一个未被记录的重要属性。
>
> **技术梗概:** 该框架结合了广泛接受的密码学建模技术，并引入了社会技术维度进行‘情境化’分析，从而提高了分析的全面性和准确性。

---
### [2026/1509] Efficient Unclonable Encryption from Pauli Eigenstates

- **作者:** Seyoon Ragavan

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1509) | [PDF](https://eprint.iacr.org/2026/1509.pdf)


> **研究背景:** 本文提出了首个基于Pauli本征态的一次性信息论安全性高效不可克隆加密方案，用于单个经典位的加密。
>
> **主要贡献:** 该工作避免了以往研究中安全损失和效率低下的问题，首次在标准模型下实现了多项式长度消息的多次使用安全的1到2不可克隆加密。
>
> **达到效果:** 此方案通过随机非恒等相位Pauli算子实现，加密与解密操作复杂度为O(n)，且具有指数安全性，恢复比特的概率上限为1/2 + O(2^(-n/2))。
>
> **技术梗概:** 该技术利用了在精确谱意义上平衡的交换-反对称结构，从而实现了高效的安全性保证。

---
### [2026/1510] Quantum Lazy Sampling and Path Recording for Any Group

- **作者:** Ben Foxman, Alex Lombardi, Fermi Ma, Barak Nehoran, John Wright

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1510) | [PDF](https://eprint.iacr.org/2026/1510.pdf)


> **研究背景:** 研究背景：在量子算法分析和密码学中，如何高效模拟具有随机群元素或acles访问权限的算法是一个核心挑战。经典方法通过懒惰采样来处理这类问题，而本文旨在提出一种适用于任何闭合子组的量子版本懒惰采样技术。
>
> **主要贡献:** 主要贡献：作者定义并分析了一种通用且可解释的路径记录oracle，能够完美模拟任意封闭子组$U(N)$中的随机元素。这种路径记录oracle存储了$t$个输入输出对的叠加态，透明地记录了算法可能从查询中学到的信息。
>
> **达到效果:** 达到的效果：该工作提供了一种新的量子数据结构，使得在理论上可以更清晰地理解并分析具有随机群元素访问权限的量子算法的行为和信息获取情况。
>
> **技术梗概:** 技术梗概：通过构建一种路径记录oracle，作者实现了对查询过程的透明记录，并且这种oracle能够适用于任何封闭子组中的随机元素模拟。

---
### [2026/1511] Unconditional Unclonable Encryption

- **作者:** Prabhanjan Ananth, Amit Sahai

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1511) | [PDF](https://eprint.iacr.org/2026/1511.pdf)


> **研究背景:** 本文提出了首个基于信息论完全安全的一次性私钥不可克隆加密方案，适用于单比特消息的加密与解密。
>
> **主要贡献:** 该研究首次构建了此类加密方案，并证明其具有指数级小的不可克隆区分优势。
>
> **达到效果:** 实验表明，在理想条件下，该方案能够提供无条件的安全性保障。
>
> **技术梗概:** 通过引入新的技术方法，实现了高效的加密与解密过程。

---

## 更新: 2026-07-24 19:21

*新增 16 篇论文 (编号 1476--1492)*

### [推荐] [2026/1483] MQ on my Hardware: Performance Analysis of MQOM on FPGA

- **匹配关键字:** lattice, post-quantum

- **作者:** Stelios Manasidis, Quinten Norga, Suparna Kundu, Ingrid Verbauwhede

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1483) | [PDF](https://eprint.iacr.org/2026/1483.pdf)


> **研究背景:** 近期在MPCitH范式下的算法进步使得后量子数字签名方案更加高效，MQOM作为基于此的候选方案，在软件中与格基和多元基方案竞争。
>
> **主要贡献:** 本文开发了针对MQOM的专用硬件加速器，并通过精心设计高吞吐量的对称原语引擎和高度优化的多项式算术核心，实现了无缓冲的全即时计算，减少了延迟并降低了内存需求。
>
> **达到效果:** 我们的设计在NIST L1参数下仅需15,812/9,384 LUTs/FFs和4.5 BRAMs，在FPGA上0.46毫秒内完成签名生成，并显著减少了硬件成本。
>
> **技术梗概:** 通过优化多项式算术核心和精心设计的对称原语引擎，实现了无缓冲计算，同时分析了不同参数集对硬件性能的影响。

---
### [推荐] [2026/1484] The SecureDrop Protocol: End-to-End Encrypted Whistleblowing for All

- **匹配关键字:** post-quantum

- **作者:** Giulio Berra, Felix Linker, Luca Maier, Cory Francis Myers, Kenneth G. Paterson, Rowen Shane, Shannon Veitch

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1484) | [PDF](https://eprint.iacr.org/2026/1484.pdf)


> **研究背景:** SecureDrop是一个用于保护举报人隐私和安全的匿名通信系统，广泛应用于新闻机构。然而，当前设计需要物理服务器托管在本地，云部署虽然能提高系统的易用性和访问性，但也引入了新的安全威胁，特别是缺乏端到端加密使得云服务提供商可能被胁迫泄露信息。
>
> **主要贡献:** 本文提出并形式化分析了一个针对SecureDrop的新协议，该协议解决了离线部署的挑战，并通过结合后量子保证的加密方案和身份隐藏的消息获取机制提供了强大的匿名性保障。
>
> **达到效果:** 我们的正式安全分析结合了Tamarin证明器进行符号分析和基于游戏的证明进行计算分析。基准测试表明，在浏览器环境中，该协议实现了实用级别的性能。
>
> **技术梗概:** 通过设计端到端加密方案、提供消息级否认性和使举报人保持无状态来最小化对举报人的罪证，并使用Tamarin证明器结合符号分析和基于游戏的证明进行形式化验证。

---
### [推荐] [2026/1485] From PQC to HHE: Reusing a Co-Design Platform for Side-Channel-Protected PASTA

- **匹配关键字:** post-quantum, homomorphic encryption

- **作者:** Ahmet Malal, Tolun Tosun, Oğuz Yayla, Erkay Savas

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1485) | [PDF](https://eprint.iacr.org/2026/1485.pdf)


> **研究背景:** 现有硬件依赖专用的密码核且缺乏侧信道保护，使得在嵌入式设备上高效执行PASTA等后量子加密算法具有挑战性。
>
> **主要贡献:** 提出了一种基于RISQrypt平台的PASTA硬件/软件协同设计方法，无需额外硬件即可实现第一阶算术遮蔽，并首次提供了侧信道保护的硬件加速PASTA实现。
>
> **达到效果:** 在Artix-7 FPGA上进行的TVLA评估中，加掩码乘法未显示任何一阶泄漏；与同一核心上的纯软件基线相比，在去掩码和掩码配置下分别实现了9.68倍和9.85倍的速度提升。
>
> **技术梗概:** 利用RISQrypt平台的算术和Keccak加速器执行PASTA的仿射层、S盒和SHAKE128抽样，并通过重用遮蔽加速器实现第一阶算术遮蔽，同时保持算术运算在处理器数据路径之外。

---
### [推荐] [2026/1488] Privacy-Preserving Counterfactual Explanations for Federated AI

- **匹配关键字:** homomorphic encryption

- **作者:** Sjoerd Berning, Vincent Dunning, Thijs Veugen, Kevin Witlox

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1488) | [PDF](https://eprint.iacr.org/2026/1488.pdf)


> **研究背景:** 随着AI在敏感领域中的应用增加，隐私保护的可解释人工智能(XAI)工具的需求日益增长。本文提出了一种隐私保护的反事实解释算法，旨在解决垂直分割数据集上的决策支持模型问题。
>
> **主要贡献:** 该研究贡献了一种结合同态加密和秘密共享技术的隐私保护反事实解释算法，能够在不泄露任何单一参与方数据的情况下提供准确的解释。
>
> **达到效果:** 实验结果表明，通过整合多个数据集，该方法在保持隐私的同时提高了模型性能，并且不会影响准确性。
>
> **技术梗概:** 该算法利用了同态加密和秘密共享技术来保护多方数据隐私，在保证隐私的前提下实现了高效的反事实解释生成。

---
### [2026/1476] Constructing new permutation polynomials by the AGW Criterion

- **作者:** Qian Liu, Liwei Fang, Zhengbang Zha, Jing Zhang

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1476) | [PDF](https://eprint.iacr.org/2026/1476.pdf)


> **研究背景:** 本文旨在通过AGW准则构造新的置换多项式，以扩展已知的置换多项式构建方法。
>
> **主要贡献:** 作者提出了具有特定形式的置换多项式的多个新类，并证明了这些多项式不是已知多项式的准乘性等价物。
>
> **达到效果:** 通过确定某些方程在\( \mathbb{F}_{2^{2m}} \)中的解数，作者成功地构造了一系列新的置换多项式。
>
> **技术梗概:** 利用AGW准则和代数方法来确定特定形式的多项式的置换性质。

---
### [2026/1477] PERSEPHONE: Zero-Knowledge Multiplicative Non-Negative Proof for Sequential Private Range Verification

- **作者:** Ivan Tjuawinata, Yann Fraboni, Darian Gunamardi, Jun Jie Sim, Zhenghao Wu, Hasventhran Baskaran, Chi-Hung Chi, Pu Duan, Kwok-Yan Lam

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1477) | [PDF](https://eprint.iacr.org/2026/1477.pdf)


> **研究背景:** 在金融科技领域，零知识证明（ZKP）被用于验证信息而不泄露具体内容。然而，传统的范围验证需要访问账户余额等敏感信息，这引发了对私有范围验证的需求。
>
> **主要贡献:** 本文提出了一个新颖的零知识证明方案——零知识乘法非负证明（ZK-MultNNP），并基于此构建了PERSEPHONE框架来解决序列化私有范围验证的问题。
>
> **达到效果:** 实验结果显示，PERSEPHONE在性能上比基于ZKRPs的方法快3倍，并且能够更有效地处理连续的私有边界验证问题。
>
> **技术梗概:** 该方案通过引入乘法非负证明技术，实现了对一系列值在序列化私有边界内进行高效验证，从而提高了交易系统的效率和稳定性。

---
### [2026/1478] Provable Recovery of RSA Private Exponents below \(N^{11/42-\varepsilon}\)

- **作者:** Yiming Gao, Honggang Hu

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1478) | [PDF](https://eprint.iacr.org/2026/1478.pdf)


> **研究背景:** 研究背景：Wiener的连分数攻击为RSA私钥恢复提供了经典可证明界限，当\(d \leq N^{11/42-\varepsilon}\)时，对于平衡的RSA实例（\(e=\Theta(N)\)），可以在多项式时间内确定性分解其因子。
>
> **主要贡献:** 主要贡献：本文提出了在更小私钥情况下恢复RSA私钥的方法，并证明了当私钥低于\(N^{11/42-\varepsilon}\)时，可以实现确定性的恢复。
>
> **达到效果:** 达到的效果：通过改进的算法和技术，作者成功地扩展了Wiener攻击的有效范围，使得更多较小的私钥能够被有效恢复。
>
> **技术梗概:** 技术梗概：利用先进的数论和分析方法，结合连分数逼近理论，设计了一种新的算法来精确估计RSA私钥，并证明了其在特定条件下的有效性。

---
### [2026/1479] Conjectures and Barriers for RS-MCA

- **作者:** Przemek Chojecki

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1479) | [PDF](https://eprint.iacr.org/2026/1479.pdf)


> **研究背景:** 该研究旨在进一步探讨Reed-Solomon MCA问题，并提出新的猜想和障碍，以完善已有的边界条件。
>
> **主要贡献:** 作者提出了一个完整的有限Reed-Solomon MCA问题，并构建了基于极点的正密度轮廓包络猜想。
>
> **达到效果:** 通过证明天花板归一化矩障碍、射影共线定理及基准维度诊断等，研究达成了多项理论成果。
>
> **技术梗概:** 研究采用了精确的第一匹配编译器和实现图像矩与共现不等式来隔离安全证书所需的支付，并结合Sidon支付及Balog-Szemer\'edi-Gowers定理进行证明。

---
### [2026/1480] Doubly-Efficient Secret-Key PIR with Low Storage Overhead

- **作者:** Caicai Chen, Yuval Ishai, Aayush Jain, Tamer Mour, Alon Rosen, Chaoping Xing

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1480) | [PDF](https://eprint.iacr.org/2026/1480.pdf)


> **研究背景:** 研究背景：秘密密钥私有信息检索（sk-PIR）允许客户端使用短的密钥从数据库中检索项目，同时隐藏请求的索引，并可能隐藏数据库本身。现有方案在存储效率上存在不足，本文旨在提供一种具有常数倍存储开销且在线服务器工作量接近最优的解决方案。
>
> **主要贡献:** 主要贡献：提出了第一个候选的双高效秘密密钥PIR方案，实现了接近线性存储开销，并且通信和在线服务器工作量分别为$k^{o(1)}$和$k/	extrm{polylog}(k)$，显著优于现有技术。
>
> **达到效果:** 达到的效果：该方案在保持常数倍的存储开销的同时，将在线服务器的工作量从$k/	extrm{polylog}(k)$降低至接近线性，从而大幅提高了效率，并且适用于大规模数据库。
>
> **技术梗概:** 技术梗概：采用permuted-code蓝图进行设计，通过引入新的$t$-光滑局部可解码码族来优化率、局部性和平滑性之间的权衡，具体包括基于曲线串联的Reed-Muller编码的新局部解码器以及基于曲线上升编码的构造。

---
### [2026/1481] 88-XOR Implementation of the AES MixColumns Matrix

- **作者:** Jérémy Jean

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1481) | [PDF](https://eprint.iacr.org/2026/1481.pdf)


> **研究背景:** 本文介绍了一种使用88个XOR门实现AES MixColumns矩阵的电路，相较于之前的最佳结果减少了4个XOR门。
>
> **主要贡献:** 该工作通过优化电路设计，在减少硬件资源的同时保持了相同的功能性。
>
> **达到效果:** 所提出的电路在XOR门的数量上实现了突破，达到了88个，比之前的最小值减少了6个XOR门。
>
> **技术梗概:** 作者采用了一种新的逻辑优化技术，通过对矩阵操作的重新组织和简化来减少所需的XOR门数量。

---
### [2026/1486] The supersingular isogeny problem in time and memory $p^{1/3+o(1)}$

- **作者:** Benjamin Wesolowski

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1486) | [PDF](https://eprint.iacr.org/2026/1486.pdf)


> **研究背景:** 研究背景：超奇异Isogeny问题被认为是后量子密码学中基于Isogeny的加密方案的安全性基础，解决该问题的成本是确定安全参数时的主要因素之一。
>
> **主要贡献:** 主要贡献：证明了在合理的随机整数平滑度假设下，可以将超奇异Isogeny问题的时间和内存复杂度降低至$p^{1/3+o(1)}$，这比之前的最佳复杂度$p^{1/2} \cdot(\log p)^{O(1)}$有了显著改进。
>
> **达到效果:** 达到的效果：该算法在理论上提供了更优的解决方案，但实际应用中由于超多项式因子和高内存需求的影响，对具体参数集的实际影响尚需进一步研究。
>
> **技术梗概:** 技术梗概：通过分析随机整数的平滑性假设来优化Isogeny路径搜索的方法，从而实现了复杂度的降低。

---
### [2026/1487] LightShark: Actively Secure Machine-Learning Inference Based on Lightweight Authenticated Distributed Comparison Function

- **作者:** Chenkai Zeng, Qi Feng, Debiao He, Min Luo

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1487) | [PDF](https://eprint.iacr.org/2026/1487.pdf)


> **研究背景:** 研究背景：现有工作Shark在半诚实环境中提出了机器学习推理的安全方案，而Grotto则提出了一种具有更小密钥大小的变体分布式比较函数（DCF），但在恶意环境下的扩展性尚未解决。
>
> **主要贡献:** 主要贡献：本文首次实现了适用于恶意环境的Grotto风格DCF方案，并基于此构建了高效支持ReLU、Spline和Truncation等基本操作及VGG-16、GPT、BERT等模型的LightShark框架。
>
> **达到效果:** 达到的效果：与Shark相比，LightShark在Bert-base推理上的性能提升了1.49至2.69倍，并降低了66.7%的通信成本；对于更大的LLM模型，实验表明其表现优异。
>
> **技术梗概:** 技术梗概：设计了基于相关GGM树的认证DCF方案，保持了半诚实环境下Grotto的密钥大小优势，并实现了高效的机器学习推理框架。

---
### [2026/1489] SwitchFold: Code-Agnostic Succinct Polynomial Commitments via Recursive Code Switching

- **作者:** Mingshu Cong, Tsz Hon Yuen, Siu-Ming Yiu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1489) | [PDF](https://eprint.iacr.org/2026/1489.pdf)


> **研究背景:** 研究旨在设计一种通用的、基于哈希的大规模多项式承诺方案（PCS），以减少证明者时间，同时保持polylogarithmic大小的证明和验证者时间。这为零知识简洁非交互知识论证（zkSNARKs）的应用提供了支持，如零知识机器学习（zkML）。
>
> **主要贡献:** 提出了一种名为SwitchFold的技术，它通过递归代码切换从任意几何增长块长度序列的线性码构建多线性PCS。该方案利用了Brakedown码序列实现了具体实例，称为BrakeFold。
>
> **达到效果:** SwitchFold成功地实现了polylogarithmic大小的证明和验证者时间，并且证明者时间仅依赖于编码时间。通过这种技术，多项式承诺可以编码数十亿参数，同时避免固定点算术中的溢出问题。
>
> **技术梗概:** SwitchFold的核心是递归应用代码切换技术，将多线性扩展（MLE）声明从一个码简化为较短的码下的简单MLE声明。生成矩阵MLE声明通过累积方案进行证明，并最终通过递归进行验证。

---
### [2026/1490] On the Formal Verification of Polynomial Commitments: two KZG constructions and the Algebraic Group Model

- **作者:** Tobias Rothmann

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1490) | [PDF](https://eprint.iacr.org/2026/1490.pdf)


> **研究背景:** 本文旨在通过形式化验证多项式承诺方案（PCSs），特别是Kate, Zaverucha和Goldberg（KZG）构造的两个变体，来加强密码学协议的安全性证明。
>
> **主要贡献:** 作者使用Isabelle/HOL中的CryptHOL框架，首次正式验证了多项式承诺方案及其安全性的证明，并引入了一种新的代数组模型（AGM）形式化方法。
>
> **达到效果:** 通过这种方法，作者成功地验证了标准的（DL-）KZG和批量KZG两种具体PCS构造的知识安全性证明，这是在Isabelle中进行的第一个此类正式验证工作。
>
> **技术梗概:** 研究采用了Shoup的序列游戏方法，并结合机器检查转换来完成证明，确保了高度的形式化和可靠性。

---
### [2026/1491] Exploiting Load/Store Leakage of Sparse Vectors for Key Recovery in HQC

- **作者:** Gustavo Banegas, Benjamin Smith, Jad Zahreddine

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1491) | [PDF](https://eprint.iacr.org/2026/1491.pdf)


> **研究背景:** HQC作为一种基于码的密钥封装机制被NIST选中进行标准化，因此其对实施攻击的抵抗力至关重要。
>
> **主要贡献:** 作者提出了一种侧信道攻击方法，利用HQC稀疏秘密向量中的加载/存储泄漏来恢复密钥。
>
> **达到效果:** 通过电磁测量区分机器字为零或非零，并将恢复出的零位置转化为解码提示，从而将HQC密钥恢复问题简化为一个缩短后的综合解码问题。实验验证了预测的高低半部不对称性，并成功在32位粒度上恢复了一个HQC-1密钥。
>
> **技术梗概:** 通过对Cortex-M4参考实现进行分析，识别出由于编译器生成的寄存器溢出导致的不同强度的高低半部泄漏，并利用这些泄漏构造了一个简单的零字区分器。

---
### [2026/1492] Proof of Demand Is Not Proof of Work: On the Limits of Demand-Weighted Consensus under Free Pseudonyms

- **作者:** Ömer Demirel

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1492) | [PDF](https://eprint.iacr.org/2026/1492.pdf)


> **研究背景:** 该研究探讨了在自由匿名环境下，需求加权共识机制的有效性边界，特别是证明有用工作的局限性。
>
> **主要贡献:** 作者定义并区分了工作声学、任务绑定和需求外生性三个属性，并提出了两个无条件的结果来揭示这些属性之间的差距。
>
> **达到效果:** 研究表明，在自由匿名环境中，基于需求的共识机制无法确保经济独立请求者的证明真实性，从而限制了其安全性。进一步推导出一个稳健性上限：在零需求路径上仍需保持活跃的去许可化机制不能依赖服务收据中的需求组件。
>
> **技术梗概:** 研究通过形式化的定义和严格的数学论证来区分不同属性，并利用联盟控制下的请求者与工作者之间的支付关系进行分析。

---

## 更新: 2026-07-23 20:51

*新增 1 篇论文 (编号 1475--1475)*

### [2026/1475] Partial Derandomization for Leakage-Resilient Shamir's Secret Sharing over Composite Order Fields

- **作者:** S. Venkitesh

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1475) | [PDF](https://eprint.iacr.org/2026/1475.pdf)


> **研究背景:** 研究背景：针对复合阶域上的Shamir秘密共享方案，现有研究表明随机选择评估点可实现统计安全性，但随机性较高。本文旨在减少这种随机性以提高效率并保持安全性。
>
> **主要贡献:** 主要贡献：作者通过引入简单固定有理函数的迭代来部分去随机化评估点的选择过程，从而将随机性从$nd \log p$比特降低到$d\log p$比特。
>
> **达到效果:** 达到的效果：在特定参数范围内（$n = O(d/\log_p d)$），该方案能够实现对单一数据块泄漏的完美安全性，并且适用于任意重构建阈值$k \ge 2$。
>
> **技术梗概:** 技术梗概：本文采用部分分式非退化论证方法，利用有理迭代的不相同极点特性来设计秘密共享方案。
...
### [推荐] [2026/1447] Faster NTRU-based Bootstrapping with Extended and Sorting-based Techniques

- **匹配关键字:** LWE

- **作者:** Jingwei Feng, Baofeng Wu, Dongdai Lin

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1447) | [PDF](https://eprint.iacr.org/2026/1447.pdf)


> **研究背景:** NTRU基于的Bootstrapping提供了FHEW类似方案的一种高性能变体，尽管比其RLWE基对应物更简单，但在内存和时间效率上仍较为昂贵。关键计算瓶颈在于$q\mid 2N$约束，它将LWE密文模数$q$与NTRU维度$N$联系在一起。
>
> **主要贡献:** 作者通过扩展技术消除了$q \mid 2N$的限制，并利用大模数$q$和较小的$N$维环来提高效率。此外，他们还使用排序技术去除不必要的操作以进一步提升时间性能。
>
> **达到效果:** 在128位安全级别下，与FINAL方案（ASIACRYPT 2021）相比，单密钥Bootstrapping加速了1.82倍；与CHES 2024年的key unrolling方法相比，加速了1.24倍。对于多密钥Bootstrapping，相较于前人工作（ASIACRYPT 2024），实现了1.70倍的加速。同时，单密钥和多密钥算法的内存使用量分别减少了$\eta$倍。
>
> **技术梗概:** 通过扩展技术移除了$q \mid 2N$的限制，并利用排序技术减少不必要的操作以提高时间性能。

---
### [推荐] [2026/1449] `ANSA-IBKEM`: Practical Quantum-Safe Identity-Based Key Encapsulation via Annular NTRU Trapdoors and Standardized PQC Arithmetic Reuse

- **匹配关键字:** lattice, post-quantum, LWE

- **作者:** Zhaohui Cheng, Kaixin Xiong

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1449) | [PDF](https://eprint.iacr.org/2026/1449.pdf)


> **研究背景:** 现有的身份基于加密方案（IBE）在实现后量子安全方面存在局限性，如高解密失败率和较大的私钥大小。为了克服这些限制，研究提出了一种新的方案ANSA-IBKEM。
>
> **主要贡献:** ANSA-IBKEM通过重新设计陷阱门、提取过程、正确性和实现层，并围绕标准化后量子算术构建，实现了紧凑的身份关系并保持了较低的解密失败率。
>
> **达到效果:** 该方案在选定压缩点下，产生的密文大小分别为3200字节和7168字节，且经过单向95%上限估计后的BCH尾部解密失败率分别低于\(2^{-164}\)和\(2^{-248}\)，从而达到了高效、安全的后量子加密效果。
>
> **技术梗概:** ANSA-IBKEM结合了环NTRU陷阱门、混合采样提取、基于BCH的消息层以及在模数\(q=8380417\)上的密文压缩技术。

---
### [推荐] [2026/1451] Lightweight Hardware Accelerator for the UOV Signature Scheme with Oil Space Blinding

- **匹配关键字:** post-quantum

- **作者:** Florian Krieger, Maciej Czuprynko, Sujoy Sinha Roy

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1451) | [PDF](https://eprint.iacr.org/2026/1451.pdf)


> **研究背景:** 面对量子计算带来的安全威胁，NIST正在寻找后量子时代的数字签名方案。UOV因其保守的设计、小型的签名以及高效的签名和验证过程而成为候选者。
>
> **主要贡献:** 本文提出了一种低内存占用的UOV硬件加速器架构，支持NIST的安全级别1至5，并通过负载平衡的数据路径优化关键矩阵操作。
>
> **达到效果:** 该设计显著减少了UOV的最大内存消耗，并实现了灵活的运行时支持。同时，通过数据重新对齐和流水线高斯消元单元等技术提高了性能。
>
> **技术梗概:** 采用分片技术和流式处理方法降低公钥和中间结果的内存需求，使用16通道负载平衡的数据路径来执行UOV的关键矩阵操作。

---
### [推荐] [2026/1457] Identity-Based Encryption from Isogenies

- **匹配关键字:** post-quantum

- **作者:** Shweta Agrawal, Andrea Basso, Sikhar Patranabis

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1457) | [PDF](https://eprint.iacr.org/2026/1457.pdf)


> **研究背景:** 本文提出了首个基于Isogeny的标识基加密方案，该方案的安全性依赖于一个新颖假设——'CDH与匹配扭转假设'（CD-HwMT），该假设在代数Isogeny模型下归约到已知假设。
>
> **主要贡献:** 作者引入了CD-HwMT假设，并基于此构建了一个具有匿名性的标识基加密方案，同时实现了首个基于Isogeny的精简性盲目传输和后量子安全的高泄漏率密钥及密文相关安全性公钥加密。
>
> **达到效果:** 该方案在代数Isogeny模型下归约到已知假设，并且能够推测其具有后量子安全性。
>
> **技术梗概:** 本文提出的技术包括CD-HwMT假设和新的构建方法，这些技术为未来基于Isogeny的高级密码学研究奠定了基础。

---
### [推荐] [2026/1459] Hybrid hash function based on the DLP and SIS problems

- **匹配关键字:** lattice

- **作者:** Dimitri Koshelev, Francesc Sebé

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1459) | [PDF](https://eprint.iacr.org/2026/1459.pdf)


> **研究背景:** 该研究提出了一种基于离散对数问题(DLP)和短整数解问题(SIS)的杂交哈希函数，旨在提供双重安全性以增强加密方案的整体安全性。
>
> **主要贡献:** 作者提供了特定参数确保在256位模数下Lattice问题达到128位安全级别，并且这是首次对这种杂交版本进行明确分析。
>
> **达到效果:** 该哈希函数结合了Pedersen和Ajtai的经典版本的安全特性，可能通过椭圆曲线和格密码学中的技巧实现显著优化。
>
> **技术梗概:** 研究采用了一种将DLP和SIS问题相结合的方法来设计哈希函数，并提供了具体参数以确保安全级别。

---
### [2026/1446] Quantum Circuit Optimization with LLMs under a Structured Guideline

- **作者:** Kyungbae Jang, Hyunji Kim, Hwajeong Seo, Anupam Chattopadhyay

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1446) | [PDF](https://eprint.iacr.org/2026/1446.pdf)


> **研究背景:** 量子密码分析的成本主要由目标密码的量子电路决定，因此需要构建并测量其qubit数量、Toffoli门数量及其深度来估算攻击成本。然而，这一过程依赖于专家知识且需针对每个密码和成本目标重新进行。
>
> **主要贡献:** 本文提出了一种使用大型语言模型（LLM）根据结构化指南优化量子电路的方法，并为多个通用密码生成了最小qubit数量和最小Toffoli深度的电路。
>
> **达到效果:** 通过这种方法，实现了CRAFT、MANTIS、QARMA、mCrypton、EPCBC和Pyjamask等密码的量子电路实现，填补了这些密码在量子电路方面的空白。
>
> **技术梗概:** 本文将跨多种密码适用的量子电路优化技术编写成指南，并指导LLM生成针对特定目标密码的两种电路：一种是qubit数量最少，另一种是Toffoli深度最小。

---
### [2026/1448] Improving Skipping Fault Correction Attacks on Randomized Dilithium via MILP

- **作者:** Haobo Ouyang, Chaoran Wang, Guowei Liu, Lixuan Wu, Meiqin Wang, Yanhong Fan

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1448) | [PDF](https://eprint.iacr.org/2026/1448.pdf)


> **研究背景:** 针对Dilithium的随机化签名模式，现有研究未能完全解决其物理攻击抗性问题。Krahmer等人提出了一种利用故障注入的关键恢复攻击方法，该方法虽然操作简单但在实际应用中需要较多的有效故障来覆盖所有密钥系数。
>
> **主要贡献:** 本文通过优化跳跃故障校正攻击，证明了全秩收集策略并非必要，并推导出最小故障数M_min及其与密钥依赖的趋势。
>
> **达到效果:** 实验结果表明，在L2、L3和L5安全级别下，使用M_min可以实现较高的密钥恢复概率。
>
> **技术梗概:** 本文采用混合整数线性规划（MILP）方法进行攻击优化，并通过随机行模型分析了秘密系数的覆盖范围及唯一有界解的概率条件。

---
### [2026/1450] Optimizing Polynomial Multiplication and Fixed-Weight Sampling for HQC on ARM Cortex-M4

- **作者:** Jihoon Jang, Hanbeom Shin, Suhri Kim, Seokhie Hong, Donggeun Kwon

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1450) | [PDF](https://eprint.iacr.org/2026/1450.pdf)


> **研究背景:** 本文针对ARM Cortex-M4平台优化了Hamming Quasi-Cyclic (HQC)方案，旨在提高其在嵌入式设备上的性能和效率。
>
> **主要贡献:** 作者通过优化多项式乘法、固定权重采样的支持扩展，并提出了一种缓存策略来重用公钥变换和基于固定密钥重新计算的哈希值，从而改进了HQC方案。
>
> **达到效果:** 优化后的实现相比之前最快的两个实现，在密钥生成、封装和解封装上分别最多可减少33.1%、34.6%和29.8%，并进一步通过缓存策略减少了高达32.7%的开销。
>
> **技术梗概:** 作者提出了一种脏位感知寄存器分配策略和XOR操作重排序，以减少VMOV指令计数；同时对支持扩展进行了重构，使用预测执行和四倍展开技术降低了每字节成本。

---
### [2026/1452] Labeled Multi-Key Batched IBE

- **作者:** Guru-Vamsi Policharla

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1452) | [PDF](https://eprint.iacr.org/2026/1452.pdf)


> **研究背景:** 研究了批量身份加密（Batched IBE）与多消息签名（Multi-Message Signatures，MMS）之间的关系及其应用。
>
> **主要贡献:** 提出了从批量IBE构造多消息签名的方法，并展示了如何利用提取式见证加密来构建批量IBE方案。
>
> **达到效果:** 该研究首次实现了支持每标签$q$个密钥查询的批量IBE方案，具有线性大小的公共参数和恒定大小的密文。
>
> **技术梗概:** 通过分析现有批量IBE构造并提取可验证的多消息签名，获得了批量IBE内部机制的理解，并据此构建了新的方案。

---
### [2026/1453] Celer: A Lookup Argument for Large-Scale Queries

- **作者:** Wenjie Qu, Yanpei Guo, Zhen Xuan, Xuanming Liu, Jiaheng Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1453) | [PDF](https://eprint.iacr.org/2026/1453.pdf)


> **研究背景:** 现有的查找论证大多针对查询大小小于表大小的场景进行了优化，但在实际应用中，查询大小远大于表大小的情况普遍存在，导致效率低下。
>
> **主要贡献:** Celer通过减少承诺的数量使其证明者运行时间与查询大小成缓慢增长关系，从而提高了可扩展性。
>
> **达到效果:** 实验结果表明，对于查询大小$m=2^{28}$和表大小$n=2^{16}$的场景，Celer相比plookup实现了25.9倍的证明者时间加速，并且比LogUp快9.4倍。
>
> **技术梗概:** Celer通过减少承诺的数量并保持字段操作与查询大小线性关系来提高效率。

---
### [2026/1454] Batched Attribute-Based Encryption from Bilinear Pairings

- **作者:** Ramprasad Sarkar, Shayeef Murshid, Mriganka Mandal

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1454) | [PDF](https://eprint.iacr.org/2026/1454.pdf)


> **研究背景:** 现有批量身份基加密方案仅支持基于身份的访问控制，无法处理复杂的属性基策略。本文旨在扩展批量加密技术以适应属性基加密的需求。
>
> **主要贡献:** 首次提出了批量属性基加密（B-ABE）概念，并构建了在不对称双线性群中的安全实现。此外，还引入了阈值批量属性基加密（TB-ABE），其中主密钥被分散给多个授权机构。
>
> **达到效果:** B-ABE 能够使密钥权威一次性生成解密整个批次密文所需的密钥，并且该密钥的大小与批次大小无关，实现了高效安全的批量解密。TB-ABE 则进一步增强了系统的灵活性和安全性。
>
> **技术梗概:** 通过在双线性配对群中设计新的加密和解密算法来实现B-ABE的安全性证明，并利用Shamir秘密共享方案实现TB-ABE中的多授权机构机制。

---
### [2026/1455] Trout++: Robust Asynchronous Two-Round ECDSA for Arbitrary Thresholds

- **作者:** Ariel Nof, Luke Parker

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1455) | [PDF](https://eprint.iacr.org/2026/1455.pdf)


> **研究背景:** Trout++ 是一种基于 ECDSA 的签名方案，旨在提供灵活的门限设置和强大的安全保证，特别适用于异步环境下的数字签名。
>
> **主要贡献:** 该研究通过引入预签功能并结合 ROAST 转换，实现了首个支持任意门限（包括恶意多数）的异步、稳健的 ECDSA 签名协议。
>
> **达到效果:** Trout++ 在减少带宽和计算量方面进行了优化，并且在性能上显著优于其前身 Trout，同时保持了与领先方案相当的功能性和复杂性。
>
> **技术梗概:** 研究采用了透明设置、两轮结构以及 ROAST 转换等技术，实现了协议的稳健性和异步特性。

---
### [2026/1456] QuantumScouter: Reinforcement Learning-Based Optimization of Variational Quantum Circuits for Differential Cryptanalysis

- **作者:** Gilsang Ahn, Jiwoo Baek, Donggun Lee, Insung Kim, Changmin Lee, Seokhie Hong, Dongjae Lee

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1456) | [PDF](https://eprint.iacr.org/2026/1456.pdf)


> **研究背景:** 经典深度学习方法在差分密码分析中需要大量密文对，这使得攻击不可行或容易被检测到。
>
> **主要贡献:** 该研究通过引入基于量子机器学习的量子差分区分器来克服数据限制，并提出了一种名为QuantumScouter的方法以优化变分量子电路。
>
> **达到效果:** 实验结果显示，在SPECK32/64和SIMON32/64密码算法上，与先前方法相比，该研究提高了准确性并减少了所需量子比特的数量。
>
> **技术梗概:** QuantumScouter利用强化学习来发现紧凑的量子电路，并明确针对门计数和电路深度等指标进行优化。

---
### [2026/1458] A High-Speed Hardware Accelerator for QR-UOV Signature Scheme

- **作者:** Renma Sugai, Hiroshi Amagasa, Rei Ueno, Naofumi Homma

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1458) | [PDF](https://eprint.iacr.org/2026/1458.pdf)


> **研究背景:** 该研究针对QR-UOV多变量方案提出了一种高性能硬件加速器，旨在优化密钥生成、签名生成和验证三个核心操作的执行效率。
>
> **主要贡献:** 贡献在于设计了专门用于加速多项式矩阵乘法的操作单元，并通过引入在线拒绝采样技术和压缩对称矩阵展开过程，进一步提升了系统性能。
>
> **达到效果:** 实验结果表明，该硬件加速器显著提高了QR-UOV方案的整体处理速度和效率，特别在多项式矩阵操作上实现了加速。
>
> **技术梗概:** 技术上采用了一个$D 	imes D$的 systolic阵列来实现多项式乘加运算，并通过AES-CTR输出块的独立生成机制来保证数据流的连续性。

---

## 更新: 2026-07-18 16:31

*新增 29 篇论文 (编号 1416--1445)*

### [推荐] [2026/1416] Beyond Size: Do Hybrid PQC Certificates Actually Enforce the Classical–PQC Binding? A Cost-and-Security Study

- **匹配关键字:** post-quantum

- **作者:** Minwoo Lee, Minjoo Sim, Siwoo Eum, Subeen Cho, Yulim Hyoung, Hwajeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1416) | [PDF](https://eprint.iacr.org/2026/1416.pdf)


> **研究背景:** 随着TLS 1.3向后量子密码学(PQC)迁移，混合X.509过渡策略（如Catalyst、Composite、Chameleon和签名组合器）的成本对比研究较少关注这些策略是否真正实现了它们承诺的经典↔PQC绑定。
>
> **主要贡献:** 研究揭示了这些策略在实际应用中并未有效执行绑定，并且即使在检查绑定的堆栈中也是如此，这挑战了基于成本的传统推荐。
>
> **达到效果:** 通过构建独立非BouncyCastle生成器验证了X.509默认行为，发现Catalyst和wolfSSL在某些情况下未能强制执行绑定。研究提出了一个四轴成本模型，综合考虑策略大小、兼容性、结构绑定和安全性。
>
> **技术梗概:** 研究使用BouncyCastle库和wolfSSL等安全堆栈进行实验，并通过独立生成器验证结果，构建了一个多维度的成本评估框架来评价PQC证书策略的有效性和适用性。

---
### [推荐] [2026/1417] Accelerating the AIMer Post-Quantum Signature with AVX-512: A Field–Keccak Speedup Analysis

- **匹配关键字:** post-quantum

- **作者:** Seung-Won Lee, Si-Woo Eum, Hwa-Jeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1417) | [PDF](https://eprint.iacr.org/2026/1417.pdf)


> **研究背景:** AIMer是一种基于后量子密码学的数字签名方案，其安全性仅依赖于AIM2对称密钥单向函数和MPC-in-the-Head零知识证明。然而，标准发布的C语言实现未包含针对特定处理器的优化。
>
> **主要贡献:** 本文提出了利用AVX-512指令集优化所有六种AIMer变体的实现方案，通过并行化二进制域乘法和加速Keccak置换来提升性能。
>
> **达到效果:** 该优化后的实现比使用AVX2的版本快1.60–1.84倍。分析表明，大部分性能提升来自于Keccak，尤其是在较短密钥长度的变体中更为显著。
>
> **技术梗概:** 通过VPCLMULQDQ指令并行化二进制域乘法，并使用VPTERNLOGQ指令处理线性层中的条件异或累积；同时利用AVX-512VL加速Keccak置换过程。

---
### [推荐] [2026/1418] CT-KAT: A Multilayer Analysis Platform for Automated Screening of Constant-Time Risks in PQC C Implementations

- **匹配关键字:** post-quantum

- **作者:** Seung-Won Lee, Min-Seo Kim, Su-Min Jeong, Hwa-Jeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1418) | [PDF](https://eprint.iacr.org/2026/1418.pdf)


> **研究背景:** 随着后量子密码学（PQC）算法标准化，C语言实现的ML-KEM、ML-DSA和SLH-DSA等算法迅速部署。然而，已知答案测试（KATs）仅验证功能正确性，并未确保不存在由密钥依赖分支、内存访问或可变延迟指令引起的定时泄漏风险。
>
> **主要贡献:** 该论文提出CT-KAT平台，用于评估PQC C实现中的常时间隙风险。CT-KAT通过单一YAML配置文件集成结构检查、编译器和优化级别组合的ct-matrix分析以及汇编级asm-scan等多层分析。
>
> **达到效果:** 当应用于基于PQClean的PQC代码库时，CT-KAT产生了四种主要分类结果。基准ML-KEM目标被归类为稳健。KyberSlash复现被标记为秘密依赖变量延迟风险，并被命名为va。
>
> **技术梗概:** CT-KAT采用统一管道进行多层分析，包括基于Valgrind/Memcheck的结构检查、ct-matrix覆盖编译器和优化级别组合以及汇编级asm-scan。

---
### [推荐] [2026/1419] ML-QED-Lite: A Lightweight Machine Learning-Based Tool for Supporting Post-Quantum Cryptography Migration in Executable Binaries

- **匹配关键字:** post-quantum

- **作者:** Seung-Won Lee, Hwa-Jeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1419) | [PDF](https://eprint.iacr.org/2026/1419.pdf)


> **研究背景:** 研究背景：为了推动后量子密码学（PQC）的迁移，需要识别已部署软件中是否使用了量子易受攻击（QV）的公钥加密方案。然而，许多可执行文件缺少源代码，因此直接筛查这些二进制文件以寻找QV候选者是必要的。
>
> **主要贡献:** 主要贡献：提出了一种基于机器学习的方法ML-QED-Lite，旨在保留QED-Lite的效率的同时减少假阳性（FPs）。
>
> **达到效果:** 达到的效果：在对网络数据集和合成数据集进行评估后，ML-QED-Lite检测到与QED-Lite相同的五个正向可执行文件，并且将FPs从两个降至零。这表明ML-QED-Lite保留了由QED-Lite识别的真实阳性（TPs）并减少了FPs，从而提高了性能。
>
> **技术梗概:** 技术梗概：ML-QED-Lite直接以目标目录下的所有ELF可执行文件作为输入，并提取每个文件的功能符号、库依赖关系和二进制级属性，然后使用训练好的分类模型来判断该文件是否为PQC迁移的候选者。

---
### [推荐] [2026/1420] An Optimized Native Rust Implementation of the KpqC Algorithms

- **匹配关键字:** post-quantum

- **作者:** Yu-Lim Hyoung, Do-Yun Park, Hyun-Ji Kim, Hwa-Jeong Seo

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/1420) | [PDF](https://eprint.iacr.org/2026/1420.pdf)


> **研究背景:** 本文旨在优化KpqC算法的本地Rust实现，以提高速度和减少内存使用，并将其与C参考代码(KpqClean)及未优化的Rust实现进行比较。
>
> **主要贡献:** 贡献在于通过利用AVX2和PCLMULQDQ加速多项式乘法、4路SHAKE以及GF(2^λ)乘法，实现了速度优化；并通过即时处理、两遍采样和传输重计算减少了内存使用量。
>
> **达到效果:** 与C参考实现相比，在所有目标方案中，速度优化版本将执行时间缩短了32-76%，而内存优化实现则在所有目标方案中的总内存使用量降低了1-94%。
>
> **技术梗概:** 技术上采用了即时处理、两遍采样和传输重计算等方法来减少临时缓冲区大小，并通过AVX2、PCLMULQDQ加速多项式乘法及GF(2^λ)乘法，提高了Rust实现的速度和效率。

---
### [推荐] [2026/1422] LESS on the Cortex-M4: Characterizing the Speed–Memory Design Space of Code-Equivalence Signatures

- **匹配关键字:** post-quantum

- **作者:** Minwoo Lee, Minjoo Sim, Subeen Cho, Yulim Hyoung, Hwajeong Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1422) | [PDF](https://eprint.iacr.org/2026/1422.pdf)


> **研究背景:** LESS是一种基于线性等价问题的代码签名方案，作为NIST后量子签名征集的候选者，在Cortex-M4微控制器上实现具有挑战性。
>
> **主要贡献:** 本文首次在Cortex-M4上全面实现了LESS（v2.0），包括七个参数集，并探索了速度-内存设计空间。
>
> **达到效果:** 通过三种操作点，平衡配置以41%-59%的峰值栈使用量实现比参考平台快4.1-4.3倍的签名速度；在速度上限和内存下限之间找到优化方案。
>
> **技术梗概:** 采用与NIST已知答案测试字节对齐的方法，并通过双遍历签名技术，确保了所有七个参数集的签名测量。

---
### [推荐] [2026/1423] CMALU: Compact Fault-Tolerant Modular Arithmetic Logic Unit for Post-Quantum Cryptography

- **匹配关键字:** post-quantum

- **作者:** YoungBeom Kim, Malik Imran, Zain Ul Abideen, Ciara Rafferty, Ayesha Khalid, Máire O’Neill, Seog Chung Seo

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1423) | [PDF](https://eprint.iacr.org/2026/1423.pdf)


> **研究背景:** 随着量子计算的发展，传统公钥密码系统面临威胁，促使研究转向依赖模算术的后量子密码学（PQC）算法。现有资源受限物联网设备的硬件加速器缺乏集成故障检测机制。
>
> **主要贡献:** 本文提出CMALU，一种紧凑且容错的模算术逻辑单元，支持单重配置路径上的六种操作，并提供三种模式以不同级别实现故障检测和数据保护。
>
> **达到效果:** 在NTT加速器和Ibex RISC-V核心上进行系统级故障注入测试表明，Mode-1能够实现100%的单比特和固定位错误检测且无延迟开销，而Mode-2则完全消除了数据损坏。
>
> **技术梗概:** CMALU通过在单一可重构数据路径上支持六种操作，并利用硬件重用重新计算以消除随机数据损坏来实现其功能。

---
### [推荐] [2026/1425] Simultaneous-Message and Succinct Secure Computation: Reusable and Multiparty Protocols

- **匹配关键字:** lattice, homomorphic encryption, LWE

- **作者:** Siddharth Agarwal, Abhishek Jain, Akshayaram Srinivasan, David J. Wu

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1425) | [PDF](https://eprint.iacr.org/2026/1425.pdf)


> **研究背景:** 研究提出了同时消息和简洁(SMS)安全计算的概念，该概念允许双方在共享参考字符串(CRS)后，以同步方式交换其私有输入的编码，并获得函数输出的加法份额。
>
> **主要贡献:** 本文扩展了SMS的研究维度，引入了可重用SMS和多方SMS协议，分别支持多次函数计算和一个大输入与多个小输入的场景。
>
> **达到效果:** 在假设LWE问题多项式硬性的前提下，作者构建了一个基于布尔电路的可重用两方SMS协议，并且其CRS和编码大小仅以对数阶增长。
>
> **技术梗概:** 该研究采用了基于学习误差(LWE)假设的安全技术来实现上述目标，确保了计算过程中的安全性与效率。

---
### [推荐] [2026/1436] Verifiable and Collusion-Resistant Multi-Party Quantum Private Set Operations
...
- **链接:** [论文](https://eprint.iacr.org/2026/1436) | [PDF](https://eprint.iacr.org/2026/1436.pdf)


> **研究背景:** 私有集合交集（PSI）和更广泛的私有集合操作（PSO）是安全多方计算（SMC）中的基本原语，允许参与者在不泄露超出指定输出之外的任何信息的情况下共同计算集合关系。随着量子技术的发展，PSI 已经演进到量子安全保障阶段。
>
> **主要贡献:** 本文提出了一种多党量子私有集合交集（MP-QPSI）协议，该协议结合了可验证的量子全同态加密（vQFHE）作为可验证的外包量子评估层和门限全同态加密（TFHE）作为门限密钥管理机制。
>
> **达到效果:** 我们通过在 IBM Quantum 平台上进行模拟来实现交集计算，并分析了协议在对抗第三方、外部窃听者以及合谋行为时的正确性和参与者隐私性，同时证明了在语义安全模型下对恶意第三方的可验证性。
>
> **技术梗概:** 该协议利用 \(C^{\mathsf{AND}}\) 电路并结合门限全同态加密和可验证量子全同态加密技术来实现多党环境下的私有集合操作，并提供了一种模块化框架视角，展示了如何通过开放控制的操作将构造扩展到量子私有集合联合（QPSU）。

---
### [推荐] [2026/1437] SoK: On the Design of Secure Ranking-based E-voting Systems

- **匹配关键字:** homomorphic encryption

- **作者:** Luke Harrison, Feng Hao, Bowen Zheng, Bingsheng Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1437) | [PDF](https://eprint.iacr.org/2026/1437.pdf)


> **研究背景:** 针对电子投票系统中的选票隐私和可验证性两大核心属性，已有研究通过混排网、同态加密及零知识证明等密码技术实现了简单多数或 plurality 投票的安全保障。但对于基于排名的复杂投票系统（如 Borda 计数、Condorcet 投票、即时 runoff 投票和单转移可选票），现有设计在这些密码技术的应用及其对安全保证和社会选择属性的影响方面尚缺乏深入分析。
>
> **主要贡献:** 本文系统地分析了文献中基于排名的电子投票系统的加密设计，并探讨了所用密码技术如何影响其安全性和社会选择特性，包括支持的各种计数函数及选票投递的灵活性。
>
> **达到效果:** 研究发现，基于排名的电子投票系统可根据信息泄露程度大致分类，并且每类系统具有相似特性和相同的密码技术应用模式。
>
> **技术梗概:** 文章采用文献综述方法，对现有基于排名的电子投票系统的加密设计进行了全面分析，通过对比不同方案来揭示其安全和社会选择属性差异。

---
### [推荐] [2026/1439] ABE for Unbounded Depth Circuits from the Doubly Circular Assumption

- **匹配关键字:** LWE

- **作者:** Shweta Agrawal, Anuja Modi, Anshu Yadav, Shota Yamada

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1439) | [PDF](https://eprint.iacr.org/2026/1439.pdf)


> **研究背景:** 针对深度和宽度未受限的电路，现有ABE（属性基加密）构造依赖于不安全的假设或需要紧凑的功能加密或混淆技术。
>
> **主要贡献:** 本文提出了一种基于新颖且可验证的“双重循环”假设的新候选方案，该假设结合了循环LWE与ElGamal加密变体的关键相关性消息安全性。
>
> **达到效果:** 此构造在Shoup的双线性通用群模型中安全，排除了非通用攻击，并且在特定条件下由标准SXDH和循环LWE推导而来。
>
> **技术梗概:** 通过结合循环LWE与ElGamal加密变体的关键相关性消息安全性，本文提供了一种新的ABE方案以应对深度未受限的电路。

---
### [推荐] [2026/1444] Compressed Post-Quantum Silent OT from Isogenies

- **匹配关键字:** lattice, post-quantum

- **作者:** Pouria Fallahpour, Arthur Herlédan Le Merdy, Mahshid Riahinia

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1444) | [PDF](https://eprint.iacr.org/2026/1444.pdf)


> **研究背景:** Oblivious Transfer (OT) 是安全计算中的基础工具，而伪随机关联函数（PCFs）能够高效地生成大量关联对。然而，在后量子环境中，现有的解决方案导致了较大的密钥大小。
>
> **主要贡献:** 本文提出了第一个基于超曲线上层结构的后量子 PCF 构造方法，实现了紧凑的密钥尺寸，并提供了首个此类构造的安全性证明。
>
> **达到效果:** 该后量子 PCF 的密钥大小在 100 kB 范围内，是目前最紧凑的后量子替代方案的七分之一。此外，其性能估计为每秒处理 7 次 OT。
>
> **技术梗概:** 本文的核心技术在于一种新的紧凑约束伪随机函数（CPRF），用于内部积成员谓词，并基于自然扩展的 pa... 

---
### [推荐] [2026/1445] Sublinear-Communication Layered MPC from HSS

- **匹配关键字:** homomorphic encryption

- **作者:** Anasuya Acharya, Pierre Meyer, Divya Ravi, Rahul Satish

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1445) | [PDF](https://eprint.iacr.org/2026/1445.pdf)


> **研究背景:** 现有支持动态参与的多方计算(MPC)协议要么通信成本与电路大小成线性关系，要么依赖全同态加密(FHE)，这在实际长运行应用中造成了瓶颈。
>
> **主要贡献:** 本文提出了基于层次化密钥分享(HSS)的子线性通信层次化MPC协议，实现了在线服务器数量最少为每层2或3个的同时保持子线性通信成本。
>
> **达到效果:** 该研究显著减少了动态参与场景下的通信开销，提升了实际应用中的效率和可行性。
>
> **技术梗概:** 通过将HSS进行分层友好的重新设计并形式化为具有层次重组分享的HSS技术来实现上述目标。

---
### [2026/1421] A Cryptographic Perspective on California's Delete Request and Opt-out Platform

- **作者:** Aarushi Goel, Gabriel Kaptchuk, Peihan Miao, Phuoc Van Long Pham, Satvinder Singh, Rachel E. Thomas

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1421) | [PDF](https://eprint.iacr.org/2026/1421.pdf)


> **研究背景:** 研究背景：加州消费者隐私法(CCPA)赋予了消费者请求删除个人数据的权利，而2023年加州的删除法案通过引入一个集中的删除请求和退出平台(DROP)，使得用户能够提交单一请求并由所有注册的数据经纪人定期处理。
>
> **主要贡献:** 主要贡献在于从密码学角度评估了第一代DROP系统的隐私架构，并提出了改进方案以提高其安全性。
>
> **达到效果:** 达到的效果是识别并分析了基于哈希的记录链接方法中存在的固有风险，提出了一些简单的轻量级修改措施来增强隐私保护。
>
> **技术梗概:** 技术梗概涉及使用私有集合交集(PSI)等可证明安全的密码学协议来替代现有的哈希记录链接方法，以确保仅揭示预期匹配的记录而不会泄露其他信息。

---
### [2026/1424] Repeated Masks, Predictable Splices: Breaking AESpolyW and Its AE Applications

- **作者:** Shuping Mao, Peng Wang, Jiadong Han, Tingting Guo, Yan Jia, Ying Chen

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1424) | [PDF](https://eprint.iacr.org/2026/1424.pdf)


> **研究背景:** AESpolyW是一种宽块加密模式，旨在利用AES和多项式哈希指令级并行性。尽管它在吞吐量上表现出色，并且在大多数测试平台上优于HCTR2和EME，但该研究揭示了其安全弱点。
>
> **主要贡献:** 研究人员提出了针对AESpolyW的攻击方法，仅需少量查询即可区分其实现与理想的安全模型之间的差异。
>
> **达到效果:** 这些攻击不仅破坏了基于AESpolyW的认证加密方案，还导致了选择密文攻击成功，并完全破解了IND-CCA安全性。
>
> **技术梗概:** 研究采用了重复掩码和可预测拼接的技术，通过少量查询揭示了AESpolyW的关键弱点。

---
### [2026/1426] Coupling Leakage in Theory and Practice - Unveiling (Post-PnR) Security Flaws in Masked FPGA-Mapped Designs

- **作者:** Nicolai Müller, Daniel Lammers, Simon Osterheider, Amir Moradi

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1426) | [PDF](https://eprint.iacr.org/2026/1426.pdf)


> **研究背景:** 随着FPGA在国防和电信等安全关键行业中的广泛应用，确保敏感数据处理的安全性变得至关重要。然而，侧信道分析（SCA）构成了显著威胁，需要通过有效的对策来保护密码学原语的机密性。
>
> **主要贡献:** 本文作者揭示了基于FPGA实现的掩码设计中未被检测到的耦合效应导致的安全漏洞，并提出了实验和工具辅助评估的方法。
>
> **达到效果:** 研究结果表明，即使在形式验证工具能够验证ASIC中的安全实现的情况下，FPGA中的耦合泄漏仍可能逃过这些工具的检测，从而揭示了现有防护措施的不足。
>
> **技术梗概:** 通过结合实验方法和自动化工具对掩码FPGA设计进行评估，作者发现了低级别漏洞，并提出了改进方案以增强其安全性。

---
### [2026/1427] SPiRiT: Verifiable Contact Tracing for Balanced Security, Privacy, and Safety

- **作者:** Reyhaneh Rabaninejad, Azra Abtahi, Antonis Michalas, Amir Aminifar

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1427) | [PDF](https://eprint.iacr.org/2026/1427.pdf)


> **研究背景:** 现有的去中心化隐私保护接触追踪协议通常缺乏可验证性，这导致了数据上传行为的不透明性和潜在的安全漏洞。
>
> **主要贡献:** SPiRiT引入了一种新的可验证接触追踪协议，通过确保上传的数据合法且有效来解决这些问题，同时保持用户隐私。
>
> **达到效果:** 该研究成功地实现了在保护隐私的同时增强系统的安全性和可靠性，从而提高了公众对接触追踪应用的信任度和接受度。
>
> **技术梗概:** SPiRiT利用零知识证明技术验证上传数据的有效性，并通过密码学手段确保用户的匿名性。

---
### [2026/1428] Highly Efficient Consistent Broadcast Encryption

- **作者:** Konstantin Gegier, Eike Kiltz, Roman Langrehr, Guilherme Rito

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1428) | [PDF](https://eprint.iacr.org/2026/1428.pdf)


> **研究背景:** 公共密钥广播加密（$\mathsf{PKEBC}$）是一种多接收者加密原语，确保所有指定的接收者在解密时得到相同的消息。然而，当前没有高效的$\mathsf{PKEBC}$构造方法，已知的方法通过非交互式零知识证明来实现一致性，这导致了复杂性和成本问题。
>
> **主要贡献:** 该研究提出了两种新的通用$\mathsf{PKEBC}$构造：$\mathsf{PKEBC}_{\mathsf{SM}}$和$\mathsf{PKEBC}_{\mathsf{FO}[\mathsf{mPKE}]}$，并在标准模型和随机预言机模型中证明了它们的安全性。
>
> **达到效果:** 通过优化这些构造的实例化，研究实现了在保持线性大小密文的同时，确保解密一致性，并显著提高了效率。
>
> **技术梗概:** 该方法利用非交互式零知识证明来实现解密的一致性。特别地，$\mathsf{PKEBC}_{\mathsf{SM}}$通过精心设计的非交互式零知识证明陈述来优化这一过程。

---
### [2026/1429] Structured Search for a Separable Subclass of Generalized Integral Properties

- **作者:** Dongchen Chai, Shenghu Hu, Thomas Peyrin, Zilong Wang, Trevor Yap, Hongyi Zhang, Liu Zhang, Chunning Zhou

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1429) | [PDF](https://eprint.iacr.org/2026/1429.pdf)


> **研究背景:** 研究背景：通用积分性质扩展了经典积分区分器，但由于通用函数空间的大小问题，其搜索变得困难。本文通过限制通用布尔函数为可分形式来研究一个结构化且易于处理的子类，从而将搜索转化为明文侧抑制传播源与密文侧未知项消去之间的相互作用。
>
> **主要贡献:** 主要贡献：提出了基于矩阵的方法来避免对映射空间进行穷举枚举，包括统一的MBM框架和针对固定明文结构的专业求解器，后者通过高斯消元法解决了线性消去系统。
>
> **达到效果:** 达到的效果：实验验证了所提出方法的实际有效性，对于SPECK和SIMON，搜索扩展了轮数，并发现了新的区分器实例。
>
> **技术梗概:** 技术梗概：本文基于矩阵的方法将联合搜索转化为布尔约束系统，适用于混合整数线性规划（MILP），并通过高斯消元法解决了密文侧的低阶映射搜索问题。

---
### [2026/1431] The Isogeny Problems

- **作者:** Wouter Castryck, Luca De Feo, Steven D. Galbraith, Péter Kutas, Krijn Reijnders, Benjamin Wesolowski

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1431) | [PDF](https://eprint.iacr.org/2026/1431.pdf)


> **研究背景:** 本文综述了基于曲率的密码学领域中七个最重要的未解问题，这些问题由该领域的多位专家提出并精选而成。
>
> **主要贡献:** 贡献在于为研究者提供了明确的研究方向和具体的问题陈述，有助于推动该领域的进一步发展。
>
> **达到效果:** 通过详细阐述每个问题的具体表述和描述，本文促进了对该领域关键挑战的理解与探讨。
>
> **技术梗概:** 作者采用问卷调查的方式收集专家意见，并对收到的众多建议进行了筛选和整理。

---
### [2026/1432] Reed–Solomon Mutual Correlated Agreement Beyond the Johnson Radius

- **作者:** Sunghyeon Jo

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1432) | [PDF](https://eprint.iacr.org/2026/1432.pdf)


> **研究背景:** 研究背景：互相关协议（MCA）用于分析基于代码的证明系统的健壮性，特别是在接收单词线上多个坐标与编码词一致时。现有确定性边界仅适用于Johnson界限以下的整数错误预算。
>
> **主要贡献:** 主要贡献：对于任意指定评估集的Reed-Solomon码，在足够大的K下，提出了超越Johnson边界的MCA误差上限，为不同速率提供了所需的平滑域支持。
>
> **达到效果:** 达到的效果：在K=2^18时，首次超过Johnson边界后，MCA错误率低于2^-128，适用于1/2, 1/4, 1/8和1/16的速率；第二阶段边界在1/8速率下同样达到了极低的误差水平。
>
> **技术梗概:** 技术梗概：通过设定特定参数的Reed-Solomon码，并利用有限域中的点集，研究了在非共同编码词解释下的MCA误差界限，从而扩展了现有理论的应用范围。

---
### [2026/1433] Shuffled-ArgMax: Securing ArgMax Decisions Against Side-Channel-Guided Fault Injection in Edge-AI IoT Devices

- **作者:** Le Wu, Liji Wu, Yuyang Pan, Xiangmin Zhang, Jian Wu

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1433) | [PDF](https://eprint.iacr.org/2026/1433.pdf)


> **研究背景:** 针对边缘AI物联网设备中的神经网络推理最终决策阶段的安全性研究不足，特别是基于ArgMax的决策逻辑在物理攻击下的脆弱性。
>
> **主要贡献:** 提出了Shuffled-ArgMax防御机制，通过随机化遍历、循环完整性检查和冗余决策验证来增强安全性。
>
> **达到效果:** 实验表明，在相同的攻击设置下，Shuffled-ArgMax将目标重定向的成功率从56.92%降低到0%，显著提高了系统的鲁棒性。
>
> **技术梗概:** 该方法通过在ArgMax实现中引入随机化和冗余验证机制来混淆侧信道信息，并防止电压故障注入攻击的利用。

---
### [2026/1434] BeeKEM: Decentralized, Secure and Efficient Group Key Agreement

- **作者:** Derek Yen, Andrés Fábrega, Liangrun Da, Martin Kleppmann, John Mumm, Sunoo Park, Brooklyn Zelenka

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1434) | [PDF](https://eprint.iacr.org/2026/1434.pdf)


> **研究背景:** 现有的群组密钥协商协议通常依赖于一个半可信的中心服务来协调通信，这在某些应用中可能成为监视和审查的瓶颈。因此，研究一种去中心化的解决方案变得至关重要。
>
> **主要贡献:** BeeKEM 是首个具备对数级更新成本（最坏情况下为线性）且有安全证明的去中心化群组密钥协商协议。
>
> **达到效果:** 通过提供开源实现并展示其与OpenMLS相当的竞争性能，BeeKEM 为多种通信和协作应用提供了可能，这些应用不仅支持端到端加密，还提供元数据隐私和抗审查特性。
>
> **技术梗概:** BeeKEM 使用创新的算法和技术来减少更新成本，并通过形式化验证确保其安全性。

---
### [2026/1435] FiltrumVote: Scalable, Verifiable, and Coercion-Resistant Internet Voting

- **作者:** Leonardo Kimura, Dimitri Leskow, Diego F. Aranha, Roberto Araújo, Marcos Simplicio

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1435) | [PDF](https://eprint.iacr.org/2026/1435.pdf)


> **研究背景:** 互联网投票可以提高选民参与度并降低运营成本，但同时也增加了选民被胁迫的风险。目前缺乏同时具备可验证性、抗胁迫性和可扩展性的解决方案。
>
> **主要贡献:** FiltrumVote通过引入可否认的投票更新机制，实现了在保持投票可验证和抗胁迫的同时，还具有高度可扩展性。
>
> **达到效果:** 实验结果显示，使用标准计算机证明一百万票的所有零知识证明仅需不到5小时，显著提升了系统的效率和实用性。
>
> **技术梗概:** FiltrumVote基于零知识证明与标准Σ协议技术构建，通过非线性的清洁阶段确保了投票的私密性和可验证性。

---
### [2026/1438] Vela and Carina: Fast Pairing-Based Multilinear Polynomial Commitments from Reciprocal Polynomials

- **作者:** Yuncong Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1438) | [PDF](https://eprint.iacr.org/2026/1438.pdf)


> **研究背景:** 基于配对的多线性多项式承诺提供了基于求和检查证明系统的简洁验证，但现有方案在证明者工作量与证明大小及验证者成本之间存在权衡。
>
> **主要贡献:** 提出了两种具有不同性能权衡的方案：Vela和Carina。前者利用逆多项式的对称性减少辅助多项式的长度；后者在同一坐标上应用常数项约简，并联合证明承诺双变量多项式及其两个辅助多项式的评估声明。
>
> **达到效果:** 在BLS12-381曲线上，当$\mu=20$时，Vela提供了最小的测量证明（232字节）和最低的验证时间（2.09毫秒），而Carina则实现了精确的开窗MSM系数数量。
>
> **技术梗概:** 通过利用逆多项式的对称性，Vela减少了辅助多项式长度并使用单个一元KZG证明在$z$和$z^{-1}$处打开两个多项式。Carina在同一坐标上应用一次常数项约简，并联合证明承诺双变量多项式及其两个辅助多项式的评估声明。

---
### [2026/1440] Rarus: A Succinct and Efficient Range Proof for Polynomial-based Vector Commitment

- **作者:** Xinyang Yang, Wenjie Qu, Yanpei Guo, Jiaheng Zhang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1440) | [PDF](https://eprint.iacr.org/2026/1440.pdf)


> **研究背景:** 范围证明使证明者能够在不泄露额外信息的情况下，向验证者证明一个承诺值位于特定区间内。这对于匿名凭证、电子投票、电子现金以及如Monero和Grin等加密货币中的隐私保护系统至关重要。然而，高效地同时证明多个承诺值满足范围约束并最小化通信开销是一个关键挑战。
>
> **主要贡献:** Rarus通过引入优化的$b$-元分解，并结合双变量零测试和加速单变量求和检查协议，实现了所有度量指标下的最优渐近复杂性。
>
> **达到效果:** 实验结果表明，与Bulletproofs和Missileproof相比，Rarus在证明者时间和验证者时间上分别获得了20倍以上的速度提升。
>
> **技术梗概:** Rarus采用了优化的$b$-元分解替代二进制分解，并结合了双变量零测试和加速单变量求和检查协议来实现其高效性。

---
### [2026/1441] RainHash2.0: Hardware- and Arithmetization-friendly Hash Function

- **作者:** Jiamin Cui, Lorenzo Grassi, Katharina Koschatko, Florian Krieger, Shibam Mukherjee, Christian Rechberger, Sujoy Sinha Roy, Markus Schofnegger, Verena Schröppel

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1441) | [PDF](https://eprint.iacr.org/2026/1441.pdf)


> **研究背景:** 随着零知识（ZK）证明系统的快速发展，哈希函数已成为其核心构建块之一。由于这些哈希函数通常主导了证明者的成本，因此电路友好的哈希函数设计成为研究热点。
>
> **主要贡献:** RainHash2.0 是一种新型的密码学置换，它既适用于二元扩张域 \(\mathbb{F}_{2^n}\) 基础协议，又能在硬件和软件中高效运行，填补了现有设计在硬件加速和通用计算性能方面的空白。
>
> **达到效果:** 通过采用源自 Binius 的新技术水平拆分轮函数，RainHash2.0 实现了在不同大小有限域同时使用时的特别有效性，并成功地在 Binius 和 VOLE 基础协议中进行了实现。
>
> **技术梗概:** RainHash2.0 通过创新性地将轮函数水平拆分，结合二元扩张域特有的设计方法，实现了高效硬件和快速通用计算性能的双重目标。

---
### [2026/1442] CoSecRAG: Efficient Private Retrieval with Database and Query Privacy for RAG

- **作者:** Moyang Xie, Jiacheng Gao, Yuan Zhang, Sheng Zhong

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1442) | [PDF](https://eprint.iacr.org/2026/1442.pdf)


> **研究背景:** CoSecRAG旨在解决检索增强生成(RAG)中的隐私问题，即用户查询可能泄露敏感意图，而检索数据库中可能包含专有或机密记录。
>
> **主要贡献:** CoSecRAG通过采用内积保持查询掩码(IPQ-Mask)和评分后聚类剪枝(PSCP)两项核心技术，实现了数据库和查询嵌入的隐私保护。
>
> **达到效果:** 实验结果显示，CoSecRAG在保持检索质量的同时显著降低了检索成本，相较于最新的双服务器私有RAG检索基线，在分数计算、安全选择Top-K以及端到端在线处理速度上分别提高了202倍、107倍和123倍，并减少了高达43%的通信量。
>
> **技术梗概:** IPQ-Mask通过一次性相关掩码将秘密共享向量上的安全内积计算转换为公共掩码查询差值的本地线性计算，从而在分数计算中消除在线安全乘法；PSCP则逆转了传统的剪枝前评分流程，在评分后进行隐私保护的候选者聚类剪枝。

---
### [2026/1443] SENTRA:Privacy-Preserving Training in Outsourced Cloud Environments

- **作者:** Maryam Zarezadeh, Jana Eisoldt, Bhavish Mohee, Stefan Köpsell, Behzad Abdolmaleki

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1443) | [PDF](https://eprint.iacr.org/2026/1443.pdf)


> **研究背景:** 在不可信云环境中训练机器学习模型需要保密性、完整性和正确性的强保障，同时保持可扩展性和对节点变更的鲁棒性。现有可信执行环境(TEE)缺乏可扩展性且易受侧信道攻击，而纯粹的安全多方计算(MPC)方法在实践中会带来高昂的开销。
>
> **主要贡献:** SENTRA通过结合TEE、秘密共享和通信高效的MPC，并通过系统级机制确保整个训练生命周期的安全性，解决了这些挑战。它引入了一种可扩展的集体验证协议，该协议在任何节点可以存储或处理秘密份额之前验证所有参与的TEE并强制执行硬件独占。
>
> **达到效果:** SENTRA能够在异构云环境中提供安全、高效的机器学习模型训练，并支持动态、容错成员管理，同时提供了回滚保护和在对抗条件下的一致状态。
>
> **技术梗概:** 该方法通过TEE、秘密共享技术和通信高效MPC的混合架构实现，并结合系统级机制确保整个训练生命周期的安全性。SENTRA还引入了一种基于动态主动秘密分享(DPSS)的重新分配方案，以及安全打包-MPC计算和节点故障的自适应处理策略。
...
### [推荐] [2026/1318] Cryptanalysis of HAWK: a Guessing Game

- **匹配关键字:** lattice, post-quantum

- **作者:** Ben Nelson, Joshua Limbrey, Cong Ling, Andrew Mendelsohn

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1318) | [PDF](https://eprint.iacr.org/2026/1318.pdf)


> **研究背景:** HAWK是一种基于格同构问题（LIP）的后量子签名方案，但该研究提出了一种经典算法，能够在概率多项式时间内通过四个数论假设恢复其密钥。
>
> **主要贡献:** 研究人员提出了一个新颖的攻击策略，将HAWK底层的秩2模块-LIP实例归约到nrdPIP问题，并利用Lenstra-Silverberg算法解决特定条件下的实例以破解HAWK。
>
> **达到效果:** 该方法通过不断尝试不同的随机下三角单位矩阵U来构造新的Gram矩阵G'，并测试其是否容易求解，最终成功恢复了部分有效的HAWK私钥。
>
> **技术梗概:** 研究采用了一种新颖的“重新随机化”技术，即在固定模块-LIP实例的情况下，通过变换Gram矩阵来寻找易于解决的问题实例。

---
### [推荐] [2026/1322] Reliable Homomorphic Matching for Fuzzy Labeled PSI at Scale
...
- **链接:** [论文](https://eprint.iacr.org/2026/1322) | [PDF](https://eprint.iacr.org/2026/1322.pdf)


> **研究背景:** FLPSI允许接收方了解与查询相似的已注册记录的标签，而不会透露其他信息。该研究关注于构建基于集合阈值减少的高效实现，并探讨其同态形式以提高性能。
>
> **主要贡献:** 作者识别并解决了现有内核中的可靠性问题，提出了CSTPSI方案来降低错误接受率，并确保了零或可忽略不计的可靠性误差。
>
> **达到效果:** 通过引入独立令牌轮次和匹配幂级数，研究实现了较低的错误接受率，并提高了同态模糊匹配协议的整体可靠性。
>
> **技术梗概:** 该方法结合了层次-BFV同态加密、门电路和秘密共享技术来决定加密下的匹配结果并释放记录标签。

---
### [推荐] [2026/1331] The Most Efficient Protocol for PAKE: What Exact Stuff Do You Need to Hash at the End?
...
- **链接:** [论文](https://eprint.iacr.org/2026/1331) | [PDF](https://eprint.iacr.org/2026/1331.pdf)


> **研究背景:** PAKE协议允许双方在仅共享低熵密码的情况下，共同建立一个加密会话密钥。OEKE-2F协议因其高效性和灵活性而受到关注，但在具体实现细节上存在争议，导致了多种潜在的变体。
>
> **主要贡献:** 作者对所有16种OEKE-2F变体进行了全面分析，并证明了每一种的安全性。研究结果揭示了不同变体之间的差异及其对KEM安全性的要求。
>
> **达到效果:** 研究表明，“哈希一切”版本需要KEM最少的安全属性，而移除哈希中的项目越多，则对KEM的要求越高。
>
> **技术梗概:** 通过形式化分析和证明技术，作者系统地评估了每个变体的UC安全性，并提供了详细的比较结果。

---
### [推荐] [2026/1333] Apples, Oranges, and Signatures: Pitfalls and Methodology in ML-DSA Benchmarking

- **匹配关键字:** post-quantum

- **作者:** Sebastien Riou, Jong-Yeon Park, Liga Anwar, Axel Poschmann, Michael Hutter

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1333) | [PDF](https://eprint.iacr.org/2026/1333.pdf)


> **研究背景:** 在后量子加密环境中，密码学迁移提出了显著的实践挑战，并需要可靠的数据支持工程决策。然而，现有的机器学习数字签名算法（ML-DSA）基准测试方法往往会产生误导性的或不可比较的结果，这使得迁移和加密灵活性的努力复杂化。
>
> **主要贡献:** 该研究分析了ML-DSA签名操作基准测试中的常见陷阱，包括在比较安全性级别时的细微不一致，并提出了基于标准化输入数据集和明确合格报告指标的稳健基准测试方法。
>
> **达到效果:** 通过这种方法，可以公平地跨硬件和软件实现进行比较，并支持实时系统设计者评估最坏情况执行时间。
>
> **技术梗概:** 该研究强调了使用标准化输入数据集的重要性，并提出了清晰界定的报告指标来解决由于拒绝采样和其他数据依赖组件导致的ML-DSA签名算法执行时间变异性问题。

---
### [推荐] [2026/1374] Analysing the Post-Quantum Security of S/MIME

- **匹配关键字:** post-quantum

- **作者:** Sayan Das, Anupam Chattopadhyay

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1374) | [PDF](https://eprint.iacr.org/2026/1374.pdf)


> **研究背景:** S/MIME作为一种基于证书的电子邮件签名和加密机制，其后量子迁移技术上可行，但仍存在安全保障问题。
>
> **主要贡献:** 论文通过将分析单位从证书转变为加密消息，提出了SMIME-PQCheck验证框架，确保了每条有效路径都符合活动迁移策略。
>
> **达到效果:** 该研究揭示了一个单一的ML-KEM接收者路径不足以保证后量子保密性，除非所有其他有效的经典路径都无法恢复相同的密钥。
>
> **技术梗概:** 通过结合X.509配置文件检查、CMS接收器结构分析以及基于政策的混合处理方法，实现了对S/MIME消息的安全验证。

---
### [推荐] [2026/1381] PriFT: Private Fine-Tuning using off-the-shelf MPC and HE libraries

- **匹配关键字:** homomorphic encryption

- **作者:** Qiuxuan Ma, Eleftheria Makri, Nusa Zisaric

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1381) | [PDF](https://eprint.iacr.org/2026/1381.pdf)


> **研究背景:** 隐私保护机器学习（PPML）旨在整个机器学习管道中保持数据隐私，但在实践中，训练阶段的输入隐私加密计算仍具有较高的计算成本。
>
> **主要贡献:** PriFT框架通过结合多方计算(MPC)和同态加密(HE)，实现了在不泄露客户隐私的情况下对客户数据进行早期利用，并支持完全私有和半私有的训练模式。
>
> **达到效果:** 实验结果表明，MPC方法在半私有场景中显著优于HE方法，在性能上表现更佳。
>
> **技术梗概:** PriFT框架使用transformer作为特征提取器，并通过Crypten和TenSEAL这两个成熟的库来实现基于MPC和HE的安全训练技术。

---
### [推荐] [2026/1384] Lower Bounds for PIR with Preprocessing from Blackbox Cryptography
...
- **链接:** [论文](https://eprint.iacr.org/2026/1384) | [PDF](https://eprint.iacr.org/2026/1384.pdf)


> **研究背景:** 研究了单服务器私有信息检索（PIR）在预处理下的通信界限，特别是在黑盒密码学使用下，先前工作表明，具有亚线性通信的单服务器PIR需要每查询线性数量的服务器操作。
>
> **主要贡献:** 提出了任何利用黑盒密码学进行计算的单服务器PIR预处理方案的计算下界，证明了在线平均计算量必须为Ω(n/s)，即使在批量查询中也成立。
>
> **达到效果:** 结果表明，在客户端存储s比特的情况下，对于n比特数据库，每次查询的平均计算量至少为Ω(n/s)；否则，服务器需要执行至少Ω(n/s)次加密操作。这些下界是最佳的，因为存在满足其中一个要求但不满足另一个要求的PIR方案。
>
> **技术梗概:** 通过构建特定的客户端预处理方案和分析其通信复杂性和计算复杂性来证明下界，确保了结果的有效性和广泛适用性。

---
### [推荐] [2026/1386] Key-Recovery Attacks on TALUS: A Cryptanalytic Note
...
- **链接:** [论文](https://eprint.iacr.org/2026/1386) | [PDF](https://eprint.iacr.org/2026/1386.pdf)


> **研究背景:** 文章针对TALUS（Kao和Chang提出的一种阈值ML-DSA构造）进行了密钥恢复攻击研究，揭示了其安全性的潜在缺陷。
>
> **主要贡献:** 作者发现了两种独立的攻击方法，分别适用于TALUS-MPC和TALUS-TEE/TALUS-MPC，并指出这些攻击利用了Feldman承诺中的可逆性以及签名过程中的错误项泄露问题。
>
> **达到效果:** 通过这两种攻击方法，研究者能够直接从密钥生成广播中恢复所有$s_1$的密钥份额，并从签名广播中独立地恢复聚合nonce $y$，从而计算出秘密值。此外，对于TALUS-TEE和TALUS-MPC，研究者还发现了一个持续存在的缺陷，即标准ML-DSA中的拒绝采样检查已被移除，导致每个签名泄露一个噪声线性方程在$s_2$中；通过在cyclotomic环上应用最小二乘恢复技术，可以从数百万个签名中恢复完整的秘密。
>
> **技术梗概:** 研究采用了密钥恢复和线性方程组求解的方法来攻击TALUS构造。对于TALUS-MPC，利用了Feldman承诺的可逆性和Gaussian消元法；而对于TALUS-TEE/TALUS-MPC，则通过最小二乘方法在cyclotomic环上进行恢复以提取秘密值。

---
### [推荐] [2026/1392] Slicing Boolean Functions with Inner Products

- **匹配关键字:** homomorphic encryption

- **作者:** Pierrick Méaux, Tim Seuré

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1392) | [PDF](https://eprint.iacr.org/2026/1392.pdf)


> **研究背景:** 研究背景：具有额外结构的布尔函数在对称密码学中扮演重要角色，有助于实现强大的密码属性并促进高效实现。近期关于同态友好的对称原语的研究，特别是在混合同态加密背景下，强调了能够根据布尔立方体的结构化分区进行分解的布尔函数的兴趣。
>
> **主要贡献:** 主要贡献：本文通过用一般整数线性形式替换汉明权重，推广了这一观点，并研究了在每个v-切片上具有有界度的函数所需的切片数量及其结构特性。
>
> **达到效果:** 达到的效果：提供了由整数向量诱导的分区的边界和结构性质，并展示了这种表示如何在GSW类似设置中实现同态评估策略，同时给出了结果密文中的噪声估算。
>
> **技术梗概:** 技术梗概：通过使用不同向量家族对多种布尔函数进行直接推广，并实验性地评估了它们的代数度、代数免疫性和非线性。

---
### [推荐] [2026/1397] HANNS: Low-Storage Non-Interactive Approximate Private Nearest Neighbor Search with Sublinear Comparison Complexity

- **匹配关键字:** homomorphic encryption

- **作者:** Haowen Pan, Ruiqi Gan, Yunhao Fu, Yintai Sun, Zhou Zhang, Yuxiang Wang, Yi Chen, Bo Zhang, Haoyi Zhou, Yongxin Tong, Zhenyu Guan, Jin Dong, Song Bian

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1397) | [PDF](https://eprint.iacr.org/2026/1397.pdf)


> **研究背景:** 随着对数据隐私的关注增加，私人最近邻搜索(PNNS)引起了越来越多的研究兴趣。现有的PNNS主要分为两类：交互式PNNS利用安全多方计算协议和索引结构实现亚线性复杂度；非交互式PNNS使用全同态加密以减少通信带宽但导致超线性计算复杂度。
>
> **主要贡献:** 我们提出了HANNS，一种具有亚线性加密比较次数的非交互式PNNS协议。该方案通过开发基于FHE的集群排序方案和引入同态产品量化(PQ)方案来实现这一目标。
>
> **达到效果:** 实验结果显示，与最新的非交互式方案相比，HANNS在速度上提高了41到277倍，并减少了12到31.7倍的存储空间；同时，在低带宽场景下，通信量降低了1,258到80,536倍，相对于交互式方案实现了8到119倍的速度提升。
>
> **技术梗概:** HANNS通过开发一种基于FHE的集群排序方案和引入同态产品量化(PQ)方案来减少加密比较次数，并实现粗略搜索和重新排名，从而显著降低计算和存储开销。

---
### [推荐] [2026/1399] CHIP: Efficient Homomorphic Encryption-Based CNN Batch Inference Using Channel-Interleaved Packing with Small Rotation Key Set
...
- **链接:** [论文](https://eprint.iacr.org/2026/1399) | [PDF](https://eprint.iacr.org/2026/1399.pdf)


> **研究背景:** 随着隐私问题的日益突出，法律法规要求机器学习应用必须遵守严格的隐私规定。现有的基于同态加密（HE）的推理解决方案在单样本和多样本场景下都存在显著的计算和内存开销，并且需要大量旋转密钥，限制了其在更广泛的应用场景中的实用性。
>
> **主要贡献:** 本文提出了一种名为CHIP的方法，通过将三维数据嵌入二维密文中实现同态卷积操作，并引入高效的二维卷积技术减少HE乘法次数，同时采用部分核和小批量策略提高计算密集型推理任务的性能。
>
> **达到效果:** 实验结果表明，在单样本和多样本场景下，与Lee等人（ICML'22）和Cheon等人（IEEE TDSC'24）提出的最先进的同态加密方法相比，本方法表现出更高的效率。使用ResNet18、VGG11和VGG16模型并设置批量大小为64时，我们的解决方案可实现高达4.7倍的速度提升。
>
> **技术梗概:** 通过通道交织打包（CHIP），将三维数据嵌入二维密文中，并结合通道聚合和密文旋转来执行同态卷积操作；引入了一种高效的二维卷积技术以减少HE乘法次数；采用部分核和小批量策略迭代处理切片内核和样本子集，最终汇总结果生成输出。

---
### [推荐] [2026/1400] What Happens When integrating Modulus Switching and Lossy Source Coding: A New Dual Attack Variant on LWE
...
- **链接:** [论文](https://eprint.iacr.org/2026/1400) | [PDF](https://eprint.iacr.org/2026/1400.pdf)


> **研究背景:** 随着大规模量子计算机对经典公钥密码学构成威胁，后量子密码方案的研发成为热点。LWE问题作为这些方案的安全基础，其破解方法如双筛FFT攻击备受关注。
>
> **主要贡献:** 本文提出了一种新的双筛FFT攻击变体，结合了模切换和有损源编码技术，显著降低了FFT步骤中的枚举大小。
>
> **达到效果:** 该方法在应用于KYBER时，虽然整体复杂度提升不大，但解码和FFT成本分别减少了1-6位和2-7位，在多目标攻击场景中具有实际意义。
>
> **技术梗概:** 通过理论分析，本文展示了如何有效整合模切换与有损源编码技术来优化双筛FFT攻击流程。

---
### [推荐] [2026/1401] A New Framework for Efficient Multivariate Functional Bootstrapping

- **匹配关键字:** homomorphic encryption

- **作者:** Kunyu Wu, Kuiyuan Duan, Dengfa Liu, Hongbo Li

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1401) | [PDF](https://eprint.iacr.org/2026/1401.pdf)


> **研究背景:** 该研究致力于提高全同态加密(FHE)中可编程重采样(PBS)的效率，特别是在处理多变量函数时，通过减少查找表(LUT)的大小来优化计算过程。
>
> **主要贡献:** 作者提出了一种基于变量分离和加性内部表示的方法来压缩多变量LUT，并将其应用于非负整数除法，实现了在理论上最优参数下的盲旋转复杂度为$\widetilde{O}(1)$。同时，该方法还提供了模减法和截断除法的框架。
>
> **达到效果:** 实验结果表明，在$t=64$时，该方法在除法运算中提高了3.6倍的速度，并且对于汉明权重区间函数提高了1.9倍的速度。
>
> **技术梗概:** 研究采用了对数变换分解二元除法为两个一元对数PBS调用、一次同态减法和一次外层指数PBS调用。此外，通过模拟退火带重热的方法寻找具有小跨度的表示形式来进一步优化算法性能。

---
### [2026/1351] Hardware Private Cubic Circuits

- **作者:** Frederik Reiter, Amir Moradi

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1351) | [PDF](https://eprint.iacr.org/2026/1351.pdf)


> **研究背景:** 硬件加密实现经常通过侧信道泄露秘密信息，这可能导致攻击者在不利用算法漏洞的情况下窃取密钥等敏感数据。现有的防侧信道攻击技术如掩码技术虽然有效但会带来显著的面积、延迟和随机性开销。
>
> **主要贡献:** 本文提出了HPCC，一种低延迟的3输入乘法器Gadget，实现了任意字段下的常数周期延迟，并允许在单个周期内计算多个乘法操作，具有较低的额外开销。
>
> **达到效果:** 该工作填补了高阶PINI Gadget在实现复杂电路方面的空白，为硬件加密设计提供了新的工具和方法。
>
> **技术梗概:** HPCC通过巧妙地设计Gadget结构来保持恒定延迟，并利用共享计算技术优化多乘法操作的效率，同时确保安全性和实用性。

---
### [2026/1369] Algebraic Modelings of the Supersingular Isogeny Problem

- **作者:** Alessio Caminata, Andrea Sanguineti, Silvia Sconza

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1369) | [PDF](https://eprint.iacr.org/2026/1369.pdf)


> **研究背景:** 本文提出了一种新的超奇异曲线上同源问题的代数建模方法，将其表示为多元多项式方程组。该模型基于Renes公式，适用于通过2或3次同源连接的蒙哥马利形式或三角形形式的椭圆曲线。
>
> **主要贡献:** 作者们首次将超奇异曲线上同源问题转化为特定次数的多项式系统，并研究了这些系统的代数性质，证明了它们是零维的，并计算了最高次数部分的维度。
>
> **达到效果:** 实验结果表明，通过Gröbner基技术求解这些系统比使用模多项式方法更快。
>
> **技术梗概:** 该模型利用Renes公式将椭圆曲线表示为特定形式的多项式方程组，并分析了这些系统的代数特性。

---
### [2026/1370] smklhs: Succinct Multi key Linearly Homomorphic Signatures for Certified Statistics

- **作者:** Diego F. Aranha, Cecilia Boschini, Hanna Ek, Elena Pagnin

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1370) | [PDF](https://eprint.iacr.org/2026/1370.pdf)


> **研究背景:** 研究背景：在多个独立数据源贡献的数据集上认证统计声明的问题，需要一个不可信的服务器聚合签名数据记录并发布如总和、平均值或比率等声明。第三方可以验证这些声明是否正确，无需访问底层记录，同时保持证明大小和验证成本足够小以实现实际部署。
>
> **主要贡献:** 主要贡献：smklhs是一种多密钥线性同态签名方案，实现了在计算中涉及的不同签署者数量对最终签名大小的影响为对数级，而与输入消息总数无关的高效认证机制。
>
> **达到效果:** 达到的效果：证明了smklhs在随机Oracle和代数组模型下对适应性敌手的安全性，并使用高性能配对库RELIC实现了该方案，验证了其实用性。
>
> **技术梗概:** 技术梗概：通过设计一种新的多密钥线性同态签名方案，结合了高效的证明生成与验证算法，确保在大规模数据集上实现快速认证。

---
### [2026/1371] The Small-Field Turn in Succinct Proofs: A Systematization of Finite-Field Choice in Modern SNARKs and STARKs

- **作者:** Christos Skatharoudis

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1371) | [PDF](https://eprint.iacr.org/2026/1371.pdf)


> **研究背景:** 近年来，透明简洁论证系统从椭圆曲线上的256位标量域迁移到了较小的字段，如Goldilocks素数、BabyBear和KoalaBear素数等。本文旨在系统化这一‘小字段转向’现象。
>
> **主要贡献:** 作者将分析单位从证明系统或虚拟机转变为有限域，并根据机器字长匹配、二进制性、减少成本以及代数哈希的S-box自同构结构来组织这些领域，从而为生产证明器（Plonky2, Plonky3, Stwo, Binius及其后代）的选择提供依据。
>
> **达到效果:** 作者揭示了基础域宽度与Fiat-Shamir声学性所需的扩展次数之间的关系，并将其与非交互式FRI的猜想和可验证声学性的差距进行对比，从而填补了该领域最明显的实证空白。
>
> **技术梗概:** 通过分析有限字段的选择结构特性，本文为生产证明器的设计提供了指导，并展示了这些选择在实际系统中的实现方式。

---
### [2026/1372] Retrieve-Compute PIR and Its Applications

- **作者:** Benny Applebaum, Shahar Shechter

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1372) | [PDF](https://eprint.iacr.org/2026/1372.pdf)


> **研究背景:** 传统的两服务器私有信息检索（two-server PIR）虽然通信量可以任意小，但依赖于两个服务器不合作的强假设，这在实践中难以验证。
>
> **主要贡献:** 本文提出了一种新的两服务器PIR变体——Retrieve-Compute PIR (RC-PIR)，其中一个服务器充当标准计算服务器，另一个则作为检索限制的服务，仅提供数据库编码的块而不进行特定的PIR计算。
>
> **达到效果:** 基于学习噪声线性（LPN）假设，本文构建了具有任意小多项式通信和多项式存储的RC-PIR方案，并由此推导出多个应用，包括k服务器PIR和k服务器鲁棒PIR。
>
> **技术梗概:** 通过利用现有的静态内容或托管服务实例化检索限制的服务，将非合作假设与现实中的架构和部署约束相结合；基于LPN假设构造了RC-PIR方案，并进一步扩展到更广泛的多服务器场景。

---
### [2026/1373] Formalizing Privacy of Anonymous Credentials: A Provably Secure Framework with Predicate Proofs

- **作者:** Yu Zhang, Zongbin Wang, Jian Hou

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1373) | [PDF](https://eprint.iacr.org/2026/1373.pdf)


> **研究背景:** 现有匿名凭证系统要么不支持谓词证明，要么绑定于特定签名方案且缺乏形式化框架。
>
> **主要贡献:** 提出了首个具备内置谓词证明支持的构造性框架，并定义了实体、操作、对手模型和安全属性。
>
> **达到效果:** 该实例化版本满足所有安全属性并在标准硬件上具有实用性，凭证及证明通信成本适度增长，运行时间和存储优于基于CL的基础方案。
>
> **技术梗概:** 使用BBS签名、Pedersen承诺和环签名基础证明进行实例化，并在标准假设下形式化验证了其安全性。

---
### [2026/1375] MPC with Weighted and Fluid Participation

- **作者:** Ghada Almashaqbeh, Sumedh Ghude

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1375) | [PDF](https://eprint.iacr.org/2026/1375.pdf)


> **研究背景:** 现有的大多数MPC协议假设参与方是静态且等价的，但现代应用需要支持动态和加权的参与方式。
>
> **主要贡献:** 该研究开发了在静态和动态设置下都具有恶意安全性、支持加权参与的MPC协议。
>
> **达到效果:** 通过扩展加权斜率秘密共享方案并引入验证性和前瞻性功能，实现了加权MPC，并利用前瞻性特性支持委员会式MPC中的动态参与。
>
> **技术梗概:** 研究采用了加权斜率秘密共享和验证性技术来确保计算的正确性和安全性。

---
### [2026/1376] Secure and Efficient Federated Learning with Adaptive Differential Privacy and Verifiable Homomorphic Aggregation

- **作者:** Mohaddese Seyedi, Farhad Rahmati, Zahra Seyedi

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/1376) | [PDF](https://eprint.iacr.org/2026/1376.pdf)


> **研究背景:** 现有的联邦学习框架虽然提供了数据保密性和聚合验证性，但缺乏正式的统计隐私保证；而基于差分隐私的方法通常依赖固定的噪声注入机制，导致在隐私-实用性权衡上表现不佳。
>
> **主要贡献:** 本文提出了一种名为HEAD-FL的安全高效联邦学习框架，结合了自适应差分隐私和可验证同态聚合技术。
>
> **达到效果:** 理论分析与实验结果表明，HEAD-FL相比固定噪声注入和基于梯度的聚合方法，在隐私-实用性权衡上表现更优，并且通信效率更高，适用于隐私敏感和带宽受限环境。
>
> **技术梗概:** 通过引入基于Renyi差分隐私框架下的自适应高斯扰动机制，并采用联邦平均算法替代基于梯度的聚合方式，实现了在保护隐私的同时减少通信开销。

---
### [2026/1377] HAWK ``Guessing Game'' is not Polynomial-Time

- **作者:** Markku-Juhani O. Saarinen

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1377) | [PDF](https://eprint.iacr.org/2026/1377.pdf)


> **研究背景:** 本文展示了对HAWK攻击的复杂性分析，指出其运行时间远高于作者最初声称的多项式时间。
>
> **主要贡献:** 研究发现了攻击算法实现中的错误假设，并通过实验验证了超多项式的复杂性。
>
> **达到效果:** 研究表明，攻击的实际复杂度可能为指数级，而非多项式级，从而提升了HAWK的安全性评估。
>
> **技术梗概:** 利用AI工具快速探索和试验高级攻击技术，展示了其在现代密码分析中的应用价值。

---
### [2026/1378] (R)Icy-DVRF: A Robust Distributed Verifiable Random Function based on ROAST signatures

- **作者:** Ahmet Ramazan Ağırtaş, Oğuz Yayla, Melis Berçin Yılmaz

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1378) | [PDF](https://eprint.iacr.org/2026/1378.pdf)


> **研究背景:** 现有的分布式可验证随机函数(DVRF)协议通常依赖于同步或半同步网络假设，这使得同时确保鲁棒性和活跃性成为一个挑战。
>
> **主要贡献:** (R)Icy-DVRF通过结合ROAST签名框架到基于FROST的机制中，实现了在异步网络上操作的同时保持恒定大小的证明，从而解决了这一问题。
>
> **达到效果:** 该协议能够在恶意行为者试图破坏过程、延迟加密份额或保持无响应的情况下，确保正确的操作和输出生成。
>
> **技术梗概:** 通过使用ROAST签名框架，(R)Icy-DVRF能够在异步网络中实现鲁棒性和活跃性，同时维持较小的证明大小。

---
### [2026/1379] Hierarchical Structure in Attribute-Based Inner-Product Functional Encryption

- **作者:** Hirotomo Shinoki

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1379) | [PDF](https://eprint.iacr.org/2026/1379.pdf)


> **研究背景:** 该研究旨在通过引入层次结构来扩展属性基内积功能加密（AB-IPFE）的访问控制能力和线性计算能力，从而将其分为两步生成密钥算法。尽管在层次身份加密和可委托属性加密中广泛研究了此类结构，但在AB-IPFE中的应用却很少见。
>
> **主要贡献:** 作者提出了一个框架来定义两级层次结构，并对现有方案进行了分类。特别地，他们关注适应性安全的方案，这些方案支持表达式访问控制，如公共索引设置下的算术程序和私有索引设置下的属性隐藏内积谓词。
>
> **达到效果:** 研究提出了一种新的基于配对的构造方法，实现了适应性安全性，并且在密钥大小和密码文本大小之间进行了权衡。
>
> **技术梗概:** 通过重新审视Abdalla等人提出的方法，作者提出了三种类型的构造方案，以平衡层次结构、加密体大小和秘密密钥的特性。

---
### [2026/1380] TIM: A Sensitive-Parameter-Privacy Blind Watermarking Based on Zero-Knowledge Proof

- **作者:** Haoran Si, Xi Lin, Huiyan Chen

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1380) | [PDF](https://eprint.iacr.org/2026/1380.pdf)


> **研究背景:** 现有的盲水印方案通常需要图像所有者在提取时透露水印种子和位置，这依赖于验证者的高度信任，并且一旦这些秘密被暴露，恶意的验证者可以伪造或移除水印。
>
> **主要贡献:** TIM提出了首个基于零知识证明的具有敏感参数隐私保护的公开可验证盲水印方案。
>
> **达到效果:** TIM通过将整幅图像提取过程分解为迭代证明、引入基于阈值的投票机制和分层状态更新机制，实现了良好的不可感知性和鲁棒性，并且在实际效率上达到了实用水平。对于4K图像，生成证明耗时5.61分钟，峰值内存使用量为9.61 GB。
>
> **技术梗概:** TIM结合了Nova和Spartan技术，将整幅图像的提取过程分解成迭代证明；采用基于阈值的投票机制以增强鲁棒性；引入分层状态更新机制来降低电路和内存成本。

---
### [2026/1382] Concrete Bit-Operation Cost of XL: For Solving Multivariate Quadratic Systems Using Wiedemann and Berlekamp-Massey

- **作者:** Hülya Evkan, Ruben Niederhagen

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1382) | [PDF](https://eprint.iacr.org/2026/1382.pdf)


> **研究背景:** 本文提出了一个针对使用Wiedemann线性代数和Berlekamp-Massey序列恢复求解多元二次系统XL算法的具体位操作成本模型。
>
> **主要贡献:** 贡献在于实现了电路导向的XL，并推导出XL、Wiedemann和Berlekamp-Massey步骤的成本公式，适用于GF(2)、GF(31)和GF(256)。
>
> **达到效果:** 实验表明，该模型能准确预测电路成本，并通过渐近分析验证了其收敛性。
>
> **技术梗概:** 技术上采用CryptAttackTester方法论，结合基线、常数系数和桶化矩阵评估变体进行建模。

---
### [2026/1383] Notes on the ideal arithmetic correlations of $N$-ary sequences

- **作者:** Feifei Yan, Pinhui Ke

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1383) | [PDF](https://eprint.iacr.org/2026/1383.pdf)


> **研究背景:** 研究背景：本文探讨了具有理想算术相关性的N元序列的非存在性，特别是在特定连接整数和序的情况下。
>
> **主要贡献:** 主要贡献：证明了当连接整数为奇素幂时，在某些条件下不存在具有理想算术自相关的三元、四元或六元序列。
>
> **达到效果:** 达到的效果：这些结果进一步表明，理想算术相关性在N元设置中是非常严格的限制条件。
>
> **技术梗概:** 技术梗概：通过证明特定序和连接整数组合下不存在具有理想算术自相关的N元序列来实现上述贡献。

---
### [2026/1385] Walsh LUT Evaluation on Lazy Bits for CKKS AES Transciphering

- **作者:** Rostin Shokri, Nektarios Georgios Tsoutsos

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1385) | [PDF](https://eprint.iacr.org/2026/1385.pdf)


> **研究背景:** 本文提出了一种针对CKKS的新型布尔查找表评估方法，利用电路XOR保持懒惰（即作为加法运算处理，最低有效位保持正确），并在Walsh基上表示LUT。
>
> **主要贡献:** 该方法通过CKKS二进制布什拉夫化进行刷新步骤，将打包的懒惰校验和放置在槽中，并清洁二进制噪声以保留干净的校验和。这种方法解耦了LUT大小与周围电路的乘法深度：较大的LUT可以通过清理选定的事实上的校验和符号并仅花费少量常数深度来重组。
>
> **达到效果:** 该框架被应用于AES-CTR转置加密，其中AES S盒使用字节拆分Walsh分解评估，在CPU实验中，Walsh S-box AES-CTR算法在相同1024块批次大小下比稀疏布什拉夫化XBOOT版本快3.25倍。
>
> **技术梗概:** 该技术通过将LUT表示为Walsh基，并利用CKKS二进制布什拉夫化进行刷新步骤，包括StC阶段、CtS阶段和$\mathsf{EvalMod}_{f_{\mathrm{BinBoot}}}$清理二进制噪声。

---
### [2026/1387] ZK-Audit: Proving Power Side-Channel Resilience in Synthesized Hardware

- **作者:** Sakib Anwar Rieyan, Nektarios Georgios Tsoutsos

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1387) | [PDF](https://eprint.iacr.org/2026/1387.pdf)


> **研究背景:** 现代硬件安全依赖于预合成算法保护能够在物理制造管道中保持不变的假设，但第三方EDA工具链可能会引入数据相关的电源侧信道漏洞。现有的前硅验证方法要求暴露高度敏感的门级IP给外部审计员以验证结构安全性。
>
> **主要贡献:** 本文提出了一种新型零知识硬件审计器（ZK-Audit），它提供数学保证的物理数据无关性，无需揭示底层电路网表。
>
> **达到效果:** 实验评估表明，该框架能够识别固有泄露逻辑并捕获EDA引起的不对称性，如在理论上无泄漏的设计中引入13%的泄露率。
>
> **技术梗概:** 通过将合成门级拓扑结构转换为自定义侧信道中间语言（SCIL），我们的架构映射物理动态切换活动到可在Halo2零知识虚拟机（zkVM）内执行的算术约束，从而实现了第一个基于零知识边界翻转评估（ZK-BTA）的实现。

---
### [2026/1388] Chimera: A Hybrid GPU Backend for Sumcheck Acceleration in Zero Knowledge Provers

- **作者:** Kashfia Farheen, Nektarios Georgios Tsoutsos

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1388) | [PDF](https://eprint.iacr.org/2026/1388.pdf)


> **研究背景:** 零知识证明系统越来越多地依赖Sumcheck协议来避免早期SNARK设计中FFT密集的结构，而Sumcheck非常适合GPU加速。
>
> **主要贡献:** Chimera提出了一种GPU-CPU框架，用于在Spartan风格的SNARK验证者中加速Sumcheck。
>
> **达到效果:** 在BN254 Spartan R1CS实例上，Chimera在\(N=2^{17}\)时，Committed-Sumcheck区域的速度提高了7.96倍，并在整个从\(2^{12}\)到\(2^{27}\)约束的验证运行中实现了1.27至1.64倍的整体加速。
>
> **技术梗概:** Chimera采用混合执行模型：大型多项式评估和折叠工作在GPU上进行，活跃的多项式状态在整个盈利轮次中保留在GPU上，紧凑的轮消息返回到Spartan的主机转录，并且小型领域则回退到CPU执行。

---
### [2026/1389] SC-DT: Scalable Constant Round Secure Comparison and its Application to Privacy Decision Tree Evaluation

- **作者:** Qinghui Zhang, Xiaojun Chen, Yansong Zhang, Xudong Chen

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1389) | [PDF](https://eprint.iacr.org/2026/1389.pdf)


> **研究背景:** 现有的基于安全多方计算（MPC）的隐私决策树评估协议主要集中在两方或三方场景，并且其核心构建块——安全门限比较，通常需要对数轮通信，这限制了大规模PDTE的应用。
>
> **主要贡献:** 本文提出了一种在诚实多数设置下使用Shamir秘密共享的可扩展常数轮次安全比较协议SC-DT，通过借鉴Falcon的方法实现了零检测，并将特征选择和路径评估重新定义为PLUT功能，从而实现高效的大型PDTE。
>
> **达到效果:** 实验结果表明，与现有方案相比，SC-DT在安全性、通信效率和计算成本方面均有所提升，特别是在大规模数据集上表现尤为明显。
>
> **技术梗概:** 通过随机洗牌减少检测所需的轮次，并将随机洗牌转换为随机位移以降低离线通信开销，同时结合PLUT功能优化了特征选择与路径评估过程。

---
### [2026/1390] A Separation Principle for Lookup-Based zkML: Activation-Function Structure Cannot Reduce Per-Lookup Proving Cost

- **作者:** Min Jun Jo

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1390) | [PDF](https://eprint.iacr.org/2026/1390.pdf)


> **研究背景:** 在零知识机器学习(zkML)中，生成证明的成本远高于运行模型本身，而这一成本主要集中在变压器必须评估的非线性函数上。
>
> **主要贡献:** 研究揭示了一个分离原则：激活函数结构无法减少每项查找证明的工作量，只能影响一次性的表项成本。
>
> **达到效果:** 通过在两个独立的证明系统(EZKL/halo2和Jolt Atlas)上的测量，展示了深度与成本之间的线性关系，并通过调整地址宽度实现了精确的证明者时间缩减。
>
> **技术梗概:** 研究采用了一种Shout风格的一热查找论证方法，分析了不同层数模型中LayerNorm增益1/σ对证明成本的影响。

---
### [2026/1391] 6G Sensing Security: Distributed Game-Theoretic RL for Urban Beamforming and Attacker Detection

- **作者:** Parmida Geranmayeh, Onur Gunlu

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1391) | [PDF](https://eprint.iacr.org/2026/1391.pdf)


> **研究背景:** 随着下一代网络的发展，无线基础设施不仅用于数据传输，还将具备环境感知能力，推动了集成传感与通信(ISAC)技术的兴起。
>
> **主要贡献:** 本文通过引入博弈论和强化学习方法，提出了分布式鲁棒波束成形策略，以检测城市环境中恶意攻击者的行为。
>
> **达到效果:** 仿真结果表明，该方法能够有效应对动态6G ISAC系统中的安全挑战，提高通信系统的安全性。
>
> **技术梗概:** 研究采用了基于效用的博弈论模型，并将其整合到强化学习框架中，实现了在复杂环境下的自适应波束成形和攻击检测。

---
### [2026/1393] On the Differential Uniformity of Polynomials over Galois Rings

- **作者:** Sondre Rønjom, Arne Sandrib

- **分类:** Foundations

- **链接:** [论文](https://eprint.iacr.org/2026/1393) | [PDF](https://eprint.iacr.org/2026/1393.pdf)


> **研究背景:** 研究背景：随着高级密码学领域如多方计算和零知识协议设计的发展，对Galois环上S-box的 cryptographic 属性进行了探索。特别是关于这些函数的差分均匀性。
>
> **主要贡献:** 主要贡献：本文首次系统地研究了Galois环$\mathrm{GR}(p^k,m)$（其中$k,m\geq 2$）上的多项式的差分性质，并提供了$\mathrm{GR}(4,2)$中APN置换的分类。
>
> **达到效果:** 达到的效果：通过分析，本文为设计安全的哈希函数和块密码提供了理论依据，并填补了该领域研究的空白。
>
> **技术梗概:** 技术梗概：采用代数方法对多项式在Galois环上的差分行为进行了深入研究，利用等价变换进行分类。

---
### [2026/1395] Blind Trace-Only Segmentation of Cipher Implementations Without Algorithm Metadata

- **作者:** Hyunjun Kim, Hwajeong Seo, Anupam Chattopadhyay

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/1395) | [PDF](https://eprint.iacr.org/2026/1395.pdf)


> **研究背景:** 盲侧信道分析（BSCA）能够通过未标记的泄漏迹线推断密钥，但实际应用仍需先对迹线进行结构化处理以定位重复计算和候选兴趣点。
>
> **主要贡献:** 作者提出了一种仅基于样本均值和标准差的两阶段方法来解决无算法元数据指导下的迹线唯一结构化问题。
>
> **达到效果:** 该方法在16个不同的块密码实现中，大多数情况下能够形成一致的重复窗口，并通过后处理分离出精确或边缘包容的计数匹配以及其他关系层次，Top-5候选区域通常能覆盖强S盒CPA热点。
>
> **技术梗概:** 第一阶段利用秩合并自相似性估计重复尺度、起始相位、锚支持稳定核心和周期候选；第二阶段将稳定核心堆叠成代表性的重复并分割为相对高分和低分段落。

---
### [2026/1396] Reliable TRNG and its Challenges

- **作者:** Raja Adhithan Radhakrishnan

- **分类:** Implementation

- **链接:** [论文](https://eprint.iacr.org/2026/1396) | [PDF](https://eprint.iacr.org/2026/1396.pdf)


> **研究背景:** 研究旨在改进基于环振荡器（RO）的真随机数发生器（TRNG）的自调谐机制，并探讨长期运行中实现可靠稳定设计所面临的挑战。
>
> **主要贡献:** 贡献在于分析了应对这些挑战的方法，并通过NIST统计测试套件验证了其有效性。
>
> **达到效果:** 结果表明，提出的方法能够显著提高TRNG在长时间操作中的可靠性与稳定性。
>
> **技术梗概:** 技术上采用了自适应反馈机制来优化环振荡器的频率分布，并结合了多种噪声源以增强随机性。

---
### [2026/1398] How to Encrypt with Random Reversible Circuits

- **作者:** Ran Canetti, Ji Luo, Yiding Zhang

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1398) | [PDF](https://eprint.iacr.org/2026/1398.pdf)


> **研究背景:** 该研究重新审视了构建公钥加密的一种自然范式，其中公钥是混淆的块密码。
>
> **主要贡献:** 贡献在于展示了如何通过特定条件下的混淆和功能扩展实现高级安全性和功能性。
>
> **达到效果:** 结果包括直接应用混淆后的块密码即可达到CCA2安全性，并且可以通过额外的功能增强实现模拟安全性和同态加密能力。
>
> **技术梗概:** 技术上依赖于可互换伪随机置换和随机可逆电路的性质，以及相应的混淆方法。

---
### [2026/1402] On Extending Integral Distinguishers

- **作者:** Dachao Wang, Hosein Hadipour, Simon Gerhalter

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1402) | [PDF](https://eprint.iacr.org/2026/1402.pdf)


> **研究背景:** 传统的积分区分器方法通常针对单一输出表达式进行分析，容易忽略多个输出组合后产生的区分器，并且难以捕捉到关键依赖的积分组合。
>
> **主要贡献:** 作者提出了一种Split-and-Cancel方法，通过结合精确扩展和预言机技术来确定哪些后缀单项式可以从选定结构中存活下来，并从中提取出关键独立的组合。
>
> **达到效果:** 该方法应用于SIMON、SIMECK、SPECK、PRESENT和GIFT等密码算法，显著增强了积分区分器的结果，在某些情况下增加了额外的一轮分析。
>
> **技术梗概:** Split-and-Cancel技术通过在短尾部分进行精确扩展，并利用预言机来确定哪些后缀单项式可以存活下来，从而从矩阵的左核中提取出关键独立的组合。

---
### [2026/1403] A polynomial-time key recovery attack of Facto-DSA

- **作者:** Simon Abelard, Ludovic Perret, Hao Shi

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1403) | [PDF](https://eprint.iacr.org/2026/1403.pdf)


> **研究背景:** 该研究针对Facto-DSA签名方案提出了一种多项式时间的密钥恢复攻击，揭示了基于多元多项式因式分解构建稳健密码方案的潜在脆弱性。
>
> **主要贡献:** 研究人员开发了一种高效算法，在标准笔记本电脑上不到一分钟的时间内成功破解了所有已提出的参数集，包括最大的一组。
>
> **达到效果:** 此研究表明，使用多元多项式因式分解作为构建安全签名方案基础的有效性受到质疑。
>
> **技术梗概:** 攻击方法基于多项式时间复杂度的算法实现，通过利用特定结构的弱点来恢复密钥。

---
### [2026/1404] Anonymous Communication on Expander Networks

- **作者:** Megumi Ando, Hannah Lynn, Anna Lysyanskaya, Eli Upfal

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1404) | [PDF](https://eprint.iacr.org/2026/1404.pdf)


> **研究背景:** 研究背景：本文探讨了在扩张网络中实现匿名通信的方法，面对被动和主动两种强敌手模型。目标是在确保匿名性的同时，提高协议的效率。
>
> **主要贡献:** 主要贡献：提出了针对扩张网络的匿名通信方案，并证明了当路由路径中的每个跳点均匀随机选择时，在特定条件下可以抵抗被动敌手并实现匿名性。
>
> **达到效果:** 达到的效果：通过理论分析和模型验证，该研究为在实际网络中高效实现匿名通信提供了新的解决方案。
>
> **技术梗概:** 技术梗概：采用洋葱路由机制，并结合扩张网络的随机性质来增强匿名性，同时确保协议的安全参数λ与网络规模成线性关系。
...
### [推荐] [2026/1290] A Compact Signature Scheme Based on QC-MDGM Codes
...
- **链接:** [论文](https://eprint.iacr.org/2026/1290) | [PDF](https://eprint.iacr.org/2026/1290.pdf)


> **研究背景:** 现有基于QC-MDGM码的签名方案要么签名过大，要么验证过程缓慢；而哈希-签名方案虽能提供较小的签名但公钥庞大且签名算法复杂。
>
> **主要贡献:** 提出ASTRA-Sign方案，结合哈希-签名范式与准循环中等密度生成矩阵码，实现小签名和小公钥。
>
> **达到效果:** ASTRA-Sign在128位安全级别下，公钥和签名均小于1kB，并且验证时间低于50微秒。
>
> **技术梗概:** 通过利用准循环码中寻找低重量码字的困难性和寻找与随机向量全汉明距离的码字的难度来保障安全性。

---
### [推荐] [2026/1293] Post-quantum Secure Non-Committing Registered Functional Encryption
...
- **链接:** [论文](https://eprint.iacr.org/2026/1293) | [PDF](https://eprint.iacr.org/2026/1293.pdf)


> **研究背景:** 非绑定加密(NCE)是证明适应性破坏安全性的关键原语，允许模拟器在未知加密消息的情况下生成密文。现有的基于属性的加密NCE构造通常依赖于集中式的信任机制来生成用户的私钥。
>
> **主要贡献:** 本文首次提出了非绑定注册功能加密(NC-RFE)，作为去中心化框架中非绑定属性加密的一般化，并提供了基于不可区分性混淆和零知识论证的通用构造。
>
> **达到效果:** 该构造在假设安全的基础注册功能加密(RFE)方案下被证明是安全的，并进一步提供了一个基于环LWE和可等效LWE假设的格基实例，实现了后量子安全的NC-RFE方案。
>
> **技术梗概:** 通过适应经典非绑定安全性到注册环境来形式化NC-RFE的概念，并基于不可区分性混淆和零知识论证提出了通用构造。

---
### [推荐] [2026/1298] Weak Keys Break the BUFF Security of HAWK
...
- **链接:** [论文](https://eprint.iacr.org/2026/1298) | [PDF](https://eprint.iacr.org/2026/1298.pdf)


> **研究背景:** HAWK是一种基于模块格同构问题的签名方案，是NIST第三轮后量子加密标准调用中唯一的候选方案。其规范声称HAWK实现了BUFF安全特性而无需应用通用的 BUFF 变换。
>
> **主要贡献:** 作者通过构造弱公钥证明了HAWK在某些BUFF游戏中未能实现声明的安全性，并揭示了现有证明中的漏洞。
>
> **达到效果:** 研究结果表明，HAWK在面对特定类型攻击时存在安全漏洞，特别是当对手能够选择验证密钥并提交伪造签名时。
>
> **技术梗概:** 通过构造弱公钥并利用其特性来挑战HAWK的安全性假设，并分析现有证明中的不足之处以揭示潜在的弱点。

---
### [推荐] [2026/1300] Thresholdizing Standardized FALCON Signatures

- **匹配关键字:** post-quantum

- **作者:** Radhika Garg, Daniel Escudero, Antigoni Polychroniadou, Akira Takahashi, Xiao Wang

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1300) | [PDF](https://eprint.iacr.org/2026/1300.pdf)


> **研究背景:** 随着NIST后量子标准的制定，设计与新选中的原语兼容的门限签名方案成为迫切需要。然而，目前没有基于FALCON验证算法的门限签名方案。
>
> **主要贡献:** 本文提出了首个支持FALCON验证算法的门限签名协议，并通过技术贡献实现了这一目标。
>
> **达到效果:** 该协议在不同设置下进行了实现和基准测试，证明了其可行性和效率。
>
> **技术梗概:** 作者通过适应MPC下的离散高斯采样、固定精度分析以及高效的MPC协议设计等方法，解决了FALCON签名过程中的技术挑战。

---
### [推荐] [2026/1301] STRUCTURED LATTICES AND THEIR APPLICATIONS TO SECURITY
...
- **链接:** [论文](https://eprint.iacr.org/2026/1301) | [PDF](https://eprint.iacr.org/2026/1301.pdf)


> **研究背景:** 欧几里得格是一个在多个方面都值得关注的研究对象，特别是井-rounded 格与最密球堆积问题、theta 函数最小化以及著名的 Minkowski 和 Woods 猜想紧密相关。
>
> **主要贡献:** 本文综述了结构化格，并讨论了它们在基于格的密码学和安全无线通信中的最新应用，旨在激发数学家和其他相关领域对这些交叉学科主题的兴趣。
>
> **达到效果:** 通过结合数论、密码学和无线通信的知识，研究提出了新的应用场景，增强了格理论的实际价值。
>
> **技术梗概:** 文章采用了综述的方法，回顾了井-rounded 格的构造及其在不同领域的应用，并探讨了其技术实现的可能性。

---
### [推荐] [2026/1305] SUF-CMA SQISign via Canonical Response Encoding
...
- **链接:** [论文](https://eprint.iacr.org/2026/1305) | [PDF](https://eprint.iacr.org/2026/1305.pdf)


> **研究背景:** SQIsign是基于isogeny的后量子数字签名方案，目前在NIST标准化过程中被考虑。现有的安全结果仅证明了其存在选择明文攻击下的不可伪造性（EUF-CMA），但未达到强不可伪造性（SUF-CMA）的要求。
>
> **主要贡献:** 作者提出了三项贡献：首先识别出SQIsign v2.0签名中的可变性向量；其次提出了一种最小修复方案——规范矩阵编码，以确保响应编码的单射性质；最后证明了修改后的方案在量子随机预言模型下实现了SUF-CMA。
>
> **达到效果:** 通过上述改进和证明，该研究使SQIsign达到了更高的安全性标准，即强不可伪造性（SUF-CMA），从而提升了其作为后量子数字签名方案的可靠性。
>
> **技术梗概:** 作者采用sigma协议的诚实验证者零知识性和特殊有效性来证明修改后的方案的安全性，并通过规范矩阵编码确保响应编码的单射性质。

---
### [2026/1291] Refined OJ Attacks: Tight Complexity for Rank Decoding Problems and Their Cryptographic Implications

- **作者:** Yongcheng Song, Rongmao Chen, Xinyi Huang, Jiang Zhang, Chao Lin

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1291) | [PDF](https://eprint.iacr.org/2026/1291.pdf)


> **研究背景:** 秩解码(RD)问题在基于秩的密码学中占据核心地位，多种变体如非齐次秩解码(NHRD)和块状秩解码(BRD)被提出以提高效率。然而，对于OJ攻击及其改进版IOJ攻击的理解不足，尤其是在NHRD和BRD变体中的应用，影响了对这些系统的安全评估。
>
> **主要贡献:** 作者精炼了OJ攻击和改进的IOJ攻击的模型，并获得了RD、NHRD和BRD问题的一般性和紧致复杂性。研究表明，对于某些参数区域而言，OJ攻击仍然是最强大的组合攻击之一。
>
> **达到效果:** 研究发现，对于RD问题，在小维度k和大扩展度m的情况下，OJ攻击仍然是一种强有力的选择；而对于NHRD问题，OJ攻击在特定参数下提供了高达100比特的安全性提升。
>
> **技术梗概:** 通过改进的建模方法，作者得出了针对秩解码及其变体的一般性和紧致复杂性分析。

---
### [2026/1292] CRAfT: Constant-Round Non-Polynomial Approximation and P2P Network for Secure Transformer Inference

- **作者:** Jinghao Zhao, Hongwei Yang, Xiaoyu Song, Meng Hao, Hui He, Weizhe Zhang

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1292) | [PDF](https://eprint.iacr.org/2026/1292.pdf)


> **研究背景:** Transformer模型因其在内容生成应用中的革命性表现而受到广泛关注，但现有安全协议的隐私问题和可扩展性瓶颈限制了其大规模部署。
>
> **主要贡献:** CRAfT通过提出针对多方互信不足用户的新型密文打包策略及基于三角函数的无分支迭代拟合策略，解决了这些问题。
>
> **达到效果:** 实验结果表明，CRAfT在BERT-base模型上实现了高达8.5倍的多客户端推理吞吐量提升，并且保持了模型准确性。
>
> **技术梗概:** 该系统利用高速P2P网络卸载大量通信流量，并通过变换复杂操作为标准的Sigmoid-like函数来最小化加密成本。

---
### [2026/1294] Differential Fault Attack on Atom: Bypassing the Double Key Filter using Filtered Faults

- **作者:** Vaibhav Dixit, Satyam Kumar, Santanu Sarkar

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1294) | [PDF](https://eprint.iacr.org/2026/1294.pdf)


> **研究背景:** 本文针对Banik等人在TOSC-2021提出的一款轻量级流密码Atom，设计了一种差分故障攻击（DFA），绕过了其双重密钥过滤器。
>
> **主要贡献:** 作者提出了一种无需猜测LFSR位即可构建代数方程的方法，并成功实现了对Atom的DFA，这是首次在Atom上报告成功的DFA。
>
> **达到效果:** 实验表明，在正确识别18个故障位置的情况下，可以在实际时间内通过注入18次故障实现攻击；平均需要52次故障注入才能获得正确的18个故障位置，总共需要70位密钥流数据（正常和故障结合）。
>
> **技术梗概:** 作者利用训练好的MLP和XGBoost模型的加权集合来识别注入故障的位置，并过滤掉置信度低于预定义阈值的预测结果。

---
### [2026/1295] A new attack to RSA with small private exponent and partial information.

- **作者:** Jorge Jimenez Urroz

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1295) | [PDF](https://eprint.iacr.org/2026/1295.pdf)


> **研究背景:** 研究背景：针对具有小私钥和部分p+q信息的RSA系统的攻击方法，是对经典Wiener攻击的一种改进。
>
> **主要贡献:** 主要贡献在于提出了一种新的算法，该算法是基于原Wiener攻击并利用连续分数的方法进行修改，从而能够在更宽松条件下成功破解系统。
>
> **达到效果:** 达到的效果是在d < n^0.3的情况下，即使仅有部分p+q信息也能成功分解n，改进了原有Wiener攻击的条件限制。
>
> **技术梗概:** 技术梗概：该算法通过简单地修改Wiener攻击中的连续分数方法来实现对具有小私钥和部分p+q信息的RSA系统的攻击。

---
### [2026/1296] Achieving Guaranteed Output Delivery MPC with Constant Rounds and Linear Communication in Minicrypt

- **作者:** Junru Li, Yifan Song

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1296) | [PDF](https://eprint.iacr.org/2026/1296.pdf)


> **研究背景:** 研究旨在探讨在Minicrypt假设下，如何实现具有恒定轮次和线性通信复杂度的常数轮次多方计算（MPC）协议，特别是保证输出交付（GOD）的问题。
>
> **主要贡献:** 论文提出了首个满足上述要求的MPC协议，在随机预言机假设下，通信复杂度为$O(|C|n\kappa+Dn^3\kappa^3+W_I{\sf poly}(n,\kappa))$位。
>
> **达到效果:** 该协议在保持线性通信的同时实现了恒定轮次的MPC计算，并且在GOD方面提供了保证，优于之前的工作。
>
> **技术梗概:** 通过巧妙地设计协议结构和利用随机预言机技术，论文成功平衡了通信复杂度与轮次复杂度之间的关系。

---
### [2026/1297] Breaking the $\Omega(|C|\kappa)$ Barrier on Garbled Circuit Size in the Random Oracle Model

- **作者:** Junru Li, Yifan Song

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1297) | [PDF](https://eprint.iacr.org/2026/1297.pdf)


> **研究背景:** 研究旨在突破随机预言模型下门电路加密大小的$\Omega(|C|\kappa)$障碍，针对计算无界对手和$T$次随机预言机查询。
>
> **主要贡献:** 提出了首个实现$o(|C|\kappa)$门电路加密大小的方案，并将其扩展为通信复杂度为$O(|C|\log T+D(\log T+\kappa)^2\log T+{\sf poly}(\kappa,\log T))$位的恶意安全双方计算协议。
>
> **达到效果:** 该方案在随机预言模型下打破了长期存在的门电路加密大小障碍，实现了更小的加密门电路尺寸，并且通信效率显著提升。
>
> **技术梗概:** 通过创新的加密技术，优化了门电路的表示方式和传输过程，在保证安全性的前提下大幅减少了所需的加密门电路大小。

---
### [2026/1299] Decomposition of compressions on elliptic curves and point recovery

- **作者:** Robert Dryło

- **分类:** Public-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1299) | [PDF](https://eprint.iacr.org/2026/1299.pdf)


> **研究背景:** 研究背景：本文探讨了椭圆曲线上压缩函数的分解及其在点恢复中的应用，特别是针对具有有限子群结构的压缩函数。
>
> **主要贡献:** 主要贡献在于证明了一般的$G$-压缩函数可以表示为一个分离态射与度数2压缩函数的复合，并提供了几种特殊模型下的具体分解方法。
>
> **达到效果:** 达到的效果是通过已知的度数2压缩性质，实现了点加倍、差分相加以及基于双态射诱导函数的点恢复算法。
>
> **技术梗概:** 技术梗概：利用分离态射和压缩函数的性质，将$G$-压缩分解为特定椭圆曲线上的压缩与态射的复合，并通过这些方法实现高效的算术操作。

---
### [2026/1302] TRIP: Thresholding in Regression with Input Privacy

- **作者:** Chrysa Oikonomou, Katerina Sotiraki

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1302) | [PDF](https://eprint.iacr.org/2026/1302.pdf)


> **研究背景:** 在安全计算中，多方协作评估函数时无法保护对抗恶意输入数据的问题促使了TRIP协议的提出。
>
> **主要贡献:** TRIP通过结合安全计算、鲁棒统计和差分隐私技术，解决了线性回归任务中的恶意输入问题。
>
> **达到效果:** 实验表明，TRIP在合成数据中恢复了真实值，在实际数据集中即使在40%的目标污染下仍保持接近无噪声最小二乘法的表现。
>
> **技术梗概:** 该协议采用了新颖的安全计算、鲁棒统计和差分隐私技术的组合方法。

---
### [2026/1303] Subspace Differential Uniformity

- **作者:** Sondre Rønjom, Arne Sandrib, Joakim Sunde

- **分类:** Secret-key cryptography

- **链接:** [论文](https://eprint.iacr.org/2026/1303) | [PDF](https://eprint.iacr.org/2026/1303.pdf)


> **研究背景:** 研究背景：本文旨在引入子空间差分均匀性(SDU)作为评估S盒和块密码中输入差异在仿射子空间内如何均匀地分散到输出差异中的非线性度的新指标。
>
> **主要贡献:** 主要贡献：作者提出了SDU的概念，并提供了计算SDU的高效算法，同时对一些流行类别的S盒进行了SDU的评估。
>
> **达到效果:** 达到的效果：通过分析不同类别S盒的SDU值，为设计具有更好差分密码分析抵抗性的块密码提供理论依据和数值参考。
>
> **技术梗概:** 技术梗概：利用仿射子空间的概念定义SDU，并通过数学方法推导了SDU的一些下界，进而提出了一种计算SDU的有效算法。

---
### [2026/1304] Security Analysis of One Lightweight Certificateless Mutual Authentication Scheme Based on Signatures for IIoT

- **作者:** Zhengjun Cao, Lihua Liu

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1304) | [PDF](https://eprint.iacr.org/2026/1304.pdf)


> **研究背景:** 研究背景：针对工业互联网（IIoT）中的轻量级无证书互认证方案的安全性进行了分析，指出了一种基于签名的无证书方案存在安全漏洞。
>
> **主要贡献:** 主要贡献：揭示了该方案在公共密钥替换攻击下的不安全性，证明了攻击者可以通过替换签名人公钥伪造任意消息的签名。
>
> **达到效果:** 达到的效果：发现了导致签名组件与目标消息及签名人身份松散绑定的根本缺陷，使得攻击者能够找到一个功能上等同于有效签名算法的有效签名函数。
>
> **技术梗概:** 技术梗概：通过分析签名方案中的关键组成部分，识别并证明了其在面对特定类型攻击时的脆弱性。

---
### [2026/1306] TETRIS: Automated Design Space Exploration of Randomness–Latency Trade-offs in Masked Hardware

- **作者:** Nilotpola Sarma, Tapish Patidar, Nupur Brahamanya, Chandan Karfa

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1306) | [PDF](https://eprint.iacr.org/2026/1306.pdf)


> **研究背景:** 在给定的安全级别下，掩码硬件的随机性和延迟之间存在权衡。尽管基于组件的掩码简化了使用称为‘组件’的掩码函数构建更大规模设计的过程，但缺乏有效的搜索方法来找到用户最优的设计。
>
> **主要贡献:** 本文提出了TETRIS工具，通过引入两种算法——在随机性约束下最小化延迟（MLRC）和在延迟约束下最小化随机性（MRLC），实现了基于软件的掩码硬件设计空间探索。
>
> **达到效果:** 实验结果表明，使用探针隔离非干扰（PINI）组件的基于组件的掩码硬件能够有效地进行局部优化，从而获得与全局优化相当甚至更好的面积性能。
>
> **技术梗概:** TETRIS工具通过定义随机性和延迟之间的结构化权衡来实现自动化设计空间探索，并利用启发式方法而非复杂的全局优化技术。

---
### [2026/1307] A Communication-Efficient Local-Verification Framework for Maliciously Secure MPC with a Two-Thirds Honest Majority

- **作者:** Hanchao Ku, Hikaru Tsuchida, Mingwu Zhang, Takashi Nishide

- **分类:** Cryptographic protocols

- **链接:** [论文](https://eprint.iacr.org/2026/1307) | [PDF](https://eprint.iacr.org/2026/1307.pdf)


> **研究背景:** 安全多方计算（MPC）是一种加密原语，允许多方在不泄露各自输入的情况下共同计算一个函数。通常需要提供对恶意行为的保护，但在许多实际应用场景中，更强的恶意安全性是必需的。
>
> **主要贡献:** 本文提出了一种针对最多控制$n/3$个参与者的静态恶意对手的安全MPC协议，并通过注入新鲜随机性改进了验证过程以检测作弊。
>
> **达到效果:** 该协议减少了通信开销，通过局部计算和批量开启替代多次乘法验证调用，同时保持本地验证成本与或低于现有方法。
>
> **技术梗概:** 基于Shamir的秘密共享和编译器方法，首先使用半诚实协议评估电路，然后运行一个允许诚实方检测作弊的验证过程。

---
### [2026/1308] Trust the Voice, Hide the Source: Anonymous Provenance for Verifiably Edited Audio

- **作者:** Xiyuan Fu, Zixing Wang, Hongbo Wang, Yu Chen

- **分类:** Applications

- **链接:** [论文](https://eprint.iacr.org/2026/1308) | [PDF](https://eprint.iacr.org/2026/1308.pdf)


> **研究背景:** 现有的音频认证方法在验证时需要释放原始录音，这会暴露敏感的源内容和来源信息。红噪音频可以保护隐私，但会无效化原始签名。揭示编辑操作以证明合规性也会泄露被红噪的内容，这使得现有方法难以同时保证隐私和真实性。
>
> **主要贡献:** 本文提出了一种隐私保护音频认证系统（PPAAS），通过单一关系将源来源和编辑正确性绑定到同一个隐藏见证中，使验证者可以在不学习源录音、编辑操作或产生该录音的设备身份的情况下，确认发布的音频来自授权设备并通过允许的编辑获得。
>
> **达到效果:** PPAAS提供了两种适应不同编辑场景的构造方法：一种基于分割的构造仅在主动编辑段落需要零知识证明，并适用于稀疏编辑；另一种迭代构造则针对密集编辑情况。这两种方法均能有效平衡隐私保护与音频真实性验证之间的冲突。
>
> **技术梗概:** 通过使用零知识证明技术，PPAAS能够在不泄露源录音、编辑操作或设备身份的情况下，确保发布的音频来自授权设备并通过允许的编辑获得。

---
### [2026/1309] Forensic Cryptanalysis of the Backdoored UA-8295 Message Terminal

- **作者:** Stijn Maatje, Marc Stevens

- **分类:** Attacks and cryptanalysis

- **链接:** [论文](https://eprint.iacr.org/2026/1309) | [PDF](https://eprint.iacr.org/2026/1309.pdf)


> **研究背景:** 本文探讨了国家情报机构可能通过设计后门影响加密标准和设备以获取信号情报的问题，尽管已知此类后门的存在，但鲜有公开分析案例。
>
> **主要贡献:** 作者首次详细分析了被后门感染的UA-8295消息终端，并提出了一个‘后门猜想’来帮助理解实际世界中后门的设计及其针对的攻击类型。
>
> **达到效果:** 通过分析，作者揭示了UA-8295后门的具体设计和潜在攻击方式，为理解和防范类似威胁提供了理论依据。
>
> **技术梗概:** 研究采用了先进的密码分析技术，包括对加密终端进行全面逆向工程，以识别其潜在的安全漏洞和后门机制。

---
### [2026/1310] Designing Wallet-Based User Intervention for Approval Phishing Mitigation

- **作者:** Maggie Yongqi Guan, Yuqi Xu, Yunlong Mao, Wei Tong, Xiaobo Zhou, Kanye Ye Wang

- **分类:** Unknown

- **链接:** [论文](https://eprint.iacr.org/2026/1310) | [PDF](https://eprint.iacr.org/2026/1310.pdf)


> **研究背景:** 研究背景：随着网络钓鱼攻击转向利用Web3中的代币批准机制，URL检测已不足以防范此类威胁。因此，需要在加密货币钱包层面设计干预措施以增强用户安全。
>
> **主要贡献:** 主要贡献：提出四种基于钱包的干预措施来缓解批准型网络钓鱼：支出限额建议、主动警告、被动警告和延迟确认。
>
> **达到效果:** 达到的效果：通过实验发现，支出限额建议显著提高了设置限额的概率；而主动警告和延迟确认则有效降低了欺诈交易的完成率，尽管统计显著性仅限于前者。
>
> **技术梗概:** 技术梗概：设计了基于用户交互的技术干预措施，并通过实验证明了这些方法的有效性，特别是在提高用户对批准后潜在风险的认识方面。
